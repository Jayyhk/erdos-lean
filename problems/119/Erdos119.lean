...
Erdős Problem 119. Let `zᵢ` be complex numbers with `|zᵢ| = 1`, let `pₙ(z) = ∏_{i ≤ n} (z - zᵢ)`
and `Mₙ = max_{|z| = 1} |pₙ(z)|`. Erdős asked three questions: (i) is `limsup Mₙ = ∞`?
(ii) is there `c > 0` with `Mₙ > n^c` infinitely often? (iii) is there `c > 0` with
`∑_{k ≤ n} M_k > n^(1+c)` for all large `n`? All three are answered affirmatively here.
...
`Mathlib`. It is reproduced here wrapped once in `namespace Erdos119` (moved above the `open`
...
lemma iteratedDeriv_rotationAverage
  ...
  · intro j hj
    have hcomp :
      ...
      (hfcont (ζ ^ j * 0) (by simp)).comp (0 : ℂ) (by fun_prop) (by
        intro w hw
        simpa [mem_ball, norm_mul, hζnorm] using hw)
    simpa [ζ] using hcomp
...
$n^{5/4}/\sqrt{\log n}$. -/
theorem erdos_119_quantitative :
    ∀ (z : ℕ → ℂ) (_ : ∀ i : ℕ, ‖z i‖ = 1),
      ∃ C > 0, ∀ᶠ n : ℕ in atTop,
        C * ((n : ℝ) ^ (5 / 4 : ℝ) /
          Real.sqrt (Real.log (n : ℝ))) <
...
-/
theorem erdos_119_question_two :
    ∀ (z : ℕ → ℂ) (_ : ∀ i : ℕ, ‖z i‖ = 1),
      ∃ (c : ℝ) (_ : c > 0), Infinite {n : ℕ | M z n > n ^ c} := by
  intro z hz
  obtain ⟨c, hc, hsum⟩ := sum_M_gt_one_add_pow z hz
...
-/
theorem erdos_119_question_one :
    ∀ (z : ℕ → ℂ) (_ : ∀ i : ℕ, ‖z i‖ = 1),
      atTop.limsup (fun n => (M z n : EReal)) = ⊤ := by
  intro z hz
  obtain ⟨c, hc, h_infinite⟩ := erdos_119_question_two z hz
  have hfreq :
