...
Erdős Problem 590. Let `α` be the ordinal `ω^ω`. Is it true that in any red/blue colouring of
the edges of `K_α` there is either a red `K_α` or a blue `K₃`? `erdos_590` proves that there
is. Erdős offered $250 for this.

The partition relation `α → (β, c)²` is `OrdinalCardinalRamsey` below: for every pair of
complementary graphs `red`, `blue` on `α.ToType`, either some red clique has order type `β`,
or some blue clique has cardinality `c`. The theorem is the case `α = β = ω^ω`, `c = 3`. The
proof follows Larson's argument, with `Erdos590.Larson.erdos_590` the version in her setting
and `erdos_590` the statement in the shape of the Formal Conjectures specification; the latter
is the final theorem here.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos590.lean`. It is reproduced here with the file-level provenance
header removed, its project-internal imports removed so that `Mathlib` is the only import, and
the whole wrapped once in `namespace Erdos590`, with the upstream trust-base print line
removed. No mathematical content is changed.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos590.lean` -/
...
#print axioms erdos_590
-- 'Erdos590.erdos_590' depends on axioms: [propext, Classical.choice, Quot.sound]
