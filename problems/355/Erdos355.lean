  ...
  have h_dense : DenselyFills S 0 U δ := by
    have h_apply_lm_reals : DenselyFills { s | ∃ t ⊆ Finset.Icc 1 (M + 1), s = ∑ i ∈ t, (1 : ℝ) / n (i - 1) } 0 (∑ i ∈ Finset.Icc 1 (M + 1), (1 : ℝ) / n (i - 1)) ((1 : ℝ) / n M) := by
      ...
      · have h_apply_finite_seq : ∀ i < M, (1 : ℝ) / n i ≤ (∑ j ∈ Finset.Ioc i M, (1 : ℝ) / n j) + (1 : ℝ) / n M := by
        ...
        have icc_succ : ∀ a b : ℕ, Finset.Icc (a + 1) b = Finset.Ioc a b := fun a b => by
          ext x; simp only [Finset.mem_Icc, Finset.mem_Ioc]; omega
        intro i hi; specialize h_apply_finite_seq ( i - 1 ) ; rcases i with ( _ | i ) <;> simp_all +decide [Finset.sum_Ioc_succ_top] ;
        refine le_trans ( h_apply_finite_seq ( Nat.lt_of_succ_le hi ) ) ?_;
    ...
    · ext; simp [S, Finset.Icc];
      ...
      · use t.image (fun x => x - 1);
        simp_all +decide [ Finset.subset_iff ];
        exact ⟨ fun x hx => Nat.lt_succ_of_le ( Nat.sub_le_of_le_add <| by linarith [ Finset.mem_Icc.mp ( ht hx ) ] ), by rw [ Finset.sum_image <| by intros a ha b hb hab; linarith [ Nat.sub_add_cancel <| show 1 ≤ a from Finset.mem_Icc.mp ( ht ha ) |>.1, Nat.sub_add_cancel <| show 1 ≤ b from Finset.mem_Icc.mp ( ht hb ) |>.1 ] ] ⟩;
    ...
    (∀ j, K - 1 ≤ j → j < M - 1 → lambda ≤ (n' (j + 1) : ℝ) / n' j ∧ (n' (j + 1) : ℝ) / n' j ≤ 2) := by
      ...
      obtain ⟨N, d, M, hd₀, hd₁, hd₂⟩ : ∃ N : ℕ, ∃ d : ℕ → ℕ, ∃ M : ℕ,
        ...
        (∀ j < M, lambda ≤ (d (j + 1) : ℝ) / d j ∧ (d (j + 1) : ℝ) / d j ≤ 2) := by
          ...
          exact fun i hi => h_n_pos i ( Nat.lt_succ_of_lt hi );
      refine' ⟨ K + M + 1, by linarith, fun i => if i < K then n i else d ( i - K ) * n K, _, _, _, _, _ ⟩ <;> simp_all +decide;
      · intro i hi₁ hi₂; rw [ show i = K by linarith ] ; aesop;
      · exact dvd_mul_of_dvd_left ( dvd_of_mul_left_dvd hd₀ ) _;
      · intro i hi; split_ifs;
        · refine' dvd_mul_of_dvd_left _ _;
          exact dvd_trans ( dvd_mul_of_dvd_left ( Finset.dvd_prod_of_mem _ ( Finset.mem_range.mpr ‹_› ) ) _ ) hd₀;
        · exact mul_dvd_mul ( hd₂.2.1 _ ( Nat.sub_le_of_le_add <| by linarith ) ) dvd_rfl;
      · intro a ha b hb hab; split_ifs <;> try linarith [ h_n_mono ( by linarith ) ( by linarith ) hab ] ;
      ...
      · intro j hj₁ hj₂; split_ifs <;> try linarith;
        simp_all +decide [ Nat.succ_sub ( by linarith : K ≤ j ), mul_div_mul_right, ne_of_gt ( h_n_pos _ ( by linarith : K < K + 1 ) ) ];
...
lemma n_ge_a_seq (lambda : ℝ) (n : ℕ → ℕ)
  ...
  ∀ i, n i ≥ a_seq lambda i := by
    ...
    · have := h_lac i; rw [ le_div_iff₀ ( Nat.cast_pos.mpr ( h_n_pos _ ) ) ] at this; norm_num [ a_seq ] at *;
      exact le_trans ( mul_le_mul_of_nonneg_left ( mod_cast IH i ( Nat.lt_succ_self i ) ) ( by positivity ) ) this
  ...
  0 < n_seq_thm2 lambda K h_lambda hK i := by
    ...
    have h_neg : ∀ i, n_seq_thm2 lambda K h_lambda hK i = 0 → False := by
      ...
      · unfold base_n at hi;
        ...
        have := this.2.2.1 ( show 0 ≤ base_M lambda K h_lambda hK - 1 from Nat.zero_le _ ) ( show i ≤ base_M lambda K h_lambda hK - 1 from by linarith ) ; simp_all +decide [ Nat.dvd_iff_mod_eq_zero ] ;
        rename_i h; have := h.1 0 ( by linarith ) ; simp_all +decide [ a_seq ] ;
    ...
    have h_block : ∀ i, base_M lambda K h_lambda hK - 1 < i → i < m_seq_thm2 lambda K h_lambda hK (k_of_index_thm2 lambda K h_lambda hK i) → n_seq_thm2 lambda K h_lambda hK (i + 1) > n_seq_thm2 lambda K h_lambda hK i := by
      ...
      have h_block : n_seq_thm2 lambda K h_lambda hK (i + 1) = d_at lambda (k_of_index_thm2 lambda K h_lambda hK i) (i + 1 - m_seq_thm2 lambda K h_lambda hK (k_of_index_thm2 lambda K h_lambda hK i - 1)) * n_seq_thm2 lambda K h_lambda hK (m_seq_thm2 lambda K h_lambda hK (k_of_index_thm2 lambda K h_lambda hK i - 1)) := by
        have h_block : k_of_index_thm2 lambda K h_lambda hK (i + 1) = k_of_index_thm2 lambda K h_lambda hK i := by
          ...
          exact this.1.not_gt ( lt_of_le_of_lt ( m_seq_thm2_strictMono _ _ _ _ |> StrictMono.monotone <| Nat.le_sub_one_of_lt h_lt ) h_block.1 ) ;))))
        generalize_proofs at *; (
    ...
    · by_cases hi : i < base_M lambda K h_lambda hK - 1 <;> simp_all +decide [ Nat.succ_le_iff ];
      ...
      · -- By definition of $k_of_index_thm2$, we know that $m_seq_thm2 k \geq i$ and $m_seq_thm2 (k - 1) < i$.
        ...
        have hi_gt : i > m_seq_thm2 lambda K h_lambda hK (k - 1) := by
          exact lt_of_le_of_ne ( Nat.le_of_lt_succ hk.2 ) ( Ne.symm hi ) ;
        generalize_proofs at *; (
  ...
  n_seq_thm2 lambda K h_lambda hK (m_seq_thm2 lambda K h_lambda hK k) ∣ n_seq_thm2 lambda K h_lambda hK (m_seq_thm2 lambda K h_lambda hK (k + 1)) := by
    ...
    · rw [ show k_of_index_thm2 lambda K h_lambda hK ( m_seq_thm2 lambda K h_lambda hK k + M_at lambda ( k + 1 ) ) = k + 1 from ?_ ];
      ...
      · refine' le_antisymm _ _ <;> norm_num [ k_of_index_thm2 ];
        ...
        · split_ifs <;> norm_num at * ; linarith [ Nat.find_spec ( show ∃ k, m_seq_thm2 lambda K h_lambda hK k ≥ m_seq_thm2 lambda K h_lambda hK k + M_at lambda ( k + 1 ) from ⟨ k + 1, by linarith [ show M_at lambda ( k + 1 ) > 0 from Nat.pos_of_ne_zero ( by
          grind ) ] ⟩ ) ];
          intro m hm; linarith [ show m_seq_thm2 lambda K h_lambda hK m ≤ m_seq_thm2 lambda K h_lambda hK k from by exact monotone_nat_of_le_succ ( fun n => by exact Nat.le_add_right _ _ ) ( Nat.le_of_lt_succ hm ) ] ;
    ...
    · have h_eq : k_of_index_thm2 lambda K h_lambda hK (m_seq_thm2 lambda K h_lambda hK k) = k ∧ k_of_index_thm2 lambda K h_lambda hK (m_seq_thm2 lambda K h_lambda hK k + M_at lambda (k + 1)) = k + 1 := by
        ...
        · refine' le_antisymm _ _ <;> simp_all +decide [ k_of_index_thm2 ];
          ...
          · split_ifs <;> simp_all +decide ;
            · grind;
            · intro m hm; exact lt_add_of_le_of_pos ( monotone_nat_of_le_succ ( fun n => by exact le_of_lt ( m_seq_thm2_strictMono _ _ _ _ |> StrictMono.lt_iff_lt |>.2 <| Nat.lt_succ_self _ ) ) <| Nat.le_of_lt_succ hm ) <| Nat.pos_of_ne_zero <| by
                exact ne_of_gt <| M_at_pos _ h_lambda _ <| Nat.succ_pos _;
      ...
      have h_subst : n_seq_thm2 lambda K h_lambda hK j = d_at lambda (k + 1) (j - m_seq_thm2 lambda K h_lambda hK k) * n_seq_thm2 lambda K h_lambda hK (m_seq_thm2 lambda K h_lambda hK k) ∧ n_seq_thm2 lambda K h_lambda hK (m_seq_thm2 lambda K h_lambda hK (k + 1)) = d_at lambda (k + 1) (M_at lambda (k + 1)) * n_seq_thm2 lambda K h_lambda hK (m_seq_thm2 lambda K h_lambda hK k) := by
        ...
        · rw [ n_seq_thm2 ];
          ...
          · rw [ show k_of_index_thm2 lambda K h_lambda hK ( m_seq_thm2 lambda K h_lambda hK k + M_at lambda ( k + 1 ) ) = k + 1 from ?_ ] ; simp +decide ; ring_nf;
            ...
            · grind;
            · exact ⟨ by rw [ show m_seq_thm2 lambda K h_lambda hK ( k + 1 ) = m_seq_thm2 lambda K h_lambda hK k + M_at lambda ( k + 1 ) from rfl ], fun n hn => lt_of_le_of_lt ( m_seq_thm2_strictMono lambda K h_lambda hK |> StrictMono.monotone <| Nat.le_of_lt_succ hn ) <| Nat.lt_add_of_pos_right <| Nat.pos_of_ne_zero <| by
                                exact ne_of_gt <| M_at_pos _ h_lambda _ <| Nat.succ_pos _ ⟩;
    ...
    · -- If $j = m_k$, then $n_{m_k} \mid n_{m_{k+1}}$ by `n_seq_thm2_div_prev_m`.
      ...
      · contrapose! hk; simp_all +decide [ not_or ] ;
        ...
        · split_ifs <;> simp_all +decide ;
          ...
          · intro m hm; exact lt_of_le_of_lt ( m_seq_thm2_strictMono _ _ _ _ |> StrictMono.monotone <| Nat.le_of_lt_succ hm ) ( lt_of_le_of_ne hj₁ <| by tauto ) ;
    ...
    · -- By definition of $super_Q_thm2$, we have $super_Q_thm2 (k + 1) = Q_seq (k + 1) * super_Q_thm2 k$.
      ...
      have h_n_seq_succ : n_seq_thm2 lambda K h_lambda hK (m_seq_thm2 lambda K h_lambda hK (k + 1)) = d_at lambda (k + 1) (M_at lambda (k + 1)) * n_seq_thm2 lambda K h_lambda hK (m_seq_thm2 lambda K h_lambda hK k) := by
        ...
        · rw [ show k_of_index_thm2 lambda K h_lambda hK ( m_seq_thm2 lambda K h_lambda hK k + M_at lambda ( k + 1 ) ) = k + 1 from ?_ ] ; norm_num [ Nat.sub_sub ];
          ...
          · simp +zetaDelta at *;
            exact fun h => absurd h ( not_le_of_gt ( lt_of_le_of_lt ( m_seq_thm2_strictMono _ _ _ _ |> StrictMono.monotone <| Nat.le_of_lt_succ this ) <| Nat.lt_add_of_pos_right <| Nat.pos_of_ne_zero <| by
              exact ne_of_gt ( M_at_pos _ h_lambda _ ( Nat.succ_pos _ ) ) ) );
...
lemma thm3_seq_v2_large_jumps (Lambda : ℝ) (lambda : ℝ) (h_Lambda : Lambda ≥ 2) (h_lambda : 1 < lambda ∧ lambda < Lambda / (Lambda - 1)) (k : ℕ) :
  ...
  (n_seq_thm3_final_v2 Lambda lambda h_Lambda h_lambda m_k : ℝ) > Lambda * n_seq_thm3_final_v2 Lambda lambda h_Lambda h_lambda (m_k - 1) := by
    have h_n_seq_v2_mono : StrictMono (n_seq_thm3_final_v2 Lambda lambda h_Lambda h_lambda) := by
      exact n_seq_thm3_final_v2_mono Lambda lambda h_Lambda h_lambda
    generalize_proofs at *; (
    unfold n_seq_thm3_final_v2 at *; simp_all +decide ; (
    split_ifs <;> simp_all +decide [m_seq_thm3_final_v2] ; (
    have := step_thm3_strong_v2_props Lambda lambda ( U_thm3 Lambda lambda h_Lambda h_lambda ) k ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.m ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.n ) h_Lambda ⟨ h_lambda.1, lambda_lt_two Lambda lambda h_Lambda h_lambda.2 ⟩ ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_m ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_pos ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_mono ) ; simp_all +decide ; (
    unfold extend_temp at *; simp_all +decide ; (
    rw [ show ( Nat.find ‹∃ k_1, m_seq_thm3_final_v2 Lambda lambda h_Lambda h_lambda k - 1 < ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k_1 ).m› ) = k from ?_ ] at * ; simp_all +decide [m_seq_thm3_final_v2] ; (
    rw [ show Nat.find ‹∃ k_1, m_seq_thm3_final_v2 Lambda lambda h_Lambda h_lambda k < ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k_1 ).m› = k + 1 from ?_ ] ; simp_all +decide [m_seq_thm3_final_v2] ; (
    rw [ show ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda ( k + 1 ) ).n ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k ).m = jump_val Lambda ( ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k ).n ( ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k ).m - 1 ) ) from ?_ ] ; exact_mod_cast Nat.lt_of_floor_lt ( Nat.lt_succ_self _ ) ;
    convert this.2.1 ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.m ) ( by linarith ) using 1 ; simp +decide ; (
    exact rfl));
    refine' le_antisymm _ _ <;> simp_all +decide ;
    · exact ⟨ k + 1, le_rfl, by linarith! [ this.1, show ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda ( k + 1 ) ).m = ( step_thm3_strong_v2 Lambda lambda ( U_thm3 Lambda lambda h_Lambda h_lambda ) k ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k ).m ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k ).n h_Lambda ⟨ h_lambda.1, lambda_lt_two Lambda lambda h_Lambda h_lambda.2 ⟩ ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_m ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_pos ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_mono ) ).1 from rfl ] ⟩ ;
    · exact fun m mn => by exact monotone_nat_of_le_succ ( fun n => by exact Nat.le_of_lt ( show ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda n |> Thm3Data.m ) < ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda ( n + 1 ) |> Thm3Data.m ) from by
                                                                                              have := step_thm3_strong_v2_props Lambda lambda ( U_thm3 Lambda lambda h_Lambda h_lambda ) n ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda n |> Thm3Data.m ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda n |> Thm3Data.n ) h_Lambda ⟨ h_lambda.1, lambda_lt_two Lambda lambda h_Lambda h_lambda.2 ⟩ ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda n |> Thm3Data.h_m ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda n |> Thm3Data.h_pos ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda n |> Thm3Data.h_mono ) ; simp_all +decide [ m_seq_thm3_final_v2 ] ; (
                                                                                              exact lt_of_lt_of_le ( by omega ) ( this.1 ) |> lt_of_lt_of_le <| by rfl;) ) ) ( Nat.le_of_lt_succ mn ) ;);
    rw [ Nat.find_eq_iff ] ; simp_all +decide [ m_seq_thm3_final_v2 ] ; (
    exact ⟨ Nat.pos_of_ne_zero ( by
      grind ), fun n hn => Nat.le_sub_one_of_lt ( by
      exact ( show StrictMono ( fun k => ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.m ) ) from by
                exact strictMono_nat_of_lt_succ fun k => by
                  have := step_thm3_strong_v2_props Lambda lambda ( U_thm3 Lambda lambda h_Lambda h_lambda ) k ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.m ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.n ) h_Lambda ⟨ h_lambda.1, lambda_lt_two Lambda lambda h_Lambda h_lambda.2 ⟩ ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_m ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_pos ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_mono ) ; simp_all +decide ; (
                  exact lt_of_lt_of_le ( by linarith ) ( this.1 ) |> lt_of_lt_of_le <| by rfl;) ) hn ) ⟩))));
    · rename_i h₁ h₂; have := h₂ ( k + 1 ) ; simp_all +decide ; (
      exact absurd ( h₂ ( k + 1 ) ) ( by linarith! [ m_seq_thm3_final_v2 ( Lambda := Lambda ) ( lambda := lambda ) ( h_Lambda := h_Lambda ) ( h_lambda := h_lambda ) ( k + 1 ), m_seq_thm3_final_v2 ( Lambda := Lambda ) ( lambda := lambda ) ( h_Lambda := h_Lambda ) ( h_lambda := h_lambda ) k, show ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda ( k + 1 ) |> Thm3Data.m ) > ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.m ) from by
                                                                                                                                                                                                                                                                                                    exact step_thm3_strong_v2_props Lambda lambda ( U_thm3 Lambda lambda h_Lambda h_lambda ) k ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.m ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.n ) h_Lambda ( ⟨ h_lambda.1, lambda_lt_two Lambda lambda h_Lambda h_lambda.2 ⟩ ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_m ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_pos ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_mono ) |> fun h => h.1.trans_lt' ( by linarith [ thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_m ] ) ; ] ) ;);
    · grind;
    · rename_i h₁ h₂; specialize h₁ ( k + 1 ) ; specialize h₂ ( k + 1 ) ; ( have := h_n_seq_v2_mono ( Nat.lt_succ_self k ) ; ( have := h_n_seq_v2_mono ( Nat.lt_succ_self ( k + 1 ) ) ; ( simp_all +decide ; ) ) ) ;
      exact absurd h₁ ( not_le_of_gt ( Nat.lt_of_lt_of_le ( Nat.sub_lt ( Nat.succ_le_of_lt ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |>.h_m ) ) zero_lt_one ) ( Nat.le_of_lt_succ ( by
        exact Nat.le_of_lt_succ ( by linarith [ show ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda ( k + 1 ) |> Thm3Data.m ) > ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.m ) from by
                                                  exact step_thm3_strong_v2_props Lambda lambda ( U_thm3 Lambda lambda h_Lambda h_lambda ) k ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.m ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.n ) h_Lambda ⟨ h_lambda.1, lambda_lt_two Lambda lambda h_Lambda h_lambda.2 ⟩ ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_m ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_pos ) ( thm3_seq_v2 Lambda lambda h_Lambda h_lambda k |> Thm3Data.h_mono ) |>.1 |> lt_of_lt_of_le ( by linarith [ U_thm3 Lambda lambda h_Lambda h_lambda ] ) ; ] ) ; ) ) ) ) ;))
...
lemma extended_seq_growth_v3 (S : Set ℕ) (K : ℕ) (b : ℕ) (hS : S_cond S) (hb : b > 0) (hK : K ≥ 1) (i : ℕ) :
  extended_seq_v3 S K b hS hb (i + 1) ≤ 2 * extended_seq_v3 S K b hS hb i := by
    ...
    · split_ifs <;> try omega;
      ...
      · simp_all +decide ;
        rw [ show i + 1 - ( K + m_val_v3 S K b hS hb - 1 ) = 0 by omega ] ; norm_num ; ring_nf ;
  ...
  SubsetSums (fun i => (1 : ℝ) / (Nat.nth (· ∈ S) i)) ⊆
    (TargetInterval (fun i => (1 : ℝ) / (Nat.nth (· ∈ S) i))) ∩ {x | ∃ q : ℚ, x = q} := by
      ...
      have h_interval : x ∈ TargetInterval (fun i => (1 : ℝ) / Nat.nth (· ∈ S) i) := by
        ...
        · exact Set.mem_Ici.mpr <| Finset.sum_nonneg fun _ _ => inv_nonneg.mpr <| Nat.cast_nonneg _)
  ...
  StrictMono (extended_seq_v3 S K b hS hb) := by
    ...
    · split_ifs <;> try linarith [ show enum_S S ( n + 1 ) > enum_S S n from Nat.nth_strictMono ( show Set.Infinite S from S_infinite S hS ) ( by linarith ) ] ;
      ...
      · have h_enum_lt : enum_S S n < enum_S S (J_idx_v3 S K b hS hb) := by
        ...
        exact lt_of_lt_of_le h_enum_lt ( Nat.le_mul_of_pos_left _ ( pow_pos ( by decide ) _ ) );
    ...
    · split_ifs <;> try omega;
      ...
      · have h_exp : 2 ^ (m_val_v3 S K b hS hb - 1) * enum_S S (K - 1) < enum_S S (J_idx_v3 S K b hS hb) := by
        ...
        generalize_proofs at *; (
        refine' lt_of_le_of_lt _ ( mul_lt_mul_of_pos_left h_exp ( pow_pos ( by decide ) _ ) );
        rw [ ← mul_assoc, ← pow_add ];
        exact Nat.mul_le_mul_right _ ( pow_le_pow_right₀ ( by decide ) ( by omega ) ));
