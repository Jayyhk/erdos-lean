...
Erdős Problem 442. Is it true that if `A ⊆ ℕ` satisfies
`(1 / log log x) * ∑_{n ∈ A ∩ [1,x)} 1/n → ∞`, then
`(∑_{n ∈ A ∩ [1,x)} 1/n)^{-2} * ∑_{a < b ∈ A ∩ (1,x]} 1/lcm(a,b) → ∞`? `erdos_442`
disproves this.
...
`src/latest/ErdosProblems/Erdos442.lean` together with the modules of
`src/latest/ErdosProblems/Erdos442/`. Those files are concatenated here in dependency order,
...
`namespace Erdos442`, the upstream trust-base print line and trailing `alias` removed, and the
final theorem renamed from `not_erdos_442` to `erdos_442`. No mathematical content is changed.
-/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos469.lean` -/
...
instance : Fintype LowFrontierChildKind where
  elems := {.terminalHit, .laterLow, .largeCandidate, .largeForced}
...
theorem expandedRootCharge_rawRootKeyEncoding (key : DecoratedRootKey) :
  ...
  change (if key.c.primeFactors ∈ key.w.primeFactors.powerset then
      rootChargeChoice key.w key.a key.c.primeFactors else 0) = _
...
theorem finitePrimeS_eq_classical_add_correction (x : Nat) :
  ...
  rw [finitePrimeS, ← Finset.sum_add_distrib]
  apply Finset.sum_congr rfl
  intro p hp
  simp only [primeLogCorrection]
  ring
...
private theorem _root_.Nat.Prime.dvd_finset_prod {p : Nat} (hp : p.Prime)
...
theorem squarefree_abundancyIndex_injective :
  ...
  have hidxA : a.abundancyIndex = A.prod primeEulerFactor := by
    ...
    calc
      ...
      _ = A.prod (fun p => 1 + ((p : Rat)⁻¹)) := by
        simpa only [primeSetProduct] using
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos442.lean` -/

section
/- leanprover/lean4:v4.33.0  mathlib v4.33.0 -/
/-
This is a Lean formalization of a solution to Erdős Problem 442.
https://www.erdosproblems.com/forum/thread/442
...
URLs:
- https://github.com/plby/lean-proofs/blob/main/ErdosProblems/Erdos442.md
- https://github.com/google-deepmind/formal-conjectures/blob/main/FormalConjectures/ErdosProblems/442.lean
-/
...
#print axioms erdos_442
-- 'Erdos442.erdos_442' depends on axioms: [propext, Classical.choice, Quot.sound]
