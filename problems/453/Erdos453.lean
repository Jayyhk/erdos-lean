...
/-- **Erdős Problem 453** (Erdős–Straus). Is it true that for all sufficiently large `n`
there exists some `i < n` with `pₙ² < p_{n+i}·p_{n−i}`, where `p_k = Nat.nth Nat.Prime k`
is the `k`-th prime (0-indexed)? The answer is **no** (Pomerance): there are infinitely
many `n` with `pₙ² > p_{n+i}·p_{n−i}` for all `i`. -/
theorem erdos_453 :
    ¬ ∃ N : ℕ, ∀ n : ℕ, n ≥ N →
        ∃ i : ℕ, i < n ∧
