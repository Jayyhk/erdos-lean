...
Erdős Problem 310. Let `α > 0` and `N ≥ 1`. Is it true that for any `A ⊆ {1, …, N}` with
`|A| ≥ α N` there is some `S ⊆ A` with

  `a / b = ∑_{n ∈ S} 1/n`,  where `a ≤ b = O_α(1)`?

`erdos_310` proves that it is. Liu and Sawhney observed that Bloom's theorem on unit
fractions implies this; the analytic input here is that theorem, from the repository's
`UnitFractions` development.

`b = O_α(1)` is rendered by the quantifier order: `C` is chosen after `α` but before `N`
and `A`, so it depends on `α` alone. Two guards make the statement non-vacuous: `S` is
required to be `Nonempty` and `a` to satisfy `1 ≤ a`, which together rule out the empty
subset and its zero reciprocal sum. The sum is taken exactly in `ℚ`.
-/
...
scoped[Chebyshev] notation "ϑ" => Erdos310.chebyshev_first
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos310.lean` -/
...
#print axioms erdos_310
-- 'Erdos310.erdos_310' depends on axioms: [propext, Classical.choice, Quot.sound]
