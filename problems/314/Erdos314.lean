...
/-! ## Erdős Problem 314: `liminf n²·ε(n) = 0` -/

open Filter Topology

/-- The harmonic numbers tend to infinity. -/
lemma harmonicReal_tendsto_atTop : Tendsto harmonicReal atTop atTop := by
  refine Real.tendsto_sum_range_one_div_nat_succ_atTop.congr (fun n => ?_)
  simp [harmonicReal, one_div]

/-- For `n ≥ 1`, some block `[n, m]` of reciprocals reaches `1` (harmonic divergence). -/
lemma exists_block_ge_one {n : ℕ} (hn : 1 ≤ n) : ∃ m, 1 ≤ harmonicPartialSum n m := by
  obtain ⟨m, hm1, hm2⟩ :=
    ((harmonicReal_tendsto_atTop.eventually_ge_atTop (harmonicReal (n - 1) + 1)).and
      (eventually_ge_atTop n)).exists
  exact ⟨m, by rw [harmonicPartialSum_eq_diff hn hm2]; linarith⟩

/-- `m(n)`: the minimal `m` with `∑_{n ≤ k ≤ m} 1/k ≥ 1`. -/
noncomputable def mMin (n : ℕ) : ℕ := sInf {m | 1 ≤ harmonicPartialSum n m}

/-- `ε(n) = (∑_{n ≤ k ≤ m(n)} 1/k) − 1`, the overshoot of the minimal block reaching `1`. -/
noncomputable def eps (n : ℕ) : ℝ := harmonicPartialSum n (mMin n) - 1

lemma one_le_block_mMin {n : ℕ} (hn : 1 ≤ n) : 1 ≤ harmonicPartialSum n (mMin n) :=
  Nat.sInf_mem (exists_block_ge_one hn)

lemma eps_nonneg {n : ℕ} (hn : 1 ≤ n) : 0 ≤ eps n := by
  have := one_le_block_mMin hn; simp only [eps]; linarith

/-- Extending the upper endpoint can only increase the block sum. -/
lemma harmonicPartialSum_mono_right (n : ℕ) {m m' : ℕ} (h : m ≤ m') :
    harmonicPartialSum n m ≤ harmonicPartialSum n m' := by
  simp only [harmonicPartialSum]
  refine Finset.sum_le_sum_of_subset_of_nonneg (Finset.Icc_subset_Icc_right h) ?_
  intro i _ _; positivity

lemma eps_le_of_block {n m : ℕ} (h1 : 1 ≤ harmonicPartialSum n m) {S : ℝ}
    (hS : harmonicPartialSum n m ≤ 1 + S) : eps n ≤ S := by
  have hle : mMin n ≤ m := Nat.sInf_le h1
  have := harmonicPartialSum_mono_right n hle
  simp only [eps]; linarith

/-- For every `c > 0`, the quantity `n²·ε(n)` is `≤ c` infinitely often (Lim–Steinerberger). -/
lemma freq_le {c : ℝ} (hc : 0 < c) :
    ∃ᶠ (n : ℕ) in atTop, (n : ℝ) ^ 2 * eps n ≤ c := by
  rw [frequently_atTop]
  intro N
  obtain ⟨m, n, hnN, h1, h2⟩ := main_theorem c hc (max N 1)
  have hn1 : 1 ≤ n := by omega
  have hnpos : (0 : ℝ) < (n : ℝ) ^ 2 := by positivity
  have heps : eps n ≤ c / (n : ℝ) ^ 2 := eps_le_of_block h1 h2
  rw [le_div_iff₀ hnpos] at heps
  refine ⟨n, ?_, ?_⟩
  · omega
  · nlinarith [heps]

/-- `n²·ε(n)` is bounded below (by `0`). -/
lemma bddBelow_eps :
    IsBoundedUnder (· ≥ ·) atTop (fun n : ℕ => (n : ℝ) ^ 2 * eps n) := by
  refine ⟨0, ?_⟩
  rw [eventually_map]
  filter_upwards [eventually_ge_atTop 1] with n hn
  exact mul_nonneg (by positivity) (eps_nonneg hn)

/-- `n²·ε(n)` is cobounded from above in the `liminf` sense. -/
lemma cobounded_eps :
    IsCoboundedUnder (· ≥ ·) atTop (fun n : ℕ => (n : ℝ) ^ 2 * eps n) := by
  refine ⟨0, fun a ha => ?_⟩
  simp only [eventually_map, ge_iff_le, eventually_atTop] at ha
  obtain ⟨N₀, hN₀⟩ := ha
  by_contra hpos
  push_neg at hpos
  have hf := freq_le (show (0 : ℝ) < a / 2 by linarith)
  rw [frequently_atTop] at hf
  obtain ⟨n, hnN₀, hle⟩ := hf N₀
  have hge := hN₀ n hnN₀
  linarith

/-- **Erdős Problem 314.** With `ε(n) = (∑_{n ≤ k ≤ m(n)} 1/k) − 1`, where `m(n)` is the
minimal `m` with block sum `≥ 1`, one has `liminf n²·ε(n) = 0` (Lim–Steinerberger). -/
theorem erdos_314 :
    Filter.liminf (fun n : ℕ => (n : ℝ) ^ 2 * eps n) atTop = 0 := by
  refine le_antisymm ?_ ?_
  · refine le_of_forall_pos_le_add (fun c hc => ?_)
    calc Filter.liminf (fun n : ℕ => (n : ℝ) ^ 2 * eps n) atTop
        ≤ c := liminf_le_of_frequently_le (freq_le hc) bddBelow_eps
      _ = 0 + c := by ring
