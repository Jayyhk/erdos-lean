...
lemma merged_component_closure (n m : ℕ) (G₁ : SimpleGraph (Fin (n + 2)))
  ...
  · obtain ⟨i, j, rfl, rfl, hij⟩ := h₂
    ...
    rcases hu with hu | hu <;>
      simp_all only [Set.mem_union, Set.mem_image, (mergeEmb₂_injective n m).eq_iff, exists_eq_right]
    · obtain ⟨x, hx, hx'⟩ := hu
...
/-- **Erdős 150.** The limit `α = lim c(n)^{1/n}` exists and satisfies
`α ≤ 2^{H₂(1/3)} = 1.8899… < 2`, where `c(n)` is the maximum number of minimal
vertex cuts of a graph on `n` vertices (Bradač's solution). -/
theorem erdos_150 :
...
-- 'Erdos150.erdos_150' depends on axioms: [propext, Classical.choice, Quot.sound]
