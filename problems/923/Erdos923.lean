...
theorem not_colorable_of_le_chromaticNumber {V : Type*} {G : SimpleGraph V} {n : ℕ}
  ...
  absurd (h.trans hc.chromaticNumber_le) (by push_neg; exact_mod_cast Nat.lt_succ_of_le le_rfl)
...
/-- **Erdős Problem 923** (Rödl 1977).
For every `n` there exists `f(n)` such that every graph `G` with `χ(G) ≥ f(n)`
contains a triangle-free subgraph `H` with `χ(H) ≥ n`. -/
theorem erdos_923 (n : ℕ) :
...
-- 'Erdos923.erdos_923' depends on axioms: [propext, Classical.choice, Quot.sound]
