...
rainbow arithmetic progression (i.e. all elements are different colours). Is it true that
$$H(k)^{1/k}/k \to \infty$$
as $k \to \infty$?

The answer is **yes**, proved by J. H. Bae. `erdos_190` states exactly that, with `H` defined as
`sInf {N | 0 < N ∧ Good k N}`. The existence of a canonical `N` is *proved* here
(`good_nonempty`), not assumed, so `H` really is the canonical van der Waerden number; and the
statement is self-guarding, since an empty defining set would give `H k = 0` and a normalised
root tending to `0` rather than `atTop`.

The argument needs a prime in the window $(19k/20, k)$, supplied by `prime_between`, a
consequence of the prime number theorem that Mathlib does not carry. Its proof is vendored here
from `AlexKontorovich/PrimeNumberTheoremAnd` (the closure of `Consequences.lean`: the
Wiener--Ikehara chain and its supporting Fourier/Sobolev development), truncated after
`prime_between` and with `Architect`'s `@[blueprint]` attributes and `blueprint_comment`
commands removed, since that library is not importable here.
-/

/-! ### Vendored: `PrimeNumberTheoremAnd` — the prime number theorem chain for `prime_between` -/

/-! #### vendored: `PrimeNumberTheoremAnd/Mathlib/Analysis/SpecialFunctions/Log/Basic.lean` -/

section PNT_Mathlib_Analysis_SpecialFunctions_Log_Basic
...
end PNT_Mathlib_Analysis_SpecialFunctions_Log_Basic

/-! #### vendored: `PrimeNumberTheoremAnd/Sobolev.lean` -/
...
/-! #### vendored: `PrimeNumberTheoremAnd/Fourier.lean` -/
...
/-! #### vendored: `PrimeNumberTheoremAnd/Defs.lean` -/
...
/-! #### vendored: `PrimeNumberTheoremAnd/Mathlib/Analysis/Asymptotics/Asymptotics.lean` -/

section PNT_Mathlib_Analysis_Asymptotics_Asymptotics
...
open _root_.Asymptotics in
private theorem _root_.Asymptotics.isLittleO_const_id_cocompact [ProperSpace F'']
...
open _root_.Asymptotics in
private theorem _root_.Asymptotics.isLittleO_const_id_atTop2 [LinearOrder F''] [NoMaxOrder F''] [ClosedIciTopology F'']
...
open _root_.Asymptotics in
private theorem _root_.Asymptotics.isLittleO_const_id_atBot2 [LinearOrder F''] [NoMinOrder F''] [ClosedIicTopology F'']
...
end PNT_Mathlib_Analysis_Asymptotics_Asymptotics

/-! #### vendored: `PrimeNumberTheoremAnd/Mathlib/Algebra/Notation/Support.lean` -/

section PNT_Mathlib_Algebra_Notation_Support
...
end PNT_Mathlib_Algebra_Notation_Support

/-! #### vendored: `PrimeNumberTheoremAnd/SmoothExistence.lean` -/

section PNT_SmoothExistence
...
end PNT_SmoothExistence

/-! #### vendored: `PrimeNumberTheoremAnd/Wiener.lean` -/
...
lemma second_fourier_integrable_aux1 (hcont : Measurable ψ) (hsupp : Integrable ψ) (hσ : 1 < σ') :
  ...
  · apply Measurable.aestronglyMeasurable
    -- `Real.fourierChar` now takes a plain `ℝ`, so the `Multiplicative.ofAdd` wrapper is
    -- definitionally redundant; `fun_prop` has no rule for it, so strip it with `show`.
    simp only [neg_mul, ofReal_exp, ofReal_neg, ofReal_mul, ofReal_sub, ofReal_one,
      smul_eq_mul]
    show Measurable (Function.uncurry fun (u : ℝ) (a : ℝ) =>
...
/- `prelim_decay_2`, `prelim_decay_3`, `AbsolutelyContinuous` and `decay_alt` are omitted:
upstream leaves the first two unproved as placeholders, and nothing in the closure of
`erdos_190` depends on them (the check at the foot of this file confirms it). -/
...
open _root_.Finset in
private lemma _root_.Finset.sum_shift_front' {E : Type*} [Ring E] {u : ℕ → E} :
...
open _root_.Finset in
private lemma _root_.Finset.sum_shift_back' {E : Type*} [Ring E] {u : ℕ → E} :
...
private lemma _root_.BoundedAtFilter.add_const {u : ℕ → ℝ} {c : ℝ} :
...
private lemma _root_.BoundedAtFilter.comp_add {u : ℕ → ℝ} {N : ℕ} :
...
theorem sum_le_integral {x₀ : ℝ} {f : ℝ → ℝ} {n : ℕ} (hf : AntitoneOn f (Ioc x₀ (x₀ + n)))
  ...
  have l1 : f (x₀ + 1) ≤ ∫ x in x₀..x₀ + 1, f x := by
    rw [← l6] ; apply intervalIntegral.integral_mono_ae_restrict (by linarith)
      intervalIntegrable_const l4
    apply eventually_of_mem _ l5
    have h₀ : (Ioc x₀ (x₀ + 1))ᶜ ∩ Icc x₀ (x₀ + 1) = {x₀} := by simp [← sdiff_eq_compl_inter]
    rw [mem_ae_iff, Measure.restrict_apply' measurableSet_Icc, h₀]
    simp
...
/-! #### vendored: `PrimeNumberTheoremAnd/Consequences.lean` -/

section PNT_Consequences
...
open _root_.Set in
private lemma _root_.Set.Ico_subset_Ico_of_Icc_subset_Icc {a b c d : ℝ} (h : Set.Icc a b ⊆ Set.Icc c d) :
...
end PNT_Consequences

/-! ### Upstream module `ErdosProblems/Erdos190.lean` (plby/lean-proofs) -/
...
-- 'Erdos190.erdos_190' depends on axioms: [propext, Classical.choice, Quot.sound]
