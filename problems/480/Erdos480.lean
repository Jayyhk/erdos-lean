...
Erdős Problem 480. Let `x₁, x₂, … ∈ [0,1]`. Is `inf_n liminf_{m→∞} n |x_{m+n} − x_m| ≤ 5^(-1/2)`? `erdos_480` proves this.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos480.lean`, a single self-contained file whose only import is
`Mathlib`. It is reproduced here with the file-level comment header, the upstream trust-base
print lines and the trailing `alias` removed, wrapped once in `namespace Erdos480`, and with the
final theorem named `erdos_480`. No mathematical content is changed.
-/
...
#print axioms erdos_480
-- 'Erdos480.erdos_480' depends on axioms: [propext, Classical.choice, Quot.sound]
