...
theorem erdos_619_solution : erdos_619_target := by
  intro hconj
...
`FC.erdos_619_solved_statement`) implies this repository's
...
lemma erdos_619_conjecture_of_fc
    (hfc : ∃ c > (0 : ℝ), ∀ (V : Type) [Fintype V] (G : SimpleGraph V),
      G.Connected → G.CliqueFree 3 →
      (FC.minNewEdges 4 G : ℝ) < (1 - c) * Fintype.card V) :
...
theorem erdos_619_fc_solution : FC.erdos_619_solved_statement :=
  iff_of_false id fun hfc =>
    erdos_619_solution (erdos_619_conjecture_of_fc hfc)

/-- **Erdős Problem 619.** The conjecture is **false** (resolved by Claude Fable 5, prompted by
Kuhn): there is **no** absolute constant `c > 0` such that for every connected triangle-free
graph `G` on `n` vertices, the minimum number `m = h₄(G)` of edges needed to extend `G` to a
triangle-free supergraph of diameter `≤ 4` is strictly less than `(1 - c)·n`. -/
theorem erdos_619 :
    ¬ ∃ c : ℝ, 0 < c ∧
      ∀ (n : ℕ) (G : SimpleGraph (Fin n)) (m : ℕ),
        G.Connected → G.CliqueFree 3 →
        (∃ H : SimpleGraph (Fin n),
          G ≤ H ∧ H.CliqueFree 3 ∧ H.ediam ≤ (4 : ℕ∞) ∧
          (H.edgeFinset \ G.edgeFinset).card = m ∧
          ∀ K : SimpleGraph (Fin n),
            G ≤ K → K.CliqueFree 3 → K.ediam ≤ (4 : ℕ∞) →
            m ≤ (K.edgeFinset \ G.edgeFinset).card) →
        (m : ℝ) < (1 - c) * n :=
  erdos_619_solution

#print axioms erdos_619
-- 'Erdos619.erdos_619' depends on axioms: [propext, Classical.choice, Quot.sound]
