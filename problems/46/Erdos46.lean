...
specialized to Erdős problem 46.
...
integers `≥ 2` whose reciprocals sum to 1. It follows from Erdős 298
...
lemma Finset.cast_lcm {x : Finset ℕ} : ((x.lcm id : ℕ) : ℤ) = x.lcm (fun n => (n : ℤ)) := by
  ...
  · intro a s ha hs
    simpa only [Finset.lcm_insert, id_eq] using
      hs ▸ (Erdos46.cast_lcm (x := a) (y := s.lcm id))
...
-- 'Erdos46.erdos_46' depends on axioms: [propext, Classical.choice, Quot.sound]
