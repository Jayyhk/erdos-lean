...
-/
theorem B_diff_disjoint (A : Set ℕ) (m : ℕ) (hm : 2 ≤ m) (i j : ℕ) (hij : i < j) (hj : j < m)
  (hA : IsSidonSetNat A) :
...
theorem sum_distinct_pos_ints_ge (S : Finset ℕ) (h_pos : ∀ x ∈ S, 0 < x) :
  ...
  have h_sum_ge : ∑ x ∈ Finset.image x Finset.univ, x ≥ ∑ i ∈ Finset.range (Finset.card S), (i + 1) := by
    rw [ Finset.sum_image <| by simp +decide [ hx.1.injective.eq_iff ] ] ; simpa only [ Finset.sum_range ] using Finset.sum_le_sum fun i _ => h_x_ge i;
...
theorem disjoint_diffs_le_s_set (A : Finset ℕ) (m : ℕ) (hm : 2 ≤ m) (s : ℕ → ℕ) (hA : IsSidonSetNat (A : Set ℕ))
  ...
  have h_diff_disjoint : ∀ (x y : ℕ), x ∈ B_finset_new A m i → y ∈ B_finset_new A m i → ∀ (z w : ℕ), z ∈ B_finset_new A m j → w ∈ B_finset_new A m j → x > y → z > w → |(x - y : ℤ)| ≠ |(z - w : ℤ)| := by
    ...
    have h_diff : (x - y : ℤ) ≠ (z - w : ℤ) := by
      ...
      · convert this ( lt_of_le_of_ne ‹_› hij ) x ( B_finset_new_eq_B A m i ( by linarith ) ▸ hx ) y ( B_finset_new_eq_B A m i ( by linarith ) ▸ hy ) z ( B_finset_new_eq_B A m j ( by linarith ) ▸ hz ) w ( B_finset_new_eq_B A m j ( by linarith ) ▸ hw ) ( mod_cast hxy ) ( mod_cast hzw ) using 1;
        grind;
  ...
  obtain ⟨x, y, hx, hy, hxy⟩ : ∃ x y : ℕ, x ∈ B_finset_new A m i ∧ y ∈ B_finset_new A m i ∧ x > y ∧ a = x - y := by
    ...
    · by_cases h : l + ( k + 1 ) < List.length ( Finset.sort ( fun x1 x2 => x1 ≤ x2 ) ( B_finset_new A m i ) ) <;> simp_all +decide;
      · exact Finset.mem_sort ( α := ℕ ) ( fun x1 x2 => x1 ≤ x2 ) |>.1 ( by aesop );
      · omega;
    · have h_mem : (Finset.sort (fun x1 x2 => x1 ≤ x2) (B_finset_new A m i))[l]?.getD 0 ∈ Finset.sort (fun x1 x2 => x1 ≤ x2) (B_finset_new A m i) := by
        by_cases hl₂ : l < List.length ( Finset.sort ( fun x1 x2 => x1 ≤ x2 ) ( B_finset_new A m i ) ) <;> simp_all +decide;
        omega;
      exact (Finset.mem_sort fun x1 x2 => x1 ≤ x2).mp h_mem;
    · have h_sorted : ∀ (l₁ l₂ : ℕ), l₁ < l₂ → l₁ < (B_finset_new A m i).card → l₂ < (B_finset_new A m i).card → (Finset.sort (· ≤ ·) (B_finset_new A m i))[l₁]?.getD 0 < (Finset.sort (· ≤ ·) (B_finset_new A m i))[l₂]?.getD 0 := by
        intros l₁ l₂ hl₁₂ hl₁ hl₂;
        have h_sorted : List.Pairwise (fun x y => x < y) (Finset.sort (· ≤ ·) (B_finset_new A m i)) := by
  ...
  obtain ⟨z, w, hz, hw, hzw⟩ : ∃ z w : ℕ, z ∈ B_finset_new A m j ∧ w ∈ B_finset_new A m j ∧ z > w ∧ a = z - w := by
    ...
    · by_cases h : l + ( k + 1 ) < List.length ( Finset.sort ( fun x1 x2 => x1 ≤ x2 ) ( B_finset_new A m j ) ) <;> simp_all +decide;
...
theorem sum_diffs_order_eq_complement (A : Finset ℕ) (s : ℕ) (hs : s < A.card) :
  ...
  · simp +decide [Finset.sum_range];
    ...
    have h_sorted : ∀ i j : Fin (A.card), i < j → (Finset.sort (· ≤ ·) A)[i]?.getD 0 ≤ (Finset.sort (· ≤ ·) A)[j]?.getD 0 := by
      intros i j hij;
      have h_sorted : List.Pairwise (· ≤ ·) (Finset.sort (· ≤ ·) A) := by
        exact Finset.sort_sorted _ _;
...
theorem sum_diffs_le_s_eq_sum_set (A : Finset ℕ) (s : ℕ) (hA : IsSidonSetNat A) (hs : s < A.card) :
  sum_diffs_le_s A s = ∑ x ∈ diffs_le_s_set A s, x := by
    ...
    · refine' Finset.sum_congr rfl fun v hv => _;
      ...
      exact h_inj ( by omega ) ( by omega ) ( by omega ) ( by simpa [ List.getElem?_eq_getElem ( show j + ( v + 1 ) < List.length ( Finset.sort ( fun x1 x2 => x1 ≤ x2 ) A ) from by simpa using by omega ), List.getElem?_eq_getElem ( show j < List.length ( Finset.sort ( fun x1 x2 => x1 ≤ x2 ) A ) from by simpa using by omega ), List.getElem?_eq_getElem ( show j' + ( v + 1 ) < List.length ( Finset.sort ( fun x1 x2 => x1 ≤ x2 ) A ) from by simpa using by omega ), List.getElem?_eq_getElem ( show j' < List.length ( Finset.sort ( fun x1 x2 => x1 ≤ x2 ) A ) from by simpa using by omega ) ] using h_eq );
    ...
    have h_diff_bound : ∀ k ∈ Finset.Ico (A.card - s) A.card, ∑ j ∈ Finset.range (A.card - k), (((A.sort (· ≤ ·)).get? (j + k)).getD 0 - ((A.sort (· ≤ ·)).get? j).getD 0) ≤ (A.card - k) * M := by
      ...
      have h_diff_bound : ∀ j ∈ Finset.range (A.card - k), (((A.sort (· ≤ ·)).get? (j + k)).getD 0 - ((A.sort (· ≤ ·)).get? j).getD 0) ≤ M := by
        ...
        have h_diff_bound : ∀ x ∈ A, x ≤ M := by
          assumption;
        by_cases h : j + k < List.length ( Finset.sort ( fun x1 x2 => x1 ≤ x2 ) A ) <;> by_cases h' : j < List.length ( Finset.sort ( fun x1 x2 => x1 ≤ x2 ) A ) <;> simp_all +decide;
...
theorem S_subset_lower_bound (A : Finset ℕ) (m : ℕ) (hm : 2 ≤ m) (s : ℕ → ℕ)
  ...
  2 * (∑ x ∈ total_diffs_subset A m s I, x : ℝ) > (∑ i ∈ I, ((B_finset_new A m i).card : ℝ) * (Real.sqrt (B_finset_new A m i).card - 2)) ^ 2 := by
    ...
    have h_term_bound : ∀ x ∈ total_diffs_subset A m s I, x ≥ 1 := by
      ...
      have h_sorted : ∀ i j : ℕ, i < j → i < (B_finset_new A m y).card → j < (B_finset_new A m y).card → (Finset.sort (fun x1 x2 => x1 ≤ x2) (B_finset_new A m y))[i]?.getD 0 < (Finset.sort (fun x1 x2 => x1 ≤ x2) (B_finset_new A m y))[j]?.getD 0 := by
        intros i j hij hi hj;
        have h_sorted : List.Pairwise (fun x1 x2 => x1 < x2) (Finset.sort (fun x1 x2 => x1 ≤ x2) (B_finset_new A m y)) := by
...
/-- Helper: the proportion of `A` in residue class `r mod m` (normalised by `|A|`, not `√N`)
converges to `1/m`. Combines `sidon_density_limit` (normalised by `√N`) with `h_card`. -/
private lemma residue_ratio_tendsto
    (m : ℕ) (hm : 2 ≤ m)
    (n_seq : ℕ → ℕ) (A_seq : ℕ → Finset ℕ)
    (h_n_tendsto : Filter.Tendsto (fun k => (n_seq k : ℝ)) Filter.atTop Filter.atTop)
    (h_subset : ∀ k, ∀ x ∈ A_seq k, x ≤ n_seq k)
    ...
    (r : ℕ) (hr : r < m) :
    Filter.Tendsto
      (fun k => (((A_seq k).filter (fun a => a % m = r)).card : ℝ) / ((A_seq k).card : ℝ))
      Filter.atTop (nhds (1 / m)) := by
  have hdens := sidon_density_limit m hm n_seq A_seq h_n_tendsto h_subset h_sidon h_card r hr
  -- c_r/|A| = (c_r/√N) / (|A|/√N); numerator → 1/m, denominator → 1.
  have hquot := hdens.div h_card (by norm_num)
  rw [div_one] at hquot
  refine Filter.Tendsto.congr' ?_ hquot
  filter_upwards [h_card.eventually_ne (by norm_num : (1:ℝ) ≠ 0),
    (h_n_tendsto.eventually_gt_atTop 0)] with k hAcard hNpos
  simp only [Pi.div_apply]
  have hsqrt : Real.sqrt (n_seq k) ≠ 0 := by positivity
  have hAne : ((A_seq k).card : ℝ) ≠ 0 := by
    intro h
    apply hAcard
    rw [h, zero_div]
  field_simp

/-! ## §6 Bridge: `A + A` is well-distributed (the literal Erdős #154 statement)

Lindström's theorem (`sidon_density_limit`) gives equidistribution of `A` itself in
residue classes.  The forum note says the analogue for `A + A` "follows immediately using
the Sidon property".  We make that precise.  Writing `c r = #{a ∈ A : a ≡ r}`, the key
facts are:

* `pairCount_eq_conv` — the number of ordered pairs `(a,b) ∈ A × A` with `a + b ≡ i`
  is the convolution `∑_{r} c r · c ((i - r) mod m)`;
* `two_mul_upper_card` — for any predicate `Q` on the sum,
  `2 · #{a ≤ b : Q(a+b)} = #{(a,b) : Q(a+b)} + #{a : Q(2a)}` (off-diagonal symmetry);
* `sum_injOn_upper` / `sumset_eq_upper_image` — for a Sidon set the sum map is injective on
  `{(a,b) : a ≤ b}`, so `A + A` is in bijection with that upper triangle.

Together: `2 · #{s ∈ A+A : s ≡ i} = P_i + D_i` and `2 · |A+A| = |A|(|A|+1)`, where
`P_i / |A|² → 1/m` (convolution + Lindström) and `D_i / |A|² → 0` (diagonal is `≤ |A|`). -/

/-- Residue bookkeeping: if `a ≡ r (mod m)` then `a + b ≡ i` iff `b ≡ i - r (mod m)`. -/
private lemma mod_add_class (m i r a b : ℕ) (hm : 0 < m) (hi : i < m) (hr : r < m)
    (ha : a % m = r) : (a + b) % m = i ↔ b % m = (i + m - r) % m := by
  have hx : b % m < m := Nat.mod_lt b hm
  rw [Nat.add_mod, ha]
  -- goal: `(r + b % m) % m = i ↔ b % m = (i + m - r) % m`
  constructor
  · intro h
    have e1 : (r + (i + m - r)) % m = (r + b % m) % m := by
      rw [show r + (i + m - r) = i + m by omega, Nat.add_mod_right, Nat.mod_eq_of_lt hi, h]
    have e2 : (b % m) % m = (i + m - r) % m := (Nat.ModEq.add_left_cancel' r e1).symm
    rwa [Nat.mod_eq_of_lt hx] at e2
  · intro h
    rw [h]
    have hcancel : (r + (i + m - r) % m) % m = (r + (i + m - r)) % m :=
      Nat.ModEq.add_left r (Nat.mod_modEq _ _)
    rw [hcancel, show r + (i + m - r) = i + m by omega, Nat.add_mod_right, Nat.mod_eq_of_lt hi]

/-- The sumset is the image of the upper triangle `{(a,b) : a ≤ b}` under addition
(true for any finset: every sum `a + b` is realised with `a ≤ b`). -/
private lemma sumset_eq_upper_image (A : Finset ℕ) :
    A + A = ((A ×ˢ A).filter (fun p => p.1 ≤ p.2)).image (fun p => p.1 + p.2) := by
  ext s
...
/-- For a Sidon set, addition is injective on the upper triangle `{(a,b) : a ≤ b}`. -/
private lemma sum_injOn_upper (A : Finset ℕ) (hA : IsSidonSetNat (A : Set ℕ)) :
    Set.InjOn (fun p : ℕ × ℕ => p.1 + p.2)
      ↑((A ×ˢ A).filter (fun p => p.1 ≤ p.2)) := by
  intro p hp q hq hpq
  simp only [Finset.coe_filter, Finset.mem_product, Set.mem_setOf_eq] at hp hq
  obtain ⟨⟨hp1, hp2⟩, hple⟩ := hp
  obtain ⟨⟨hq1, hq2⟩, hqle⟩ := hq
  obtain ⟨e1, e2⟩ :=
    hA p.1 hp1 p.2 hp2 q.1 hq1 q.2 hq2 hple hqle hpq
  exact Prod.ext e1 e2

/-- Off-diagonal symmetry: `2 · (upper triangle) = (whole square) + (diagonal)`,
filtered by any predicate `Q` of the coordinate sum. -/
private lemma two_mul_upper_card (A : Finset ℕ) (Q : ℕ → Prop) [DecidablePred Q] :
    2 * ((A ×ˢ A).filter (fun p => p.1 ≤ p.2 ∧ Q (p.1 + p.2))).card
      = ((A ×ˢ A).filter (fun p => Q (p.1 + p.2))).card
        + (A.filter (fun a => Q (a + a))).card := by
  classical
  set up := (A ×ˢ A).filter (fun p => p.1 ≤ p.2 ∧ Q (p.1 + p.2)) with hup
  set lo := (A ×ˢ A).filter (fun p => p.2 ≤ p.1 ∧ Q (p.1 + p.2)) with hlo
  -- whole square = up ∪ lo
  have h_union : (A ×ˢ A).filter (fun p => Q (p.1 + p.2)) = up ∪ lo := by
    rw [hup, hlo, ← Finset.filter_or]
    refine Finset.filter_congr (fun p _ => ?_)
    constructor
    · intro hq; rcases le_total p.1 p.2 with h | h
      · exact Or.inl ⟨h, hq⟩
      · exact Or.inr ⟨h, hq⟩
    · rintro (⟨_, hq⟩ | ⟨_, hq⟩) <;> exact hq
  -- up ∩ lo = diagonal
  have h_inter : up ∩ lo = (A ×ˢ A).filter (fun p => p.1 = p.2 ∧ Q (p.1 + p.2)) := by
    rw [hup, hlo, ← Finset.filter_and]
    refine Finset.filter_congr (fun p _ => ?_)
    constructor
    · rintro ⟨⟨h1, hq⟩, h2, _⟩
      exact ⟨le_antisymm h1 h2, hq⟩
    · rintro ⟨he, hq⟩
      exact ⟨⟨he.le, hq⟩, he.ge, hq⟩
  -- swap of coordinates gives |lo| = |up|
  have h_swap : lo.card = up.card := by
    have himg : lo = up.image Prod.swap := by
      rw [hlo, hup]
      ext p; obtain ⟨a, b⟩ := p
      ...
      constructor
      · rintro ⟨⟨ha, hb⟩, hba, hq⟩
        exact ⟨b, a, ⟨⟨hb, ha⟩, hba, by rw [Nat.add_comm]; exact hq⟩, rfl, rfl⟩
      · rintro ⟨x, y, ⟨⟨hx, hy⟩, hxy, hq⟩, rfl, rfl⟩
        exact ⟨⟨hy, hx⟩, hxy, by rw [Nat.add_comm]; exact hq⟩
    rw [himg, Finset.card_image_of_injective _ Prod.swap_injective]
  -- diagonal is in bijection with `A`
  have h_diag : ((A ×ˢ A).filter (fun p => p.1 = p.2 ∧ Q (p.1 + p.2))).card
      = (A.filter (fun a => Q (a + a))).card := by
    have himg : (A ×ˢ A).filter (fun p => p.1 = p.2 ∧ Q (p.1 + p.2))
        = (A.filter (fun a => Q (a + a))).image (fun a => (a, a)) := by
      ext p; obtain ⟨a, b⟩ := p
      simp only [Finset.mem_filter, Finset.mem_product, Finset.mem_image, Prod.mk.injEq]
      constructor
      · rintro ⟨⟨ha, _hb⟩, rfl, hq⟩
        exact ⟨a, ⟨ha, hq⟩, rfl, rfl⟩
      · rintro ⟨x, ⟨hx, hq⟩, rfl, rfl⟩
        exact ⟨⟨hx, hx⟩, rfl, hq⟩
    rw [himg, Finset.card_image_of_injective _ (fun a b h => congrArg Prod.fst h)]
  -- combine via inclusion–exclusion
  have h_ce := Finset.card_union_add_card_inter up lo
  rw [← h_union, h_inter, h_diag, h_swap] at h_ce
  omega

/-- Convolution identity: the number of ordered pairs `(a,b) ∈ A × A` with
`a + b ≡ i (mod m)` equals `∑_{r < m} c r · c ((i - r) mod m)`, where
`c r = #{a ∈ A : a ≡ r}`. -/
private lemma pairCount_eq_conv (A : Finset ℕ) (m i : ℕ) (hm : 0 < m) (hi : i < m) :
    ((A ×ˢ A).filter (fun p => (p.1 + p.2) % m = i)).card
      = ∑ r ∈ Finset.range m,
          (A.filter (fun a => a % m = r)).card
            * (A.filter (fun a => a % m = (i + m - r) % m)).card := by
  rw [Finset.card_eq_sum_card_fiberwise
        (f := fun p => p.1 % m) (t := Finset.range m)
        (fun p _ => Finset.mem_range.mpr (Nat.mod_lt p.1 hm))]
  refine Finset.sum_congr rfl (fun r hr => ?_)
  rw [Finset.mem_range] at hr
  rw [← Finset.card_product, Finset.filter_filter]
  congr 1
  ...
  constructor
  · rintro ⟨hsum, har⟩
    exact ⟨har, (mod_add_class m i r a b hm hi hr har).mp hsum⟩
  · rintro ⟨har, hbσ⟩
    exact ⟨(mod_add_class m i r a b hm hi hr har).mpr hbσ, har⟩

/-- For a Sidon set, the residue-`Q` part of the sumset `A + A` is counted by the
corresponding part of the upper triangle `{(a,b) : a ≤ b}` (the sum map is injective there). -/
private lemma sumset_filter_card (A : Finset ℕ) (hA : IsSidonSetNat (A : Set ℕ))
    (Q : ℕ → Prop) [DecidablePred Q] :
    ((A + A).filter (fun s => Q s)).card
      = ((A ×ˢ A).filter (fun p => p.1 ≤ p.2 ∧ Q (p.1 + p.2))).card := by
  have key : (A + A).filter (fun s => Q s)
      = ((A ×ˢ A).filter (fun p => p.1 ≤ p.2 ∧ Q (p.1 + p.2))).image (fun p => p.1 + p.2) := by
    rw [sumset_eq_upper_image A, Finset.filter_image]
    congr 1
    rw [Finset.filter_filter]
  rw [key]
  refine Finset.card_image_of_injOn ((sum_injOn_upper A hA).mono ?_)
  intro p hp
  rw [Finset.mem_coe, Finset.mem_filter] at hp
  rw [Finset.mem_coe, Finset.mem_filter]
  exact ⟨hp.1, hp.2.1⟩

/-- **Erdős Problem #154** (the `A + A` form).  For a sequence of Sidon sets with
`Aₖ ⊆ [0, nₖ]`, `nₖ → ∞` and `|Aₖ| ∼ √nₖ`, the sumset `Aₖ + Aₖ` is asymptotically
equidistributed in every residue class mod `m`: the proportion of `Aₖ + Aₖ` lying in
class `i` tends to `1/m`.  This transfers Lindström's equidistribution of `A`
(`sidon_density_limit`) to `A + A`, which — as the problem statement notes — "follows
immediately using the Sidon property". -/
theorem erdos_154
    (m : ℕ) (hm : 2 ≤ m)
    (n_seq : ℕ → ℕ) (A_seq : ℕ → Finset ℕ)
    (h_n_tendsto : Filter.Tendsto (fun k => (n_seq k : ℝ)) Filter.atTop Filter.atTop)
  ...
  -- `|Aₖ| → ∞`
  have h_card_atTop : Filter.Tendsto (fun k => ((A_seq k).card : ℝ))
      Filter.atTop Filter.atTop := by
    have hsqrt : Filter.Tendsto (fun k => Real.sqrt (n_seq k)) Filter.atTop Filter.atTop := by
      have h := (tendsto_rpow_atTop (show (0:ℝ) < 1 / 2 by norm_num)).comp h_n_tendsto
      exact h.congr (fun k => (Real.sqrt_eq_rpow _).symm)
    have hmul := h_card.pos_mul_atTop (show (0:ℝ) < 1 by norm_num) hsqrt
    refine hmul.congr' ?_
    filter_upwards [h_n_tendsto.eventually_gt_atTop 0] with k hk
    have hs : Real.sqrt (n_seq k) ≠ 0 := (Real.sqrt_pos.mpr hk).ne'
    field_simp
  -- `1/|Aₖ| → 0`
  have hinv : Filter.Tendsto (fun k => 1 / ((A_seq k).card : ℝ)) Filter.atTop (nhds 0) := by
    simpa only [one_div] using tendsto_inv_atTop_zero.comp h_card_atTop
  -- residue ratios `cᵣ/|A| → 1/m`
  have hcr : ∀ r < m, Filter.Tendsto
      (fun k => (((A_seq k).filter (fun a => a % m = r)).card : ℝ) / ((A_seq k).card : ℝ))
      Filter.atTop (nhds (1 / m)) :=
    fun r hr => residue_ratio_tendsto m hm n_seq A_seq h_n_tendsto h_subset h_sidon h_card r hr
  -- ordered-pair (convolution) count, normalised by `|A|²`, tends to `1/m`
  have hP : Filter.Tendsto
      (fun k => (((A_seq k ×ˢ A_seq k).filter (fun p => (p.1 + p.2) % m = i)).card : ℝ)
                  / ((A_seq k).card : ℝ) ^ 2)
      Filter.atTop (nhds (1 / m)) := by
    have hfun : (fun k => (((A_seq k ×ˢ A_seq k).filter (fun p => (p.1 + p.2) % m = i)).card : ℝ)
                  / ((A_seq k).card : ℝ) ^ 2)
        = (fun k => ∑ r ∈ Finset.range m,
            (((A_seq k).filter (fun a => a % m = r)).card : ℝ) / ((A_seq k).card : ℝ)
              * ((((A_seq k).filter (fun a => a % m = (i + m - r) % m)).card : ℝ)
                  / ((A_seq k).card : ℝ))) := by
      funext k
      rw [pairCount_eq_conv (A_seq k) m i hm0 hi, Nat.cast_sum, Finset.sum_div]
      exact Finset.sum_congr rfl (fun r _ => by push_cast; ring)
    rw [hfun]
    have hsum := tendsto_finset_sum (Finset.range m)
      (fun r hr => (hcr r (Finset.mem_range.mp hr)).mul
        (hcr ((i + m - r) % m) (Nat.mod_lt _ hm0)))
    have hval : (∑ _r ∈ Finset.range m, (1 / (m : ℝ)) * (1 / (m : ℝ))) = 1 / m := by
      rw [Finset.sum_const, Finset.card_range, nsmul_eq_mul]
      field_simp
    rwa [hval] at hsum
  -- diagonal count, normalised by `|A|²`, tends to `0`
  have hD : Filter.Tendsto
      (fun k => (((A_seq k).filter (fun a => (a + a) % m = i)).card : ℝ)
                  / ((A_seq k).card : ℝ) ^ 2)
    ...
    filter_upwards [h_card_atTop.eventually_gt_atTop 0] with k hk
    have hDc : (((A_seq k).filter (fun a => (a + a) % m = i)).card : ℝ)
        ≤ ((A_seq k).card : ℝ) := by exact_mod_cast Finset.card_filter_le _ _
    have hck : ((A_seq k).card : ℝ) ≠ 0 := ne_of_gt hk
    calc (((A_seq k).filter (fun a => (a + a) % m = i)).card : ℝ) / ((A_seq k).card : ℝ) ^ 2
        ≤ ((A_seq k).card : ℝ) / ((A_seq k).card : ℝ) ^ 2 := by gcongr
      _ = 1 / ((A_seq k).card : ℝ) := by rw [sq, div_mul_eq_div_div, div_self hck]
  -- `2·Sᵢ/|A|² → 1/m`
  have hS2 : Filter.Tendsto
      (fun k => 2 * (((A_seq k + A_seq k).filter (fun s => s % m = i)).card : ℝ)
                  / ((A_seq k).card : ℝ) ^ 2)
      Filter.atTop (nhds (1 / (m : ℝ))) := by
    have hfun : (fun k => 2 * (((A_seq k + A_seq k).filter (fun s => s % m = i)).card : ℝ)
                  / ((A_seq k).card : ℝ) ^ 2)
        = (fun k => (((A_seq k ×ˢ A_seq k).filter (fun p => (p.1 + p.2) % m = i)).card : ℝ)
                      / ((A_seq k).card : ℝ) ^ 2
            + (((A_seq k).filter (fun a => (a + a) % m = i)).card : ℝ)
                      / ((A_seq k).card : ℝ) ^ 2) := by
      funext k
      have h2 : 2 * ((A_seq k + A_seq k).filter (fun s => s % m = i)).card
          = ((A_seq k ×ˢ A_seq k).filter (fun p => (p.1 + p.2) % m = i)).card
            + ((A_seq k).filter (fun a => (a + a) % m = i)).card := by
        rw [sumset_filter_card (A_seq k) (h_sidon k) (fun s => s % m = i)]
        exact two_mul_upper_card (A_seq k) (fun s => s % m = i)
      rw [← add_div]
      congr 1
      exact_mod_cast h2
    rw [hfun]
    simpa only [add_zero] using hP.add hD
  -- `2·|A+A|/|A|² → 1`
  have hden : Filter.Tendsto
      (fun k => 2 * ((A_seq k + A_seq k).card : ℝ) / ((A_seq k).card : ℝ) ^ 2)
      Filter.atTop (nhds 1) := by
    have hform : (fun k => 2 * ((A_seq k + A_seq k).card : ℝ) / ((A_seq k).card : ℝ) ^ 2)
        =ᶠ[Filter.atTop] (fun k => 1 + 1 / ((A_seq k).card : ℝ)) := by
      filter_upwards [h_card_atTop.eventually_gt_atTop 0] with k hk
      have hck : ((A_seq k).card : ℝ) ≠ 0 := ne_of_gt hk
      have h2 : 2 * (A_seq k + A_seq k).card
          = (A_seq k).card * (A_seq k).card + (A_seq k).card := by
        have hAA : (A_seq k + A_seq k).card
            = ((A_seq k ×ˢ A_seq k).filter (fun p => p.1 ≤ p.2 ∧ True)).card := by
          have h := sumset_filter_card (A_seq k) (h_sidon k) (fun _ => True)
          simpa only [Finset.filter_True] using h
        rw [hAA, two_mul_upper_card (A_seq k) (fun _ => True)]
        simp [Finset.card_product]
      have hcast : 2 * ((A_seq k + A_seq k).card : ℝ)
          = ((A_seq k).card : ℝ) * ((A_seq k).card : ℝ) + ((A_seq k).card : ℝ) := by
        exact_mod_cast h2
      rw [hcast]; field_simp
    refine Filter.Tendsto.congr' hform.symm ?_
    simpa using tendsto_const_nhds.add hinv
  -- combine: `Sᵢ/|A+A| = (2Sᵢ/|A|²) / (2|A+A|/|A|²) → (1/m)/1 = 1/m`
  have hfinal := hS2.div hden (by norm_num : (1 : ℝ) ≠ 0)
  rw [div_one] at hfinal
  refine hfinal.congr' ?_
  filter_upwards [h_card_atTop.eventually_gt_atTop 0] with k hk
  have hck : ((A_seq k).card : ℝ) ≠ 0 := ne_of_gt hk
  have hAA_ne : ((A_seq k + A_seq k).card : ℝ) ≠ 0 := by
    have hne : (A_seq k).Nonempty := Finset.card_pos.mp (by exact_mod_cast hk)
    exact_mod_cast (Finset.card_pos.mpr (hne.add hne)).ne'
  simp only [Pi.div_apply]
  rw [div_div_div_cancel_right₀ (pow_ne_zero 2 hck)]
  ring

end

#print axioms erdos_154
-- 'Erdos154.erdos_154' depends on axioms: [propext, Classical.choice, Quot.sound]
