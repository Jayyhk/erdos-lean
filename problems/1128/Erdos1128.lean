...
Erdős Problem 1128. Let `A, B, C` have cardinality `ℵ₁`. In any 2-colouring of `A × B × C`, must there be `A₁ ⊂ A`, `B₁ ⊂ B`, `C₁ ⊂ C`, all of cardinality `ℵ₀`, with `A₁ × B₁ × C₁` monochromatic? `erdos_1128` disproves this.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos1128.lean`, a single self-contained file whose only import is
`Mathlib`. It is reproduced here with the file-level comment header, the upstream trust-base
print lines and the trailing `alias` removed, wrapped once in `namespace Erdos1128`, and with the
final theorem named `erdos_1128` (upstream `not_erdos_1128`). No mathematical content is changed.
-/
...
#print axioms erdos_1128
-- 'Erdos1128.erdos_1128' depends on axioms: [propext, Classical.choice, Quot.sound]
