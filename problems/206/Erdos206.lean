...
* `Erdos206.erdos_206`: The set of positive reals with eventually greedy best Egyptian
    ...
    · intro T hT₁ hT₂ hT₃ hT₄
      ...
      · obtain ⟨a, ha₁, ha₂⟩ : ∃ a ∈ T, ∀ m ∈ T, a ≤ m := by
        ...
        have hT_minus_a : egyptianSum T = 1 / (a : ℝ) + egyptianSum (T \ {a}) := by
          simp +decide [ egyptianSum, Finset.sum_eq_add_sum_diff_singleton ha₁ ]
...
lemma per_interval_bound (s t : ℕ) (hs : 100 ≤ s) (hst : s < t)
  ...
  calc volume (X_set s (t + 2) ∩ Set.Ioc q r)
    ...
    _ = volume (Set.Ioc q r) - volume A :=
        measure_diff hA_sub hA_meas.nullMeasurableSet h_vol_ne_top
