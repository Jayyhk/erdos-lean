...
/-- Edges receiving colour `c` under an edge-colouring `χ` of the complete graph. -/
def colourClass {m k : ℕ} (χ : Sym2 (Fin m) → Fin k) (c : Fin k) :
    SimpleGraph (Fin m) where
  Adj a b := a ≠ b ∧ χ s(a, b) = c
  symm := by
    rintro a b ⟨hne, hc⟩
    exact ⟨hne.symm, by rwa [Sym2.eq_swap]⟩
  loopless := ⟨fun a h => h.1 rfl⟩

instance {m k : ℕ} (χ : Sym2 (Fin m) → Fin k) (c : Fin k) :
    DecidableRel (colourClass χ c).Adj := fun a b => by
  unfold colourClass; infer_instance

/--
**Erdős problem #557.** Let `R_k(G)` be the least `m` such that every `k`-colouring of the
edges of `K_m` contains a monochromatic copy of `G`. Then `R_k(T) ≤ k*n + O(1)` for every
tree `T` on `n` vertices, with an absolute constant (here `2`) uniform in both `k` and `n`.

This is the implication from the Erdős–Sós conjecture (`erdos_sos`, Erdős problem #548)
noted on erdosproblems.com: `k`-colour `K_m`, take a colour class carrying at least a
`1/k` share of the `binom(m,2)` edges, and check that `m ≥ k*n + 2` pushes that class past
the Erdős–Sós edge threshold `(n-2)*m/2` for trees on `n` vertices.
-/
theorem erdos_557 :
    ∃ C : ℕ, ∀ (k t m : ℕ), 0 < k → k * (t + 1) + C ≤ m →
      ∀ χ : Sym2 (Fin m) → Fin k, ∀ T : SimpleGraph (Fin (t + 1)), T.IsTree →
        ∃ c : Fin k, T.IsContained (colourClass χ c) := by
  classical
  refine ⟨2, fun k t m hk hm χ T hT => ?_⟩
  -- each colour class is the corresponding fiber of `χ` on the complete edge set
  have hfib : ∀ c : Fin k, (colourClass χ c).edgeFinset
      = (⊤ : SimpleGraph (Fin m)).edgeFinset.filter (fun e => χ e = c) := by
    intro c; ext e
    induction e using Sym2.ind with
    | _ a b => simp [colourClass, SimpleGraph.mem_edgeFinset, SimpleGraph.mem_edgeSet]
  -- the fibers partition the edges of the complete graph
  have hsum : ∑ c : Fin k, (colourClass χ c).edgeFinset.card
      = (⊤ : SimpleGraph (Fin m)).edgeFinset.card := by
    simp only [hfib]
    exact (Finset.card_eq_sum_card_fiberwise (fun x _ => Finset.mem_univ (χ x))).symm
  -- pigeonhole: some colour carries at least a `1/k` share
  have hex : ∃ c : Fin k, (⊤ : SimpleGraph (Fin m)).edgeFinset.card
      ≤ k * (colourClass χ c).edgeFinset.card := by
    by_contra h
    push_neg at h
    have hne : (Finset.univ : Finset (Fin k)).Nonempty :=
      Finset.univ_nonempty_iff.mpr (Fin.pos_iff_nonempty.mp hk)
    have hlt : ∑ c : Fin k, k * (colourClass χ c).edgeFinset.card
        < ∑ _c : Fin k, (⊤ : SimpleGraph (Fin m)).edgeFinset.card :=
      Finset.sum_lt_sum_of_nonempty hne (fun c _ => h c)
    rw [← Finset.mul_sum, hsum, Finset.sum_const, Finset.card_univ,
      Fintype.card_fin, smul_eq_mul] at hlt
    omega
  obtain ⟨c, hc⟩ := hex
  refine ⟨c, ?_⟩
  have hchoose : (⊤ : SimpleGraph (Fin m)).edgeFinset.card = m.choose 2 := by
    rw [SimpleGraph.card_edgeFinset_top_eq_card_choose_two, Fintype.card_fin]
  have htm : t + 1 ≤ m := le_trans (Nat.le_mul_of_pos_left _ hk) (by omega)
  refine erdos_sos m t htm (colourClass χ c) ?_ T hT
  -- arithmetic: (m choose 2) ≤ k * E_c  and  m ≥ k(t+1)+2  give the Erdős–Sós threshold
  have hEc : ((colourClass χ c).edgeSet.ncard : ℚ) = ((colourClass χ c).edgeFinset.card : ℚ) := by
    congr 1; simp [SimpleGraph.edgeFinset, Set.ncard_eq_toFinset_card']
  rw [hEc]
  set Ec : ℕ := (colourClass χ c).edgeFinset.card with hEcdef
  have h1 : (m.choose 2 : ℚ) ≤ (k : ℚ) * Ec := by
    have := hc; rw [hchoose] at this; exact_mod_cast this
  have h2 : (m.choose 2 : ℚ) = (m : ℚ) * ((m : ℚ) - 1) / 2 := by rw [Nat.cast_choose_two]
  have h3 : ((k : ℚ)) * ((t : ℚ) + 1) + 2 ≤ (m : ℚ) := by exact_mod_cast hm
  have hk' : (1 : ℚ) ≤ (k : ℚ) := by exact_mod_cast hk
  have hm1 : (1 : ℚ) ≤ (m : ℚ) := by
    have : (1 : ℕ) ≤ m := by omega
    exact_mod_cast this
  rw [h2] at h1
  nlinarith [h1, h3, hk', hm1, sq_nonneg ((m : ℚ) - 1)]

#print axioms erdos_557
-- 'Erdos557.erdos_557' depends on axioms: [propext, Classical.choice, Quot.sound]
