...
Erdős Problem 755. The number of equilateral triangles of size `1` formed by any set of `n`
points in `ℝ⁶` is at most `(1/27 + o(1)) n³`. `erdos_755` proves this.

Erdős and Purdy [ErPu75] gave Lenz's construction of three orthogonal circles carrying `n/3`
points each, showing `T₆^u(n) ≥ n³/27 − O(n²)`, and Erdős conjectured the matching upper
bound. It was proved in the stronger any-size form by Clemen, Dumitrescu and Liu [CDL25b],
who determine `T₆(n) = (1/27 + o(1)) n³` exactly; the unit-triangle bound follows because
unit equilateral triangles are a subset of the equilateral triangles of any positive size.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos755.lean`, a single self-contained file. It is reproduced here
with the twenty-one explicit `Mathlib` sub-imports replaced by the single `import Mathlib`,
the file-level provenance header removed, wrapped once in `namespace Erdos755` (moved above
the `open` lines), and with the upstream test theorem `erdos_755_test_dim_one` placed before
the final theorem. No mathematical content is changed. The statement is that of the Formal
Conjectures entry, whose Apache 2.0 notice is reproduced below.
-/
...
/-- A degenerate sanity check: the empty point set in `ℝ¹` spans no unit
...
-- 'Erdos755.erdos_755' depends on axioms: [propext, Classical.choice, Quot.sound]
