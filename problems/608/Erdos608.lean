...
Disproof of Erdős Problem 608: "Let $G$ be a graph with $n$ vertices and $>n^2/4$ many
edges. Are there at least $\frac{2}{9}n^2$ edges of $G$ which are contained in a $C_5$?"

The answer is no, by the Füredi–Maleki construction (unpublished; described by Grzesik, Hu
and Volec, *Minimum number of edges that occur in odd cycles*, arXiv:1605.09055, who also
proved the optimal constant $\frac{2+\sqrt 2}{16}$).

`erdos_608` refutes the intended asymptotic reading, "for all sufficiently large $n$",
which is the reading under which erdosproblems.com's DISPROVED verdict cites Füredi–Maleki.
The literal unqualified $\forall n$ sentence is separately refuted by `literal_form_false`,
degenerately at $n = 3$; since the asymptotic form is logically weaker, refuting it covers
the literal reading too. `strong_disproof` gives the explicit uniform gap
$\varepsilon = 47/7056$ below $2/9$.

Flattened single-file vendoring of the six-module Lean development by primateria
(github.com/primateria/erdos608), concatenated in dependency order with project-internal
imports removed. The conjecture is inlined into `erdos_608` rather than carried as a
separate `def`.
-/
...
All inequalities are cleared to naturals; the vertex type is `Fin n`.
-/
...
/-
The word-for-word `∀ n` reading of the problem is degenerately FALSE, which is
why `erdos_608` below refutes the eventually-form (`∃ n₀, ∀ n ≥ n₀, …`) instead.
-/
...
This lemma documents WHY `erdos_608` uses the eventually-form
`∃ n₀, ∀ n, n₀ ≤ n → …`: under the literal `∀ n` form, a
...
theorem literal_form_false :
  ...
  have hpent : Erdos608.pentEdges (⊤ : SimpleGraph (Fin 3)) = ∅ := by
    ext e
    simp only [Erdos608.pentEdges, Set.mem_setOf_eq, Set.mem_empty_iff_false, iff_false, not_and]
    intro _ hc5
...
/-! ### `OnC5` agrees with Mathlib's notion of a 5-cycle -/
...
/-! ### The Füredi–Maleki construction -/

/-
The witness construction: a rational specialization of the Füredi–Maleki
template.
...
Provided here:
* `V`, `FM`, `card_V` (`Fintype.card (V m) = 28m`).
* per-part degree lemmas, `sum_degrees`, `card_edgeFinset`, and
  `edgeSet_ncard` (`(FM m).edgeSet.ncard = 197m² − 5m`).
* `onC5_dd`, `onC5_cd`, `onC5_bc` — every D×D, C×D, B×C edge lies on a
  pentagon (for `1 ≤ m`).
-/


/-! #### The vertex type and the graph -/
...
/-- The Füredi–Maleki graph on `28m` vertices has `197m² − 5m` edges
(so `4e > n²` for `m ≥ 6`). -/
...
/-- **D×D.** Every edge inside the clique D lies on a pentagon:
...
/-- **C×D.** Every C×D edge lies on a pentagon: `c, d₁, d₂, d₃, d₄` with
...
/-- **B×C.** Every B×C edge lies on a pentagon: `b, c₁, d₁, d₂, c₂` with
...
/-! ### Counting the pentagonal edges -/

/-
Pentagonal edges of the Füredi–Maleki graph.

* `not_onC5_AB`: no pentagon of `FM m` meets part A; in particular no
  ...
  pentagon has.
* `pentEdges_ncard`: `(pentEdges (FM m)).ncard = 169·m² − 5·m` for
  `1 ≤ m`: the pentagonal edges are exactly the edges outside the A×B block
  (`not_onC5_AB` gives ⊆, `onC5_bc`/`onC5_cd`/`onC5_dd` give ⊇), and the A×B
  block has `28·m²` edges.
-/


/-! #### No pentagon meets part A -/
...
/-- Finite-arithmetic core of `not_onC5_AB`: the five adjacency-shaped constraints of a
...
/-- No A×B edge of `FM m` lies on a pentagon. -/
...
/-! #### The A×B block and the pentagonal-edge count -/
...
/-- Every edge of `FM m` outside the A×B block lies on a pentagon (the
...
/-- The Füredi–Maleki graph has `169·m² − 5·m` pentagonal edges. -/
...
/-! ### Transport to `Fin n` and the disproof -/
...
`28m` elements) while the statements below quantify over
...
correspond under the injection `Sym2.map (toFin m)`), and proves:

* `erdos_608`       : for `m ≥ 6` the graph `FMFin m` on `n = 28m` vertices
  has `4e = 788m² − 20m > 784m² = n²` edges but only `169m² − 5m` pentagonal
  edges, and `9(169m² − 5m) = 1521m² − 45m < 1568m² = 2n²`.
...
/-- With `ε = 47/7056` there are arbitrarily
...
/-- **Erdős Problem 608.** The Füredi–Maleki construction disproves
...
#print axioms erdos_608
-- 'Erdos608.erdos_608' depends on axioms: [propext, Classical.choice, Quot.sound]
