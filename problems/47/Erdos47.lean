...
of Bloom's unit-fractions theorem (Bloom-Mehta) — the same source as problem 298.
...
then some `S ⊆ A` has `∑_{n∈S} 1/n = 1`). It is derived from Bloom's
...
lemma Finset.cast_lcm {x : Finset ℕ} : ((x.lcm id : ℕ) : ℤ) = x.lcm (fun n => (n : ℤ)) := by
  ...
  · intro a s ha hs
    simpa only [Finset.lcm_insert, id_eq] using
      hs ▸ (Erdos47.cast_lcm (x := a) (y := s.lcm id))
...
/-- **Erdős Problem #47** (Bloom's $100 theorem).  If `δ > 0` and `N` is sufficiently
large in terms of `δ`, then any `A ⊆ {1,…,N}` with `∑_{a∈A} 1/a > δ·log N` has a subset
`S ⊆ A` with `∑_{n∈S} 1/n = 1`.  This is the logarithmic-density form of the unit-fractions
problem, derived from Bloom's quantitative `unit_fractions_upper_log_density`: its threshold
`≫ (log log log N / log log N)·log N` is `o(log N)`, so it lies below `δ·log N` once `N`
is large. -/
theorem erdos_47 : ∀ δ : ℝ, 0 < δ → ∀ᶠ N : ℕ in atTop, ∀ A : Finset ℕ,
    A ⊆ Finset.Icc 1 N → δ * Real.log N < rec_sum A → ∃ S ⊆ A, rec_sum S = 1 := by
  intro δ hδ
  obtain ⟨C, hC, hlog⟩ := unit_fractions_upper_log_density
  have hll : Filter.Tendsto (fun N : ℕ => Real.log (Real.log N)) atTop atTop :=
    Real.tendsto_log_atTop.comp (Real.tendsto_log_atTop.comp tendsto_natCast_atTop_atTop)
  have hf0 : Filter.Tendsto
      (fun N : ℕ => C * (Real.log (Real.log (Real.log N)) / Real.log (Real.log N)))
      atTop (nhds 0) := by
    have hdiv := (Real.isLittleO_log_id_atTop.tendsto_div_nhds_zero).comp hll
    simpa using hdiv.const_mul C
  have hthr : ∀ᶠ N : ℕ in atTop,
      C * (Real.log (Real.log (Real.log N)) / Real.log (Real.log N)) < δ :=
    hf0.eventually (Iio_mem_nhds hδ)
  filter_upwards [hlog, hthr, eventually_ge_atTop 1] with N hN hthrN hN1 A hAsub hArec
  refine hN A hAsub ?_
  have hlogN : (0 : ℝ) ≤ Real.log N := Real.log_nonneg (by exact_mod_cast hN1)
  calc C * (Real.log (Real.log (Real.log N)) / Real.log (Real.log N) * Real.log N)
      = (C * (Real.log (Real.log (Real.log N)) / Real.log (Real.log N))) * Real.log N := by ring
    _ ≤ δ * Real.log N := mul_le_mul_of_nonneg_right hthrN.le hlogN
    _ ≤ rec_sum A := le_of_lt hArec
...
-- 'Erdos47.erdos_47' depends on axioms: [propext, Classical.choice, Quot.sound]
