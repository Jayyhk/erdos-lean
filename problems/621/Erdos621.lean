...
lemma sos_eq_2D_sub_2A (G : Trigraph V) :
  ...
  rw [ show ∑ x : V, ∑ x_1 : V, ∑ x_2 : V, ∑ x_3 : V, G.n x x_2 * ( G.n x x_3 * ( ( G.n x x_1 + G.c x x_1 ) * ( G.s x_1 x_2 * G.s x_1 x_3 ) ) ) = ∑ x : V, ∑ x_1 : V, ∑ x_2 : V, ∑ x_3 : V, G.n x_1 x_2 * ( G.n x_1 x_3 * ( ( G.n x x_1 + G.c x x_1 ) * ( G.s x x_2 * G.s x x_3 ) ) ) from ?_ ] ; ring;
  · congr! 3;
...
lemma K13_minus_P4_C4_eq_half_sos (G : Trigraph V) :
  ...
  have h_simplify : ∑ u : V, ∑ v : V, ∑ w : V, ∑ x : V, (G.s u v * G.s u w * G.s u x + G.s u v * G.s v w * G.s v x - G.s u v * G.s u w * G.s v x - G.s u v * G.s v w * G.s u x) = 2 * ∑ u : V, ∑ v : V, ∑ w : V, ∑ x : V, G.s u v * G.s u w * G.s u x - 2 * ∑ u : V, ∑ v : V, ∑ w : V, ∑ x : V, G.s u v * G.s u w * G.s v x := by
    ...
    have h_simplify2 : ∑ u : V, ∑ v : V, ∑ w : V, ∑ x : V, G.s u v * G.s v w * G.s u x = ∑ u : V, ∑ v : V, ∑ w : V, ∑ x : V, G.s u v * G.s u w * G.s v x := by
      exact Finset.sum_congr rfl fun _ _ => Finset.sum_congr rfl fun _ _ => Finset.sum_comm.trans ( Finset.sum_congr rfl fun _ _ => Finset.sum_congr rfl fun _ _ => by ring )
...
lemma fbound4 (G : Trigraph V) :
  ...
  rw [ ← Finset.sum_sub_distrib ] ; congr ; ext v ; rw [ ← Finset.sum_sub_distrib ] ; congr ; ext w ; rw [ ← Finset.sum_sub_distrib ] ; congr ; ext x ; ring;
  unfold Trigraph.n; ring;
...
lemma B_expansion (G : Trigraph V) :
  ...
  simp +decide only [← Finset.mul_sum _ _ _, ← sum_mul] ; ring
...
lemma sum_s_s_s_s_eq_C4 (G : Trigraph V) :
  ...
  exact Finset.sum_congr rfl fun u hu => Finset.sum_congr rfl fun v hv => Finset.sum_congr rfl fun w hw => Finset.sum_congr rfl fun x hx => by rw [ G.s_symm u v, G.s_symm u x ] ; ring;
...
lemma F2_eq (G : Trigraph V) :
  ...
  have h_F2_decomp : G.F2 = ∑ u : V, ∑ v : V, ∑ w : V, ∑ x : V, 2 * G.s u v * (3 * G.s w x + G.c w x) * (G.s u w + G.s v w) * (1 - G.s u x - G.s v x) + ∑ u : V, ∑ v : V, ∑ w : V, ∑ x : V, G.s u v * (1 - G.s u w - G.s v w) * (1 - G.s u x - G.s v x) + ∑ u : V, ∑ v : V, ∑ w : V, ∑ x : V, 4 * G.s u v * G.s w x * G.s u w * G.s v x := by
    simpa only [ ← sum_add_distrib ] using Finset.sum_congr rfl fun u hu => Finset.sum_congr rfl fun v hv => Finset.sum_congr rfl fun w hw => Finset.sum_congr rfl fun x hx => by unfold Trigraph.f2; ring;
  have h_A : ∑ u : V, ∑ v : V, ∑ w : V, ∑ x : V, 2 * G.s u v * (3 * G.s w x + G.c w x) * (G.s u w + G.s v w) * (1 - G.s u x - G.s v x) = 12 * G.P4 + 2 * ∑ u : V, ∑ v : V, ∑ w : V, ∑ x : V, G.s u v * G.c w x * (G.s u w + G.s v w) * (1 - G.s u x - G.s v x) := by
    have h_A : ∑ u : V, ∑ v : V, ∑ w : V, ∑ x : V, 2 * G.s u v * (3 * G.s w x) * (G.s u w + G.s v w) * (1 - G.s u x - G.s v x) = 12 * G.P4 := by
      convert congr_arg ( · * 6 ) ( fbound1 G ) using 1 ; ring;
      · simp +decide only [Finset.sum_mul _ _ _] ; congr ; ext ; congr ; ext ; congr ; ext ; congr ; ext ; ring;
      · ring;
    simp +decide only [mul_add, mul_left_comm, mul_assoc, Finset.mul_sum _ _ _];
    rw [ ← h_A ] ; rw [ ← Finset.sum_add_distrib ] ; congr ; ext ; rw [ ← Finset.sum_add_distrib ] ; congr ; ext ; rw [ ← Finset.sum_add_distrib ] ; congr ; ext ; rw [ ← Finset.sum_add_distrib ] ; congr ; ext ; ring;
      ...
      have h_sum : ∑ u : V, ∑ v : V, ∑ w : V, ∑ x : V, G.f2 u v w x > (Fintype.card V) ^ 2 * ∑ u : V, ∑ v : V, G.s u v := by
        have h_sum : ∑ u : V, ∑ v : V, ∑ w : V, ∑ x : V, G.f2 u v w x > ∑ u : V, ∑ v : V, (G.s u v * (Fintype.card V) ^ 2) := by
          ...
          · obtain ⟨u₀, v₀, huv⟩ : ∃ u₀ v₀ : V, G.s u₀ v₀ = 1 := by
              ...
              · exact by push_neg at h_zero; obtain ⟨ u, v, h ⟩ := h_zero; exact ⟨ u, v, Or.resolve_left ( G.s_eq_zero_or_one u v ) h ⟩ ;
...
private lemma two_coloring_sum_le (G : Trigraph V) (u₀ v₀ : V) (huv : G.s u₀ v₀ = 1)
  ...
  have h_sum : 4 * ∑ w : V, ∑ x : V, (if w ∈ Z then 1 else 0) * (if x ∈ Z then 1 else 0) * ((G.c w x + G.s w x) * (if χ₁ w = χ₁ x then 1 else 0) + G.s w x) ≤ 2 * (Z.card : ℤ) ^ 2 := by
    ...
    convert mul_le_mul_of_nonneg_left h_sum zero_le_two using 1 ; ring;
    exact congrArg₂ _ ( Finset.sum_congr rfl fun _ _ => Finset.sum_congr rfl fun _ _ => by split_ifs <;> ring ) rfl;
  have h_sum : ∑ w : V, ∑ x : V, (2 * ((G.c w x + G.s w x) * (if χ₁ w = χ₁ x then 1 else 0) + (G.c w x + G.s w x) * (if χ₂ w = χ₂ x then 1 else 0) + 2 * G.s w x)) = 2 * (G.edgesWithin χ₁ + G.edgesWithin χ₂) + 4 * G.S_total := by
    simp +decide [ Finset.mul_sum _ _ _, Trigraph.edgesWithin, Trigraph.S_total ] ; ring;
    simp +decide only [sum_add_distrib, sum_mul _ _ _] ; ring;
  ...
  have h_card_filter : (G.edgeFinset.filter (fun e => Sym2.lift ⟨fun u v => χ u = χ v, by intros; simp [eq_comm]⟩ e)).card ≤ (∑ u : V, ∑ v : V, (if G.Adj u v ∧ χ u = χ v then 1 else 0)) / 2 := by
    have h_card_filter : (G.edgeFinset.filter (fun e => Sym2.lift ⟨fun u v => χ u = χ v, by intros; simp [eq_comm]⟩ e)).card * 2 ≤ (∑ u : V, ∑ v : V, (if G.Adj u v ∧ χ u = χ v then 1 else 0)) := by
      have h_card_filter : (G.edgeFinset.filter (fun e => Sym2.lift ⟨fun u v => χ u = χ v, by intros; simp [eq_comm]⟩ e)).card * 2 ≤ Finset.card (Finset.filter (fun (uv : V × V) => G.Adj uv.1 uv.2 ∧ χ uv.1 = χ uv.2) (Finset.univ : Finset (V × V))) := by
        have h_card_filter : Finset.card (Finset.filter (fun (uv : V × V) => G.Adj uv.1 uv.2 ∧ χ uv.1 = χ uv.2) (Finset.univ : Finset (V × V))) ≥ Finset.card (Finset.biUnion (Finset.filter (fun e => Sym2.lift ⟨fun u v => χ u = χ v, by intros; simp [eq_comm]⟩ e) G.edgeFinset) (fun e => Finset.filter (fun (uv : V × V) => Sym2.mk uv = e) (Finset.univ : Finset (V × V)))) := by
...
/-! ## Erdős Problem 621 (literal site form) -/

open TriangleIndep

/-- **Erdős Problem 621** (Erdős–Gallai–Tuza, proved by Norin–Sun).
For every finite simple graph `G` on `n` vertices,
`α₁(G) + τ₁(G) ≤ n²/4`. This is the literal site phrasing; the underlying
`erdos_conjecture` is the equivalent integer form
`4 * (α₁ G + τ₁ G) ≤ n²`. -/
theorem erdos_621 {V : Type*} [Fintype V] [DecidableEq V]
    (G : SimpleGraph V) [DecidableRel G.Adj] :
    (alpha1 G + tau1 G : ℝ) ≤ (Fintype.card V : ℝ) ^ 2 / 4 := by
  have h := erdos_conjecture G
  have h4 : (0 : ℝ) < 4 := by norm_num
  rw [le_div_iff₀ h4]
  have : ((4 * (alpha1 G + tau1 G) : ℕ) : ℝ) ≤ ((Fintype.card V) ^ 2 : ℕ) := by exact_mod_cast h
  push_cast at this
  linarith
...
-- 'Erdos621.erdos_621' depends on axioms: [propext, Classical.choice, Quot.sound]
