...
Erdős Problem 965. Is it true that for any 2-colouring of `ℝ` there is a set `A ⊆ ℝ` of
cardinality `ℵ₁` such that all sums `a + b` with `a ≠ b` and `a, b ∈ A` are the same colour?
`erdos_965` disproves this.

In [Er75b] Erdős reported that he could prove this false assuming the continuum hypothesis;
a published proof under CH is due to Hindman, Leader and Strauss [HLS17]. The result was
proved in ZFC, without assuming CH, independently by Komjáth [Ko16] and by Soukup and Weiss.
The formalisation follows Komjáth: a finite-union colouring is transferred through a Hamel
basis of `ℝ` over `ℚ`. The statement quantifies over uncountable `A` rather than `|A| = ℵ₁`;
since every set of cardinality `ℵ₁` is uncountable, this form is the stronger one and implies
the question as posed on erdosproblems.com. It is the statement of the Formal Conjectures
entry.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos965.lean` together with the nine modules of
`src/latest/ErdosProblems/Erdos965/`. Those ten files are concatenated here in dependency
...
`namespace Erdos965`, the upstream trust-base print line and trailing `alias` removed, and the
final theorem renamed from `not_erdos_965` to `erdos_965`. No mathematical content is changed.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos965/Countability.lean` -/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos965/CriticalPair.lean` -/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos965/UniformPrefix.lean` -/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos965/CoordinateNormalize.lean` -/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos965/CylinderSplit.lean` -/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos965/FiniteColoring.lean` -/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos965/DeltaSystem.lean` -/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos965/HamelTransfer.lean` -/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos965/FiniteMain.lean` -/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos965.lean` -/
...
/-- **Erdős Problem 965 (disproof).** There is a two-colouring of `ℝ` for which no uncountable
set has all its distinct pair sums the same colour. Since a set of cardinality `ℵ₁` is
uncountable, this refutes the question as Erdős asked it. -/
theorem erdos_965 :
...
#print axioms erdos_965
-- 'Erdos965.erdos_965' depends on axioms: [propext, Classical.choice, Quot.sound]
