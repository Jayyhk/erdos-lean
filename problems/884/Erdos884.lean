...
open _root_.ArithmeticFunction.IsMultiplicative
...
theorem _root_.Aux.sum_inv_le_log (n : ℕ) (hn : 1 ≤ n) :
  ...
  · apply @AntitoneOn.sum_le_integral_Ico 2 (n + 1) fun x : ℝ => (x - 1)⁻¹
    · linarith [hn]
    apply Aux.inv_sub_antitoneOn_Icc; norm_num
...
theorem _root_.Aux.sum_pow_cardDistinctFactors_le_self_mul_log_pow {P h : ℕ} (x : ℝ) (hx : 1 ≤ x)
  ...
  rw [←mul_sum];
  gcongr
  apply Aux.sum_pow_cardDistinctFactors_div_self_le_log_pow x hx hP
...
theorem _root_.SelbergSieve.one_div_selbergTerms_eq_conv_moebius_nu (l : ℕ) (hl : Squarefree l)
  ...
  rw [←Aux.div_mult_of_dvd_squarefree ν s.nu_mult l d (dvd_of_mem_divisors hd) hl, inv_div]
  · ring
...
theorem _root_.SelbergSieve.conv_selbergTerms_eq_selbergTerms_mul_nu {d : ℕ} (hd : d ∣ P) :
  ...
  calc
    (∑ l ∈ divisors P, if l ∣ d then g l else 0) =
      ...
      rw [← Aux.sum_over_dvd_ite prodPrimes_ne_zero hd,
        ← Nat.sum_divisorsAntidiagonal fun x _ => g x,
        Nat.sum_divisorsAntidiagonal' fun x _ => g x, Aux.sum_over_dvd_ite prodPrimes_ne_zero hd]
    _ = g d * ∑ l ∈ divisors P, if l ∣ d then 1 / g l else 0 := by
      ...
      rw [← Aux.div_mult_of_dvd_squarefree g (SelbergSieve.selbergTerms_mult s) d l h]
      · ring
...
private theorem _root_.SelbergSieve.lambdaSquared_eq_zero_of_support_wlog {w : ℕ → ℝ} {y : ℝ}
    (hw : ∀ (d : ℕ), ¬d ^ 2 ≤ y → w d = 0)
...
theorem _root_.SelbergSieve.upperMoebius_of_lambda_sq (weights : ℕ → ℝ) (hw : weights 1 = 1) :
  ...
  have h_sq :
    ...
    rw [sq, mul_sum, Aux.conv_lambda_sq_larger_sum _ n, sum_comm]
...
theorem _root_.SelbergSieve.lambdaSquared_mainSum_eq_quad_form (w : ℕ → ℝ) :
  ...
  dsimp only [mainSum, SelbergSieve.lambdaSquared]
  ...
  trans (∑ d ∈ divisors P, ∑ d1 ∈ divisors P, ∑ d2 ∈ divisors P,
          if d = d1.lcm d2 then w d1 * w d2 * ν d else 0)
  · apply Aux.conv_lambda_sq_larger_sum
...
theorem _root_.SelbergSieve.lambdaSquared_mainSum_eq_diag_quad_form (w : ℕ → ℝ) :
  ...
  by
  rw [SelbergSieve.lambdaSquared_mainSum_eq_quad_form s w]
  ...
  · apply sum_congr rfl; intro d1 hd1; apply sum_congr rfl; intro d2 _
    ...
    rw [SelbergSieve.nu_eq_conv_one_div_selbergTerms s _ hgcd_dvd, mul_sum]
    apply sum_congr rfl; intro l _
    rw [mul_ite_zero]; apply if_congr Iff.rfl _ rfl
    ring
  trans (∑ l ∈ divisors P, ∑ d1 ∈ divisors P, ∑ d2 ∈ divisors P,
        if l ∣ Nat.gcd d1 d2 then 1 / SelbergSieve.selbergTerms s l * (ν d1 * w d1) * (ν d2 * w d2) else 0)
...
theorem _root_.SelbergSieve.selbergBoundingSum_pos :
  ...
  · intro l hl
    ...
    · apply SelbergSieve.selbergTerms_pos _ _ (hl.1.1)
...
theorem _root_.SelbergSieve.selbergWeights_mul_mu_nonneg (d : ℕ) (hdP : d ∣ P) :
  ...
  · exact le_of_lt <| SelbergSieve.selbergTerms_pos _ d hdP
...
theorem _root_.SelbergSieve.selbergWeights_eq_dvds_sum (d : ℕ) :
  ...
  by_cases h_dvd : d ∣ P
  swap
  · dsimp only [SelbergSieve.selbergWeights]; rw [if_neg h_dvd]
    rw [sum_eq_zero]
    · ring
  ...
  apply symm
  rw [SelbergSieve.sum_mul_subst d P]
  · apply sum_congr rfl
    intro m hm
    ...
    · intro h
      ...
      · rw [inv_mul_cancel₀ (nu_ne_zero h_dvd),
          (SelbergSieve.selbergTerms_mult _).map_mul_of_coprime <| coprime_comm.mp h.2]
        ring
      ring
  · intro l _ hdl
    rw [if_neg, mul_zero]
...
theorem _root_.SelbergSieve.selbergWeights_diagonalisation (l : ℕ) (hl : l ∈ divisors P) :
  ...
  calc
    (∑ d ∈ divisors P, if l ∣ d then ν d * γ d else 0) =
      ...
      rw [SelbergSieve.selbergWeights_eq_dvds_sum, ← boole_mul, mul_sum, mul_sum]
...
theorem _root_.SelbergSieve.weight_one_of_selberg : γ 1 = 1 := by
  ...
  simp only [inv_one, mul_one, isUnit_one, IsUnit.squarefree, moebius_apply_of_squarefree,
    cardFactors_one, _root_.pow_zero, Int.cast_one, SelbergSieve.selbergBoundingSum, one_mul,
...
theorem _root_.SelbergSieve.selberg_bound_simple_mainSum :
  ...
  · apply sum_congr rfl; intro l hl
    ...
    rw [one_div_mul_cancel <| _root_.ne_of_gt <| SelbergSieve.selbergTerms_pos _ l <| dvd_of_mem_divisors hl]
    ring
...
private lemma _root_.SelbergSieve._helper {k m d : ℕ} (hkd : k ∣ d) (hk : k ∈ divisors P) (hm : m ∈ divisors P) :
...
theorem _root_.SelbergSieve.selbergBoundingSum_ge {d : ℕ} (hdP : d ∣ P) :
  ...
  _ = (∑ k ∈ divisors P,
    ...
    rw [SelbergSieve.sum_mul_subst k P, sum_congr rfl]
    · intro m hm
      rw [mul_ite_zero, ← ite_and]
      apply if_ctx_congr _ _ fun _ => rfl
      · exact_mod_cast s._helper hkd hk hm
      · intro h
        apply (SelbergSieve.selbergTerms_mult _).map_mul_of_coprime
  ...
  _ ≥ (∑ k ∈ divisors P, if k ∣ d
    ...
    swap
    · rfl
    apply mul_le_mul le_rfl _ _ (le_of_lt <| SelbergSieve.selbergTerms_pos _ k <| hkd.trans hdP)
    · apply sum_le_sum; intro m hm
      ...
      · exfalso; apply h'
        ...
        · trans ((d*m)^2:ℝ)
          ...
          exact h.1
      · refine le_of_lt <| SelbergSieve.selbergTerms_pos _ m <| dvd_of_mem_divisors hm
      · rfl
    apply sum_nonneg; intro m hm
    split_ifs
    · apply le_of_lt <| SelbergSieve.selbergTerms_pos _ m <| dvd_of_mem_divisors hm
    · rfl
  _ = _ := by
    conv => enter [1, 2, k]; rw [← ite_zero_mul]
    rw [←sum_mul, SelbergSieve.conv_selbergTerms_eq_selbergTerms_mul_nu _ hdP]
...
theorem _root_.SelbergSieve.selberg_bound_simple :
    siftedSum (s := s.toBoundingSieve) ≤
      X / S +
        ∑ d ∈ divisors P, if (d : ℝ) ≤ y then (3:ℝ) ^ ω d * |R d| else 0 := by
  let μPlus := SelbergSieve.selbergUbSieve s
  calc
    siftedSum ≤ X * mainSum μPlus + errSum μPlus :=
      SelbergSieve.siftedSum_le_mainSum_errSum_of_UpperBoundSieve _ μPlus
    _ ≤ _ := ?_
  gcongr
  ...
  · rw [hP]
    ...
    · intro hd
      ...
      · intro p hpp hpy
        ...
        rw [Sieve.prime_dvd_primorial_iff _ _ hpp]
    ...
    · intro h
      ...
      intro p hp
      erw [Sieve.prime_dvd_primorial_iff _ _ hp]
      intro hpy
      apply h.2 p hp
      trans ↑(Nat.floor z)
      · norm_cast
...
theorem _root_.Sieve.selbergBoundingSum_ge_sum_div (s : SelbergSieve) (hP : ∀ p:ℕ, p.Prime → (p:ℝ) ≤ s.level → p ∣ s.prodPrimes)
  ...
  calc ∑ l ∈ s.prodPrimes.divisors, (if l ^ 2 ≤ s.level then SelbergSieve.selbergTerms _ l else 0)
  ...
  · rw [←Finset.sum_filter]; apply Finset.sum_le_sum; intro l hl
    ...
    · rw [Sieve.prod_factors_sum_pow_compMult (Nat.floor s.level) _ s.nu]
      · exact hnu
      · exact hlsq
      · rw [ne_eq, Nat.floor_eq_zero, not_lt]
        exact s.one_le_level
    rw [SelbergSieve.selbergTerms_apply _ l]
    apply Sieve.prod_factors_one_div_compMult_ge _ _ hnu _ _ hlsq
    · intro p hpp hpl
      ...
      · rw [←Nat.factorization_le_iff_dvd _ hprod_ne_zero, Nat.factorization_pow]
        · intro p
          ...
          · trans m
            ...
            apply Real.sqrt_le_self _ s.one_le_level
  ...
  · intro i hi j hj hij t hti htj x hx
    ...
    have h : ∀ i j {n}, i ∣ s.prodPrimes → i ∣ x → x ∣ j ^ n → i ∣ j := by
      ...
      exact ((squarefree_of_dvd_prodPrimes hiP).dvd_pow_iff_dvd
        (by rintro rfl; simp_all)).mp (Trans.trans hix hij)
    have hidvdj : i ∣ j := by
...
theorem _root_.Sieve.boundingSum_ge_sum (s : SelbergSieve) (hnu : s.nu = (ζ : ArithmeticFunction ℝ).pdiv .id)
  ...
  · rw[←hnu]
    apply Sieve.selbergBoundingSum_ge_sum_div
    · intro p hpp hple
      apply hP p hpp hple
    · rw[hnu]
      exact Sieve.CompletelyMultiplicative.zeta.pdiv Sieve.CompletelyMultiplicative.id
    · intro n
      rw[hnu]
      apply div_nonneg
...
theorem _root_.Sieve.boundingSum_ge_log (s : SelbergSieve) (hnu : s.nu = (ζ : ArithmeticFunction ℝ).pdiv .id)
  ...
  · exact Sieve.boundingSum_ge_sum s hnu hP
...
theorem prod_gcd_of_dvd_prod {r : ℕ} {m : Fin r → ℕ} (_hm : ∀ i, m i ≠ 0)
...
theorem card_primeFactors_eq_sum_gcd {r : ℕ} {m : Fin r → ℕ} (hm : ∀ i, m i ≠ 0)
    (hcop : Pairwise (Nat.Coprime on m)) {d : ℕ} (hd : d ∣ ∏ i, m i) (_hd0 : d ≠ 0) :
...
lemma sum_sum_divisorsAntidiagonal_le (F : ℕ → ℝ) (_hF : ∀ n, 0 ≤ F n) (y : ℕ) :
...
lemma totient_powerset_swap (N : ℕ) (g : Finset ℕ → ℝ) :
    ∑ h ∈ Finset.Icc 1 N, ∑ S ∈ h.primeFactors.powerset, g S
      = ∑ S ∈ ((Finset.Icc 1 N).filter Nat.Prime).powerset,
          ∑ _h ∈ (Finset.Icc 1 N).filter (fun h => (∏ p ∈ S, p) ∣ h), g S := by
  classical
  apply Finset.sum_comm'
  intro h S
...
theorem pairSieve_boundingSum_ge (M h : ℕ) (z : ℝ) (hz : 1 ≤ z) (_heven : 2 ∣ h)
    (_hh : 0 < h) :
...
theorem oneScale_primePart_le {B : Finset ℕ} {N K : ℕ}
    (_hprime : ∀ p ∈ B, p.Prime) (hcard : B.card = K)
...
theorem oneScale_diffCard_le {B : Finset ℕ} {x₀ H K : ℕ}
    ...
    (hwin : ∀ p ∈ B, x₀ < p ∧ p ≤ x₀ + H)
    (hKH : 2*K*H ≤ x₀) (hx₀ : 4 ≤ x₀) (hH : 1 ≤ H) (_hK : 1 ≤ K) :
    pairSumOn ((∏ p ∈ B, p).divisors)
...
lemma os_neg_injOn (v : Finset ℕ) : Set.InjOn (fun p : ℕ => -(p:ℝ)) ↑v :=
  fun _p _ _q _ h => os_neg_injective h
...
theorem oneScale_sameCard_le {B : Finset ℕ} {x₀ H N K : ℕ}
    (hprime : ∀ p ∈ B, p.Prime) (hcard : B.card = K) (_hK : 2 ≤ K) (hN : 1 ≤ N) (hH : 1 ≤ H)
    (hwin : ∀ p ∈ B, x₀ < p ∧ p ≤ x₀ + H)
    (hsep : ∀ p ∈ B, ∀ q ∈ B, p < q → N < q - p)
    (hbig : 8 * (2*H)^(K+1) ≤ x₀)
    (hlogK : 2 * (1 + Real.log K) ≤ N) :
...
theorem sum_inv_divisors_mul {a b : ℕ} (h : a.Coprime b) (_ha : a ≠ 0) (_hb : b ≠ 0) :
    ∑ d ∈ (a*b).divisors, ((d:ℝ))⁻¹ =
...
/-- **Erdős Problem 884** (Daniel Larsen). It is NOT the case that
`sumDivisorInvPairwiseDifference =O[atTop] (1 + sumDivisorInvConsecutiveDifference)`,
i.e. the site problem #884 has a negative answer. -/
theorem erdos_884 :
