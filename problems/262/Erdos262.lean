...
Erdős Problem 262. Suppose `a₁ < a₂ < ⋯` is a sequence of positive integers such that for
every integer sequence `tₙ ≥ 1` the sum `∑ 1/(tₙ aₙ)` is irrational — an *irrationality
sequence*. How slowly can `aₙ` grow? Hančl showed such a sequence must satisfy

  `limsup_{n→∞} log₂(log₂ aₙ) / n ≥ 1`,

which is what `erdos_262` proves. The limsup is taken in `EReal`, so the statement keeps its
intended meaning when the quotient is unbounded above. The index is one-based, so the `n`-th
term of the sequence is `a n` and the denominator is `n + 1`.

Mathematical source: J. Hančl, "Expression of real numbers with the help of infinite
series", Acta Arith. 59 (1991), 97--104.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos262.lean` -/

section
/- leanprover/lean4:v4.33.0  mathlib v4.33.0 -/
/-
This is a Lean formalization of a solution to Erdős Problem 262.
https://www.erdosproblems.com/forum/thread/262
...
URLs:
- https://github.com/plby/lean-proofs/blob/main/ErdosProblems/Erdos262.md
-/
/-
...
#print axioms erdos_262
-- 'Erdos262.erdos_262' depends on axioms: [propext, Classical.choice, Quot.sound]
