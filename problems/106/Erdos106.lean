...
Erdős Problem 106: let `f(n)` be the largest total side length of `n` squares packed
inside the unit square. Is `f(k² + 1) = k` for every `k`? `erdos_106` proves this is
false.

Disproved by A. Raj Singh at `k = 4`, where `k² + 1 = 17`. The witness is an explicit
packing of seventeen squares — sixteen axis-parallel and one rotated — described in a
square of side four and then scaled by a quarter, whose total side length exceeds `4`.
Since `f` is a supremum, `4 < f 17` already contradicts `f (4 ^ 2 + 1) = 4`.

`attainableSideSums n` is `{t | ∃ P : Fin n → Square, IsPackingIn 1 P ∧ totalSideLength P = t}`
and `f n` is its `sSup`, so `f` is exactly the extremal function of the problem.
-/
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos106.lean` -/
...
#print axioms erdos_106
-- 'Erdos106.erdos_106' depends on axioms: [propext, Classical.choice, Quot.sound]
