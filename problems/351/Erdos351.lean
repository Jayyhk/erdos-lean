/-
Erdős Problem 351 — https://www.erdosproblems.com/351

Flattened single-file vendoring of Shashi456/erdos-formalizations `Erdos/P283/Proof_flat.lean`
(the shared `PolynomialEgyptianSums` development; the same bundle proves #283). The whole body
is wrapped in `namespace Erdos351`, the #283 wrapper is dropped, and the `formal-conjectures`
`True ↔ …` sentinel on `erdos_351` is removed. The superfluous `0 < natDegree` hypothesis from
the upstream wrapper is also dropped, since `corollary_7_pos_leading` needs only positive
leading coefficient.

The May 3 2026 proof (GPT-5.5 Pro, cleaned up by Liam Price; Kevin Barreto noticed #351 follows).
-/
import Mathlib
...
/-- `A` is strongly complete: for every finite `B`, every sufficiently large integer is a finite
subset sum of `A \ B`. -/
def HasCompleteImage (P : ℚ[X]) : Prop :=
  PolynomialEgyptianSums.IsStronglyComplete (imageSet P)

/-- **Erdős Problem 351.** For every `P : ℚ[X]` with positive leading coefficient, the set
`{P(n) + 1/n : n ∈ ℕ}` is strongly complete (Graham; the rational/Egyptian version, following
`PolynomialEgyptianSums.corollary_7_pos_leading`). -/
theorem erdos_351 : ∀ P : ℚ[X], 0 < P.leadingCoeff → HasCompleteImage P :=
  fun P hlc => PolynomialEgyptianSums.corollary_7_pos_leading P hlc
