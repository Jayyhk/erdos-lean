...
Erdős Problem 146 (the Erdős–Simonovits *degeneracy conjecture*): if `H` is bipartite and
`r`-degenerate, must its Turán number satisfy `ex(n, H) = O(n ^ (2 - 1/r))`?
`DegeneracyConjectureStatement` states the conjecture; `erdos_146`
refutes it, via a 2-degenerate bipartite `H` whose extremal number is at least
`c * n ^ (3/2 + ε)`, exceeding the conjectured `O(n ^ (3/2))`.

The original proof was found by: OpenAI (github.com/openai/ten-proofs, file
`CompactnessAndDegeneracy.lean`), linked from https://www.erdosproblems.com/146.

Vendored from that file, which develops Erdős problems 146 and 180 together; the whole
file is kept because the degeneracy argument uses lemmas from the compactness half. The
file's two top-level namespaces `CompactnessConjecture` and `TwoDegenerateGraphs` become
sub-namespaces of `Erdos146`, and the refutation — upstream `not_erdos_146`
...
theorem twoDegenerateExtremalCounterexample :
  ...
  · obtain ⟨minimum, hminimum⟩ :=
    ...
    have hmonotone :
      ...
      extremalNumber_monotone_of_no_isolated
...
theorem erdos_146 :
  ...
  have hupper' :
    ...
    norm_num
  have hlarge :=
    eventually_constant_le_positive_nat_rpow
      (C + 1) c ε hc hε
...
#print axioms erdos_146
-- 'Erdos146.erdos_146' depends on axioms: [propext, Classical.choice, Quot.sound]
