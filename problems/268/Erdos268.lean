...
/-
The swap contribution has bounded error.
-/
...
lemma swapContrib_bound (n : ℕ) (j i : Fin 3) :
  ...
  unfold M_mat;
  fin_cases i <;> simp +decide [Matrix.mulVec];
  ...
  · fin_cases j <;> simp +decide [ Ssets, Tsets ];
    ...
    · unfold S₂ T₂ c_coeff; norm_num [ Fin.sum_univ_succ, vecHead, vecTail ] ; ring_nf; norm_num;
      ...
      unfold m_const; norm_num; ring_nf;
      field_simp;
      exact ⟨ by exact le_of_sub_nonneg ( by ring_nf; positivity ), by exact le_of_sub_nonneg ( by ring_nf; positivity ) ⟩;
