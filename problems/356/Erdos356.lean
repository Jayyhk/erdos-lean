...
Erdős Problem 356. Is there some `c > 0` such that, for all sufficiently large `n`, there
exist integers `a₁ < ⋯ < a_k ≤ n` for which at least `c n ^ 2` distinct integers have the
form `∑_{u ≤ i ≤ v} aᵢ`? `erdos_356` proves that there is, with `c = 1 / 300000`.

Solved in the affirmative by Beker, whose explicit construction is formalized here: the
partial sums are `p t = t ^ 2 + t / b`, eventually taken with `b = Nat.sqrt N`, and a finite
collision-energy estimate gives the count. Note the question fails for the obvious choice
`aᵢ = i`.

`consecutiveSums a` is the image, over pairs `u ≤ v`, of `∑ i ∈ Finset.Icc u v, a i`.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos822/FiniteEnergy.lean` -/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos356.lean` -/
...
#print axioms erdos_356
-- 'Erdos356.erdos_356' depends on axioms: [propext, Classical.choice, Quot.sound]
