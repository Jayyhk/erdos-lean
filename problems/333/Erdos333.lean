...
theorem A_density_zero :
  ...
  have hcard_mono :
    ...
    have hIcc : Finset.Icc 0 N ⊆ Finset.Icc 0 (2 ^ (k + 1)) := by
      ...
      exact le_trans hx (le_of_lt hN_lt_pow)
  ...
  have hdyadic_bound :
    ...
    have hI_nonempty : I.Nonempty := by
      refine ⟨3, ?_⟩
      simp [I, Finset.mem_Icc]
      -- We need to show `2 ≤ k` (since `simp` simplified `3 ≤ k + 1` to `2 ≤ k`).
      -- This follows from `N ≥ 2^(k₀+3) ≥ 2^3 = 8`, so `k ≥ 2`.
      have : 8 ≤ N := by
        have : 8 ≤ 2 ^ (k₀ + 3) := by
          have : 3 ≤ k₀ + 3 := Nat.le_add_left 3 k₀
          have : 2 ^ 3 ≤ 2 ^ (k₀ + 3) := Nat.pow_le_pow_right (by norm_num : 1 ≤ 2) this
          norm_num at this
          exact this
        exact Nat.le_trans this hN
      have : 2 ^ 2 < N := by norm_num; omega
      -- `2^k ≤ N` and `N > 2^2` implies `k ≥ 2`.
      by_contra h
      have hk_lt : k < 2 := Nat.lt_of_not_ge h
      have : 2 ^ k < 2 ^ 2 := Nat.pow_lt_pow_right (by norm_num) hk_lt
      have hN_pos : N ≠ 0 := by omega
      have hpow_le : 2 ^ k ≤ N := by
        simpa [hk_def] using (Nat.pow_log_le_self 2 hN_pos)
      omega
...
/-- **Erdős Problem 333** (disproof). It is *not* true that for every density-zero set
`A ⊆ ℕ` there exists a `B` with `A ⊆ B + B` and `|B ∩ [0,N]| = o(√N)`: the explicit
density-zero set `A` constructed above is a counterexample. -/
theorem erdos_333 :
    ¬ ∀ A' : Set ℕ,
      Filter.Tendsto (fun N : ℕ => (countingFn A' N : ℝ) / (N : ℝ)) Filter.atTop (nhds 0) →
      ∃ B : Set ℕ, (A' ⊆ {x | ∃ b b' : ℕ, b ∈ B ∧ b' ∈ B ∧ x = b + b'}) ∧
        Filter.Tendsto (fun N : ℕ => (countingFn B N : ℝ) / Real.sqrt N) Filter.atTop (nhds 0) :=
  fun h => main_obstruction.2 (h A main_obstruction.1)

#print axioms erdos_333
-- 'Erdos333.erdos_333' depends on axioms: [propext, Classical.choice, Quot.sound]
