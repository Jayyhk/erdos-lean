...
Erdős Problem 191. Let `C > 0` be arbitrary. Is it true that, if `n` is sufficiently large
depending on `C`, then in any 2-colouring of the pairs from `{2, …, n}` there is some
`X ⊆ {2, …, n}` with all pairs inside `X` of one colour and

  `∑_{x ∈ X} 1 / log x ≥ C`?

`erdos_191` proves that it is. The answer is yes, proved by Rödl; Conlon, Fox and Sudakov
later gave the best-possible quantitative form.

The 2-colouring is carried by a `SimpleGraph` on `Vertices n = {x : ℕ // x ∈ Icc 2 n}`,
with the two colours being adjacency and non-adjacency, so `Monochromatic G X` is
`G.IsClique X ∨ G.IsIndepSet X`. The weight is `weight x = (Real.log x)⁻¹`, and `N` is
chosen after `C`, which is the "sufficiently large depending on `C`" of the question.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/Util/Ramsey.lean` -/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos191.lean` -/
...
#print axioms erdos_191
-- 'Erdos191.erdos_191' depends on axioms: [propext, Classical.choice, Quot.sound]
