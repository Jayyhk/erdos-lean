...
/-
Borsuk's conjecture is false: not every bounded set of diameter 1 in ℝᵈ partitions
into at most d+1 sets of diameter <1. (The Lean equality form
`∀ d ≥ 1, BorsukNumber d = d+1` is a strictly stronger statement, since it bundles
the easy lower bound; we state the literal upper-bound form here to match the
question as posed on erdosproblems.com/505.)
-/
theorem erdos_505 : ¬ ∀ (d : ℕ), d ≥ 1 → BorsukProperty d (d + 1) := by
  intro h
  have h947 : BorsukProperty 946 947 := h 946 (by norm_num)
  have h_le : BorsukNumber 946 ≤ 947 := Nat.sInf_le h947
  linarith [f_946_ge_1650]

#print axioms erdos_505
-- 'Erdos505.erdos_505' depends on axioms: [propext, Classical.choice, Quot.sound]
