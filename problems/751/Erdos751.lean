...
Erdős Problem 751 (DISPROVED). The page asks: *"Let G be a graph with chromatic
number χ(G) = 4. If m₁ < m₂ < ⋯ are the lengths of the cycles in G then can
min(mᵢ₊₁ − mᵢ) be arbitrarily large?"* The answer is no — Bondy–Vince give the
uniform bound 2: any such G has two distinct cycle lengths within distance 2,
...
theorem erdos_751_witness
    ...
    ∃ a b : ℕ, a < b ∧ b - a ≤ 2 ∧
      ...
      (∃ C : BV.Cycle (G := G), BV.Cycle.length (G := G) C = b) := by
  obtain ⟨C1, C2, h⟩ := Main.erdos_751_strong G (le_of_eq hχ.symm)
  rcases lt_trichotomy
      (BV.Cycle.length (G := G) C1) (BV.Cycle.length (G := G) C2) with hlt | heq | hgt
  · refine ⟨_, _, hlt, ?_, ⟨C1, rfl⟩, ⟨C2, rfl⟩⟩
    unfold Nat.dist at h
    omega
  · exfalso
    rw [heq, Nat.dist_self] at h
    omega
