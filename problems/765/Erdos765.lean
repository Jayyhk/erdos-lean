...
lemma sqrt_approx_eventually {ε : ℝ} (εpos : 0 < ε) :
  ...
  · have tt : Tendsto (fun n : ℕ ↦ (4 * (1 - ε) / √n + 4 / n) / (4 * ε * (2 - ε))) atTop (𝓝 0) := by
    ...
    replace tt :
      ...
      have h_eps : (4 * ε * (2 - ε)) ≠ 0 := by nlinarith
      refine tt.congr' ?_
      filter_upwards [eventually_gt_atTop 0] with n hn
      have hn' : (n : ℝ) > 0 := by exact_mod_cast hn
      have hsqrt : √(n : ℝ) > 0 := Real.sqrt_pos.mpr hn'
      have hsq : √(n : ℝ) ^ 2 = (n : ℝ) := Real.sq_sqrt (le_of_lt hn')
      field_simp
      linear_combination (ε - 1) * hsq
...
lemma _root_.SimpleGraph.extremalNumber_mono_left_of_no_iso {W : Type*} {H : SimpleGraph W}
  ...
  have hG'_edgeFinset : G'.edgeFinset.card = G.edgeFinset.card := by
