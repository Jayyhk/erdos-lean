...
Erdős Problem 309. How many integers can be written as a sum of distinct unit fractions with
denominators from `{1, …, N}`? Are there `o(log N)` such integers? `erdos_309` proves that
there are not — and, more precisely, that the count is asymptotic to `log N`.

It is trivial that the count is at most `log N + O(1)`; Yokota proved the matching lower
bound `log N - O(log log N)`. So the answer to the question as posed is no.

`IsRepresentable N m` is `∃ A ⊆ Finset.Icc 1 N, rec_sum A = (m : ℚ)`, with the reciprocal
sum computed exactly in `ℚ` and the denominators distinct because `A` is a `Finset`.
`F N` counts the representable integers, searching `range (N + 1)`, which is exact since any
such sum is at most `∑_{n ≤ N} 1/n`.

The conclusion bundles both parts: the sharp asymptotic `F N / log N → 1` and the requested
negative answer `¬ (F =o[atTop] log)`.
-/
...
scoped[Chebyshev] notation "ϑ" => Erdos309.chebyshev_first
...
private lemma _root_.Nat.coprime_prod {ι : Type*} (s : Finset ι) (f : ι → ℕ) (n : ℕ) :
...
private lemma _root_.Function.Antiperiodic.abs_periodic {f : ℝ → ℝ} {c : ℝ}
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/UnitFractions/ErdosProblems.lean` -/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos309.lean` -/
...
#print axioms erdos_309
-- 'Erdos309.erdos_309' depends on axioms: [propext, Classical.choice, Quot.sound]
