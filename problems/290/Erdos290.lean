...
-/
/-
`a + 2 ≤ b_val a`, strengthening `blargerthana`, so the smaller index `b_val a - 1`
of the consecutive pair is strictly greater than `a` (matching the problem's `b > a`).
-/
lemma a_add_two_le_bval (a : ℕ) : a + 2 ≤ b_val a := by
  have h_exp : a < 3 ^ (Nat.log 3 a + 1) := Nat.lt_pow_succ_log_self (by decide) _
  rw [show b_val a = 2 * 3 ^ (Nat.log 3 a + 1) from rfl]
  omega

theorem erdos_290 (a : ℕ) (ha : a > 0) :
    ∃ b, a < b ∧ b ≤ 6 * a ∧ v a (b + 1) < v a b := by
  use b_val a - 1
  have hb2 : 2 ≤ b_val a := batleast2 a
  have hbig : a + 2 ≤ b_val a := a_add_two_le_bval a
  have hsmall : b_val a ≤ 6 * a := bsmallerthansixa a ha
  ...
  have h_div : v a (b_val a) ∣ v a (b_val a - 1) := by
    ...
    · exact Rat.den_nz _;
    · exact Nat.ne_of_gt <| Rat.pos _;
  have heq : b_val a - 1 + 1 = b_val a := by omega
  refine ⟨by omega, by omega, ?_⟩
  rw [heq]
  refine lt_of_le_of_ne (Nat.le_of_dvd (Nat.pos_of_ne_zero ?_) h_div) ?_
  · exact Rat.den_nz _
  · have := pequaltothreevaluationofvab_final a ha
    have := pequaltothreevaluationofvabminusone a ha
    aesop

#print axioms erdos_290
-- 'Erdos290.erdos_290' depends on axioms: [propext, Classical.choice, Quot.sound]
