...
https://github.com/Woett/Miscellaneous/blob/main/Blueprint%20for%20formalizing%20solution%20to%20%23845.pdf
...
-/
theorem erdos_845_explicit (n : ℕ) (h : 0 < n) :
...
/-- A positive integer `n` is "`C`-representable" if it can be written as a sum of
distinct 3-smooth numbers whose largest is `≤ C` times the smallest. -/
def representable_with (C : ℝ) (n : ℕ) : Prop :=
  ∃ I : Finset ℕ, n = ∑ i ∈ I, A i ∧
    ∃ min_i ∈ I, ∀ i ∈ I, A min_i ≤ A i ∧ (A i : ℝ) ≤ C * A min_i

/-- Site-form conjecture of Erdős #845: for every `C > 0`, the set of `C`-representable
integers has asymptotic density 0. -/
def conjecture_845 : Prop :=
  ∀ C : ℝ, 0 < C →
    Filter.Tendsto
      (fun N : ℕ => (((Finset.range N).filter (fun n => representable_with C n)).card : ℝ) / N)
      Filter.atTop (nhds 0)

/-- Erdős #845 (disproved): the universal density-0 claim fails — for `C = 6`,
every positive integer is representable, so the density is `1`, not `0`. -/
theorem erdos_845 : ¬ conjecture_845 := by
  intro h
  have h6 := h 6 (by norm_num)
  have key : ∀ n : ℕ, 0 < n → representable_with 6 n := by
    intro n hn
    obtain ⟨I, hI_sum, min_i, hmin_mem, hbound⟩ := erdos_845_explicit n hn
    refine ⟨I, hI_sum, min_i, hmin_mem, fun i hi => ?_⟩
    obtain ⟨h1, h2⟩ := hbound i hi
    refine ⟨h1, ?_⟩
    have h2' : (A i : ℝ) < 6 * (A min_i : ℝ) := by exact_mod_cast h2
    linarith
  have h_ge : ∀ᶠ N : ℕ in Filter.atTop,
      (1 : ℝ) / 2 ≤
        (((Finset.range N).filter (fun n => representable_with 6 n)).card : ℝ) / N := by
    filter_upwards [Filter.eventually_ge_atTop 2] with N hN
    have hN_pos : 0 < N := by linarith
    have hN_real : (0 : ℝ) < N := by exact_mod_cast hN_pos
    have hsub : Finset.Ioo 0 N ⊆
        (Finset.range N).filter (fun n => representable_with 6 n) := by
      intro n hn
      rw [Finset.mem_Ioo] at hn
      rw [Finset.mem_filter, Finset.mem_range]
      exact ⟨hn.2, key n hn.1⟩
    have hcard : (N - 1 : ℕ) ≤
        ((Finset.range N).filter (fun n => representable_with 6 n)).card := by
      have h_ioo : (Finset.Ioo 0 N).card = N - 1 := by rw [Nat.card_Ioo]; omega
      calc (N - 1 : ℕ) = (Finset.Ioo 0 N).card := h_ioo.symm
        _ ≤ _ := Finset.card_le_card hsub
    have hcast : ((N - 1 : ℕ) : ℝ) = (N : ℝ) - 1 := by
      have h1 : (1 : ℕ) ≤ N := by linarith
      rw [Nat.cast_sub h1, Nat.cast_one]
    rw [le_div_iff₀ hN_real]
    have hN_ge_2 : (2 : ℝ) ≤ N := by exact_mod_cast hN
    have hcard_real : ((N : ℝ) - 1) ≤
        (((Finset.range N).filter (fun n => representable_with 6 n)).card : ℝ) := by
      have := (Nat.cast_le (α := ℝ)).mpr hcard
      rw [hcast] at this
      exact this
    linarith
  have h_lt : ∀ᶠ N : ℕ in Filter.atTop,
      (((Finset.range N).filter (fun n => representable_with 6 n)).card : ℝ) / N < 1 / 2 :=
    h6.eventually (eventually_lt_nhds (show (0 : ℝ) < 1 / 2 by norm_num))
  obtain ⟨N, hN_ge, hN_lt⟩ := (h_ge.and h_lt).exists
  linarith
