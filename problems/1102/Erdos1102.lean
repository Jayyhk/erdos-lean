...
/-! ## --- vendored: ErdosProblem1102PropertyP.lean --- -/
...
/-! ## --- vendored: ErdosProblem1102PropertyQDensity.lean --- -/
  ...
  upperDensity A ≤ ((B ∩ Set.Icc 1 M).ncard : ℝ) / M := by
    ...
    · -- Since B is periodic with period M, its natural density exists and is equal to the density in one period, which is |B ∩ [1, M]| / M.
      have hB_nat_density : HasNaturalDensity B ((B ∩ Set.Icc 1 M).ncard / M : ℝ) := by
        ...
        · simpa using Filter.Tendsto.add ( tendsto_const_nhds.mul (show Filter.Tendsto (fun n : ℕ => (n : ℝ)⁻¹) Filter.atTop (nhds 0) from tendsto_inv_atTop_nhds_zero_nat) ) ( tendsto_const_nhds.mul (show Filter.Tendsto (fun n : ℕ => (n : ℝ)⁻¹) Filter.atTop (nhds 0) from tendsto_inv_atTop_nhds_zero_nat) );
   ...
   ((B ∩ Set.Icc 1 M).ncard : ℝ) / M = ∏ p ∈ Finset.filter Nat.Prime (Finset.range (C + 1)), (1 - 1/(p:ℝ)^2)) := by
     ...
     · refine' ⟨ ∏ p ∈ Finset.filter Nat.Prime ( Finset.range ( C + 1 ) ), p ^ 2, Finset.prod_pos fun p hp => pow_pos ( Nat.Prime.pos ( Finset.mem_filter.mp hp |>.2 ) ) _, _, _ ⟩;
       ·
         ...
         intro n; constructor <;> intros h p hp hp' <;> specialize h p hp hp' <;> simp_all +decide [ Finset.prod_eq_prod_diff_singleton_mul <| show p ∈ Finset.filter Nat.Prime ( Finset.range ( C + 1 ) ) from Finset.mem_filter.mpr ⟨ Finset.mem_range.mpr <| by linarith, hp' ⟩, Nat.add_mod ] ;
       · -- The number of integers in [1, M] that are not congruent to b_p modulo p^2 for any prime p <= C is given by the Euler's totient function of M.
         have h_card : (Finset.filter (fun n => ∀ p ∈ Finset.filter Nat.Prime (Finset.range (C + 1)), ¬n ≡ b p [MOD p^2]) (Finset.Icc 1 (∏ p ∈ Finset.filter Nat.Prime (Finset.range (C + 1)), p^2))).card = (∏ p ∈ Finset.filter Nat.Prime (Finset.range (C + 1)), (p^2 - 1)) := by
           ...
           · simp +zetaDelta at *;
             intro a ha₁ ha₂ ha₃; split_ifs <;> simp_all +decide [ Nat.ModEq ] ;
           ...
           · grind;
           · simp +zetaDelta at *;
             intro n hn hn'; use if n = 0 then ∏ p ∈ Finset.range ( C + 1 ) with Nat.Prime p, p ^ 2 else n; split_ifs <;> simp_all +decide ;
             · refine' ⟨ Finset.prod_pos fun p hp => hn p ( Finset.mem_range_succ_iff.mp ( Finset.mem_filter.mp hp |>.1 ) ) ( Finset.mem_filter.mp hp |>.2 ), fun p hp hp' => _ ⟩;
               simp_all +decide [ Nat.ModEq, Finset.prod_eq_prod_diff_singleton_mul <| Finset.mem_filter.mpr ⟨ Finset.mem_range.mpr <| Nat.lt_succ_of_le hp, hp' ⟩ ];
  ...
  ((bad_candidates_i n x).card : ℝ) / (candidates x (W_sq n)).card < 1/3 := by
    ...
    obtain ⟨x₀, hx₀⟩ : ∃ x₀ : ℕ, ∀ x ≥ x₀, C * (W_sq n * n / (Nat.sqrt x : ℝ)) < 1 / 6 ∧ x / 2 ≥ W_sq n := by
      have hx₀ : Filter.Tendsto (fun x : ℕ => C * (W_sq n * n / (Nat.sqrt x : ℝ))) Filter.atTop (nhds 0) := by
        simpa using tendsto_const_nhds.mul ( tendsto_const_nhds.mul (show Filter.Tendsto (fun n : ℕ => (n : ℝ)⁻¹) Filter.atTop (nhds 0) from tendsto_inv_atTop_nhds_zero_nat) |> Filter.Tendsto.comp <| Filter.tendsto_atTop_atTop.mpr fun x => ⟨ x ^ 2, fun y hy => by nlinarith [ Nat.lt_succ_sqrt' y ] ⟩ );
  ...
  Filter.Tendsto (fun x => ((max_k n x ε + 5 : ℝ) * ((W_sq n : ℝ) * n / Real.sqrt x))) Filter.atTop (nhds 0) := by
    ...
    · -- We'll use the fact that `max_k` grows logarithmically with `x`.
      ...
      have h_factor : Filter.Tendsto (fun x => ((max_k n x ε : ℝ) + 5) / Real.sqrt x) Filter.atTop (nhds 0) := by
        simpa [ add_div ] using h_max_k_log.add ( tendsto_const_nhds.mul ( (show Filter.Tendsto (fun n : ℕ => (n : ℝ)⁻¹) Filter.atTop (nhds 0) from tendsto_inv_atTop_nhds_zero_nat).sqrt ) );
...
lemma natural_density_of_densities (A : Set ℕ) (d : ℝ)
    ...
    HasNaturalDensity A d := by
      ...
      · intro a' ha'; contrapose! ha'; simp_all +decide [ upperDensity, lowerDensity ] ;
        ...
        · exact fun b x hx => by rw [Filter.frequently_atTop] at ha'; obtain ⟨ y, hy₁, hy₂ ⟩ := ha' x; linarith [ hx y hy₁ ] ;
...
lemma C_lower_nonneg : C_lower ≥ 0 := by
  ...
  have h_density : ∀ k ≥ 1, ∀ R, n k ≤ R → R ≤ n (k + 1) → ((Finset.Icc 1 R).filter (fun a => a ∈ SF)).card / (R : ℝ) ≥ 6 / Real.pi^2 - C / k := by
    ...
    gcongr;
    simp +decide only [Finset.mem_Icc] at *; tauto;
...
theorem erdos_1102.upper_density_Q
  ...
  have h_card_ge_j_div_Aj : ∀ j, ((Set.range A ∩ Set.Icc 1 (A j)).ncard : ℝ) ≥ j := by
    ...
    have h_card_ge_j_div_Aj : ((Set.range A ∩ Set.Icc 1 (A j)).ncard : ℝ) ≥ Finset.card (Finset.image A (Finset.Icc 1 j)) := by
      ...
      · exact fun x hx => by obtain ⟨ y, hy, rfl ⟩ := Finset.mem_image.mp (Finset.mem_coe.mp hx); exact ⟨ Set.mem_range_self _, ⟨ Nat.one_le_iff_ne_zero.mpr <| by linarith [ h_inc <| show 0 < y from Finset.mem_Icc.mp hy |>.1 ], h_inc.monotone <| Finset.mem_Icc.mp hy |>.2 ⟩ ⟩ ;
...
/-! ## --- vendored: ErdosProblem1102PropertyOverP.lean --- -/
...
-/
def relevant_primes_overp (P : ℕ) (x : ℝ) : Finset ℕ :=
...
def relevant_pairs (P : ℕ) (x : ℝ) : Finset (ℕ × ℕ) :=
  (relevant_primes_overp P x).biUnion (fun p => (Finset.Icc 1 (a_bound p)).image (fun a => (p, a)))
...
-/
def sum_S1 (P : ℕ) (x : ℝ) : ℝ := ∑ p ∈ relevant_primes_overp P x, (a_bound p : ℝ) / p^2
def sum_S2 (P : ℕ) (x : ℝ) : ℝ := ∑ p ∈ relevant_primes_overp P x, (a_bound p : ℝ)
...
lemma sum_S1_le_sum_bound (P : ℕ) (x : ℝ) :
  sum_S1 P x ≤ ∑ p ∈ relevant_primes_overp P x, 1 / ((p : ℝ) * (Real.log (Real.log p))^2) := by
...
lemma sum_subset_le_tail (P : ℕ) (x : ℝ) (assumps : SieveAssumptions) :
  ∑ p ∈ relevant_primes_overp P x, 1 / ((p : ℝ) * (Real.log (Real.log p))^2) ≤ tail_val P := by
    refine' le_trans _ ( Summable.sum_le_tsum _ _ _ );
    refine' Finset.sum_le_sum fun p hp => _;
    · unfold relevant_primes_overp at hp; aesop;
...
lemma sum_S2_is_littleO (P : ℕ):
  (fun x => sum_S2 P x) =o[Filter.atTop] (fun x => x) := by
    -- By definition of `sum_S2`, we have `sum_S2 P x ≤ ∑ p ∈ relevant_primes_overp P x, p / (Real.log (Real.log p))^2`.
    have h_sum_S2_le : ∀ x : ℝ, sum_S2 P x ≤ ∑ p ∈ relevant_primes_overp P x, (p : ℝ) / (Real.log (Real.log p))^2 := by
      intro x;
      refine' Finset.sum_le_sum fun p hp => _;
      exact Nat.floor_le ( div_nonneg ( Nat.cast_nonneg _ ) ( sq_nonneg _ ) );
    -- The sum over relevant primes is bounded by the sum over all primes $p \le \sqrt{2x}$ by definition of `relevant_primes_overp`.
    have h_relevant_primes_le_all_primes : ∀ x : ℝ, ∑ p ∈ relevant_primes_overp P x, (p : ℝ) / (Real.log (Real.log p))^2 ≤ ∑ p ∈ Finset.filter (fun p => Nat.Prime p ∧ (p : ℝ) ≤ Real.sqrt (2 * x)) (Finset.range (Nat.floor (Real.sqrt (2 * x)) + 1)), (p : ℝ) / (Real.log (Real.log p))^2 := by
      intros x
      simp [relevant_primes_overp];
...
lemma bad_n_card_bound (P : ℕ) (x : ℝ) (W : ℕ) (hW : W > 0)
    (h_coprime : ∀ p ∈ relevant_primes_overp P x, Nat.Coprime W (p^2)) :
  (bad_n_total P x W).card ≤ (L_x x : ℝ) / W * (sum_S1 P x) + 2 * (sum_S2 P x) := by
    ...
    have h_union_card : ((bad_n_total P x W).card : ℝ) ≤ ∑ p ∈ (relevant_primes_overp P x), ∑ a ∈ (Finset.Icc 1 (a_bound p)), ((bad_n_for_pair x W p a).card : ℝ) := by
    ...
    have h_term_bound : ∀ p ∈ relevant_primes_overp P x, ∀ a ∈ Finset.Icc 1 (a_bound p), ((bad_n_for_pair x W p a).card : ℝ) ≤ (L_x x : ℝ) / (W * p^2) + 2 := by
      intros p hp a ha
...
/-! ## --- vendored: ErdosProblem1102PropertyQFastGrowing.lean --- -/
...
lemma A2_admissible : Admissible A2 := by
  ...
  rintro a ⟨ j, hj₁, rfl ⟩ ; rcases j with ( _ | _ | j ) <;> norm_num [ Nat.pow_succ', ← mul_assoc, Nat.mul_mod ] at *;
  have : 4 ∣ 2^(j+2) := ⟨2^j, by ring⟩; omega);
  use p^2 - 1;
...
lemma A4_admissible : Admissible A4 := by
  ...
  · -- For odd primes p, the elements j! - 1 are congruent to -1 mod p^2 for all j >= 2p.
    ...
    have h_odd_primes_classes : Finset.image (fun j => (Nat.factorial j - 1) % p^2) (Finset.range (2 * p)) ∪ {p^2 - 1} ⊂ Finset.range (p^2) := by
      ...
      · intro h; have := Finset.card_le_card h; simp_all +decide [ Finset.subset_iff ] ;
        ...
        refine' lt_of_le_of_lt ( Nat.add_le_add_right ( Finset.card_image_le ) _ ) _ ; norm_num;
  ...
  · cases hp.eq_two_or_odd <;> simp_all +decide;
    use 0; norm_num; intro a ha; rcases ha with ⟨ j, hj, rfl ⟩;
    have : (Nat.factorial j - 1) % 4 ≠ 0 := by
      ...
      · have heq : j + 1 + 1 + 1 + 1 = j + 4 := by omega
        ...
        have := Nat.factorial_pos (j + 4); omega
    exact this;
  ...
  · rw [ Nat.nth_eq_sInf ];
    ...
    · refine' le_csInf _ _ <;> norm_num;
      · refine' ⟨ 2 ^ ( n + 2 ) + 1, _, _ ⟩;
        ...
        · intro k hk; exact lt_of_le_of_lt ( Nat.nth_monotone ( show { x | x ∈ A1 }.Infinite from A1_infinite ) ( hk ) ) ( by rw [ ih ] ; exact by linarith [ pow_lt_pow_right₀ ( by decide : 1 < 2 ) ( by linarith : n + 1 < n + 2 ) ] ) ;
...
lemma A3_nth (n : ℕ) : Nat.nth (· ∈ A3) n = Nat.factorial (n + 1) + 1 := by
  ...
  · -- The (n+1)-th element of A3 is the smallest element in A3 that is greater than (n+1)! + 1.
    have h_next : Nat.nth (fun x => x ∈ A3) (n + 1) = sInf {x ∈ A3 | x > (n + 1)! + 1} := by
      ...
      · intro k hk; exact lt_of_le_of_lt ( Nat.nth_monotone ( show { x | x ∈ A3 }.Infinite from by exact Set.infinite_of_forall_exists_gt fun m => ⟨ ( m + 1 ) ! + 1, ⟨ m + 1, by linarith, rfl ⟩, by linarith [ Nat.self_le_factorial ( m + 1 ) ] ⟩ ) ( hk ) ) ( lt_of_le_of_lt ( ih.le ) h.2 ) ;
    ...
    (W_val x : ℝ) ≤ Real.exp (0.4 * Real.log x) := by
      ...
      have log_W_val_bound : ∀ᶠ x in Filter.atTop, Real.log (W_val x) ≤ 2 * Real.log 4 * (0.1 * Real.log x) := by
        ...
        have h_log_W_val_def : ∀ x, 1 < x → Real.log (W_val x) = ∑ p ∈ (Finset.range (Nat.floor (0.1 * Real.log x) + 1)).filter Nat.Prime, 2 * Real.log p := by
          intros x hx; unfold W_val; rw [ Nat.cast_prod ] ; rw [ Real.log_prod ] ; norm_num [ Finset.sum_mul _ _ _ ] ;
          aesop;
...
lemma tail_sum_inv_prime_sq_isBigO :
    ...
    have h_tail_bound : ∃ C > 0, ∀ x : ℝ, x ≥ 4 → ∀ N : ℕ, N ≥ Nat.ceil x → (∑ p ∈ Finset.Icc (Nat.ceil x) N, if Nat.Prime p then (1 / (p : ℝ)^2) else 0) ≤ C / (x * Real.log x) := by
      ...
      have h_abel : ∀ x : ℝ, x ≥ 4 → ∀ N : ℕ, N ≥ Nat.ceil x → (∑ p ∈ Finset.Icc (Nat.ceil x) N, if Nat.Prime p then (1 / (p : ℝ)^2) else 0) ≤ (Nat.primeCounting N : ℝ) / N^2 + ∑ m ∈ Finset.Icc (Nat.ceil x) N, (Nat.primeCounting m : ℝ) * (1 / m^2 - 1 / (m + 1)^2) := by
        ...
        have h_sum_split : (∑ p ∈ Finset.Icc (Nat.ceil x) N, if Nat.Prime p then (1 / (p : ℝ)^2) else 0) ≤ (Nat.primeCounting N : ℝ) / N^2 + ∑ m ∈ Finset.Ico (Nat.ceil x) N, (Nat.primeCounting m : ℝ) * (1 / m^2 - 1 / (m + 1)^2) := by
          ...
          · split_ifs <;> norm_num [ Nat.primeCounting ];
            · rw [ inv_eq_one_div, div_le_div_iff_of_pos_right ] <;> norm_cast <;> norm_num [ Nat.primeCounting' ];
              · grind;
          ...
          · rename_i k hk ih;
            ...
            · rw [ Finset.Icc_eq_cons_Ico ( Nat.ceil_le.mpr hk ), Finset.sum_cons ] at ih ; norm_num at *;
              split_ifs at * <;> ring_nf at * <;> norm_num at *;
              · linarith;
              · grind +revert;
              · linarith;
              · grind;
            · rw [ Nat.primeCounting, Nat.primeCounting ];
              rw [ Nat.primeCounting', Nat.count_succ ] ; aesop;
      ...
      have h_sum_bound : ∀ x : ℝ, x ≥ 4 → ∀ N : ℕ, N ≥ Nat.ceil x → (∑ m ∈ Finset.Icc (Nat.ceil x) N, (1 / (m * Real.log m) * (1 / m))) ≤ 2 / (x * Real.log x) := by
        ...
        have h_sum_bound : (∑ m ∈ Finset.Icc (Nat.ceil x) N, (1 / (m * Real.log m) * (1 / m))) ≤ (∑ m ∈ Finset.Icc (Nat.ceil x) N, (1 / (m - 1 : ℝ) - 1 / (m : ℝ)) / Real.log x) := by
          ...
          · linarith;
          · field_simp;
            rw [ div_le_div_iff₀ ] <;> nlinarith [ Real.log_pos ( by linarith : 1 < x ), Real.log_le_log ( by linarith ) ( by linarith : x ≤ m + 1 + 1 ) ];
        ...
        have h_telescope : (∑ m ∈ Finset.Icc (Nat.ceil x) N, (1 / (m - 1 : ℝ) - 1 / (m : ℝ))) = (1 / (Nat.ceil x - 1 : ℝ)) - (1 / (N : ℝ)) := by
          erw [ Finset.sum_Ico_eq_sum_range ];
          convert Finset.sum_range_sub' _ _ using 3 <;> push_cast <;> ring_nf;
          rw [ Nat.cast_sub ] <;> push_cast <;> linarith [ Nat.ceil_le_floor_add_one x ];
      ...
      have h_prime_counting_bound : ∃ C > 0, ∀ x : ℝ, x ≥ 4 → ∀ N : ℕ, N ≥ Nat.ceil x → (Nat.primeCounting N : ℝ) / N^2 ≤ C / (N * Real.log N) := by
        have h_prime_counting_bound : ∃ C > 0, ∀ N : ℕ, N ≥ 4 → (Nat.primeCounting N : ℝ) ≤ C * N / Real.log N := by
          ...
          · have := hM N ( Nat.lt_of_floor_lt ( not_le.mp hN' ) |> le_of_lt ) ; simp_all +decide [ mul_div_assoc ];
            exact le_trans this ( mul_le_mul_of_nonneg_right ( le_max_left _ _ ) ( by positivity ) );
        obtain ⟨ C, hC₀, hC ⟩ := h_prime_counting_bound;
        use C, hC₀;
        intro x hx N hN; rw [ div_le_iff₀ ] <;> ring_nf at * <;> norm_num at *;
        · convert ‹∀ N : ℕ, 4 ≤ N → ( N.primeCounting : ℝ ) ≤ C * N * ( Real.log N ) ⁻¹› N ( by exact_mod_cast hx.trans hN ) using 1 ; by_cases hN : N = 0 <;> simp +decide [ sq, mul_assoc, hN ];
  ...
  obtain ⟨ C, hC₀, hC ⟩ := h_bound; use C, 4; intros x hx; rw [ abs_of_nonneg ( tsum_nonneg fun _ => by positivity ), abs_of_nonneg ( Real.log_nonneg <| by linarith ), abs_of_nonneg ( by linarith ) ] ; convert hC x hx using 1 ; ring;
...
lemma sum_inv_sq_part_O :
  ...
  =O[Filter.atTop] (fun x => 1 / (Real.log x * Real.log (Real.log x))) := by
    ...
    have h_tail_sum : (fun x : ℝ => ∑' p : ℕ, if (p : ℝ) ≥ 0.1 * Real.log x ∧ Nat.Prime p then 1 / (p ^ 2 : ℝ) else 0) =O[Filter.atTop] (fun x : ℝ => 1 / ((0.1 * Real.log x) * Real.log (0.1 * Real.log x))) := by
      convert tail_sum_inv_prime_sq_isBigO.comp_tendsto ( show Filter.Tendsto ( fun x : ℝ => 0.1 * Real.log x ) Filter.atTop Filter.atTop from Filter.Tendsto.const_mul_atTop ( by norm_num ) ( Real.tendsto_log_atTop ) ) using 1;
    ...
    suffices h_simp : ∀ᶠ x in Filter.atTop, (Finset.card (Finset.filter (fun p : ℕ => 0.1 * Real.log x < p ∧ p ≤ Real.sqrt (2 * x) ∧ Nat.Prime p) (Finset.range (Nat.floor (Real.sqrt (2 * x)) + 1))) : ℝ) ≤ Real.sqrt (2 * x) by
      ...
      have h_final : ∀ᶠ x in Filter.atTop, Real.sqrt (2 * x) * (Real.exp (0.4 * Real.log x) / x) ≤ 1 / (Real.log x * Real.log (Real.log x)) := by
        ...
        suffices h_simplify' : Filter.Tendsto (fun x : ℝ => Real.sqrt 2 * Real.exp (-0.1 * Real.log x) * (Real.log x * Real.log (Real.log x))) Filter.atTop (nhds 0) by
          refine h_simplify'.congr' ?_ ; filter_upwards [ Filter.eventually_gt_atTop 0 ] with x hx
          norm_num [ Real.exp_neg, Real.exp_log hx, hx.le ] ; ring_nf;
          rw [ show x = ( Real.exp ( Real.log x ) ) by rw [ Real.exp_log hx ] ] ; norm_num [ Real.sqrt_eq_rpow, ← Real.exp_mul, ← Real.exp_neg, mul_assoc, mul_comm, mul_left_comm, hx.ne' ];
          norm_num [ ← Real.exp_add ] ; ring_nf ; norm_num;
  ...
  ∀ᶠ x in Filter.atTop, (Nat.floor x - Nat.ceil (x / 2) + 1 : ℝ) ≥ (W_val x : ℝ) := by
    ...
    have h_x_half_minus_one_ge_x_pow : ∀ᶠ x in Filter.atTop, (x / 2 - 1 : ℝ) ≥ Real.exp (0.4 * Real.log x) := by
      ...
      suffices h_div : ∀ᶠ x in Filter.atTop, (x : ℝ) ^ (3 / 5 : ℝ) / 2 ≥ 1 + 1 / (x : ℝ) ^ (2 / 5 : ℝ) by
        ...
        rw [ show ( 3 / 5 : ℝ ) = 1 - 2 / 5 by norm_num, Real.rpow_sub ] at hx₁ <;> norm_num at * <;> try linarith;
        rw [ show ( 2 / 5 : ℝ ) * Real.log x = Real.log ( x ^ ( 2 / 5 : ℝ ) ) by rw [ Real.log_rpow ( by positivity ) ] ] ; rw [ Real.exp_log ( by positivity ) ] ; ring_nf at * ; nlinarith [ inv_mul_cancel₀ ( ne_of_gt ( Real.rpow_pos_of_pos ( zero_lt_one.trans hx₂ ) ( 2 / 5 : ℝ ) ) ), Real.rpow_pos_of_pos ( zero_lt_one.trans hx₂ ) ( 2 / 5 : ℝ ) ];
...
lemma W_coprime_primes (x_nat : ℕ) (W : ℕ) (hW : W = W_val x_nat) :
    ∀ p ∈ PrimesInInterval (x_nat : ℝ), Nat.Coprime W (p^2) := by
      ...
      have h_gt : ∀ q ∈ Finset.range (Nat.floor (0.1 * Real.log x_nat) + 1), Nat.Prime q → q < p := by
        intro q hq hq'; have := Finset.mem_filter.mp hp; simp only [Finset.mem_range, PrimesInInterval, Finset.mem_filter] at *
...
lemma sufficient_condition_for_Q  (A : Set ℕ) (hA_adm : Admissible A) (hA_inf : A.Infinite) (C : ℝ)
    ...
    (h_growth : GrowthCondition A C) : PropertyQ A := by
      ...
      have h_exists_good_n : ∀ᶠ k in Filter.atTop, ∃ n ∈ Finset.Icc (Nat.ceil ((Nat.nth (· ∈ A) (J k - 1) : ℝ) / 2)) (Nat.floor (Nat.nth (· ∈ A) (J k - 1) : ℝ)), ∀ a ∈ A, a < n → Squarefree (n + a) := by
        have h_exists_good_n : ∀ᶠ k in Filter.atTop, (Nat.nth (· ∈ A) (J k - 1) : ℝ) ≥ 100 ∧ (J k : ℝ) * C_freq * failure_prob_sum_2 (Nat.nth (· ∈ A) (J k - 1)) < 1 ∧ (Nat.floor (Nat.nth (· ∈ A) (J k - 1) : ℝ) - Nat.ceil ((Nat.nth (· ∈ A) (J k - 1) : ℝ) / 2) + 1 : ℝ) ≥ W_val (Nat.nth (· ∈ A) (J k - 1)) := by
          ...
          have h_length_condition : Filter.Tendsto (fun k => (Nat.nth (· ∈ A) (J k - 1) : ℝ)) Filter.atTop Filter.atTop := by
            ...
            intro b; use b + 1; intro a ha; refine' le_trans _ ( Nat.le_nth _ );
        ...
        · obtain ⟨ n, hn₁, hn₂, hn₃ ⟩ := this;
          ...
          · have := Nat.nth_mem_of_infinite hA_inf ( Nat.count ( fun x => x ∈ A ) a ) ; simp_all +decide ;
        ...
        · norm_cast at *;
          rw [ Int.subNatNat_of_le ] at hk <;> norm_cast at * ; aesop;
          rw [ Nat.ceil_le ];
          rw [ Nat.floor_natCast ] ; linarith;
      ...
      have h_inf_good_n : Set.Infinite {n : ℕ | ∃ k, n ∈ Finset.Icc (Nat.ceil ((Nat.nth (· ∈ A) (J k - 1) : ℝ) / 2)) (Nat.floor (Nat.nth (· ∈ A) (J k - 1) : ℝ)) ∧ ∀ a ∈ A, a < n → Squarefree (n + a)} := by
        ...
        refine Set.infinite_of_forall_exists_gt ?_;
        intro b; obtain ⟨ i, hi ⟩ := h_inf_good_n ( 2 * b + 2 ) ; obtain ⟨ n, hn₁, hn₂ ⟩ := ha ( Max.max a i ) ( le_max_left _ _ ) ; use n; simp_all +decide [Nat.ceil_le] ;
        exact ⟨ ⟨ _, hn₁ ⟩, by rw [ div_le_iff₀ ] at hn₁ <;> norm_cast at * ; linarith [ hi ( Max.max a i ) ( le_max_right a i ) ] ⟩;
      exact h_inf_good_n.mono fun n hn => by obtain ⟨ k, hk₁, hk₂ ⟩ := hn; exact hk₂;
...
/-- **Erdős problem 1102** (solved). Van Doorn–Tao characterize the densities and
explicit examples of sequences satisfying properties $P$, $Q$, $\overline{P}$, and
$\overline{P}_\infty$, including the specific sequences $2^n \pm 1$ and $n! \pm 1$. -/
theorem erdos_1102 :
    -- Theorem 1 (P): every strict-mono enumeration of a HasPropertyP set has A j / j → ∞;
    -- and for every f → ∞ with f n ≠ 0 there is such an enumeration with A j / j ≤ f j.
    (∀ (A : ℕ → ℕ), StrictMono A → HasPropertyP (Set.range A) →
        Filter.Tendsto (fun j => (A j / j : ℝ)) Filter.atTop Filter.atTop) ∧
    (∀ (f : ℕ → ℕ), Filter.Tendsto f Filter.atTop Filter.atTop → (∀ n, f n ≠ 0) →
        ∃ A : ℕ → ℕ, StrictMono A ∧ HasPropertyP (Set.range A) ∧
        ∀ j : ℕ, (A j : ℝ) / j ≤ f j) ∧
    -- Theorem 2 (Q): every HasPropertyQ set has limsup (j / A j) ≤ 6/π²; and an enumeration
    -- exists achieving Tendsto (j / A j) → 6/π² with all A j squarefree.
    (∀ (A : ℕ → ℕ), StrictMono A → HasPropertyQ (Set.range A) →
        Filter.limsup (fun j : ℕ ↦ (j / A j : ℝ)) Filter.atTop ≤ 6 / Real.pi ^ 2) ∧
    (∃ A : ℕ → ℕ, StrictMono A ∧ (∀ j, Squarefree (A j)) ∧ HasPropertyQ (Set.range A) ∧
        Filter.Tendsto (fun j : ℕ ↦ (j / A j : ℝ)) Filter.atTop (nhds (6 / Real.pi ^ 2))) ∧
    -- Theorem 3 (P̄, P̄∞): every PropertyP_bar_infty set has upper density < 6/π²;
    -- ∀ ε > 0 some PropertyP_bar set has lower density ≥ 6/π² − ε.
    (∀ A : Set ℕ, PropertyP_bar_infty A → upperDensity A < 6 / Real.pi ^ 2) ∧
    (∀ ε > 0, ∃ A : Set ℕ, PropertyP_bar A ∧ lowerDensity A ≥ 6 / Real.pi ^ 2 - ε) ∧
    -- Theorem 4: 2ⁿ ± 1 and n! ± 1 all have PropertyQ.
    (PropertyQ A1 ∧ PropertyQ A2 ∧ PropertyQ A3 ∧ PropertyQ A4) :=
  ⟨erdos_1102.density_zero_of_P, erdos_1102.exists_sequence_with_P,
   erdos_1102.upper_density_Q, erdos_1102.lower_density_Q_exists,
   theorem_overp_i, theorem_overp_ii,
   A1_PropertyQ, A2_PropertyQ, A3_PropertyQ, A4_PropertyQ⟩

end

#print axioms erdos_1102
-- 'Erdos1102.erdos_1102' depends on axioms: [propext, Classical.choice, Quot.sound]
