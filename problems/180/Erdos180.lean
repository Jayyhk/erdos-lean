...
Erdős Problem 180 (the Erdős–Simonovits *compactness conjecture*): if a finite family of
graphs is cyclic — every member is connected, bipartite and contains a cycle — must the
family be compact, i.e. must its Turán-type extremal function be governed by a single
member?  `CompactnessConjectureStatement` states the conjecture; `erdos_180` refutes it.

The counterexample is `proposedFamily`, for which `proposedFamily_familyLittleO` and
`proposedFamily_uniformMemberLower` give incompatible upper and lower bounds.

The original proof was found by: OpenAI (github.com/openai/ten-proofs, file
`CompactnessAndDegeneracy.lean`), linked from https://www.erdosproblems.com/180.

Vendored from that file, which develops Erdős problems 180 and 146 together. Only the
part needed for 180 is kept: everything up to and including the refutation, which is the
whole of the file's `CompactnessConjecture` namespace up to that point (nothing after it
can be one of its dependencies). The namespace is renamed from `CompactnessConjecture` to
...
#print axioms erdos_180
-- 'Erdos180.erdos_180' depends on axioms: [propext, Classical.choice, Quot.sound]
