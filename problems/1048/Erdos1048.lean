...
-- The conjecture `erdos_1048` is inlined in the main theorem below (per repo convention).
...
/-- **Erdős Problem 1048** (Erdős–Herzog–Piranian [EHP58, p.142], refuted by
Pommerenke [Po61]): for any monic non-constant `f ∈ ℂ[x]` with all roots satisfying
`|z| ≤ r` for some `r < 2`, the open sublevel set `{z : |f(z)| < 1}` need NOT have
a connected component with diameter `> 2 − r`. Pommerenke's counterexample is
`f(z) = z^n − r^n` with `r > 1`: all `n` components of the level set have diameter
`→ 0` as `n → ∞`. The Lean adds `degree f ≥ 1` (Boris's note: ruling out the
constant polynomial `f = 1` which is otherwise a Mathlib-vacuous edge case). -/
theorem erdos_1048 :
    ¬ (∀ f : ℂ[X],
        f.Monic →
        degree f ≥ 1 →
  ...
  -- By `erdos_1048`, $U = \{z : |P(z)| < 1\}$ has a component $K$ with diameter $> 2 - 1.5 = 0.5$.
  ...
  have h_pom : Metric.diam (Subtype.val '' connectedComponent C) < 0.5 :=
    ‹∀ n ≥ hN, ∀ x : S n 1.5, Metric.diam ( Subtype.val '' connectedComponent x ) < 0.5›
      n (le_max_left _ _) C
  have h_chain : (0.5 : ℝ) < Metric.diam (Subtype.val '' connectedComponent C) :=
    lt_of_lt_of_le hx hC
  linarith

#print axioms erdos_1048
-- 'Erdos1048.erdos_1048' depends on axioms: [propext, Classical.choice, Quot.sound]
