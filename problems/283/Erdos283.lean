/-
**STANDALONE FLAT BUNDLE** for Erdős Problem #283 — POLYNOMIAL EGYPTIAN SUMS.
...
Adapted from the upstream P283+P351 bundle by stripping the #351 wrapper and the
`formal-conjectures` `True ↔ …` sentinel, leaving the literal headline `erdos_283`.
-/
...
/-
Erdős Problem 283 — literal headline.

`Erdos283.erdos_283` states the problem directly: no `formal-conjectures`
`True ↔ …` sentinel, statement inlined (no `Condition` abbreviation). It bridges
from the core `PolynomialEgyptianSums` development (`theorem_1`, with `α = 1`,
...
/-- **Erdős Problem #283.** For every integer polynomial `p` with positive leading
coefficient and no fixed divisor (no `d ≥ 2` dividing `p(n)` for all `n ≥ 1`),
every sufficiently large `m` is `p(n₁) + ⋯ + p(n_k)` for some distinct positive
integers `1 ≤ n₁ < ⋯ < n_k` with `1 = 1/n₁ + ⋯ + 1/n_k`.

Bridges from `PolynomialEgyptianSums.theorem_1` (with `α = 1`, `L = 1`, `p`
coerced from `ℤ[X]` to `ℚ[X]`). -/
theorem erdos_283 : ∀ p : ℤ[X],
    p.leadingCoeff > 0 → ¬ (∃ d ≥ 2, ∀ n ≥ 1, d ∣ p.eval n) →
    ∀ᶠ m in atTop, ∃ k ≥ 1, ∃ n : Fin (k + 1) → ℤ, 0 = n 0 ∧ StrictMono n ∧
      1 = ∑ i ∈ Finset.Icc 1 (Fin.last k), (1 : ℚ) / (n i) ∧
      m = ∑ i ∈ Finset.Icc 1 (Fin.last k), p.eval (n i) := by
  intro p hlc hnf
  rw [Filter.eventually_atTop]
