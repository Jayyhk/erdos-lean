...
/-- The Nat.factorization of a multiset product equals the sum of Nat.factorizations. -/
private lemma Nat.factorization_multiset_prod (s : Multiset ℕ) (h : (0 : ℕ) ∉ s) (p : ℕ) :
...
theorem Factorization.zero_total_imbalance {n : ℕ} (f : Factorization n)
  ...
  · have hbal := h_balance_zero p (mem_primesBelow.mpr ⟨lt_succ_of_le hp_le, hp⟩)
    ...
    simp only [Nat.factorization_multiset_prod f.a h0]
    omega
  · simp only [Nat.factorization_multiset_prod f.a h0,
      Nat.factorization_factorial_eq_zero_of_lt (lt_of_not_ge hp_le)]
    exact Multiset.sum_eq_zero fun x hx ↦ by
      obtain ⟨m, hm, rfl⟩ := Multiset.mem_map.mp hx
      exact Nat.factorization_eq_zero_of_lt ((f.ha m hm).trans_lt (lt_of_not_ge hp_le))
  · aesop
  · aesop
...
/-- Given a Nat.factorization `f`, an element `m` in it, and a new number `m' ≤ n`,
`replace` returns a new Nat.factorization with `m` replaced by `m'`. -/
...
/-- The sum of a function `F` over a Nat.factorization after replacing `m` with `m'`
...
/-- Extends a Nat.factorization by adding a new factor `m`. -/
...
/-- Adding a factor increases balance by the factor's Nat.factorization. -/
...
lemma Factorization.deficit_implies_le_n {n : ℕ} (f : Factorization n) (p : ℕ)
  ...
  contrapose! hp; simp_all [Nat.factorization_factorial_eq_zero_of_lt hp, balance]

/-- The Nat.factorization of a product of primes at `p` equals the count of `p` in the multiset. -/
lemma Nat.factorization_prod_eq_count {D : Multiset ℕ} (hD : ∀ p ∈ D, p.Prime) (p : ℕ) :
  ...
  | cons q D ih =>
    ...
    simp only [Multiset.prod_cons, Nat.factorization_mul hq.ne_zero hprod_ne, Finsupp.add_apply,
...
/-- Adding the full deficit multiset product to a Nat.factorization with no surplus primes and all
...
lemma Factorization.addFactor_deficit_balance_eq_zero {n : ℕ} (f : Factorization n) (L : ℕ)
  ...
  · simp_all only [CanonicallyOrderedAdd.multiset_prod_pos]
    simp only [Nat.factorization_prod_eq_count (fun q hq ↦ (mem_deficitMultiset f L q hq).1)]
...
deficit multiset yields a Nat.factorization with zero imbalance and lower score. -/
...
/-- Adding a submultiset `M` of deficit primes to a Nat.factorization reduces the total imbalance
by `M.card`. -/
lemma Factorization.addFactor_submultiset_total_imbalance {n : ℕ} (f : Factorization n) (L : ℕ)
  ...
  have h_bal : ∀ p ∈ (n + 1).primesBelow,
    ...
    rw [addFactor_balance, h_m_prod, Nat.factorization_prod_eq_count]
...
lemma Factorization.score_le_of_add_submultiset {n : ℕ} (f : Factorization n) (L : ℕ)
  ...
  have h_sum_eq : (∑ p ∈ (n + 1).primesBelow,
    ...
    · simp only [f', addFactor_balance]
      rw [h_m_prod, Nat.factorization_prod_eq_count (fun q hq ↦
...
lemma Factorization.lower_score_3_case2a {n : ℕ} (f : Factorization n) (L : ℕ)
  ...
  have h_bal_eq : ∀ q ∈ (n + 1).primesBelow, q ≠ p →
    ...
    rw [addFactor_balance, Nat.factorization_eq_zero_of_not_dvd, Int.ofNat_zero, add_zero]
    exact fun hdvd ↦
...
theorem Factorization.lower_score_3 {n : ℕ} (f : Factorization n) (L : ℕ)
  ...
  · by_cases hp : p.Prime
    · by_cases hp' : p ≤ n
      ...
      · have := fun m hm ↦ Nat.factorization_eq_zero_of_not_dvd fun h ↦ hp' <| (f.ha m hm).trans'
          (le_of_dvd (f.hpos m hm) h); simp [balance, sum, map_congr rfl this]
    · simp [balance, sum, Nat.factorization_eq_zero_of_not_prime _ hp]
  · have hp_prime : p.Prime := of_not_not fun hnp ↦ hp.ne <| by simp [balance, sum,
      Nat.factorization_eq_zero_of_not_prime _ hnp]
...
/-- Elements of the initial Nat.factorization lie in the interval `[n - n/M, n)`. -/
...
/-- For elements `m` in the initial Nat.factorization, `n/m ≤ (1 - 1/M)⁻¹`. -/
...
theorem Params.initial.balance_large_prime_ge (P : Params) {p : ℕ}
  ...
  have hsum : (P.initial.a.map (·.factorization p)).sum = 0 := sum_eq_zero fun x hx ↦ by
    ...
    obtain ⟨m, ⟨_, hsmooth⟩, rfl⟩ := hx
    rw [Nat.factorization_eq_zero_iff, mem_smoothNumbers'] at *
...
/-- For primes `p > √n`, the sum of `p`-adic valuations in the initial Nat.factorization is bounded by
...
lemma Params.initial.sum_valuation_le_M_mul_interval_count (P : Params) {p : ℕ}
  ...
  have hval : ∀ m ∈ S, m.factorization p ≤ if p ∣ m then 1 else 0 := fun m hm ↦ by
    ...
    split_ifs <;> simp_all [Nat.factorization_eq_zero_iff]
...
lemma Params.initial.valuation_eq_indicator (P : Params) {p m : ℕ} (hp : p.Prime)
  ...
  split_ifs with hs hd hd' <;> simp_all only [gt_iff_lt, Nat.factorization_eq_zero_iff,
      ...
      exact ⟨fun ⟨⟨h1, _⟩, h2⟩ ↦ ⟨h1, Nat.lt_succ_iff.mpr <| le_of_not_gt fun h ↦
        ...
        fun ⟨h1, h2⟩ ↦
          ⟨⟨h1, (le_of_lt_succ h2).trans_lt
            (Nat.factorization_lt p hm') |>.trans_le
...
/-- The sum of valuations in the initial Nat.factorization is bounded by `M` times the sum of
valuations in the interval. This is because the initial Nat.factorization is a subset of `M` copies
of the interval. -/
...
/-- For a small prime `p`, the sum of `p`-adic valuations in the initial Nat.factorization equals `M`
...
lemma Params.initial.sum_valuation_eq_small (P : Params) {p : ℕ} (hp : p.Prime)
  ...
  have h_sum_factorizations : (P.initial.a.map (·.factorization p)).sum =
    ...
    specialize hmp
    contrapose! hmp
    simp_all +decide only [Finset.mem_Ico, Nat.factorization_eq_zero_iff, false_or]
  ...
  have h_sum_factorizations_eq : ∀ m ∈ Finset.Ico (P.n - P.n / P.M) P.n, m.factorization p =
    ...
    · simp +contextual only [Finset.subset_iff, Finset.mem_Ico, true_and, and_imp]
      ...
      linarith [Finset.mem_Ico.mp hm, le_of_dvd (pos_of_ne_zero (by aesop_cat)) (ordProj_dvd m p),
        Nat.pow_le_pow_right hp.one_lt.le (show k ≤ Nat.factorization m p from by grind)]
    · simp +contextual only [Finset.mem_Ico, true_and, not_lt, ite_eq_right_iff, one_ne_zero,
      imp_false, and_imp]
      intro x hx₁ hx₂ hx₃
      contrapose! hx₃
      rw [← Nat.factorization_le_iff_dvd] at hx₃ <;> norm_num at *
      · simpa [hp] using hx₃ p
...
lemma Params.initial.balance_ge_neg_M_mul_log (P : Params) {p : ℕ} (hp : p.Prime)
  ...
  rw [Factorization.balance, Factorization.sum, initial.sum_valuation_eq_small P hp hp_le hp_gt,
    Nat.factorization_def _ hp, padicValNat_factorial]
...
theorem Params.initial.balance_small_prime_ge (P : Params) {p : ℕ} (hp : p ≤ Real.sqrt P.n)
  ...
  have h_bound_ℝ : (P.initial.balance p : ℝ) ≥ -(P.M * (Real.log P.n / Real.log p)) := by
    ...
    · simp only [cast_mul, Factorization.balance, hp_prime, not_false_eq_true,
        Nat.factorization_eq_zero_of_not_prime, CharP.cast_eq_zero, sub_zero]
      rw [neg_le_iff_add_nonneg']
      positivity
  exact le_trans (by
    rw [mul_div]; ring_nf; gcongr; norm_cast; linarith [P.hL_pos]) h_bound_ℝ

/-- The Nat.factorization consisting of `M` copies of `[n - n/M, n)`, without filtering for smooth
numbers. -/
...
private lemma Params.initial.score_bound_aux_imbalanced (P : Params)
  ...
  have hA4 : A4 ≤ RHS3 := by
    ...
    have hset :
      ...
      ext x
      simp [primesBelow, and_assoc, _root_.and_left_comm, and_comm]
...
/-- `A n` is the least number of factors, each in `[1, n²]`, whose product is `n!`.
This is the quantity `A(n)` of Erdős Problem 392. -/
noncomputable def A (n : ℕ) : ℕ :=
  sInf {t : ℕ | ∃ a : Fin t → ℕ, (∀ i, 1 ≤ a i ∧ a i ≤ n ^ 2) ∧ ∏ i, a i = n.factorial}

lemma A_set_nonempty (n : ℕ) :
    {t : ℕ | ∃ a : Fin t → ℕ, (∀ i, 1 ≤ a i ∧ a i ≤ n ^ 2) ∧ ∏ i, a i = n.factorial}.Nonempty := by
  refine ⟨n, fun i ↦ (i : ℕ) + 1, fun i ↦ ⟨Nat.le_add_left 1 _, ?_⟩, ?_⟩
  · calc (i : ℕ) + 1 ≤ n := i.isLt
      _ ≤ n ^ 2 := Nat.le_self_pow (by norm_num) n
  · rw [Fin.prod_univ_eq_prod_range (fun k ↦ k + 1) n]
    exact Finset.prod_range_add_one_eq_factorial n

lemma A_mem (n : ℕ) :
    ∃ a : Fin (A n) → ℕ, (∀ i, 1 ≤ a i ∧ a i ≤ n ^ 2) ∧ ∏ i, a i = n.factorial :=
  Nat.sInf_mem (A_set_nonempty n)

/-- Lower half via Stirling: eventually `n·log n − n ≤ log (n!)`. -/
lemma log_factorial_ge : ∀ᶠ n : ℕ in Filter.atTop,
    (n : ℝ) * Real.log n - n ≤ Real.log (n ! : ℝ) := by
  have h_ratio : Filter.Tendsto
      (fun n : ℕ ↦ (n ! : ℝ) / (Real.sqrt (2 * n * π) * (n / Real.exp 1) ^ n))
      Filter.atTop (nhds 1) := by
    have h := Stirling.factorial_isEquivalent_stirling
    rw [isEquivalent_iff_tendsto_one] at h
    · exact h
    · filter_upwards [Filter.eventually_gt_atTop 0] with n hn; positivity
  have h_ratio_ge : ∀ᶠ n : ℕ in Filter.atTop,
      (1 : ℝ) / 2 ≤ (n ! : ℝ) / (Real.sqrt (2 * n * π) * (n / Real.exp 1) ^ n) :=
    h_ratio.eventually (Metric.ball_mem_nhds 1 (by norm_num : (0:ℝ) < 1/2)) |>.mono fun n hn ↦ by
      simp only [Real.dist_eq] at hn; linarith [abs_sub_lt_iff.mp hn]
  filter_upwards [h_ratio_ge, Filter.eventually_gt_atTop 0] with n h_rat hn_pos
  have hn : (0 : ℝ) < n := Nat.cast_pos.mpr hn_pos
  have hn1 : (1 : ℝ) ≤ n := by exact_mod_cast hn_pos
  have h2npi : (0 : ℝ) < 2 * n * π := by positivity
  have hsqrt : Real.sqrt (2 * n * π) > 0 := by positivity
  have hpow : (n / Real.exp 1 : ℝ) ^ n > 0 := by positivity
  have hdenom : Real.sqrt (2 * n * π) * (n / Real.exp 1) ^ n > 0 := by positivity
  rw [le_div_iff₀ hdenom] at h_rat
  have hcorr : (0 : ℝ) ≤ Real.log (1 / 2) + Real.log (2 * n * π) / 2 := by
    have h4 : (4 : ℝ) ≤ 2 * n * π := by nlinarith [Real.pi_gt_three, hn1]
    have hmono : Real.log 4 ≤ Real.log (2 * n * π) := Real.log_le_log (by norm_num) h4
    have hlog4 : Real.log 4 = 2 * Real.log 2 := by
      rw [show (4 : ℝ) = 2 ^ 2 by norm_num, Real.log_pow]; ring
    have hlhalf : Real.log (1 / 2) = -Real.log 2 := by rw [one_div, Real.log_inv]
    rw [hlhalf]; linarith
  calc (n : ℝ) * Real.log n - n
      ≤ Real.log (1 / 2) + Real.log (2 * n * π) / 2 + n * Real.log n - n := by linarith [hcorr]
    _ = Real.log (1 / 2 * (Real.sqrt (2 * n * π) * (n / Real.exp 1) ^ n)) := by
        rw [Real.log_mul (by norm_num) hdenom.ne', Real.log_mul hsqrt.ne' hpow.ne',
          Real.sqrt_eq_rpow, Real.log_rpow h2npi, Real.log_pow,
          Real.log_div hn.ne' (Real.exp_pos 1).ne', Real.log_exp]
        ring
    _ ≤ Real.log (n ! : ℝ) := Real.log_le_log (by positivity) h_rat

/-- Upper half (from `Solution_2`): for every `ε>0`, eventually
`A n - (n/2 - n/(2 log n)) ≤ ε · (n / log n)`. -/
lemma A_upper (ε : ℝ) (hε : ε > 0) :
    ∀ᶠ n : ℕ in Filter.atTop,
      (A n : ℝ) - ((n : ℝ) / 2 - n / (2 * Real.log n)) ≤ ε * ((n : ℝ) / Real.log n) := by
  filter_upwards [Solution_2 ε hε, Filter.eventually_gt_atTop 1] with n hsol hn1
  obtain ⟨t, a, hprod, hcond⟩ := hsol
  have ht_pos : 0 < t := by
    rcases Nat.eq_zero_or_pos t with h | h
    · exfalso; subst h
      rw [Fin.prod_univ_zero] at hprod
      have := Nat.one_lt_factorial.mpr hn1
      omega
    · exact h
  have ha1 : ∀ i, 1 ≤ a i := by
    intro i
    rcases Nat.eq_zero_or_pos (a i) with h | h
    · exact absurd (hprod ▸ Finset.prod_eq_zero (Finset.mem_univ i) h) (Nat.factorial_pos n).ne'
    · exact h
  have ha2 : ∀ i, a i ≤ n ^ 2 := fun i ↦ (hcond i).1
  have hbound : (t : ℝ) ≤ (n : ℝ) / 2 - n / (2 * Real.log n) + ε * n / Real.log n :=
    (hcond ⟨0, ht_pos⟩).2
  have hmem : t ∈ {t : ℕ | ∃ a : Fin t → ℕ, (∀ i, 1 ≤ a i ∧ a i ≤ n ^ 2) ∧
      ∏ i, a i = n.factorial} := ⟨a, fun i ↦ ⟨ha1 i, ha2 i⟩, hprod⟩
  have hAt' : (A n : ℝ) ≤ (t : ℝ) := by exact_mod_cast Nat.sInf_le hmem
  rw [mul_div_assoc] at hbound
  linarith

/-- Lower half: eventually `0 ≤ A n - (n/2 - n/(2 log n))`, i.e. `A n ≥ n/2 - n/(2 log n)`
(every valid factorization has `n! ≤ (n²)^(A n)`, combined with Stirling). -/
lemma A_lower_nonneg :
    ∀ᶠ n : ℕ in Filter.atTop, 0 ≤ (A n : ℝ) - ((n : ℝ) / 2 - n / (2 * Real.log n)) := by
  filter_upwards [log_factorial_ge, Filter.eventually_gt_atTop 1] with n hstir hn1
  obtain ⟨a, hb, hp⟩ := A_mem n
  have hn : (1 : ℝ) < n := by exact_mod_cast hn1
  have hlogn : 0 < Real.log n := Real.log_pos hn
  have hprod_le : n.factorial ≤ (n ^ 2) ^ (A n) := by
    calc n.factorial = ∏ i, a i := hp.symm
      _ ≤ ∏ _i : Fin (A n), n ^ 2 := Finset.prod_le_prod' (fun i _ ↦ (hb i).2)
      _ = (n ^ 2) ^ (A n) := by rw [Finset.prod_const, Finset.card_univ, Fintype.card_fin]
  have hfact_real : (n.factorial : ℝ) ≤ ((n : ℝ) ^ 2) ^ (A n) := by exact_mod_cast hprod_le
  have hlog : Real.log (n.factorial : ℝ) ≤ (A n : ℝ) * (2 * Real.log n) := by
    calc Real.log (n.factorial : ℝ) ≤ Real.log (((n : ℝ) ^ 2) ^ (A n)) :=
          Real.log_le_log (by positivity) hfact_real
      _ = (A n : ℝ) * (2 * Real.log n) := by rw [Real.log_pow, Real.log_pow]; ring
  have h2logn : 0 < 2 * Real.log n := by positivity
  have key : ((n : ℝ) / 2 - n / (2 * Real.log n)) * (2 * Real.log n) ≤ (A n : ℝ) * (2 * Real.log n) := by
    have expand : ((n : ℝ) / 2 - n / (2 * Real.log n)) * (2 * Real.log n) = n * Real.log n - n := by
      field_simp
    rw [expand]; linarith
  linarith [le_of_mul_le_mul_right key h2logn]
...
theorem erdos_392 :
  ...
  intro ε hε
  filter_upwards [A_upper ε hε, A_lower_nonneg, Filter.eventually_gt_atTop 1]
    with n hup hlow hn1
  have hn : (1 : ℝ) < n := by exact_mod_cast hn1
  have hlogn : 0 < Real.log n := Real.log_pos hn
  have hg : 0 ≤ (n : ℝ) / Real.log n := by positivity
  rw [Real.norm_eq_abs, Real.norm_eq_abs, abs_of_nonneg hlow, abs_of_nonneg hg]
  exact hup
