...
-/
theorem erdos_476_nat (p : ℕ) [Fact p.Prime] (A : Finset (ZMod p)) :
...
/-- **Erdős Problem 476** (Erdős–Heilbronn). For `A ⊆ 𝔽_p`, with `A +̂ A = {a+b : a≠b ∈ A}`
(`restrictedSumset`), `|A +̂ A| ≥ min(2|A| − 3, p)` — here with honest integer subtraction. -/
theorem erdos_476 (p : ℕ) [Fact p.Prime] (A : Finset (ZMod p)) :
    ((restrictedSumset A).card : ℤ) ≥ min (2 * (A.card : ℤ) - 3) (p : ℤ) := by
  have h := erdos_476_nat p A
  have hcast : ((min (2 * A.card - 3) p : ℕ) : ℤ) ≤ ((restrictedSumset A).card : ℤ) := by
    exact_mod_cast h
  rw [Nat.cast_min] at hcast
  exact le_trans (min_le_min (by omega) le_rfl) hcast
