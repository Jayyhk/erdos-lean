    ...
    ·
      ...
      have h_valuation : ∀ p : ℕ, Nat.Prime p → p ∣ g → padicValNat p D > padicValNat p (Int.natAbs q) := by
        ...
        have h_valuation_step : padicValNat p D > padicValNat p (Int.natAbs q) := by
          ...
          split_ifs at h_valuation_step <;> simp_all +decide [ padicValNat ] ;
          · split_ifs <;> simp_all +decide ;
            exact absurd ‹lcm_range ( s * t ) = 0› ( by exact Nat.ne_of_gt <| Nat.recOn ( s * t ) ( by decide ) fun n ihn => Nat.lcm_pos ( Nat.succ_pos _ ) ihn );
          · have := ‹∀ x : ℕ, p ^ ( x + 1 ) ∣ lcm_range ( s * t ) › ( Nat.log p ( lcm_range ( s * t ) ) ) ; exact absurd this ( Nat.not_dvd_of_pos_of_lt ( Nat.pos_of_ne_zero ( by
              exact Nat.ne_of_gt <| Nat.recOn ( s * t ) ( by decide ) fun n ih => Nat.lcm_pos ( Nat.succ_pos _ ) ih; ) ) ( Nat.lt_pow_succ_log_self hp.one_lt _ ) ) ;
