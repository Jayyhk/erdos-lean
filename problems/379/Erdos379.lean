...
lemma round1_h_main₁ (R : ℕ) (hR : R ≥ 1) :
  ...
  have hN_gt_one : N ≠ 1 := by
    ...
    have h₃ : 2 ^ L₀ ≥ 4 := by
      have h₄ : L₀ ≥ 2 := h₁
      have h₅ : 2 ^ L₀ ≥ 2 ^ 2 := Nat.pow_le_pow_of_le_right (by norm_num) h₄
      omega
    have h₄ : N > 1 := by omega
    omega
...
lemma subgoal3 (q i R : ℕ) (hi1 : 1 ≤ i) (h_q_gt_R : q > R) (hR : R ≥ 1) :
  ...
  have h4 : q ^ i ≥ q := by
    ...
    have h7 : q ^ i ≥ q ^ 1 := by
      apply Nat.pow_le_pow_of_le_right h6 h5
    simpa using h7
  linarith
...
theorem S_unbounded_from_lemmas
  ...
  have h_main : ∃ (n : ℕ), S n ≥ b := by
    ...
    have h_bdd : BddAbove A := by
      ...
      have h₉ : p ^ s ≥ 2 ^ s := Nat.pow_le_pow_of_le_left h₈ s
      have h₁₀ : s ≥ n.choose 1 + 1 := by omega
      have h₁₁ : 2 ^ s ≥ 2 ^ (n.choose 1 + 1) := Nat.pow_le_pow_of_le_right (by norm_num) h₁₀
