...
/-- Site-form headline for Erdős Problem 897: both conjectures (Q1 and Q2) are false. -/
theorem erdos_897 :
    (¬ ∀ (f : ℕ → ℝ),
      (∀ᵉ (a > 0) (b > 0), a.Coprime b → f (a * b) = f a + f b) →
      ((Filter.atTop ⊓ Filter.principal {(p, k) : ℕ × ℕ | p.Prime}).limsup
  ...
  · exact (Bool.false_ne_true (erdos_897.parts.i.mp h)).elim
  · exact (Bool.false_ne_true (erdos_897.parts.ii.mp h)).elim

#print axioms erdos_897
-- 'Erdos897.erdos_897' depends on axioms: [propext, Classical.choice, Quot.sound]
