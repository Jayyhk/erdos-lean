...
Erdős Problem 405. Let `p` be an odd prime. Is it true that the equation

  `(p - 1)! + a ^ (p - 1) = p ^ k`

has only finitely many solutions? `erdos_405` proves that it does.

`IsSolution p a k` is the conjunction `p.Prime ∧ p ≠ 2 ∧ 0 < a ∧ 0 < k` with the equation
above, so the oddness of `p` is part of the predicate. Erdős and Graham stated the question
allowing `p = 2`, which is presumably an oversight since that case visibly has infinitely
many solutions.

Brindza and Erdős proved finiteness; Yu and Liu determined the solutions exactly, and that
sharper classification is available here as `erdos_405_classification`:
`IsSolution p a k` holds iff `(p, a, k)` is `(3, 1, 1)`, `(3, 5, 3)` or `(5, 1, 2)` — that
is, `2! + 1 ^ 2 = 3`, `2! + 5 ^ 2 = 3 ^ 3` and `4! + 1 ^ 4 = 5 ^ 2`.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos405.lean` -/
...
The main theorem `erdos_405_classification` classifies all positive-integer solutions of
...
theorem erdos_405_classification {p a k : ℕ} :
    IsSolution p a k ↔
      (p = 3 ∧ a = 1 ∧ k = 1) ∨
      (p = 3 ∧ a = 5 ∧ k = 3) ∨
      (p = 5 ∧ a = 1 ∧ k = 2) :=
...
theorem solution_set_eq_exceptional :
  ...
  ext t
  rcases t with ⟨p, a, k⟩
  simp [erdos_405_classification, exceptionalSolutions]
...
-- 'Erdos405.erdos_405' depends on axioms: [propext, Classical.choice, Quot.sound]
