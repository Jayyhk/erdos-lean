...
Erdős Problem 2 (Erdős's "perhaps my favourite problem", $1000). Can the smallest modulus
of a covering system be arbitrarily large? `erdos_2` proves that it cannot: there is a
single `M` such that every distinct covering system has a modulus below `M`.

The answer is no, contrary to Erdős's expectations, by Hough — building on Filaseta, Ford,
Konyagin, Pomerance and Yu — with a simpler proof and a better bound later given by
Balister, Bollobás, Morris, Sahasrabudhe and Tiba.

`IsDistinctCoveringSystem D a` is `(∀ d ∈ D, 2 ≤ d) ∧ ∀ z : ℤ, ∃ d ∈ D, z ≡ a d [ZMOD d]`.
The moduli are distinct because `D` is a `Finset`, and `2 ≤ d` excludes the trivial modulus
`1`, which would cover `ℤ` by itself. The equivalent literal negation of the question,
`¬ HasArbitrarilyLargeMinimum`, is also proved above as `not_hasArbitrarilyLargeMinimum`.
-/
...
scoped[Chebyshev] notation "ϑ" => Erdos2.chebyshev_first
...
-- 'Erdos2.erdos_2' depends on axioms: [propext, Classical.choice, Quot.sound]
