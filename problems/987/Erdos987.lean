...
Erdős Problem 987. For an infinite sequence `x₁, x₂, … ∈ (0,1)` let
`A_k = limsup_{n→∞} |∑_{j ≤ n} e(k xⱼ)|`, where `e(x) = e^{2πix}`. Is it true that
`limsup_{k→∞} A_k = ∞`? And is it possible for `A_k = o(k)`? `erdos_987` answers both
affirmatively.

This is Problem 7.21 in [Ha74], attributed to Erdős [Er64b, Er65b]. Erdős found a proof that
`A_k ≫ log k` infinitely often; Clunie [Cl67] improved this to `A_k ≫ k^{1/2}` infinitely
often and showed sequences with `A_k ≤ k` for all `k` exist. The two questions are
`erdos_987_question_one` and `erdos_987_question_two` below, and `erdos_987` is their
conjunction. `erdos_987_sqrt_log_upper_bound` records the stronger quantitative form used for
the second: a sequence with `‖∑_{j < n} e(k xⱼ)‖ ≤ C √(k log k)` for all `n` and all `k ≥ 2`,
which is `o(k)`.

Here `A x k` is `limsup_n ‖∑_{j ∈ range n} e (k * x j)‖` taken in `EReal`, so that the value
`∞` of the first question is the literal `⊤`; summing over `range n` rather than `j ≤ n` only
shifts the index and leaves the limsup unchanged.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos987.lean`. It is reproduced here with the file-level provenance
header removed, its project-internal imports removed so that `Mathlib` is the only import, and
the whole wrapped once in `namespace Erdos987`, with the upstream trust-base print lines and
trailing `alias`es removed. The parts are renamed from `erdos_987`, `erdos_987.parts.ii` and
`erdos_987.variants.sqrt_log_upper_bound`. No mathematical content is changed.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos987.lean` -/

section
/- leanprover/lean4:v4.33.0  mathlib v4.33.0 -/
/- Original license: Apache 2.0. Note: This file has been modified. -/
/-
This is a Lean formalization of a solution to Erdős Problem 987.
https://www.erdosproblems.com/forum/thread/987
...
URLs:
- https://github.com/plby/lean-proofs/blob/main/ErdosProblems/Erdos987.md
- https://github.com/google-deepmind/formal-conjectures/blob/main/FormalConjectures/ErdosProblems/987.lean
-/
/-
...
[teorth/analysis](https://github.com/teorth/analysis/blob/main/Analysis/Misc/erdos_987.lean).
-/
theorem erdos_987_question_one :
...
`erdos_987_sqrt_log_upper_bound`, which gives a $\sqrt{k \log k}$
...
* **Sub-Gaussian** (M=2 and linear; *deferred to the helper lemmas*): `μ{‖B‖ ≥ t} ≤ 4·exp(...)`,
...
Two focused sub-goals are discharged separately: `apssvShortEnvelope_summable_at_C100`
and `apssvLongEnvelope_summable_at_C100`. -/
...
constants and case analysis, modulo the deferred sub-Gaussian bounds
...
theorem erdos_987_sqrt_log_upper_bound :
...
theorem erdos_987_question_two :
  ...
  obtain ⟨x, hx, C, hC, hbound⟩ := erdos_987_sqrt_log_upper_bound
...
/-- **Erdős Problem 987.** Both questions on erdosproblems.com have an affirmative answer:
`limsup_k A_k = ∞` for every sequence in `(0,1)`, and there is a sequence for which
`A_k = o(k)`. -/
theorem erdos_987 :
    (∀ (x : ℕ → ℝ) (_ : ∀ j : ℕ, x j ∈ Set.Ioo (0 : ℝ) 1),
        atTop.limsup (fun k : ℕ => A x k) = ⊤) ∧
      (∃ (x : ℕ → ℝ) (_ : ∀ j : ℕ, x j ∈ Set.Ioo (0 : ℝ) 1) (b : ℕ → ℝ),
        b =o[atTop] (fun k : ℕ => (k : ℝ)) ∧
          ∀ᶠ k : ℕ in atTop, A x k ≤ ((b k : ℝ) : EReal)) :=
  ⟨erdos_987_question_one, erdos_987_question_two⟩

end

#print axioms erdos_987
-- 'Erdos987.erdos_987' depends on axioms: [propext, Classical.choice, Quot.sound]
