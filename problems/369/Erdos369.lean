...
lemma exists_nat_mul_mod_near (α β : ℝ) (hβ : 0 < β)
  ...
  have hγ_pos : 0 < γ ∨ γ < 0 := by
    ...
    have h_eq : (u₂ - u₁) * α = (⌊(u₂ * α) / β⌋ - ⌊(u₁ * α) / β⌋) * β := by
      exact eq_of_sub_eq_zero ( by push_neg at hγ_zero; linarith ) ;
...
Main theorem: Erdős Problem #369.
    ...
    for all m ∈ {a-k+1, ..., a}.
-/
/-- Stronger result actually established (SkyYang's construction): the `k` consecutive
integers in `[N/2, N]` are each `mᵉ`-smooth (i.e. `P⁺(m) ≤ mᵉ`), which is stronger than
`Nᵉ`-smooth. The intended statement `erdos_369` is derived from this. -/
theorem erdos_369_strong (ε : ℝ) (hε : 0 < ε) (k : ℕ) (hk : 2 ≤ k) :
    ∃ N₀ : ℕ, ∀ N : ℕ, N₀ ≤ N →
      ∃ a : ℕ, N / 2 ≤ a - (k - 1) ∧ a ≤ N ∧ k ≤ a ∧
        ∀ j : ℕ, j < k → (Nat.largestPrimeFactor (a - j) : ℝ) ≤ ((a - j : ℕ) : ℝ) ^ ε := by
  by_cases hε1 : ε ≥ 1;
...
theorem erdos_369 (ε : ℝ) (hε : 0 < ε) (k : ℕ) (hk : 2 ≤ k) :
    ∃ N₀ : ℕ, ∀ N : ℕ, N₀ ≤ N →
      ∃ a : ℕ, N / 2 ≤ a - (k - 1) ∧ a ≤ N ∧ k ≤ a ∧
        ∀ j : ℕ, j < k → (Nat.largestPrimeFactor (a - j) : ℝ) ≤ (N : ℝ) ^ ε := by
  obtain ⟨N₀, hN₀⟩ := erdos_369_strong ε hε k hk
  refine ⟨N₀, fun N hN => ?_⟩
  obtain ⟨a, h1, h2, h3, h4⟩ := hN₀ N hN
  refine ⟨a, h1, h2, h3, fun j hj => (h4 j hj).trans ?_⟩
  apply Real.rpow_le_rpow (by positivity) _ hε.le
  exact_mod_cast (Nat.sub_le a j).trans h2

end

#print axioms erdos_369
-- 'Erdos369.erdos_369' depends on axioms: [propext, Classical.choice, Quot.sound]
