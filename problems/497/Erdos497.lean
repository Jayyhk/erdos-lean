...
theorem erdos_497 :
    ∃ ε : ℕ → ℝ, Filter.Tendsto ε Filter.atTop (nhds 0) ∧
      ∀ n : ℕ, (A n : ℝ) = 2 ^ ((1 + ε n) * (n.choose (n / 2) : ℝ)) := by
  have hApos : ∀ n, 0 < A n := fun n =>
    Finset.card_pos.mpr ⟨∅, by simp [antichains, IsAntichain, Set.Pairwise]⟩
  have hne : ∀ᶠ n : ℕ in Filter.atTop, ((n.choose (n / 2) : ℝ)) ≠ 0 := by
    apply Filter.Eventually.of_forall; intro n
    exact_mod_cast (Nat.choose_pos (Nat.div_le_self n 2)).ne'
  refine ⟨fun n => Real.logb 2 (A n) / (n.choose (n / 2) : ℝ) - 1, ?_, ?_⟩
  · have hdiv := (Asymptotics.isEquivalent_iff_tendsto_one hne).mp erdos_497_log
    simpa using hdiv.sub_const 1
  · intro n
    have hC : (0:ℝ) < (n.choose (n / 2) : ℝ) := by exact_mod_cast Nat.choose_pos (Nat.div_le_self n 2)
    have hA : (0:ℝ) < (A n : ℝ) := by exact_mod_cast hApos n
    rw [show (1 + (Real.logb 2 (A n) / (n.choose (n / 2):ℝ) - 1)) = Real.logb 2 (A n) / (n.choose (n / 2):ℝ) by ring,
        div_mul_cancel₀ _ (ne_of_gt hC), Real.rpow_logb (by norm_num) (by norm_num) hA]
