...
/-- **Erdős Problem 1026** — exact per-`n` form (Tao, Boris Alexeev, llllvvuu, KoishiChan,
Aristotle, AlphaEvolve, Baek–Koizumi–Ueoro): for every `n = k² + 1 + 2a` with `1 < k`
and `−k < a ≤ k`, the optimal constant
`c(n) := inf_x maxMonotoneSubseqSum(x) / ∑x` (over distinct-positive sequences of
length `n`) equals `k / (k² + a)`. -/
theorem erdos_1026_exact
    (k n : ℕ) (a : ℤ)
    (hk : 1 < k)
    (ha_low : -k < a)
    (ha_high : a ≤ k)
    (hn : (n : ℤ) = k^2 + 1 + 2 * a) :
    c_opt n = (k : ℝ) / (k^2 + a) :=
  c_opt_eq_k_div_sq_add_a k n a hk ha_low ha_high hn

/-- **Erdős Problem 1026** — Cambie's conjecture as stated on erdosproblems.com/1026
(proved by Tidor–Wang–Yang [TWY16] / implicit in Wagner [Wa17]; formalized by
Aristotle — the site's recorded formalization): for every `k ≥ 1` and every
distinct-positive sequence `x₁,…,x_{k²}` summing to `1`, there is a monotonic
subsequence of sum at least `1 / k`. This is the site's "stronger conjecture"
clause, and in particular gives the site's headline answer `c = 1` (since
`√(k²) · (1/k) = 1`). For `k = 1` the singleton subsequence works; for `k ≥ 2`
derived from `erdos_1026_exact` by setting `K = k - 1, A = (k - 1 : ℤ)`. -/
theorem erdos_1026 (k : ℕ) (hk : 1 ≤ k)
    ...
    ∃ (m : ℕ) (s : Fin (m + 1) → Fin (k ^ 2)),
      ...
      (Monotone (fun i => x (s i)) ∨ Antitone (fun i => x (s i))) ∧
      (1 : ℝ) / k ≤ ∑ i, x (s i) := by
  rcases eq_or_lt_of_le hk with hk_eq | hk_gt
  · -- k = 1: the singleton subsequence works.
    have hk1 : k = 1 := hk_eq.symm
    have hk_sq_pos : 0 < k ^ 2 := by rw [hk1]; norm_num
    refine ⟨0, fun _ => ⟨0, hk_sq_pos⟩, ?_, .inl ?_, ?_⟩
    · -- StrictMono via val comparison on Fin (0 + 1).
      rintro ⟨a, ha⟩ ⟨b, hb⟩ hab
      have h_lt : a < b := hab
      omega
    · -- Monotone: constant function.
      intro a b _
      exact le_refl _
    · -- 1/1 ≤ x at the unique index.
      subst hk1
      simp only [Nat.cast_one, div_one]
      -- Reduce the subsequence sum (constant function over Fin 1).
      rw [Finset.sum_const, Finset.card_univ, Fintype.card_fin, zero_add, one_smul]
      -- Reduce `∑ i : Fin (1^2), x i` to `x ⟨0, _⟩` using that the univ is a singleton.
      have h_univ_singleton :
          (Finset.univ : Finset (Fin (1 ^ 2))) = {⟨0, hk_sq_pos⟩} := by
        ext i
        simp only [Finset.mem_univ, Finset.mem_singleton, true_iff]
        apply Fin.ext
        have hi := i.isLt
        have h1sq : (1 : ℕ) ^ 2 = 1 := rfl
        omega
      rw [h_univ_singleton, Finset.sum_singleton] at h_sum
      linarith
  · -- k ≥ 2: use Boris's machinery.
    have hk_ge_2 : 2 ≤ k := hk_gt
    have hk1 : (1 : ℕ) ≤ k := by omega
    have hK_pos : 0 < k - 1 := by omega
    have hA_low : -((k - 1 : ℕ) : ℤ) ≤ ((k - 1 : ℕ) : ℤ) := by push_cast; omega
    have hA_high : ((k - 1 : ℕ) : ℤ) ≤ ((k - 1 : ℕ) : ℤ) := le_refl _
    have hn : ((k ^ 2 : ℕ) : ℤ) =
        ((k - 1 : ℕ) : ℤ) ^ 2 + 1 + 2 * ((k - 1 : ℕ) : ℤ) := by
      push_cast [Nat.cast_sub hk1]; ring
    obtain ⟨m, s, hs_mono, hs_monAnt, hs_sum⟩ :=
      exists_monotone_subseq_sum_ge_general_range (k - 1) (k ^ 2) ((k - 1 : ℕ) : ℤ)
        hK_pos hA_low hA_high hn x h_pos h_inj h_sum
    refine ⟨m, s, hs_mono, hs_monAnt, ?_⟩
    have hk_pos_R : (0 : ℝ) < (k : ℝ) := by exact_mod_cast (by omega : 0 < k)
    have hk1_pos_R : (0 : ℝ) < (k : ℝ) - 1 := by
      have : (1 : ℝ) < (k : ℝ) := by exact_mod_cast (by omega : 1 < k)
      linarith
    have hk_ne : (k : ℝ) ≠ 0 := ne_of_gt hk_pos_R
    have hk1_ne : (k : ℝ) - 1 ≠ 0 := ne_of_gt hk1_pos_R
    push_cast [Nat.cast_sub hk1] at hs_sum
    have hbound_eq :
        ((k : ℝ) - 1) / (((k : ℝ) - 1) ^ 2 + ((k : ℝ) - 1)) = 1 / (k : ℝ) := by
      have hfact : ((k : ℝ) - 1) ^ 2 + ((k : ℝ) - 1) = ((k : ℝ) - 1) * (k : ℝ) := by ring
      rw [hfact]
      field_simp
    have h_one_le : (1 : ℝ) / (k : ℝ) ≤
        ((k : ℝ) - 1) / (((k : ℝ) - 1) ^ 2 + ((k : ℝ) - 1)) := le_of_eq hbound_eq.symm
    exact h_one_le.trans hs_sum

#print axioms erdos_1026
-- 'Erdos1026.erdos_1026' depends on axioms: [propext, Classical.choice, Quot.sound]
