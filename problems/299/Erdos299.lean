...
Is there an infinite sequence `a₁ < a₂ < ⋯` with `a_{i+1} - a_i = O(1)` and no finite
...
reciprocal sum `1`, contradicting the hypothesis.  `not_erdos299` is taken verbatim from
`plby/unit-fractions` `src4/ErdosProblems.lean`; `erdos_299` restates it in the
`formal-conjectures` form (Big-O gaps, real index-sum).
...
lemma Finset.cast_lcm {x : Finset ℕ} : ((x.lcm id : ℕ) : ℤ) = x.lcm (fun n => (n : ℤ)) := by
  ...
  · intro a s ha hs
    simpa only [Finset.lcm_insert, id_eq] using
      hs ▸ (Erdos299.cast_lcm (x := a) (y := s.lcm id))
...
/-- Bloom's density theorem (problem 298), `rec_sum` form. -/
...
/-- **Problem 299 (plby/unit-fractions form).** No strictly-increasing positive integer
sequence with uniformly bounded gaps avoids every finite reciprocal subsum equal to `1`. -/
theorem not_erdos299 :
  ...
  have hfreq : ∃ᶠ N : ℕ in atTop, (1 / (D : ℝ)) ≤ partial_density A N := by
    rw [Nat.frequently_atTop_iff_infinite]
    exact hTinf
  have hupper : (1 / (D : ℝ)) ≤ upper_density A := by
    exact le_limsup_of_frequently_le hfreq (is_bounded_under_le_partial_density (A := A))
  have hA : 0 < upper_density A := by
    exact lt_of_lt_of_le (one_div_pos.mpr hDpos) hupper
  rcases erdos298 A hA with ⟨S, hS, hrecS⟩
...
open Asymptotics in
/-- **Erdős Problem 299** (`formal-conjectures` form): there is *no* infinite sequence
`a₁ < a₂ < ⋯` of positive integers with `a_{i+1} - a_i = O(1)` such that no finite sum of
`1/aᵢ` equals `1`.  Derived from `not_erdos299`. -/
theorem erdos_299 :
    ¬ ∃ a : ℕ → ℕ, StrictMono a ∧ (∀ n, 0 < a n) ∧
      (fun n ↦ (a (n + 1) : ℝ) - a n) =O[atTop] (1 : ℕ → ℝ) ∧
      ∀ S : Finset ℕ, ∑ i ∈ S, (1 : ℝ) / a i ≠ 1 := by
  rintro ⟨a, ha, hpos, hbigO, hnosum⟩
  refine not_erdos299 ⟨a, ha, fun i => hpos i, ?_, ?_⟩
  · -- bounded gaps from the Big-O hypothesis
    obtain ⟨c, hc⟩ := isBigO_iff.mp hbigO
    rw [eventually_atTop] at hc
    obtain ⟨N₀, hN₀⟩ := hc
    refine ⟨⌈c⌉₊ + (Finset.range N₀).sup (fun i => a (i + 1) - a i), fun i => ?_⟩
    by_cases hi : N₀ ≤ i
    · have h2 : ((a (i + 1) - a i : ℕ) : ℝ) = (a (i + 1) : ℝ) - a i := by
        rw [Nat.cast_sub (ha.monotone (Nat.le_succ i))]
      have h1 : ‖(a (i + 1) : ℝ) - a i‖ ≤ c := by
        simpa [Pi.one_apply] using hN₀ i hi
      rw [Real.norm_eq_abs, abs_of_nonneg (by rw [← h2]; positivity)] at h1
      have hle : a (i + 1) - a i ≤ ⌈c⌉₊ := by
        have : ((a (i + 1) - a i : ℕ) : ℝ) ≤ c := by rw [h2]; exact h1
        exact_mod_cast this.trans (Nat.le_ceil c)
      omega
    · have hmem : i ∈ Finset.range N₀ := Finset.mem_range.mpr (by omega)
      have hsup := Finset.le_sup (f := fun i => a (i + 1) - a i) hmem
      dsimp only at hsup
      omega
  · -- index-sum ↔ image-sum via injectivity
    intro S hrec
    refine hnosum S ?_
    have hcast : ((rec_sum (S.image a) : ℚ) : ℝ) = ∑ i ∈ S, (1 : ℝ) / a i := by
      simp only [rec_sum, Rat.cast_sum]
      push_cast
      rw [Finset.sum_image (fun x _ y _ h => ha.injective h)]
    rw [← hcast, hrec]
    norm_num

#print axioms erdos_299
-- 'Erdos299.erdos_299' depends on axioms: [propext, Classical.choice, Quot.sound]
