...
Erdős Problem 358. Let `A = {a₁ < a₂ < ⋯}` be an infinite sequence of integers and let `f(n)`
count the solutions of `n = ∑_{u ≤ i ≤ v} aᵢ`. Is there such an `A` with `f(n) → ∞`? Or even
one with `f(n) ≥ 2` for all large `n`? `erdos_358` answers both affirmatively.

`f A n` below is `Nat.card {(u, v) | 0 < u ∧ 0 < v ∧ n = ∑ i ∈ Icc u v, A i}`, the number of
representations of `n` as a sum of consecutive terms. The two questions are
`erdos_358_question_one` and `erdos_358_question_two`, and `erdos_358` is their conjunction.
The second follows from the first, since `f A n → ∞` forces `2 ≤ f A n` eventually; a single
sequence therefore witnesses both.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos358.lean`. It is reproduced here with the file-level provenance
header removed, its project-internal imports removed so that `Mathlib` is the only import, and
the whole wrapped once in `namespace Erdos358`, with the upstream trust-base print lines and
trailing `alias`es removed. The two parts are renamed from `erdos_358` and
`erdos_358_part_ii` (upstream `erdos_358.parts.i` and `erdos_358.parts.ii`). No mathematical
content is changed.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos358.lean` -/
...
/-- **Erdős Problem 358.** Both questions on erdosproblems.com have an affirmative answer:
there is a strictly increasing sequence whose consecutive-sum representation function tends to
infinity, and hence one for which it is at least `2` from some point on. -/
theorem erdos_358 :
    (∃ A, StrictMono A ∧ atTop.Tendsto (Erdos358.f A) atTop) ∧
      (∃ A, StrictMono A ∧ ∀ᶠ n in atTop, 2 ≤ Erdos358.f A n) :=
  ⟨erdos_358_question_one, erdos_358_question_two⟩

end

#print axioms erdos_358
-- 'Erdos358.erdos_358' depends on axioms: [propext, Classical.choice, Quot.sound]
