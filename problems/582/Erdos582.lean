...
-/
theorem erdos_582 : ∃ (V : Type) (_ : Fintype V) (_ : DecidableEq V) (G : SimpleGraph V), G.CliqueFree 4 ∧ EdgeRamseyTriangle G := by
  refine ⟨VertexG, inferInstance, inferInstance, GraphG, ?_, folkman_theorem⟩
  intro t hclique
  have hle : 4 ≤ GraphG.cliqueNum := hclique.card_eq ▸ hclique.isClique.card_le_cliqueNum
  rw [GraphG_cliqueNum_eq_three] at hle
  omega
