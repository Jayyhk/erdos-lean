...
-/
/-! ## NthPrimeAsymp vendored proof of nth_prime_asymp -/
...
/-! ## --- Dependency chain for nth_prime_asymp (vendored from PrimeNumberTheoremAnd, Defs.lean + Consequences.lean lines 901-960) --- -/
...
theorem log_Q_k_bound_eventually :
  ...
  have h_log_Q_upper_bound : ∃ C > 0, ∀ᶠ E in Filter.atTop, ∀ k < E, Real.log (Q_k E k) ≤ E * Real.log (C * (E : ℝ)^2 * Real.log (E : ℝ)) := by
    ...
    rw [ Real.log_prod fun i hi => Nat.cast_ne_zero.mpr <| Nat.Prime.ne_zero <| by unfold p_kj; aesop ] ; exact le_trans ( Finset.sum_le_sum fun i hi => hE i ( Finset.mem_Icc.mp hi |>.1 ) ( Finset.mem_Icc.mp hi |>.2 ) ) ( by norm_num ) ;
...
theorem log_M_bound_eventually :
  ...
  have hM_log : ∀ E ≥ 10, Real.log (M E) ≤ Real.log (2^E) + Real.log 3 + ∑ k ∈ Finset.range E, Real.log (Q_k E k) := by
    ...
    · rw [ Real.log_prod fun i hi => Nat.cast_ne_zero.mpr <| by exact Finset.prod_ne_zero_iff.mpr fun j hj => Nat.Prime.ne_zero <| Nat.prime_nth_prime _ ];
...
#print axioms erdos_205
-- 'Erdos205.erdos_205' depends on axioms: [propext, Classical.choice, Quot.sound]
