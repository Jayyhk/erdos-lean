...
lemma g_eq_two_f (P : Finset Point) (u : ℝ) : g P u = 2 * f P u := by
  ...
  have hmap : (s : Set (Point × Point)).MapsTo Sym2.mk t := by
    intro pq hpq
    have hpq' : pq ∈ P.offDiag := (Finset.mem_filter.mp hpq).1
    ...
    have hmem : Sym2.mk pq ∈ P.sym2 := by
  ...
  have h_fiber_card :
    ...
    have hpair' :
      ...
      · intro hpq
        rcases (Finset.mem_filter.mp hpq) with ⟨hpq_s, hpq_eq⟩
        rcases hpq_eq with hpq_eq | hpq_eq
        · exact Finset.mem_filter.mpr ⟨hpq_s, (Sym2.mk_eq_mk_iff.mpr (Or.inl hpq_eq))⟩
        · exact Finset.mem_filter.mpr ⟨hpq_s, (Sym2.mk_eq_mk_iff.mpr (Or.inr hpq_eq))⟩
    have hcard : #{pq ∈ s | Sym2.mk pq = Sym2.mk (a, b)} = 2 := by
      calc
        #{pq ∈ s | Sym2.mk pq = Sym2.mk (a, b)}
  ...
  have hcard :
      s.card = ∑ z ∈ t, 2 := by
    calc
      s.card = ∑ z ∈ t, #{pq ∈ s | Sym2.mk pq = z} := h_fiber
      _ = ∑ z ∈ t, 2 := by
            refine Finset.sum_congr rfl ?_
            intro z hz
...
-- 'Erdos94.erdos_94' depends on axioms: [propext, Classical.choice, Quot.sound]
