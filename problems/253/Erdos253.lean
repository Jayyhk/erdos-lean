...
Erdős Problem 253. Let `1 ≤ a₁ < a₂ < ⋯` be integers with `a_{i+1}/a_i → 1`. If every infinite arithmetic progression contains infinitely many integers that are sums of distinct `a_i`, must every sufficiently large integer be a sum of distinct `a_i`? `erdos_253` disproves this.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos253.lean`, a single self-contained file whose only import is
`Mathlib`. It is reproduced here with the file-level comment header, the upstream trust-base
print lines and the trailing `alias` removed, wrapped once in `namespace Erdos253`, and with the
final theorem named `erdos_253` (upstream `not_erdos_253`). No mathematical content is changed.
-/
...
#print axioms erdos_253
-- 'Erdos253.erdos_253' depends on axioms: [propext, Classical.choice, Quot.sound]
