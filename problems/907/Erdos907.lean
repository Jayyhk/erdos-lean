...
lemma reduction_to_periodic (f : ℝ → ℝ) (hf : HasContinuousDifferences f) :
  ...
  · intro h
    ...
    rw [this]
    exact (hf h).sub ((buildG_continuous c hc |>.comp (continuous_add_right h)).sub
...
theorem erdos_907_of_all_h (f : ℝ → ℝ)
...
theorem erdos_907 (f : ℝ → ℝ)
  ...
  erdos_907_of_all_h f <| hasContinuousDifferences_of_pos f hf

#print axioms erdos_907
-- 'Erdos907.erdos_907' depends on axioms: [propext, Classical.choice, Quot.sound]
