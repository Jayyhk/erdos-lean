...
/-- **Erdős Problem 1080** ([Er75; Er79g], refuted by de Caen–Székely [DeSz92]):
there is **no** constant `c > 0` such that every bipartite graph `G` on `n` vertices
in which one part has `⌊n^(2/3)⌋` vertices and `G` has `≥ c · n` edges must contain
a `C_6`. The Lazebnik–Ustimenko–Woldar graph family `B(q)` (formalized here) gives
the counterexample. -/
theorem erdos_1080 :
...
#print axioms erdos_1080
-- 'Erdos1080.erdos_1080' depends on axioms: [propext, Classical.choice, Quot.sound]
