...
/-- **Erdős Problem 613** (Pikhurko disproof, n = 5 case in literal decomposition form).
There exists a graph `G` with `Nat.choose 11 2 - Nat.choose 5 2 - 1 = 44` edges that
is **not** the union of a bipartite graph and a graph of maximum degree `< 5`,
disproving Erdős's conjecture ([Er81e]) for `n = 5`. We use Tao's witness graph
(`PikhurkoN5.G`) and derive the literal decomposition disproof from
`PikhurkoN5.main`. -/
theorem erdos_613 :
    ∃ (V : Type) (_ : Fintype V) (_ : DecidableEq V) (G : SimpleGraph V),
      G.edgeSet.ncard = 44 ∧
      ¬ ∃ (G₁ G₂ : SimpleGraph V),
        G = G₁ ⊔ G₂ ∧ G₁.IsBipartite ∧ ∀ v : V, (G₂.neighborSet v).ncard < 5 := by
  refine ⟨PikhurkoN5.V, inferInstance, inferInstance, PikhurkoN5.G,
    PikhurkoN5.edge_count_44, ?_⟩
  rintro ⟨G₁, G₂, hG_eq, hG₁_bip, hG₂_deg⟩
  classical
  -- Coloring derived from the decomposition: 0 if in G₂, 1 if not (i.e. in G₁).
  let color : Sym2 PikhurkoN5.V → Fin 2 :=
    fun e => if e ∈ G₂.edgeSet then 0 else 1
  by_cases hStar : hasMonoStar PikhurkoN5.G color 0 5
  · -- Mono-star case: 5 color-0 (i.e. G₂) neighbours of some x
    obtain ⟨x, S, hcard, _, hprop⟩ := hStar
    have hSnbr : ∀ y ∈ S, y ∈ G₂.neighborSet x := by
      intro y hy
      obtain ⟨_, hcol⟩ := hprop hy
      have hin : s(x, y) ∈ G₂.edgeSet := by
        by_contra h
        change (if s(x, y) ∈ G₂.edgeSet then (0 : Fin 2) else 1) = 0 at hcol
        rw [if_neg h] at hcol
        exact absurd hcol (by decide)
      exact (SimpleGraph.mem_edgeSet G₂).mp hin
    have h5 : 5 ≤ (G₂.neighborSet x).ncard := by
      have hsubset : (↑S : Set PikhurkoN5.V) ⊆ G₂.neighborSet x :=
        fun y hy => hSnbr y hy
      have hScard : (↑S : Set PikhurkoN5.V).ncard = 5 := by
        rw [Set.ncard_coe_finset]; exact hcard
      rw [← hScard]
      exact Set.ncard_le_ncard hsubset (Set.toFinite _)
    exact absurd (hG₂_deg x) (by omega)
  · -- ¬ mono-star ⇒ mono-triangle in color 1 (= G₁), contradicting bipartite
    have hTri := PikhurkoN5.red_triangle_of_no_blue_star color hStar
    obtain ⟨a, b, c, hab, hbc, hac, hcab, hcbc, hcac⟩ := hTri
    have toG₁ : ∀ {u v : PikhurkoN5.V}, PikhurkoN5.G.Adj u v →
        color s(u, v) = 1 → G₁.Adj u v := by
      intro u v huv hcol
      by_cases h : s(u, v) ∈ G₂.edgeSet
      · change (if s(u, v) ∈ G₂.edgeSet then (0 : Fin 2) else 1) = 1 at hcol
        rw [if_pos h] at hcol
        exact absurd hcol (by decide)
      · have hsup : (G₁ ⊔ G₂).Adj u v := hG_eq ▸ huv
        rcases hsup with h1 | h2
        · exact h1
        · exact absurd ((SimpleGraph.mem_edgeSet G₂).mpr h2) h
    have h1ab : G₁.Adj a b := toG₁ hab hcab
    have h1bc : G₁.Adj b c := toG₁ hbc hcbc
    have h1ac : G₁.Adj a c := toG₁ hac hcac
    have htri : G₁.IsNClique 3 {a, b, c} :=
      SimpleGraph.is3Clique_triple_iff.mpr ⟨h1ab, h1ac, h1bc⟩
