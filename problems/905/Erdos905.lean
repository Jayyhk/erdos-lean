...
Khadžiivanov–Nikiforov independently in 1979).

Every graph on `n` vertices with more than `n^2 / 4` edges contains an edge
which lies in at least `n / 6` triangles. Stated cleared of fractions:
`n ≤ 6 * t(e)`, where `t(e) = triangleDegree G e`.
-/
theorem erdos_905 (G : SimpleGraph V) [DecidableRel G.Adj]
  ...
  refine ⟨e, heE, ?_⟩
  have hlt := card_div_six_lt_maxTriangleDegree_of_quarter_sq_lt_edges G h
  rw [← hmax] at hlt
  omega

#print axioms erdos_905
-- 'Erdos905.erdos_905' depends on axioms: [propext, Classical.choice, Quot.sound]
