...
/-- **Erdős Problem 1037** (Chen–Erdős conjecture [Er93,p.347], refuted by
Cambie–Hunter–KoishiChan): for every `ε > 0` and every `C > 0`, every sufficiently
large graph `G` on `n` vertices with at least `(1/2 + ε) · n` distinct degrees
contains a trivial (empty or complete) subgraph on more than `C · log n` vertices.
The formal statement follows Thomas Bloom's precise interpretation of Erdős's
"imprecise question" (erdosproblems.com/forum/thread/1037#post-3422). The
construction of Cambie–Hunter–KoishiChan refutes it. -/
theorem erdos_1037 :
    ¬ (∀ ε : ℝ, 0 < ε →
        ∀ C : ℝ, 0 < C →
        ∃ n₀ : ℕ, ∀ n ≥ n₀,
...
#print axioms erdos_1037
-- 'Erdos1037.erdos_1037' depends on axioms: [propext, Classical.choice, Quot.sound]
