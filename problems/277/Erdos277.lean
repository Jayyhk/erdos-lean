...
Erdős Problem 277. Is it true that for every `c` there is `n` with `σ(n) > c n` but no covering system whose moduli are all distinct divisors of `n` greater than `1`? `erdos_277` proves this.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos277.lean`, a single self-contained file whose only import is
`Mathlib`. It is reproduced here with the file-level comment header, the upstream trust-base
print lines and the trailing `alias` removed, wrapped once in `namespace Erdos277`, and with the
final theorem named `erdos_277`. No mathematical content is changed.
-/
...
#print axioms erdos_277
-- 'Erdos277.erdos_277' depends on axioms: [propext, Classical.choice, Quot.sound]
