...
lemma _root_.SimpleGraph.IsGreedyClique.isPrefix {l' : List V} (hl' : l' <+: l) : G.IsGreedyClique l' := by
  ...
  | nil =>
    rw [prefix_nil] at hl'
    simp [hl', SimpleGraph.IsGreedyClique.nil]
...
lemma _root_.SimpleGraph.not_isPSequence_of_qIndex_eq_length (hl : G.qIndex l = l.length) : ¬G.IsPSequence l := by
  ...
  contrapose! hl
  unfold SimpleGraph.IsPSequence at hl
  replace hl := hl.2
...
lemma _root_.SimpleGraph.IsPSequence.sum_degree_get_le_mul : ∑ i, G.degree (pl.get i) ≤ G.qIndex l * n V := by
  ...
  simp only [SimpleGraph.IsPSequence.get, Fin.getElem_fin, get_eq_getElem, getElem_map, map_take, getElem_take]
  congr!
  simp
...
lemma _root_.SimpleGraph.IsPSequence.maximalFor_qClass_self {c : Fin (G.qIndex l + 1)} :
  ...
  exact ⟨pl.greedyClique.qClass_getElem_eq bc (by lia),
    fun v hv _ ↦ (pl.greedyClique.maximalFor_take bc).le
      (adj_take_of_qClass (Fin.val_eq_of_eq hv))⟩
...
lemma _root_.SimpleGraph.IsPSequence.card_qClass_eq_cast {i : Fin (G.qIndex l)} :
  ...
  simp_rw [decide_not, Bool.not_eq_eq_eq_not, Bool.not_true, decide_eq_false_iff_not,
    Bool.not_false, decide_eq_true_eq, mem_sdiff, SimpleGraph.IsPSequence.cnr, mem_filter_univ, SimpleGraph.IsPSequence.get]
  conv_rhs =>
    enter [2, 1, j]
...
lemma _root_.SimpleGraph.IsPSequence.card_qClass_eq_last : #{v | G.qClass l v = Fin.last _} = #(pl.cnr (Fin.last _)) := by
  ...
  rw [Fin.sum_univ_sub (fun i ↦ #(pl.cnr i)) hf, show #(pl.cnr 0) = n V by simp [SimpleGraph.IsPSequence.cnr]] at key
  ...
  calc
    ...
    _ = ∑ i : Fin (G.qIndex l), #(pl.cnr i.castSucc) * (d i.castSucc : ℤ) +
      ...
      have n_anti {i : Fin (G.qIndex l)} : #(pl.cnr i.succ) ≤ #(pl.cnr i.castSucc) :=
        card_le_card pl.cnr_succ_subset_cnr_castSucc
      simp_rw [Fin.sum_univ_castSucc, SimpleGraph.IsPSequence.card_qClass_eq_cast pl, SimpleGraph.IsPSequence.card_qClass_eq_last pl,
    ...
    _ = n V * d 0 + ∑ i : Fin (G.qIndex l), #(pl.cnr i.succ) * (d i.succ : ℤ) -
      ...
      simp [SimpleGraph.IsPSequence.cnr]
    _ = _ := by
...
lemma _root_.SimpleGraph.IsPSequence.equation_9 {i : Fin (G.qIndex l)} : n V - ∑ j ≤ i, pl.adeg j.castSucc ≤ #(pl.cnr i.succ) := by
  ...
  · simp_rw [SimpleGraph.IsPSequence.cnr, mem_filter_univ] at hv ⊢
    exact fun j hj ↦ hv ⟨j.1, by lia⟩ (Nat.le_of_lt_succ hj)
  · classical
    rw [sum_comm]
    exact sum_le_sum fun v _ ↦ by rw [sum_ite, sum_const_zero, zero_add, ← card_eq_sum_ones, SimpleGraph.IsPSequence.adeg,
  ...
  calc
    ...
    _ ≤ n V * G.degree (pl.get 0) +
      ...
      · rw [sub_nonpos, Nat.cast_le]
        have := pl.greedyClique.degree_antitone
        rw [pairwise_iff_get] at this
...
lemma _root_.SimpleGraph.IsPSequence.equation_11c :
  ...
  have {i : Fin (G.qIndex l)} : (n V - pl.adeg i.castSucc : ℤ) = G.degree (pl.get i.castSucc) := by
    rw [SimpleGraph.IsPSequence.adeg, Fin.lastCases_castSucc,
      Nat.cast_sub (G.degree_lt_card_verts _).le, sub_sub_cancel]
...
theorem _root_.SimpleGraph.IsPSequence.equation_12 : r ≤ G.qIndex l + 1 := by
  ...
  have bq : G.qIndex l + 1 ∈ Set.Icc 1 (n V) :=
    ⟨by lia, pl.qIndex_lt_length.trans_le pl.greedyClique.nodup.length_le_card⟩
...
theorem _root_.SimpleGraph.IsPSequence.mul_length_sub_one_le_sum_take :
  ...
  congr!
  simp only [getElem_finRange, Fin.cast_mk, SimpleGraph.IsPSequence.get,
    Fin.castLE_mk, Fin.getElem_fin]
  exact getElem_take
...
theorem _root_.SimpleGraph.exists_isPSequence : ∃ l, G.IsPSequence l := by
  ...
  refine ⟨l, ?_⟩
  unfold SimpleGraph.IsPSequence
  refine ⟨gc, fun v ↦ ?_⟩
  contrapose! maxl
...
theorem _root_.SimpleGraph.erdos_904 : ∃ s, G.IsNClique r s ∧ 2 * r * #G.edgeFinset ≤ n V * ∑ v ∈ s, G.degree v := by
  ...
  have nodup_take : (l.take r).Nodup := pl.greedyClique.take.nodup
  ...
  · constructor
    · rw [IsClique, pairwise_iff_coe_toFinset_pairwise nodup_take G.symm]
      exact pl.greedyClique.pairwise_adj.take
...
/-- **Erdős Problem 904** (Bollobás–Erdős).
For all `r ≥ 2`, if a graph `G` on `n` vertices has at least `t_r(n)` edges
(where `t_r(n)` is the Turán number, the maximum number of edges on `n` vertices
with no `K_{r+1}`), then there exists an `r`-clique `x_1, …, x_r` in `G` with
...
-- 'Erdos904.erdos_904' depends on axioms: [propext, Classical.choice, Quot.sound]
