...
lemma frontier_hausdorff_ge_two_edist_pair {U : Set ℂ}
    ...
    2 * edist z w ≤ μH[1] (frontier U) := by
      ...
      exact le_trans ( mul_le_mul_right hV_re_measure _ ) hV_bound
...
lemma norm_sub_ge_of_deriv_approx {s t : ℝ} (hst : s ≤ t)
  ...
  have h_deriv_split : ∫ u in s..t, deriv γ u = (deriv γ s) * (t - s) + ∫ u in s..t, (deriv γ u - deriv γ s) := by
    rw [ intervalIntegral.integral_sub ] <;> norm_num [ hγ_intble ];
    ring;
  -- By the reverse triangle inequality:
  have h_reverse_triangle : ‖γ t - γ s‖ ≥ ‖(deriv γ s) * (t - s)‖ - ‖∫ u in s..t, (deriv γ u - deriv γ s)‖ := by
    simpa [ h_ftc, h_deriv_split ] using norm_sub_le ( γ t - γ s ) ( ∫ u in s..t, deriv γ u - deriv γ s );
  -- For the error term:
  ...
  norm_num at *;
  norm_cast at *;
  rw [ Real.norm_of_nonneg ( sub_nonneg.mpr hst ) ] at h_reverse_triangle ; linarith
...
lemma integral_le_eVariationOn {a b : ℝ} (hab : a < b)
  ...
  have h_riemann_sum : ∀ ε > 0, ∃ N : ℕ, N ≠ 0 ∧ ∑ k ∈ Finset.range N, ‖γ (a + (k + 1) * (b - a) / N) - γ (a + k * (b - a) / N)‖ ≥ (∫ t in a..b, ‖deriv γ t‖) - ε := by
    ...
    obtain ⟨δ, δ_pos, hδ⟩ : ∃ δ > 0, ∀ t ∈ Set.Icc a b, ∀ u ∈ Set.Icc a b, |t - u| < δ → ‖deriv γ t - deriv γ u‖ < ε / (2 * (b - a)) := by
      have := Metric.uniformContinuousOn_iff.mp ( isCompact_Icc.uniformContinuousOn_of_continuous hγ_deriv_cont ) ( ε / ( 2 * ( b - a ) ) ) ( div_pos hε_pos ( mul_pos zero_lt_two ( sub_pos.mpr hab ) ) ) ; aesop;
...
lemma lemniscatePetalCurve_on_lemniscate (n : ℕ) (hn : n ≥ 1)
  ...
  norm_cast; ring_nf; rw [ Real.sin_sq, Real.cos_sq ] ; ring
...
lemma petal_curve_connected_to_one (n : ℕ) (hn : n ≥ 1)
  ...
  have h_real_axis_path : ∃ γ : ℝ → ℂ, Continuous γ ∧ γ 0 = 1 ∧ γ 1 = (((1 - ε) * (2 * Real.cos (n * θ))) ^ ((1 : ℝ) / n) : ℝ) * Complex.exp (θ * Complex.I) ∧ ∀ t ∈ Set.Icc 0 1, γ t ∈ OmegaSet (modelPoly n) := by
    have h_real_axis_path : ∃ γ : ℝ → ℂ, Continuous γ ∧ γ 0 = (((1 - ε) * 2) ^ ((1 : ℝ) / n) : ℝ) ∧ γ 1 = (((1 - ε) * (2 * Real.cos (n * θ))) ^ ((1 : ℝ) / n) : ℝ) * Complex.exp (θ * Complex.I) ∧ ∀ t ∈ Set.Icc 0 1, γ t ∈ OmegaSet (modelPoly n) := by
      ...
      · intro t ht₁ ht₂; convert scaled_petal_in_omegaSet n hn ( t * θ ) ⟨ by nlinarith [ hθ.1, hθ.2, show ( n : ℝ ) ≥ 1 by norm_cast, mul_div_cancel₀ Real.pi ( by positivity : ( 2 * n : ℝ ) ≠ 0 ) ], by nlinarith [ hθ.1, hθ.2, show ( n : ℝ ) ≥ 1 by norm_cast, mul_div_cancel₀ Real.pi ( by positivity : ( 2 * n : ℝ ) ≠ 0 ) ] ⟩ ε hε₁ hε₂ using 1; ring
    ...
    · obtain ⟨seq, hseq⟩ : ∃ seq : ℕ → ℝ, (∀ k, seq k ∈ Set.Ioo (-(Real.pi / (2 * n))) (Real.pi / (2 * n))) ∧ Filter.Tendsto seq Filter.atTop (nhds θ) := by
        ...
        · use fun k => Real.pi / ( 2 * n ) - 1 / ( k + 1 ) * ( Real.pi / ( 2 * n ) )
          ...
          · intro k; constructor <;> ring_nf <;> norm_num [ Real.pi_pos, show n ≠ 0 by linarith ]
            ...
            · field_simp; norm_num
          · convert tendsto_natCast_div_add_atTop ( 1 : ℝ ) |> Filter.Tendsto.const_mul ( Real.pi / ( 2 * n ) ) using 2 <;> ring
            · field_simp; ring
...
lemma pow_re_eq_norm_mul_cos_arg (z : ℂ) (n : ℕ) :
  ...
  rw [ ← Complex.norm_mul_exp_arg_mul_I z ] ; ring;
...
lemma omegaSet_cos_arg_pos (n : ℕ) (_hn : n ≥ 1)
  ...
  have h_cos_pos : 0 < (z ^ n).re := by
    convert Set.mem_setOf.mp ( ball_one_one_sub_re_pos _ ) using 1;
    ...
    · -- z ≠ 0: use arg bound
      ...
      have h_norm_eq : ‖z‖ ^ n = 2 * Real.cos (n * Complex.arg z) := by
        have h_norm_eq : ‖z‖ ^ (2 * n) - 2 * ‖z‖ ^ n * Real.cos (n * Complex.arg z) + 1 = 1 := by
          ...
          convert h_norm_eq using 1 ; norm_num [ Complex.normSq, Complex.sq_norm ] ; ring;
          rw [ ← Complex.norm_mul_exp_arg_mul_I z ] ; ring ; norm_num [ ← Complex.exp_nat_mul, Complex.exp_re, Complex.exp_im ] ; ring;
...
lemma norm_deriv_lemniscatePetalCurve (n : ℕ) (hn : n ≥ 1) (θ : ℝ)
  ...
  · norm_num [ Complex.normSq, Complex.norm_def, Complex.exp_re, Complex.exp_im ]
    ...
    rw [ max_eq_right ( mul_nonneg zero_le_two ( Real.cos_nonneg_of_mem_Icc ⟨ by nlinarith [ Real.pi_pos, hθ.1, hθ.2, show ( n : ℝ ) ≥ 1 by norm_cast, mul_div_cancel₀ Real.pi ( by positivity : ( 2 * n : ℝ ) ≠ 0 ) ], by nlinarith [ Real.pi_pos, hθ.1, hθ.2, show ( n : ℝ ) ≥ 1 by norm_cast, mul_div_cancel₀ Real.pi ( by positivity : ( 2 * n : ℝ ) ≠ 0 ) ] ⟩ ) ) ] ; ring
    rw [ eq_comm, Real.sqrt_eq_iff_mul_self_eq ]
    · rw [ Real.sin_sq, Real.sin_sq ] ; ring
      rw [ show ( n : ℝ ) ⁻¹ = -1 + ( n : ℝ ) ⁻¹ + 1 by ring, Real.rpow_add_one ] <;> ring ; norm_num
...
lemma integral_cos_rpow_eq_betaFn (α : ℝ) (hα : α > -1) :
  ...
  have h_subst : ∫ θ in (0 : ℝ)..Real.pi / 2, Real.cos θ ^ α = (1 / 2) * ∫ x in (0 : ℝ)..1, x ^ (-1 / 2 : ℝ) * (1 - x) ^ ((α - 1) / 2 : ℝ) := by
    ...
    have h_subst : ∫ θ in (0)..Real.pi / 2, Real.cos θ ^ α = ∫ x in (0)..1, (Real.cos (Real.arcsin (Real.sqrt x))) ^ α * (1 / (2 * Real.sqrt (x * (1 - x)))) := by
      ...
      have h_subst : ∫ x in (Set.Ioo 0 (Real.pi / 2)), (Real.cos x) ^ α = ∫ x in (Set.image (fun θ => Real.sin θ ^ 2) (Set.Ioo 0 (Real.pi / 2))), (Real.cos (Real.arcsin (Real.sqrt x))) ^ α * (1 / (2 * Real.sqrt (x * (1 - x)))) := by
        ...
        · refine' MeasureTheory.setIntegral_congr_fun measurableSet_Ioo fun x hx => _;
          ...
          rw [ Real.sq_sqrt ( by nlinarith [ Real.cos_sq' x ] ) ] ; ring;
          rw [ show Real.cos x ^ 2 - Real.cos x ^ 4 = ( Real.cos x ^ 2 ) * ( 1 - Real.cos x ^ 2 ) by ring, Real.sqrt_mul ( sq_nonneg _ ), Real.sqrt_sq ( Real.cos_nonneg_of_mem_Icc ⟨ by linarith [ Real.pi_pos, hx.1 ], by linarith [ Real.pi_pos, hx.2 ] ⟩ ) ] ; ring;
          rw [ ← Real.sin_sq, Real.sqrt_sq_eq_abs ] ; ring;
    ...
    refine' MeasureTheory.setIntegral_congr_fun measurableSet_Ioo fun x hx => _;
    rw [ Real.cos_arcsin ] ; ring;
    rw [ Real.sq_sqrt hx.1.le ] ; rw [ show x - x ^ 2 = x * ( 1 - x ) by ring, Real.sqrt_mul hx.1.le ] ; norm_num [ Real.rpow_neg hx.1.le, Real.rpow_neg ( sub_nonneg.2 hx.2.le ) ] ; ring;
    rw [ Real.sqrt_eq_rpow, Real.sqrt_eq_rpow, ← Real.rpow_mul ( by linarith [ hx.1, hx.2 ] ), ← Real.rpow_neg ( by linarith [ hx.1, hx.2 ] ) ] ; ring;
    rw [ ← Real.rpow_add ( by linarith [ hx.1, hx.2 ] ) ] ; ring;
  ...
  have h_beta : ∫ x in (0 : ℝ)..1, x ^ (-1 / 2 : ℝ) * (1 - x) ^ ((α - 1) / 2 : ℝ) = Complex.re (Complex.betaIntegral (1 / 2 : ℂ) ((α + 1) / 2 : ℂ)) := by
    ...
    convert rfl using 2 ; norm_num [ Complex.ofReal_cpow ] ; ring;
  ...
  have h_beta_eq : Complex.betaIntegral (1 / 2 : ℂ) ((α + 1) / 2 : ℂ) = (Real.Gamma (1 / 2) * Real.Gamma ((α + 1) / 2)) / Real.Gamma ((α + 1) / 2 + 1 / 2) := by
    ...
    · rw [ ← Complex.Gamma_ofReal, ← Complex.Gamma_ofReal, ← Complex.Gamma_ofReal ] ; ring;
      norm_num;
    · grind +splitIndPred;
...
lemma lemniscate_arc_integral_eq (n : ℕ) (hn : n ≥ 1) :
        ...
        · refine' MeasureTheory.Integrable.const_mul _ _;
          have h_integrable : MeasureTheory.IntegrableOn (fun u => Real.cos u ^ ((1 / n : ℝ) - 1)) (Set.Ioo 0 (Real.pi / 2)) := by
            have h_beta : ∫ u in (0 : ℝ)..(Real.pi / 2), Real.cos u ^ ((1 / n : ℝ) - 1) = betaFn (1 / 2) ((1 / n : ℝ) / 2) / 2 := by
              convert integral_cos_rpow_eq_betaFn _ _ using 1 <;> ring;
  ...
  have h_integral_cos : ∫ u in (0 : ℝ)..(Real.pi / 2), (Real.cos u) ^ ((1 / n : ℝ) - 1) = betaFn (1 / 2) (1 / (2 * n)) / 2 := by
    convert integral_cos_rpow_eq_betaFn ( ( 1 / n : ℝ ) - 1 ) _ using 1 <;> ring ; norm_num [ hn ];
    linarith;
  simp_all +decide [ lemniscateLength ];
  rw [ intervalIntegral.integral_congr fun x hx => by rw [ Real.mul_rpow ( by norm_num ) ( Real.cos_nonneg_of_mem_Icc ⟨ by linarith [ Set.mem_Icc.mp ( by simpa [ Real.pi_div_two_pos.le ] using hx ) ], by linarith [ Set.mem_Icc.mp ( by simpa [ Real.pi_div_two_pos.le ] using hx ) ] ⟩ ) ] ] ; norm_num [ Real.rpow_sub ] ; ring;
  grind
      ...
      DifferentiableAt ℝ (lemniscatePetalCurve n) t := by
        ...
        · refine' DifferentiableAt.congr_of_eventuallyEq _ _;
          ...
          · refine' Complex.ofRealCLM.differentiableAt.comp _ _;
            refine' DifferentiableAt.rpow _ _ _ <;> norm_num [ mul_comm ];
            exact ne_of_gt ( Real.cos_pos_of_mem_Ioo ⟨ by nlinarith [ Real.pi_pos, show ( n : ℝ ) ≥ 1 by norm_cast, ht.1, ht.2, mul_div_cancel₀ Real.pi ( by positivity : ( 2 * n : ℝ ) ≠ 0 ) ], by nlinarith [ Real.pi_pos, show ( n : ℝ ) ≥ 1 by norm_cast, ht.1, ht.2, mul_div_cancel₀ Real.pi ( by positivity : ( 2 * n : ℝ ) ≠ 0 ) ] ⟩ );
          · filter_upwards [ Ioo_mem_nhds ht.1 ht.2 ] with θ hθ using by rw [ max_eq_right ( mul_nonneg zero_le_two ( Real.cos_nonneg_of_mem_Icc ⟨ by nlinarith [ Real.pi_pos, hθ.1, hθ.2, show ( n : ℝ ) ≥ 1 by norm_cast, mul_div_cancel₀ ( Real.pi : ℝ ) ( by positivity : ( 2 * n : ℝ ) ≠ 0 ) ], by nlinarith [ Real.pi_pos, hθ.1, hθ.2, show ( n : ℝ ) ≥ 1 by norm_cast, mul_div_cancel₀ ( Real.pi : ℝ ) ( by positivity : ( 2 * n : ℝ ) ≠ 0 ) ] ⟩ ) ) ] ;
          ...
          · refine' ContinuousOn.mul _ _;
            · refine' Complex.continuous_ofReal.comp_continuousOn _;
              ...
              · refine' ContinuousOn.mul ( ContinuousOn.mul continuousOn_const _ ) _;
                ...
                · fun_prop;
              · intro θ hθ; norm_num [ mul_comm ];
                norm_num [ show Real.cos ( θ * n ) ≠ 0 from ne_of_gt ( Real.cos_pos_of_mem_Ioo ⟨ by nlinarith [ Real.pi_pos, show ( n : ℝ ) ≥ 1 by norm_cast, mul_div_cancel₀ Real.pi ( by positivity : ( 2 * n : ℝ ) ≠ 0 ), hθ.1 ], by nlinarith [ Real.pi_pos, show ( n : ℝ ) ≥ 1 by norm_cast, mul_div_cancel₀ Real.pi ( by positivity : ( 2 * n : ℝ ) ≠ 0 ), hθ.2 ] ⟩ ) ] ; ring;
  ...
  have h_symm : ∀ k : ℕ, k < n → componentBoundaryLength (modelPoly n) (ω ^ k) = componentBoundaryLength (modelPoly n) 1 := by
    ...
    have h_symm_step : ∀ m : ℕ, m < n → componentBoundaryLength (modelPoly n) (ω ^ m) = componentBoundaryLength (modelPoly n) (ω ^ (m + 1)) := by
      ...
      have h_symm_step : componentBoundaryLength (modelPoly n) (ω ^ m) = componentBoundaryLength (modelPoly n) (ω * (ω ^ m)) := by
        ...
        · rw [ ← Complex.exp_nat_mul, mul_comm, Complex.exp_eq_one_iff ] ; use 1 ; ring ; norm_num [ show n ≠ 0 by linarith ];
...
/-- **Erdős Problem 1044** (Erdős–Herzog–Piranian [EHP58], solved by Q. Tang):
the infimum of `Λ(f)` over all monic polynomials `f` of degree ≥ 1 whose roots
lie in the closed unit disk equals `2`. Here `Λ(f)` is the supremum of
`μ_H¹`-boundary-lengths over connected components of the lemniscate region
`{z ∈ ℂ : |f(z)| < 1}`. -/
...
-- 'Erdos1044.erdos_1044' depends on axioms: [propext, Classical.choice, Quot.sound]
