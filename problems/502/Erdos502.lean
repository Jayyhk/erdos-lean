...
theorem card_monomials_le (d s : ℕ) :
    (monomials_le d s).card = Nat.choose (d + s) s := by
      ...
      rw [ h_bij, Finset.card_filter ];
      rw [ Finset.sum_image ] <;> norm_num [ Finsupp.sum_fintype ];
      -- The number of non-negative integer solutions to the equation $x_1 + x_2 + \cdots + x_d \leq s$ is given by the stars and bars theorem, which states that the number of solutions is $\binom{d+s}{s}$.
      have h_stars_and_bars : ∀ (d s : ℕ), Finset.card (Finset.filter (fun x : Fin d → ℕ => ∑ i, x i ≤ s) (Finset.Iic (fun _ => s))) = Nat.choose (d + s) s := by
        intro d s; induction' d with d ih generalizing s <;> simp_all +decide [ add_comm, add_left_comm, Fin.sum_univ_succ ] ;
        · rfl;
        · -- For the inductive step, we can split the sum into the sum over the first element and the sum over the remaining elements.
          have h_split : Finset.filter (fun x : Fin (d + 1) → ℕ => x 0 + ∑ i : Fin d, x i.succ ≤ s) (Finset.Iic (fun _ => s)) = Finset.biUnion (Finset.range (s + 1)) (fun k => Finset.image (fun x : Fin d → ℕ => Fin.cons k x) (Finset.filter (fun x : Fin d → ℕ => ∑ i : Fin d, x i ≤ s - k) (Finset.Iic (fun _ => s - k)))) := by
            ext x; simp [Finset.mem_biUnion, Finset.mem_image];
            constructor <;> intro h;
            · use x 0, by
                linarith [ h.1 0, h.1 1, Finset.sum_nonneg fun i ( hi : i ∈ Finset.univ ) => Nat.zero_le ( x ( Fin.succ i ) ) ], fun i => x i.succ, by
                exact ⟨ fun i => Nat.le_sub_of_add_le' <| by linarith [ h.1 i.succ, Finset.single_le_sum ( fun a _ => Nat.zero_le ( x ( Fin.succ a ) ) ) ( Finset.mem_univ i ) ], Nat.le_sub_of_add_le' <| by linarith [ h.1 0, Finset.sum_nonneg fun a ( _ : a ∈ Finset.univ ) => Nat.zero_le ( x ( Fin.succ a ) ) ] ⟩, by
                exact funext fun i => by cases i using Fin.inductionOn <;> rfl;
            · rcases h with ⟨ a, ha, b, hb, rfl ⟩ ; exact ⟨ fun i => by cases i using Fin.inductionOn <;> [ exact Nat.le_trans ( Nat.le_of_lt_succ ha ) ( Nat.le_refl _ ) ; exact le_trans ( hb.1 _ ) ( Nat.sub_le _ _ ) ], by simpa [ Fin.sum_univ_succ ] using by linarith [ Nat.sub_add_cancel ( show a ≤ s from Nat.le_of_lt_succ ha ) ] ⟩ ;
          rw [ h_split, Finset.card_biUnion ] <;> norm_num [ Finset.card_image_of_injective, Function.Injective, ih ];
          · exact Nat.recOn s ( by norm_num ) fun n ih => by simp_all +decide [ Nat.choose, add_comm, add_left_comm, Finset.sum_range_succ' ] ;
          · intro k hk l hl hkl; simp_all +decide [ Finset.disjoint_left ] ;
            intro a x hx₁ hx₂ hx₃ y hy₁ hy₂ hy₃; contrapose! hkl; aesop;
      exact h_stars_and_bars d s
...
theorem dim_s_le_card_monomials_le (d s : ℕ) (A : Set (EuclideanSpace ℝ (Fin d))) :
    dim_s d s A ≤ (monomials_le d s).card := by
      ...
      have h_span : Submodule.span ℝ (Set.image (fun m => MvPolynomial.monomial m (1 : ℝ)) (monomials_le d s).toSet) = polynomials_of_degree_le d s := by
        ext p
        refine' ⟨ fun hp => _, fun hp => _ ⟩;
        · refine' Submodule.span_induction _ _ _ _ hp;
          · rintro _ ⟨ m, hm, rfl ⟩;
            refine' Submodule.subset_span _;
            simp_all +decide [ monomials_le ];
            rcases hm with ⟨ i, hi, y, rfl ⟩ ; simp_all +decide [ Finsupp.sum_fintype ];
            linarith;
        ...
        · refine' Submodule.span_induction _ _ _ _ hp;
          · intro p hp;
            ...
            refine' Finset.mem_biUnion.mpr ⟨ m.sum fun i n => n, _, _ ⟩ <;> norm_num;
            · exact Nat.lt_succ_of_le ( le_trans ( Finset.le_sup ( f := fun m => m.sum fun i n => n ) hm ) hp );
            · refine' ⟨ ⟨ m.support.val.bind fun i => Multiset.replicate ( m i ) i, _ ⟩, _ ⟩ <;> norm_num;
              · rfl
              · ext i; simp +decide [ Multiset.toFinsupp ] ;
                simp +decide [ Multiset.count_bind, Multiset.count_replicate ];
                exact fun h => h.symm;
          · exact Submodule.zero_mem _;
          · exact fun x y hx hy hx' hy' => Submodule.add_mem _ hx' hy';
          · exact fun a x hx₁ hx₂ => Submodule.smul_mem _ _ hx₂;
      -- The dimension of the image of a linear map is less than or equal to the dimension of the domain.
      have h_dim_le : Module.finrank ℝ (Submodule.map (restriction_map d A) (Submodule.span ℝ (Set.image (fun m => MvPolynomial.monomial m (1 : ℝ)) (monomials_le d s).toSet))) ≤ Module.finrank ℝ (Submodule.span ℝ (Set.image (fun m => MvPolynomial.monomial m (1 : ℝ)) (monomials_le d s).toSet)) := by
...
def Omega (d s : ℕ) (A : Set (EuclideanSpace ℝ (Fin d))) [Fintype A] : Submodule ℝ (A → ℝ) :=
  Submodule.span ℝ {f : A → ℝ | ∀ g ∈ Submodule.map (restriction_map d A) (polynomials_of_degree_le d s), dotProduct f g = 0}
      ...
      have h_monomial : ∀ (m : Fin d ⊕ Fin d →₀ ℕ), m.sum (fun _ n => n) ≤ 2 * s + 1 → (∀ (f g : A → ℝ), f ∈ Omega d s A → g ∈ Omega d s A → (Phi_monomial d m A) f g = 0) := by
        ...
        · have h_ortho : ∀ (p : MvPolynomial (Fin d) ℝ), p.totalDegree ≤ s → (∑ x : A, f x * (eval x p)) = 0 := by
            ...
            have h_ortho : ∀ (g : A → ℝ), g ∈ Submodule.map (restriction_map d A) (polynomials_of_degree_le d s) → (∑ x : A, f x * g x) = 0 := by
              intro g hg;
              have h_ortho : ∀ (g : A → ℝ), g ∈ Submodule.map (restriction_map d A) (polynomials_of_degree_le d s) → dotProduct f g = 0 := by
                intro g hg
                have h_ortho : f ∈ Submodule.span ℝ {f : A → ℝ | ∀ g ∈ Submodule.map (restriction_map d A) (polynomials_of_degree_le d s), dotProduct f g = 0} := by
                  exact hf
                rw [ Finsupp.mem_span_iff_linearCombination ] at h_ortho;
                obtain ⟨ l, hl ⟩ := h_ortho;
                rw [ ← hl, Finsupp.linearCombination_apply ];
                simp +decide [ Finsupp.sum, dotProduct ];
                simp +decide [ Finset.sum_mul _ _ _];
                rw [ Finset.sum_comm ];
                exact Finset.sum_eq_zero fun x hx => by simpa only [ mul_assoc, Finset.mul_sum _ _ _ ] using mul_eq_zero_of_right _ ( x.2 g hg ) ; ;
              exact h_ortho g hg;
            convert h_ortho _ _;
        ...
        · have h_g_zero : ∑ y : A, g y * (eval y (monomial my 1)) = 0 := by
            have h_g_zero : ∀ (p : MvPolynomial (Fin d) ℝ), p.totalDegree ≤ s → ∑ y : A, g y * (eval y p) = 0 := by
              intros p hp
              have h_g_zero : g ∈ Submodule.span ℝ {f : A → ℝ | ∀ g ∈ Submodule.map (restriction_map d A) (polynomials_of_degree_le d s), dotProduct f g = 0} := by
                exact hg;
              refine' Submodule.span_induction _ _ _ _ h_g_zero;
              · simp +zetaDelta at *;
                exact fun f hf => hf p ( Submodule.subset_span hp );
              · norm_num;
              · simp +contextual [ add_mul, Finset.sum_add_distrib ];
              · simp +contextual [ mul_assoc ];
                exact fun a x hx hx' => by rw [ ← Finset.mul_sum _ _ _, hx', MulZeroClass.mul_zero ] ;
...
theorem finrank_Omega (d s : ℕ) (A : Set (EuclideanSpace ℝ (Fin d))) [Fintype A] [DecidableEq A] :
  ...
  -- Omega is the orthogonal complement of the image of the restriction map
  let U := Submodule.map (restriction_map d A) (polynomials_of_degree_le d s)
  have h_omega_eq_orth : Omega d s A = Submodule.span ℝ {f | ∀ g ∈ U, dotProduct f g = 0} := rfl
  -- The dot product is non-degenerate, so dim(U) + dim(U_perp) = dim(V)
  -- dim(V) = |A|
  -- dim(U) = dim_s d s A
  have h_orthogonal_complement : Module.finrank ℝ (Submodule.span ℝ {f : A → ℝ | ∀ g ∈ U, dotProduct f g = 0}) = Fintype.card A - Module.finrank ℝ U := by
    have h_dim_orthogonal : Module.finrank ℝ (Submodule.span ℝ {f : A → ℝ | ∀ g ∈ U, dotProduct f g = 0}) = Module.finrank ℝ (Submodule.dualAnnihilator U) := by
      refine' LinearEquiv.finrank_eq _;
      refine' ( LinearEquiv.ofBijective _ ⟨ _, _ ⟩ );
      refine' { toFun := fun f => ⟨ _, _ ⟩, map_add' := _, map_smul' := _ };
      exact ( ∑ i : A, f.val i • LinearMap.proj i );
      all_goals norm_num [ Function.Injective, Function.Surjective ];
      · intro w hw; have := f.2; simp_all +decide [ dotProduct] ;
        refine' Submodule.span_induction _ _ _ _ f.2;
        · exact fun f hf => hf w hw;
        · norm_num;
        · simp +contextual [ add_mul, Finset.sum_add_distrib ];
        · simp +contextual [ mul_assoc];
          exact fun a x hx hx' => by rw [ ← Finset.mul_sum _ _ _, hx', MulZeroClass.mul_zero ] ;
      · simp +decide [ add_smul, Finset.sum_add_distrib ];
      · simp +decide [ Finset.smul_sum, MulAction.mul_smul ];
      · intro a ha b hb h; ext x; replace h := congr_arg ( fun f => f ( fun y => if y = x then 1 else 0 ) ) h; simp_all +decide [ dotProduct ] ;
      · intro a ha;
        refine' ⟨ fun x => a ( Pi.single x 1 ), _, _ ⟩ <;> norm_num;
        · refine' Submodule.subset_span _;
          intro g hg; specialize ha g hg; simp_all +decide [ dotProduct ] ;
          convert ha using 1;
          rw [ show g = ∑ x : A, Pi.single x ( g x ) by ext x; simp +decide [ Pi.single_apply ] ] ; simp +decide [ mul_comm ];
          exact Finset.sum_congr rfl fun x _ => by rw [ ← smul_eq_mul, ← map_smul ] ; congr; ext y; by_cases hy : y = x <;> aesop;
        · ext x; simp +decide [ Pi.single_apply ] ;
    have := Submodule.finrank_quotient_add_finrank U.dualAnnihilator;
    have h_dual_annihilator : Module.finrank ℝ (Module.Dual ℝ (A → ℝ) ⧸ U.dualAnnihilator) = Module.finrank ℝ U := by
      have h_iso : (Module.Dual ℝ (A → ℝ) ⧸ U.dualAnnihilator) ≃ₗ[ℝ] Module.Dual ℝ U := by
        exact Subspace.quotAnnihilatorEquiv U
      have := h_iso.finrank_eq;
      aesop;
    simp_all +decide;
    exact eq_tsub_of_add_eq <| by linarith;
  convert h_orthogonal_complement using 1
...
/-- **Erdős Problem 502** (Bannai–Bannai–Stanton). A finite set `A ⊆ ℝⁿ` with exactly two
distinct pairwise distances satisfies `|A| ≤ C(n+2, 2)`. -/
theorem erdos_502 (n : ℕ) (A : Set (EuclideanSpace ℝ (Fin n))) [Fintype A] [DecidableEq A]
    (hA : is_s_distance_set A 2) : Fintype.card A ≤ Nat.choose (n + 2) 2 :=
  bannai_bannai_stanton n 2 A hA

end

#print axioms erdos_502
-- 'Erdos502.erdos_502' depends on axioms: [propext, Classical.choice, Quot.sound]
