...
lemma no_strictMono_arithProgression (r : ℝ → ℝ → Prop) (hchaotic : ChaoticOrder r)
  ...
  · intro h
    ...
    have h_ap' : ArithProgression a d k ⟨2, by omega⟩ + ArithProgression a d k ⟨0, by omega⟩ =
        2 * ArithProgression a d k ⟨1, by omega⟩ := by linarith
...
-/
lemma orderZ_trans {a b c : ℤ} (hab : OrderZ a b) (hbc : OrderZ b c) : OrderZ a c := by
  revert a b c;
  -- Apply the induction hypothesis to conclude the proof.
  apply Classical.byContradiction
  intro h_contra;
  obtain ⟨a, b, c, hab, hbc, hac⟩ : ∃ a b c : ℤ, OrderZ a b ∧ OrderZ b c ∧ ¬OrderZ a c ∧ (a.natAbs + b.natAbs + c.natAbs) = sInf {n : ℕ | ∃ a b c : ℤ, OrderZ a b ∧ OrderZ b c ∧ ¬OrderZ a c ∧ n = a.natAbs + b.natAbs + c.natAbs} := by
    have h_nonempty : {n : ℕ | ∃ a b c : ℤ, OrderZ a b ∧ OrderZ b c ∧ ¬OrderZ a c ∧ n = a.natAbs + b.natAbs + c.natAbs}.Nonempty := by
      push_neg at h_contra;
      exact ⟨ _, ⟨ h_contra.choose, h_contra.choose_spec.choose, h_contra.choose_spec.choose_spec.choose, h_contra.choose_spec.choose_spec.choose_spec.1, h_contra.choose_spec.choose_spec.choose_spec.2.1, h_contra.choose_spec.choose_spec.choose_spec.2.2, rfl ⟩ ⟩;
  ...
  have h_mod : a % 2 = b % 2 ∧ b % 2 = c % 2 := by
    have h_mod : a % 2 ≤ b % 2 ∧ b % 2 ≤ c % 2 ∧ c % 2 ≤ a % 2 := by
      grind +locals;
    grind +extAll;
  -- By definition of `OrderZ`, we know that `OrderZ (a / 2) (b / 2)` and `OrderZ (b / 2) (c / 2)`.
  ...
  have h_div_c : OrderZ (a / 2) (c / 2) := by
    have h_div_c : (a / 2).natAbs + (b / 2).natAbs + (c / 2).natAbs < a.natAbs + b.natAbs + c.natAbs := by
      grind;
    contrapose! h_div_c;
...
lemma orderZ_tri (a b : ℤ) : OrderZ a b ∨ a = b ∨ OrderZ b a := by
  ...
  have h_ind : ∀ a b : ℤ, a.natAbs + b.natAbs = 0 → OrderZ a b ∨ a = b ∨ OrderZ b a := by
    aesop;
  -- Apply induction on the sum of the absolute values of `a` and `b`.
  have h_ind : ∀ n : ℕ, (∀ a b : ℤ, a.natAbs + b.natAbs ≤ n → OrderZ a b ∨ a = b ∨ OrderZ b a) → ∀ a b : ℤ, a.natAbs + b.natAbs = n + 1 → OrderZ a b ∨ a = b ∨ OrderZ b a := by
    intros n ih a b h_sum;
    by_cases h_cases : a % 2 < b % 2 ∨ b % 2 < a % 2;
    · cases h_cases <;> unfold OrderZ <;> simp +decide [ *, OrderZBool ];
      · grind;
      · grobner;
    · -- Since $a$ and $b$ have the same parity, we can apply the induction hypothesis to $a/2$ and $b/2$.
      have h_ind_step : OrderZ (a / 2) (b / 2) ∨ a / 2 = b / 2 ∨ OrderZ (b / 2) (a / 2) := by
        grind +revert;
      cases h_ind_step <;> simp_all +decide [ OrderZ ];
      · grind +suggestions;
      · unfold OrderZBool; simp_all +decide [ Int.emod_eq_emod_iff_emod_sub_eq_zero ] ;
        grind;
  -- By induction, we can conclude that the statement holds for all pairs of integers.
  have h_all : ∀ n : ℕ, (∀ a b : ℤ, a.natAbs + b.natAbs ≤ n → OrderZ a b ∨ a = b ∨ OrderZ b a) := by
    intro n; induction n <;> simp_all +decide ;
    exact fun a b hab => if h : a.natAbs + b.natAbs = _ + 1 then h_ind _ ‹_› _ _ h else ‹∀ a b : ℤ, a.natAbs + b.natAbs ≤ _ → OrderZ a b ∨ a = b ∨ OrderZ b a› _ _ ( Nat.le_of_lt_succ ( lt_of_le_of_ne hab h ) );
...
theorem orderZ_chaotic : ChaoticOrder OrderZ := by
  ...
  obtain ⟨a, b, c, h_sum, h_order⟩ : ∃ a b c : ℤ, a + c = 2 * b ∧ OrderZ a b ∧ OrderZ b c := by
    unfold ChaoticOrder at h_contra; aesop;
  -- Choose a counterexample minimizing the sum of absolute values.
  obtain ⟨a, b, c, h_sum, h_order, h_min⟩ : ∃ a b c : ℤ, a + c = 2 * b ∧ OrderZ a b ∧ OrderZ b c ∧ ∀ a' b' c', a' + c' = 2 * b' → OrderZ a' b' → OrderZ b' c' → Int.natAbs a' + Int.natAbs b' + Int.natAbs c' ≥ Int.natAbs a + Int.natAbs b + Int.natAbs c := by
    have h_well_founded : WellFounded fun x y : ℕ => x < y := by
      exact wellFounded_lt;
    have := h_well_founded.has_min { n : ℕ | ∃ a b c : ℤ, a + c = 2 * b ∧ OrderZ a b ∧ OrderZ b c ∧ n = Int.natAbs a + Int.natAbs b + Int.natAbs c } ⟨ _, ⟨ a, b, c, h_sum, h_order.1, h_order.2, rfl ⟩ ⟩;
    rcases this with ⟨ n, ⟨ a, b, c, h₁, h₂, h₃, rfl ⟩, h₄ ⟩ ; exact ⟨ a, b, c, h₁, h₂, h₃, fun a' b' c' h₅ h₆ h₇ => not_lt.1 fun h₈ => h₄ _ ⟨ a', b', c', h₅, h₆, h₇, rfl ⟩ h₈ ⟩ ;
  ...
  have h_parity_ac : a % 2 = c % 2 := by
    omega;
  -- By `orderZ_recurse`, `OrderZ a b ↔ OrderZ (a/2) (b/2)` and `OrderZ b c ↔ OrderZ (b/2) (c/2)`.
...
lemma finiteQ_hasChaoticOrder (n : ℕ) (vals : Fin n → ℚ) (hvals : Function.Injective vals) :
    ∃ r : Fin n → Fin n → Prop, (∀ i, ¬r i i) ∧ (∀ i j k, r i j → r j k → r i k) ∧
      (∀ i j, i ≠ j → r i j ∨ r j i) ∧ (∀ i j k, vals i + vals k = 2 * vals j → ¬(r i j ∧ r j k)) := by
  obtain ⟨m, hm_pos, h_shift⟩ : ∃ m : ℕ, 0 < m ∧ (∀ i : Fin n, ∃ n : ℤ, ((m : ℚ) * vals i = ↑n)) := by
...
def TreeT (n : ℕ) : Type :=
  {r : Fin (n + 1) → Fin (n + 1) → Bool //
    ...
    (∀ i j, i ≠ j → r i j = true ∨ r j i = true) ∧ (∀ i j k : Fin (n + 1),
      Qenum (i : ℕ) + Qenum (k : ℕ) = 2 * Qenum (j : ℕ) → ¬(r i j = true ∧ r j k = true))}
...
lemma treeQ_nonemptyLevels (n : ℕ) :
  ...
  obtain ⟨r, hr⟩ := finiteQ_hasChaoticOrder (n + 1)
      (fun i : Fin (n + 1) => Qenum i) (by exact fun i j h => by simpa [Fin.ext_iff] using Qenum.injective h)
...
lemma treeQ_infiniteBranch :
  ...
  set π : {i j : ℕ} → i ≤ j → α j → α i :=
    fun hij r => ⟨fun i j => r.val (Fin.castLE (by omega) i) (Fin.castLE (by omega) j), by grind⟩
...
lemma compatibleChain_agree {n m : ℕ} (hnm : n ≤ m) (i j : Fin (n + 1)) :
    (CompatibleChain m).val (⟨i, by omega⟩) (⟨j, by omega⟩) = (CompatibleChain n).val i j := by
  induction hnm with
  | refl => rfl
  | step hmx ih =>
...
lemma orderQ_at_level (a b : ℚ) (n : ℕ)
    ...
    ((CompatibleChain n).val ⟨(Denumerable.eqv ℚ) a, by omega⟩ ⟨(Denumerable.eqv ℚ) b, by omega⟩ = true) ↔ orderQ a b := by
  rw [orderQ]
  rw [← compatibleChain_agree (show max (Denumerable.eqv ℚ a) (Denumerable.eqv ℚ b) ≤ n from max_le ha hb)]
...
lemma orderQ_linear : LinearOrdering orderQ := by
  ...
  · intro a
    ...
    grind
  · intro a b c hab hbc
    obtain ⟨n, hn⟩ : ∃ n, (Denumerable.eqv ℚ) a ≤ n ∧ (Denumerable.eqv ℚ) b ≤ n ∧ (Denumerable.eqv ℚ) c ≤ n := by
      exact ⟨Max.max (Denumerable.eqv ℚ a) (Max.max (Denumerable.eqv ℚ b) (Denumerable.eqv ℚ c)),
        le_max_left _ _, le_max_of_le_right (le_max_left _ _), le_max_of_le_right (le_max_right _ _)⟩
    have h_chain : (CompatibleChain n).val ⟨(Denumerable.eqv ℚ) a, by omega⟩ ⟨(Denumerable.eqv ℚ) b, by omega⟩ = true ∧
        (CompatibleChain n).val ⟨(Denumerable.eqv ℚ) b, by omega⟩ ⟨(Denumerable.eqv ℚ) c, by omega⟩ = true := by
      exact ⟨orderQ_at_level a b n hn.1 hn.2.1 |>.2 hab, orderQ_at_level b c n hn.2.1 hn.2.2 |>.2 hbc⟩
  ...
  · intro a b
    ...
    set n := max ia ib
    have h_tree :
        ∀ i j : Fin (n + 1), (CompatibleChain n).val i j ∨ i = j ∨ (CompatibleChain n).val j i := by
      grind
...
theorem orderQ_chaotic : ChaoticOrder orderQ := by
  ...
  obtain ⟨n, hn⟩ : ∃ n : ℕ, (Denumerable.eqv ℚ) a ≤ n ∧ (Denumerable.eqv ℚ) b ≤ n ∧ (Denumerable.eqv ℚ) c ≤ n := by
    exact ⟨Max.max (Denumerable.eqv ℚ a) (Max.max (Denumerable.eqv ℚ b) (Denumerable.eqv ℚ c)),
      le_max_left _ _, le_max_of_le_right (le_max_left _ _), le_max_of_le_right (le_max_right _ _)⟩
  have h_chain :
      (CompatibleChain n).val ⟨(Denumerable.eqv ℚ) a, by omega⟩ ⟨(Denumerable.eqv ℚ) b, by omega⟩ = true ∧
      (CompatibleChain n).val ⟨(Denumerable.eqv ℚ) b, by omega⟩ ⟨(Denumerable.eqv ℚ) c, by omega⟩ = true := by
    exact ⟨orderQ_at_level a b n hn.1 hn.2.1 |>.2 h_order.1, orderQ_at_level b c n hn.2.1 hn.2.2 |>.2 h_order.2⟩
  have := (CompatibleChain n).2.2.2.2 ⟨(Denumerable.eqv ℚ) a, by linarith⟩ ⟨(Denumerable.eqv ℚ) b, by linarith⟩ ⟨(Denumerable.eqv ℚ) c, by linarith⟩ ?_ <;> simp_all [two_mul]
  unfold Qenum; aesop
...
lemma orderR_linear : LinearOrdering orderR := by
  ...
  · intro a b c
    ...
    · refine ⟨i, ?_, ?_, ?_⟩ <;> simp_all [WellOrderingRel]
      exact fun k hk ↦ hj.2.1 k (lt_trans hk hij)
    · by_cases hji : WellOrderingRel j i
      · have := hi.2.1 j hji
        use j
        exact ⟨by aesop, fun k hk => by rw [hi.2.1 k (lt_trans hk hji), hj.2.1 k hk],
          by rw [this]; exact hj.2.2⟩
      · have hij_eq : i = j := by
          exact Classical.not_not.1 fun h => hij <| by have := basisOrder_def.trichotomous i j; tauto
        ...
        grind +locals
  · intro a b
    by_cases hab : a = b <;> simp_all [orderR]
    obtain ⟨i, hi⟩ : ∃ i : QBASIS_R, HamelBasis.repr a i ≠ HamelBasis.repr b i ∧
        ∀ j : QBASIS_R, HamelBasis.repr a j ≠ HamelBasis.repr b j → ¬WellOrderingRel j i := by
      have h_nonempty : ∃ i : QBASIS_R, HamelBasis.repr a i ≠ HamelBasis.repr b i :=
        first_differing_coefficient_exists a b hab
      have h_well_founded : WellFounded (WellOrderingRel : QBASIS_R → QBASIS_R → Prop) := IsWellFounded.wf
      exact WellFounded.has_min h_well_founded
        (fun a_1 => (HamelBasis.repr a) a_1 = (HamelBasis.repr b) a_1 → False) h_nonempty
    rcases orderQ_linear.tri (HamelBasis.repr a i) (HamelBasis.repr b i) with h | h <;>
      simp_all [eq_comm]
    · exact Or.inl ⟨i, hi.1, fun j hj => Classical.not_not.1 fun hj' => hi.2 j hj' hj, h⟩
    · exact Or.inr ⟨i, hi.1, fun j hj => Classical.not_not.1 fun hj' => hi.2 j hj' hj, h⟩
...
lemma midpoint_coeffwise (a b c : ℝ) (h : a + c = 2 * b) (i : QBASIS_R) :
  ...
  replace h := congr_arg HamelBasis.repr h
  simp_all [two_mul]
  exact congr_arg (fun f ↦ f i) h
