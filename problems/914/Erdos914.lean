...
lemma equitable_coloring_of_edgeless (G : SimpleGraph V) [DecidableRel G.Adj]
  ...
  exact ⟨h_eq.choose, by intro v w hvw; specialize hG (Sym2.mk (v, w)); tauto, h_eq.choose_spec⟩
  ...
  have h_edges : (Finset.card (Finset.filter (fun v => ne.f v ≠ ne.small) Finset.univ)) ≤ r * (Finset.card (Finset.filter (fun v => ne.f v = ne.small) Finset.univ)) := by
    ...
    have h_edges : ∀ v ∈ Finset.filter (fun v => ne.f v = ne.small) Finset.univ, Finset.card (Finset.filter (fun w => ne.f w ≠ ne.small ∧ G.Adj v w) Finset.univ) ≤ r := by
      ...
      have h_deg_v : Finset.card (Finset.filter (fun w => G.Adj v w) Finset.univ) ≤ r := by
        exact le_trans ( SimpleGraph.degree_le_maxDegree G v ) hd |> le_trans ( by simp +decide [ SimpleGraph.degree, SimpleGraph.neighborFinset_def ] ) ;
      generalize_proofs at *; (
...
lemma solo_weight_contradiction
  ...
  have h_solo_deg_bound : ∀ z : V, ne.f z = W₀ →
    ...
    have h_deg_bound : Finset.card (Finset.filter (fun w => G.Adj z w) (Finset.univ : Finset V)) ≤ r := by
      simpa [ SimpleGraph.degree, SimpleGraph.neighborFinset_def ] using hd.trans' ( G.degree_le_maxDegree z );
...
lemma T_vertex_B_degree_bound
  ...
  have h_total_neighbors : (Finset.filter (fun w => G.Adj x w) Finset.univ).card ≤ r := by
    have := hd.trans' ( G.degree_le_maxDegree x ) ; simp_all +decide [ SimpleGraph.degree, SimpleGraph.neighborFinset_def ] ;
...
lemma terminal_weight_contradiction
  ...
  have h_bad_from_terminal : ∀ (W : Fin (r + 1)) (z y₁ : V),
    ...
    have h_not_movable : ∀ X, X ≠ W → IsAccessible G ne.f ne.small X →
        ∃ w, ne.f w = X ∧ G.Adj z w := by
      intro X hXW hXacc
      by_contra h_no_nbr; push_neg at h_no_nbr
...
lemma B_minus_equitable
  ...
  have h_ind : HasEquitableColoring (G.induce B_minus) q := by
    have h_ind : (G.induce B_minus).maxDegree ≤ q - 1 := by
      ...
      · exact Finset.sup_le fun v _ => WithBot.coe_le_coe.mpr ( h_ind v );
    ...
    · simp_all +decide [ Finset.ext_iff ];
      ...
      simp +zetaDelta at *;
      grind;
    · have h_ind : Fintype.card B_minus < Fintype.card V := by
        rw [ Fintype.card_subtype ] ; exact Finset.card_lt_card ( Finset.filter_ssubset.mpr ⟨ y₁, by aesop ⟩ ) ;
      contrapose! ih;
      refine' ⟨ _, _, _, _, _, q - 1, _, _, _, _ ⟩;
      exact ULift ( Fin ( Fintype.card B_minus ) );
      all_goals try infer_instance;
      exact SimpleGraph.comap ( fun x => Fintype.equivFin B_minus |>.symm x.down ) ( induce B_minus G );
      all_goals norm_num [ Nat.sub_add_cancel ( Nat.one_le_iff_ne_zero.mpr hq ) ] at *;
      any_goals tauto;
      · convert ‹ ( induce B_minus G ).maxDegree ≤ q - 1 › using 1;
        unfold SimpleGraph.maxDegree; simp +decide [ SimpleGraph.degree, SimpleGraph.neighborFinset ] ;
        congr! 2;
        ext; simp [Finset.mem_image];
        constructor;
        · rintro ⟨ x, rfl ⟩;
          refine' ⟨ _, _, _ ⟩;
          exact ( Fintype.equivFin B_minus ).symm x |>.1;
          grind;
          refine' Finset.card_bij ( fun y hy => ⟨ ( Fintype.equivFin B_minus ) y, _ ⟩ ) _ _ _ <;> simp +decide [ Finset.mem_filter ];
          · exact Fin.is_lt _ |> lt_of_lt_of_le <| by simp +decide [ Fintype.card_subtype ] ;
          · exact fun y hy => ⟨ _, hy, ( Fintype.equivFin B_minus ).symm y |>.2, by simp +decide ⟩;
        · rintro ⟨ a, ha, rfl ⟩;
          use Fintype.equivFin B_minus ⟨ a, ha ⟩ ; simp +decide;
          refine' Finset.card_bij ( fun x hx => ⟨ ( Fintype.equivFin B_minus ).symm x.down, _ ⟩ ) _ _ _ <;> simp +decide [ Finset.mem_filter, Finset.mem_univ ];
          exact fun b hb hab => ⟨ Fintype.equivFin B_minus ⟨ b, hb ⟩, by simpa using hab, by simp +decide ⟩;
      · intro h;
        obtain ⟨ f, hf₁, hf₂ ⟩ := h;
        refine' ih ⟨ fun x => f ⟨ Fintype.equivFin B_minus x ⟩, _, _ ⟩ <;> simp_all +decide [ IsProper, IsEquitable ];
        intro c₁ c₂; convert hf₂ c₁ c₂ using 1; simp +decide [ cClass ] ;
        · rw [ Finset.card_filter, Finset.card_filter ];
          refine' Finset.sum_bij ( fun x _ => ⟨ Fintype.equivFin B_minus x ⟩ ) _ _ _ _ <;> simp +decide;
          exact fun x => ⟨ _, ( Fintype.equivFin B_minus ).symm x |>.2, by simp +decide ⟩;
        · simp +decide [ cClass ];
          rw [ Finset.card_filter, Finset.card_filter ];
          refine' Finset.sum_bij ( fun x _ => ⟨ Fintype.equivFin B_minus x ⟩ ) _ _ _ _ <;> simp +decide;
          exact fun x => ⟨ _, ( Fintype.equivFin B_minus ).symm x |>.2, by simp +decide ⟩
...
lemma A_plus_equitable
  ...
  have hproper : IsProper (G.induce A_plus) f_A := by
    intro u v; by_cases hu : u.val = z <;> by_cases hv : v.val = z <;> simp_all +decide ; (
    intro huv h_eq; have := hg_acc v v.2; simp_all +decide [ SimpleGraph.adj_comm ] ;
    specialize hXmov v ; simp_all +decide ;
    exact hXmov ( by have := φ.injective h_eq; aesop ));
    · contrapose! hXmov; simp_all +decide [ SimpleGraph.adj_comm ] ;
      use u.val; simp_all +decide ; (
      have := φ.injective hXmov.2; aesop;);
    · intro huv h_eq;
      -- Since $u$ and $v$ are not $z$ or $y₁$, their colors are determined by $ne.f$. Therefore, if $f_A u = f_A v$, then $ne.f u = ne.f v$.
      have h_ne_f_eq : ne.f u.val = ne.f v.val := by
        have h_ne_f_eq : φ ⟨g u.val, hg_acc u.val u.prop⟩ = φ ⟨g v.val, hg_acc v.val v.prop⟩ → g u.val = g v.val := by
          exact fun h => by simpa using congr_arg Subtype.val ( φ.injective h ) ;
        grind +suggestions;
  ...
  · -- X ≠ ne.small: construct NearlyEquitable and use accessibility
    ...
    have card_total : Fintype.card ↥A_plus = m1 * s := by
      ...
      · rw [ Nat.sub_add_cancel ( Nat.one_le_iff_ne_zero.mpr _ ) ] ; contrapose! hacc ; aesop;
      · ext v; simp [A_plus];
        grind
...
lemma non_accessible_case1
  ...
  have hA_plus_card : Fintype.card A_plus = (Finset.univ.filter (fun c : Fin (r + 1) => IsAccessible G ne.f ne.small c)).card * s := by
    ...
    have := non_acc_verts_minus_one_card G r s hs ne hacc y₁ hy₁_nacc; simp_all +decide ;
    rw [ show ( Finset.filter ( Membership.mem B_minus ) Finset.univ : Finset V ) = Finset.filter ( fun v => ¬IsAccessible G ne.f ne.small ( ne.f v ) ) Finset.univ \ { y₁ } from ?_ ] ; simp +decide [ Finset.card_sdiff, * ] ; ring_nf;
    · rw [ show ( Finset.univ.filter fun c : Fin ( r + 1 ) => IsAccessible G ne.f ne.small c ) = Finset.univ \ ( Finset.univ.filter fun c : Fin ( r + 1 ) => ¬IsAccessible G ne.f ne.small c ) by ext; simp +decide, Finset.card_sdiff ] ; simp +decide [ Finset.card_univ ] ; ring_nf;
  ...
  · convert hB_minus_card using 1;
    simp +decide [ Set.compl_setOf, A_plus, B_minus ];
...
/-- **Erdős Problem 914** (Hajnal–Szemerédi). For `r ≥ 2` and `m ≥ 1`, every
graph on `r * m` vertices with minimum degree at least `m * (r - 1)` contains
`m` vertex-disjoint copies of `K_r`. -/
theorem erdos_914 {V : Type*} [Fintype V] [DecidableEq V]
    (G : SimpleGraph V) [DecidableRel G.Adj] (r m : ℕ) (hr : 2 ≤ r) (hm : 1 ≤ m)
    (hcard : Fintype.card V = r * m) (hmin : m * (r - 1) ≤ G.minDegree) :
    HajnalSzemeredi.HasDisjointCliques G r m :=
  HajnalSzemeredi.hajnal_szemeredi_clique_cover G r m (by lia) hcard hmin

#print axioms erdos_914
-- 'Erdos914.erdos_914' depends on axioms: [propext, Classical.choice, Quot.sound]
