...
Erdős Problem 615. Does there exist a constant `c > 0` such that every graph `G` on `n`
vertices with at least `(1/8 − c)n²` edges must contain either a `K₄` or an independent set on
at least `n / log n` vertices? `erdos_615` disproves this.

A problem of Erdős, Hajnal, Simonovits, Sós and Szemerédi [EHSSS93]; equivalently, it asks
whether the Ramsey–Turán number satisfies `rt(n; 4, n/log n) < (1/8 − c)n²`. Erdős, Hajnal,
Sós and Szemerédi [EHSS83] proved `rt(n; 4, εn) < (1/8 + o(1))n²` for fixed `ε > 0`. The
negative answer formalised here comes from a quantitative form of the Bollobás–Erdős
construction, whose Brunn–Minkowski input is the first module below. In the statement
`¬ G.CliqueFree 4` says `G` contains a `K₄`, and `n / log n ≤ G.indepNum` says `G` has an
independent set of at least that size.

Fox, Loh and Zhao quote the original question from [EHSSS93] as their Problem 1.4 ("The
critical window for the classical Ramsey-Turán problem", arXiv:1208.3276): "Is it true for
some constant `c > 0` that `RT(n, K₄, n/log n) < (1/8 - c)n²`?", and their Theorem 1.10
answers it negatively. Since `RT(n, K₄, m)` is the largest number of edges of an `n`-vertex
`K₄`-free graph with independence number less than `m`, that question is exactly the
contrapositive of the implication refuted below: a `K₄`-free `G` with
`G.indepNum < n / log n` would have to satisfy `e(G) < (1/8 - c)n²`.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos615.lean` together with the two modules of
`src/latest/ErdosProblems/Erdos615/`. The three files are concatenated here in dependency
...
`namespace Erdos615`, the upstream trust-base print line and trailing `alias` removed, and the
final theorem renamed from `not_erdos_615` to `erdos_615`. No mathematical content is changed.
-/
...
-- 'Erdos615.erdos_615' depends on axioms: [propext, Classical.choice, Quot.sound]
