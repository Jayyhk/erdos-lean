...
`Erdos1190.erdos1190_main` in `Erdos/P1190/Proof.lean`.
-/
...
lemma minimalMembersIn_upClosureIn_card_le
    ...
    (hUniform : Erdos1190.UniformFamily A k)
    (hAX : ∀ S ∈ A, S ⊆ X)
...
theorem not_pSmall_of_spread
    ...
    (_hUniform : Erdos1190.UniformFamily A k)
    (hSpread : Erdos1190.SpreadFamily A κ)
    (hκ : 1 < κ)
    (hAX : ∀ S ∈ A, S ⊆ X) :
...
lemma member_nonempty_of_uniform
    ...
    (hk : 1 ≤ k) (hUniform : Erdos1190.UniformFamily A k) (hS : S ∈ A) :
    S.Nonempty := by
...
theorem disjoint_members_of_many_successful_colors {X : Finset α} {m r k : ℕ}
    ...
    (hk : 1 ≤ k) (hUniform : Erdos1190.UniformFamily A k)
    (hSuccess : r ≤ (successfulColors A c).card) :
    ∃ B : Finset (Finset α),
      B ⊆ A ∧ B.card = r ∧ Erdos1190.PairwiseDisjointMembers B := by
  classical
...
Matches the historical `Erdos1190.spread_disjointness_input` interface in
`SpreadCore.lean`.
...
`Erdos1190.ParkPham.spread_disjointness_theorem`; the historical name
...
theorem spread_disjointness_input :
  ...
  Erdos1190.ParkPham.spread_disjointness_theorem
...
`Erdos1190.bfv_omega_count_input` in a later review step.
-/
...
* downstream: `Erdos.P202.BFV.LowerPathConstruction.lowerPath_f_lower_bound_eventually`
  ...
  → supplies the historical `Erdos1190.bfv_lower_bound_input` interface.
-/
...
`Erdos1190.bfv_lower_bound_input`, using the source-aligned rooted path lower
...
/-- **Erdős Problem 1190 — main theorem.** The sharp BFV asymptotic
`ε_m = exp(-(1 + o(1)) · sqrt(log m · log log m))`. -/
theorem erdos_1190 :
    ∀ ε : ℝ, 0 < ε → ∀ᶠ m : ℕ in atTop,
      Lscale (-(1 + ε)) m ≤ epsilon1190 m ∧
      epsilon1190 m ≤ Lscale (-(1 - ε)) m := by
  intro ε hε
  filter_upwards [eps1190_lower ε hε, eps1190_upper ε hε] with m hlow hup
  exact ⟨hlow, hup⟩

#print axioms erdos_1190
-- 'Erdos1190.erdos_1190' depends on axioms: [propext, Classical.choice, Quot.sound]
