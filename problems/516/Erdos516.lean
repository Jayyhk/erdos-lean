...
Erdős Problem 516. Let `f(z) = ∑_{k ≥ 1} aₖ z^{nₖ}` be an entire function of finite order with
`nₖ / k → ∞`, and set `M(r) = max_{|z| = r} |f(z)|`, `m(r) = min_{|z| = r} |f(z)|`. Is it true
that `limsup log m(r) / log M(r) = 1`? `erdos_516` proves that it is.

The affirmative answer is due to Fuchs [Fu63]. Below, `HasFabryGaps n` is
`StrictMono n ∧ nₖ / k → ∞` (Fabry gaps), `OfFiniteOrder f` is the usual finite-order growth
bound `‖f z‖ ≤ c * exp (‖z‖ ^ a)` for a differentiable `f`, and `ratio r f` is
`log (⨅_{‖z‖ = r} ‖f z‖) / log (⨆_{‖z‖ = r} ‖f z‖)`, i.e. `log m(r) / log M(r)`. The
hypothesis `aₖ ≠ 0` is the usual convention for a gap series, in which the listed coefficients
are the nonzero ones; it is also what the Formal Conjectures entry assumes, and the statement
below is that entry verbatim.

Erdős states this as item 7 on p.249 of [Er61] ("Some unsolved problems", Magyar Tud. Akad.
Mat. Kutató Int. Közl. 6 (1961), 221--254), attributing it to Pólya [Po29]: "Pólya's problem.
Let `f(z) = ∑_{k=1}^∞ aₖ z^{nₖ}` be an entire function of finite order. Assume that
`lim_{k→+∞} nₖ/k = ∞`. Does it then follow that `limsup log m(r)/log M(r) = 1`?" The statement
below is that question. Two small differences from Erdős's wording: he does not write the
hypothesis `aₖ ≠ 0` (it is the usual convention that the listed coefficients of a gap series
are the nonzero ones, and it is what the Formal Conjectures entry assumes), and monotonicity
of `n` is explicit here as part of `HasFabryGaps` where Erdős leaves it implicit in the
notation. Note also that on the same page, in the unrelated item 5, Erdős uses `m(r)` for the
maximum term `max |aₙ rⁿ|`; in Pólya's problem `m(r)` is the minimum modulus
`min_{|z| = r} |f(z)|`, which is what `ratio` uses.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos516.lean` together with the modules of
`src/latest/ErdosProblems/Erdos516/`. Those files are concatenated here in dependency order,
...
once in `namespace Erdos516` with the upstream trust-base print line removed. No mathematical
content is changed.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos516/Check516.lean` -/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos516.lean` -/
...
-- 'Erdos516.erdos_516' depends on axioms: [propext, Classical.choice, Quot.sound]
