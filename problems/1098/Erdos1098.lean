...
theorem exists_monochromatic_subsequence (c : ℕ → Bool) :
  ...
  obtain ⟨b, hb⟩ : ∃ b : Bool, Set.Infinite {n : ℕ | c n = b} := by
    ...
    · push_neg at h_finite; exact h_finite
...
def PairwiseNonCommuting (S : Set G) : Prop :=
  S.Pairwise fun x y => x * y ≠ y * x

def IsPEGroup (G : Type*) [Group G] : Prop :=
  ∀ S : Set G, PairwiseNonCommuting S → S.Finite
...
private theorem seqF_commutes_seqG (hFC : IsFCGroup G) (hnFIZ : ¬IsFIZGroup G)
  ...
  · have := fgAux_alpha_beta_props hFC hnFIZ j;
    ...
    grind +locals;
  · have := fgAux_alpha_beta_props hFC hnFIZ i; simp_all +decide [ seqF, seqG ] ;
...
/-- **Erdős Problem 1098** ([Ne76], proved by B.H. Neumann): if the non-commuting
graph of a group `G` has no infinite complete subgraph (i.e. every set of pairwise
non-commuting elements is finite), then there is a finite bound on the size of
complete subgraphs (i.e. of pairwise non-commuting finite sets). -/
theorem erdos_1098 (G : Type*) [Group G]
    (h : ∀ S : Set G, PairwiseNonCommuting S → S.Finite) :
    ∃ n : ℕ, ∀ S : Finset G,
      PairwiseNonCommuting (↑S : Set G) → S.card ≤ n := by
...
#print axioms erdos_1098
-- 'Erdos1098.erdos_1098' depends on axioms: [propext, Classical.choice, Quot.sound]
