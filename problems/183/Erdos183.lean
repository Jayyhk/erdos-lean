...
Erdős Problem 183: "Let $R(3;k)$ be the minimal $n$ such that if the edges of $K_n$ are
coloured with $k$ colours then there must exist a monochromatic triangle. Determine
$\\lim_{k \\to \\infty} R(3;k)^{1/k}$."

`erdos_183` determines this limit: it is $+\\infty$. Erdős offered \\$100 for a proof that
the limit is *finite*, so this settles that question in the negative.

The original proof was found by: OpenAI (github.com/openai/ten-proofs, file
`MulticolorTriangleRamsey.lean`), linked from https://www.erdosproblems.com/183.

Vendored verbatim from that file, with only these changes: the namespace is renamed from
`ErdosProblems.MulticolourTriangleRamsey` to `Erdos183`; the proof-carrying theorem
`divergentRamseyRoot` is renamed to `erdos_183` (upstream had `erdos_183` as a one-line
alias of it); and upstream's `erdos_problem_183_explicit`, which merely paired
...
`R(3;k) = triangleRamseyNumber k` is the least `n` such that every `k`-colouring of the
edges of `K n` contains a monochromatic triangle. This is the limit the problem asks for:
`R(3;k)^(1/k)` tends to infinity, i.e. **the limit is `+∞`**.

Equivalently, `R(3;k)` grows superexponentially. This answers in the negative the
...
`triangleRamseyNumber_succ_le` and `triangleRamseyNumber_factorial_upper`
(`R(3;k) ≤ 4 · k!`). -/
theorem erdos_183 :
    Filter.Tendsto
      (fun k : ℕ =>
