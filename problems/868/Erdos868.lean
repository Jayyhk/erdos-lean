...
Erdős Problem 868. If `A` is an additive basis of order `2` and `1_A * 1_A(n) → ∞`, must `A`
contain a minimal additive basis of order `2` -- one for which deleting any element leaves
infinitely many `n ∉ A + A`? And what if instead `1_A * 1_A(n) > ε log n` for all large `n`,
...
`src/latest/ErdosProblems/Erdos868.lean` together with the modules of
`src/latest/ErdosProblems/Erdos868/`. Those files are concatenated here in dependency order,
...
`namespace Erdos868`, and the upstream trust-base print lines and trailing `alias`es removed.
The two parts are renamed from `not_erdos_868` and `not_erdos_868_part_ii` (upstream
`erdos_868.parts.i` and `erdos_868.parts.ii`). No mathematical content is changed.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos868.lean` -/
...
/-- An asymptotic additive basis of order `o`: its `o`-fold pointwise Set.sum is cofinite.
...
/-- **Erdős Problem 868.** Both of the questions on erdosproblems.com have a negative answer:
...
#print axioms erdos_868
-- 'Erdos868.erdos_868' depends on axioms: [propext, Classical.choice, Quot.sound]
