...
lemma erdosTerm_summable (a : ℕ → ℕ) (ha : ∀ n, 0 < a (n + 1))
  ...
  convert h_pseries.mul_right ( 2 ^ N / ( Q a N : ℝ ) ) using 2 ; norm_num ; ring_nf;
...
lemma tail_geometric_bound (a : ℕ → ℕ) (ha : ∀ n, 0 < a (n + 1))
  ...
  have h_bound : ∑' n, erdosTerm a (N + n) ≤ ∑' n, (Λ / (M : ℝ)) ^ (n + 1) / (Q a N : ℝ) := by
    ...
    · have h_term_bound : ((N + n + 1).divisors.card : ℝ) / (Q a (N + n + 1) : ℝ) ≤ (Λ ^ (n + 1) : ℝ) / (Q a N * M ^ (n + 1)) := by
      ...
      convert h_term_bound using 1 ; ring!;
  ...
  have h_geo_series : ∑' n, (Λ / (M : ℝ)) ^ (n + 1) / (Q a N : ℝ) = (Λ / (M : ℝ)) / (1 - Λ / (M : ℝ)) / (Q a N : ℝ) := by
    ...
    exact Or.inl ( by erw [ ← tsum_geometric_of_lt_one ( by positivity ) ( by rw [ mul_inv_lt_iff₀ ( by norm_cast; linarith [ show M > 0 from Nat.pos_of_ne_zero ( by rintro rfl; norm_num at hM; nlinarith ) ] ) ] ; nlinarith ) ] ; erw [ ← tsum_mul_left ] ; exact tsum_congr fun _ => by ring );
