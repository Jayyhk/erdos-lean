...
Erdős Problem 116. Let `p(z) = ∏_{i=1}^n (z - zᵢ)` with `|zᵢ| ≤ 1`. Is it true that the
planar measure of the strict unit lemniscate satisfies

  `|{z : |p(z)| < 1}| > n^(-O(1))`?

`erdos_116` proves that it is: there are absolute constants `c > 0` and `C : ℕ`, chosen
before `n`, with `c / n ^ C < volume {z : ℂ | ‖∏ i, (z - a i)‖ < 1}` for every `n > 0` and
every choice of roots in the closed unit disk. The witnesses are `c = π / 2 ^ 31` and
`C = 14`, established in explicit form by `erdos_116_explicit`.

Conjectured by Erdős, Herzog and Piranian; the `≫ n⁻⁴` bound follows from a result of
Pommerenke. The measure is two-dimensional Lebesgue measure on `ℂ`.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos116.lean` -/
...
-- 'Erdos116.erdos_116' depends on axioms: [propext, Classical.choice, Quot.sound]
