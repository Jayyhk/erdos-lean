...
Erdős Problem 109 (the Erdős sumset conjecture). Any `A ⊆ ℕ` of positive upper density
contains a sumset `B + C` with both `B` and `C` infinite. `erdos_109` proves this.

The conjecture is recorded in [ErGr80, p.85] and was proved by Moreira, Richter and Robertson
[MRR19]; the proof formalised here follows their ergodic-theoretic argument, working with an
...
`src/latest/ErdosProblems/Erdos109/`. Those files are concatenated here in dependency order,
...
once in `namespace Erdos109` with the upstream trust-base print line removed. No mathematical
content is changed.
-/
...
/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos109.lean` -/
...
#print axioms erdos_109
-- 'Erdos109.erdos_109' depends on axioms: [propext, Classical.choice, Quot.sound]
