...
Erdős Problem 1112: "Let $1 \le d_1 < d_2$ and $k \ge 3$. Does there exist an integer $r$
such that if $B = \{b_1 < b_2 < \cdots\}$ is a lacunary sequence of positive integers with
$b_{i+1} \ge r b_i$ then there exists a sequence of positive integers
$A = \{a_1 < a_2 < \cdots\}$ such that $d_1 \le a_{i+1} - a_i \le d_2$ for all $i \ge 1$
and $(kA) \cap B = \emptyset$, where $kA$ is the $k$-fold sumset?"

`erdos_1112` answers this: such an $r$ exists **if and only if** $d_2 \ge k + 1$.

`Question` encodes the question with the ratio quantified over `ℕ`; `QuestionInt` uses the
problem's literal integer phrasing, and `question_iff_questionInt` proves the two agree.
The halves are `erdos_1112_existence_bound` (the explicit ratio $192 d_2$ works when
$d_2 \ge k+1$) and `erdos_1112_strong_nonexistence` (when $d_2 \le k$ no growth condition
on $B$ whatsoever suffices, so no $r$ can exist).
...
dependency is dropped, leaving `Mathlib` as the only import. Each upstream module keeps its
own `section` so that file-scoped `variable`/`open` declarations do not leak across the
concatenation. The mathematical content is unchanged.
-/


/-! ### Upstream module `Erdos1112` -/
...
The problem says "an integer `r`", but the quantitative parameter `r_k(d₁, d₂)` of
[BHJ97] — under which this question is usually discussed — is naturally a *real*
threshold. Neither choice changes the answer: the class of admissible `B` only shrinks
as the ratio grows, so a real witness is dominated by the natural number `⌈r⌉₊`, and a
natural witness is already a real one. Proving this means `erdos_1112` resolves the
question under all three readings of the ratio (`ℕ`, `ℤ`, `ℝ`). -/

/-- `B` is lacunary with a *real* ratio `r`. -/
def IsLacunaryWithReal (r : ℝ) (b : ℕ → ℕ) : Prop :=
  0 < b 0 ∧ StrictMono b ∧ ∀ i, r * (b i : ℝ) ≤ (b (i + 1) : ℝ)

/-- `RatioWorks` with the ratio quantified over `ℝ`. -/
def RatioWorksReal (k d₁ d₂ : ℕ) (r : ℝ) : Prop :=
  ∀ b : ℕ → ℕ, IsLacunaryWithReal r b →
    ∃ a : ℕ → ℕ, HasGapsIn d₁ d₂ a ∧
      Disjoint (kFoldSumset k a) (Set.range b)

/-- The question with the ratio quantified over `ℝ`. -/
def QuestionReal (k d₁ d₂ : ℕ) : Prop :=
  ∃ r : ℝ, RatioWorksReal k d₁ d₂ r

/-- **The ℝ-to-ℕ bridge.** A real ratio gives exactly the same existence question as a
natural one: forwards, cast; backwards, replace `r` by `⌈r⌉₊ ≥ r`. -/
theorem question_iff_questionReal (k d₁ d₂ : ℕ) :
    Question k d₁ d₂ ↔ QuestionReal k d₁ d₂ := by
  constructor
  · rintro ⟨r, hr⟩
    refine ⟨(r : ℝ), fun b hb => hr b ⟨hb.1, hb.2.1, fun i => ?_⟩⟩
    have h := hb.2.2 i
    exact_mod_cast h
  · rintro ⟨r, hr⟩
    refine ⟨⌈r⌉₊, fun b hb => hr b ⟨hb.1, hb.2.1, fun i => ?_⟩⟩
    have hn : (⌈r⌉₊ * b i : ℕ) ≤ b (i + 1) := hb.2.2 i
    have h1 : r ≤ (⌈r⌉₊ : ℝ) := Nat.le_ceil r
    have hb0 : (0 : ℝ) ≤ (b i : ℝ) := Nat.cast_nonneg _
    calc r * (b i : ℝ) ≤ (⌈r⌉₊ : ℝ) * (b i : ℝ) := by nlinarith
      _ = ((⌈r⌉₊ * b i : ℕ) : ℝ) := by push_cast; ring
      _ ≤ (b (i + 1) : ℝ) := by exact_mod_cast hn

end


/-! ### Upstream module `Erdos1112Proof.Basic` -/
...
/-! ### Upstream module `Erdos1112Proof.Existence.Beatty` -/
...
/-! ### Upstream module `Erdos1112Proof.Existence.FreeGap` -/
...
/-! ### Upstream module `Erdos1112Proof.Existence.Nested` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.TailCovering` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.Kit` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.Certificate` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.GapWord` -/
...
/-! ### Upstream module `Erdos1112Proof.SubsetSums` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.SlotLemmaParts` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.Defs` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.TwoGen` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.Frame` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.Lift` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.Tables` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.TablesData` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseBClasses` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseB` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.Staircase` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseD` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseE` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseL` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseP` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseTCore` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseTScanE1` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseTScanE10` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseTScanE2` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseTScanE3` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseTScanE4` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseTScanE5` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseTScanE6` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseTScanE7` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseTScanE8` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseTScanE9` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseTScan` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.CaseT` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.Graham` -/
...
/-! ### Upstream module `Erdos1112Proof.Sharp.Main` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.SlotLemma` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.TwoLetter.Core` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.TwoLetter.MH.Slope` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.TwoLetter.MH.IrrationalCase` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.TwoLetter.MH.RationalCase` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.TwoLetter.Balanced` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.TwoLetter.MH.Subwindow` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.TwoLetter.MH.Walk` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.TwoLetter.Sturmian` -/
...
/-! ### Upstream module `Erdos1112Proof.NonEx.Main` -/
...
/-! ### Upstream module `Erdos1112Proof.Final` -/
...
For `k ≥ 3` and `1 ≤ d₁ < d₂`: *an integer* `r` of the kind the problem demands — one
forcing every lacunary `B` with `b_{i+1} ≥ r · b_i` to admit an `A` with all gaps in
`[d₁, d₂]` and `(kA) ∩ B = ∅` — exists **if and only if** `d₂ ≥ k + 1`.

`QuestionInt` quantifies `r : ℤ`, matching the problem's literal wording. The proof
passes through `question_iff_questionInt` to the `ℕ` phrasing `Question` that the
development works with; with `question_iff_questionReal` this shows the `ℕ`, `ℤ` and `ℝ`
readings of the ratio all give the same answer.

The remark of Erdős and Graham that this problem comes from ([ErGr80], p. 18) fixes
`b₁ ≥ 5`, ratio `2`, gaps in `[2, 3]` and two summands, and asks only "whether such
behavior can hold for `A + A + A` (or more summands)"; it contains no ratio `r`. The
parametrised question above is due to [BHJ97], and its `k = 3, d₁ = 2, d₂ = 3` corner is
exactly the case Erdős and Graham left open — answered here in the negative, since
`3 < 4`. -/
theorem erdos_1112 (k d₁ d₂ : ℕ) (hk : 3 ≤ k) (hd₁ : 1 ≤ d₁)
    (hd : d₁ < d₂) :
    QuestionInt k d₁ d₂ ↔ k + 1 ≤ d₂ := by
  rw [← question_iff_questionInt]
  constructor
  · rintro ⟨r, hr⟩
    by_contra hlt
    push_neg at hlt
