...
/-- **Erdős Problem 1047** (Grunsky's question, EHP58 Problem 1b, p.145, refuted by
Aristotle's counterexample `f(z) = z⁶ − z` with `c = 0.582`): for a monic polynomial
`f ∈ ℂ[x]` with `m` distinct root locations (i.e. `f.roots.toFinset.card = m`) and
`c > 0` small enough that `{z : |f(z)| ≤ c}` has `m` distinct connected components
(one around each root), it is **not** the case that all such components must be
convex. -/
theorem erdos_1047 :
  ...
  rcases main_result with
    ⟨f, c, m, hfMonic, _hfNodup, hcard, hcpos, himgcard, ⟨K, hKmem, hKnotConvex⟩⟩
  have hKconv : Convex ℝ K :=
    (h f c m hfMonic hcard hcpos himgcard) K hKmem
  exact hKnotConvex hKconv

#print axioms erdos_1047
-- 'Erdos1047.erdos_1047' depends on axioms: [propext, Classical.choice, Quot.sound]
