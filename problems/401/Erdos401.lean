...
/-
The weight function ω(r) tends to infinity as r → ∞ (since p_{r+1} → ∞ and (q-1)/log q → ∞).
-/
theorem omega_tendsto : Filter.Tendsto ω Filter.atTop Filter.atTop := by
  have h_ulogu : Filter.Tendsto (fun u : ℝ => u / Real.log u) Filter.atTop Filter.atTop := by
    suffices h : Filter.Tendsto (fun v : ℝ => Real.exp v / v) Filter.atTop Filter.atTop by
      have hc := h.comp Real.tendsto_log_atTop
      exact hc.congr' (by filter_upwards [Filter.eventually_gt_atTop 0] with x hx using by rw [Function.comp_apply, Real.exp_log hx])
    simpa using Real.tendsto_exp_div_pow_atTop 1
  have h_q1logq : Filter.Tendsto (fun q : ℝ => (q - 1) / Real.log q) Filter.atTop Filter.atTop := by
    refine Filter.tendsto_atTop_mono' Filter.atTop ?_
      (h_ulogu.const_mul_atTop (show (0 : ℝ) < 1 / 2 by norm_num))
    filter_upwards [Filter.eventually_ge_atTop (2 : ℝ)] with q hq
    have hlog : 0 < Real.log q := Real.log_pos (by linarith)
    rw [mul_div_assoc']
    exact (div_le_div_iff_of_pos_right hlog).mpr (by linarith)
  have h_pr : Filter.Tendsto (fun r : ℕ => ((p (r + 1) : ℕ) : ℝ)) Filter.atTop Filter.atTop := by
    have hsm : Filter.Tendsto (fun r : ℕ => Nat.nth Nat.Prime r) Filter.atTop Filter.atTop :=
      (Nat.nth_strictMono Nat.infinite_setOf_prime).tendsto_atTop
    refine (tendsto_natCast_atTop_atTop.comp hsm).congr (fun r => ?_)
    simp [p]
  have hcomp := (h_q1logq.comp h_pr).const_mul_atTop
    (show (0 : ℝ) < γ / 16 by norm_num [γ])
  refine hcomp.congr (fun r => ?_)
  simp only [ω, Function.comp_apply]
  rw [mul_div_assoc]

/-
**Erdős Problem 401.** There is a function `f(r) → ∞` such that, for every `r ≥ 1`, for
infinitely many `n` there exist `a₁, a₂ > 0` with `a₁ + a₂ > n + f(r) log n` and
`a₁! a₂! ∣ n! · P_r^n` (where `P_r = 2·3⋯p_r`). Witnessed by `f = ω`.
-/
theorem erdos_401 :
    ∃ f : ℕ → ℝ, Filter.Tendsto f Filter.atTop Filter.atTop ∧
      ∀ r : ℕ, r ≥ 1 → Set.Infinite {n : ℕ | ∃ a1 a2 : ℕ, a1 > 0 ∧ a2 > 0 ∧
        (a1 : ℝ) + a2 > n + f r * Real.log n ∧
        (Nat.factorial a1 * Nat.factorial a2) ∣ (Nat.factorial n * (P r)^n)} :=
  ⟨ω, omega_tendsto, theorem_1⟩

#print axioms erdos_401
-- 'Erdos401.erdos_401' depends on axioms: [propext, Classical.choice, Quot.sound]
