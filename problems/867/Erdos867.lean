...
* `erdos_867`: the conjecture `|A| ≤ N/2 + O(1)`
  is **false**.
...
lemma freudSet_cross_AB_card4_sum_gt (y : ℕ) (hy : 1 ≤ y) (a b : ℕ)
  ...
  have h_third : (36*y-5) ∈ {x ∈ freudSet y | a ≤ x ∧ x ≤ b} ∨
    ...
    obtain ⟨x, hx⟩ : ∃ x ∈ {x ∈ freudSet y | a ≤ x ∧ x ≤ b},
      ...
      · push_neg at h_cases; obtain ⟨x, hx₁, hx₂, hx₃⟩ := h_cases
...
lemma freudSet_interval_sum_cross_AB_or_all_ge_48 (y : ℕ) (hy : 1 ≤ y) (a b : ℕ)
  ...
  · push_neg at hall; obtain ⟨w, hw, hwlt⟩ := hall
...
lemma freudSet_interval_sum_not_in_D (y : ℕ) (hy : 1 ≤ y) (a b : ℕ)
  ...
  · push_neg at h; obtain ⟨z, hz, hzge⟩ := h
...
#print axioms erdos_867
-- 'Erdos867.erdos_867' depends on axioms: [propext, Classical.choice, Quot.sound]
