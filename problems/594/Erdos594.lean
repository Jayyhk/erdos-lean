...
Erdős Problem 594. Does every graph `G` with chromatic number `≥ ℵ₁` contain all sufficiently large odd cycles? `erdos_594` proves this.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos594.lean`, a single self-contained file whose only import is
`Mathlib`. It is reproduced here with the file-level comment header, the upstream trust-base
print lines and the trailing `alias` removed, wrapped once in `namespace Erdos594`, and with the
final theorem named `erdos_594`. No mathematical content is changed.
-/
...
-- 'Erdos594.erdos_594' depends on axioms: [propext, Classical.choice, Quot.sound]
