      ...
      · rcases h with ( h | ⟨ r₁, r₂, hr₁, hr₂, h ⟩ );
        ...
        · -- Since $r₁$ and $r₂$ are positive, we can divide both sides of the equation $r₁ • (u - C) = r₂ • (v - C)$ by $r₁$ and $r₂$ respectively.
          have h_div : (r₁ / r₂) • (u - C) = v - C := by
            simp +decide [ div_eq_inv_mul, MulAction.mul_smul, h, hr₂.ne' ];
          by_cases hr₁₂ : r₁ / r₂ ≤ 1;
          · by_cases hr₁₂' : r₁ / r₂ < 1;
      ...
      · intro y hy z hz hx
        ...
        have h_av_bv_zero : ∀ v ∈ s, v ≠ x → a v = 0 ∧ b v = 0 := by
          ...
          have h_av_zero : a v = 0 := by
            rw [ Finset.sum_eq_add_sum_diff_singleton ‹x ∈ s› ] at ha_sum;
          ...
          have h_bv_zero : b v = 0 := by
            rw [ Finset.sum_eq_add_sum_diff_singleton ( show x ∈ s from by assumption ) ] at hb_sum;
...
lemma centroid_angle_injective_on_s {s : Finset V}
  ...
  · exact convex_convexHull ℝ _;
  · exact IsCompact.isClosed ( Set.Finite.isCompact_convexHull <| Finset.finite_toSet s )
...
lemma angle_ge_of_dist_ge {A B C : V} (hAB : A ≠ B) (hBC : B ≠ C) (hAC : A ≠ C)
  ...
  -- Apply the lemma `angle_gt_of_dist_gt` to obtain a contradiction.
  have h_contra : EuclideanGeometry.angle A B C > EuclideanGeometry.angle B A C := by
    apply angle_gt_of_dist_gt hAB hBC hAC h_not_col;
    refine' lt_of_le_of_ne h fun h' => h_contra_contra _;
    simp_all +decide [ EuclideanGeometry.angle, dist_eq_norm ];
    simp_all +decide [ InnerProductGeometry.angle, norm_sub_rev ];
    simp_all +decide [ norm_eq_sqrt_real_inner ];
    rw [ Real.sqrt_inj ( by rw [ real_inner_self_eq_norm_sq ] ; positivity ) ( by rw [ real_inner_self_eq_norm_sq ] ; positivity ) ] at h' ; simp_all +decide [ inner_sub_left, inner_sub_right ];
    simp_all +decide [ real_inner_comm, real_inner_self_eq_norm_sq ];
    grind;
  exact h_contra_contra h_contra.le
...
lemma exists_halfplane_functional_of_angle_span {s : Set ℂ} (a b : ℝ)
    ...
    ∃ f : ℂ →L[ℝ] ℝ, f ≠ 0 ∧ ∀ z ∈ s, f z ≤ 0 := by
      ...
      have hw : ∀ z ∈ s, z ≠ 0 → Complex.re (z * Complex.exp (-α * Complex.I)) ≥ 0 := by
        ...
        convert mul_nonneg ( norm_nonneg z ) h_cos_nonneg using 1 ; rw [ ← Complex.norm_mul_cos_arg, ← Complex.norm_mul_sin_arg ] ; ring_nf;
        norm_num [ Real.cos_add, Real.sin_add, mul_assoc, mul_comm Real.pi ] ; ring_nf;
        rw [ ← Complex.norm_mul_cos_arg, ← Complex.norm_mul_sin_arg ] ; ring_nf;
...
lemma consecutive_angle_diff_pos {n : ℕ} {z : Fin n → ℂ}
    ...
    0 < diff := by
      ...
      · convert sub_pos.mpr ( h_sorted i ⟨ ( i + 1 ) % n, Nat.mod_lt _ ( by linarith ) ⟩ _ ) using 1;
        norm_num [ Fin.lt_iff_val_lt_val ];
    ...
    have h_arg_diff : 0 < (if i.val < n - 1 then Complex.arg (z (⟨(i + 1) % n, Nat.mod_lt _ (lt_of_lt_of_le (by norm_num) h_n)⟩)) - Complex.arg (z i) else 2 * Real.pi + Complex.arg (z (⟨(i + 1) % n, Nat.mod_lt _ (lt_of_lt_of_le (by norm_num) h_n)⟩)) - Complex.arg (z i)) ∧ (if i.val < n - 1 then Complex.arg (z (⟨(i + 1) % n, Nat.mod_lt _ (lt_of_lt_of_le (by norm_num) h_n)⟩)) - Complex.arg (z i) else 2 * Real.pi + Complex.arg (z (⟨(i + 1) % n, Nat.mod_lt _ (lt_of_lt_of_le (by norm_num) h_n)⟩)) - Complex.arg (z i)) < Real.pi := by
      have h_int : 0 ∈ interior (convexHull ℝ (Set.range z)) := by
        have h_arg_diff : 0 ∈ interior (convexHull ℝ (Set.image (fun x => to_complex_map (x - C)) s)) := by
          ...
          refine' ⟨ ι, t, w, fun i => to_complex_map ( z i - C ), hw, hw', _, _ ⟩ <;> simp_all +decide [ Finset.centerMass ];
          simp +decide [ mul_sub, ← Finset.sum_mul ];
          norm_cast ; aesop;
    ...
    · -- Case vi != vj: collinearity contradiction
      ...
      · -- Case k > 0: Same ray
        ...
        have h_eq_pts : vi = vj := unique_ray_intersection_frontier (convex_convexHull ℝ _)
          (IsCompact.isClosed (Set.Finite.isCompact_convexHull <| Finset.finite_toSet s)) hC_int
      ...
      · -- Case k < 0: Opposite rays
        ...
        have h_opp : SameRay ℝ (vi - C) (-(vj - C)) := by
          apply (SameRay.sameRay_map_iff to_complex_map).mp
          simp only [map_neg, map_sub, zi, zj, hk_eq]
          exact Or.inr (Or.inr ⟨-k, 1, neg_pos.mpr hk_neg, zero_lt_one, by simp⟩)
        ...
        have h_C_seg : C ∈ openSegment ℝ vi vj := by
          ...
          apply to_complex_map.injective
          dsimp [t]
          simp only [to_complex_map.map_add, to_complex_map.map_smul]
          have h_vi_map : to_complex_map vi = zi + to_complex_map C := by
             simp [zi, map_sub]
          have h_vj_map : to_complex_map vj = zj + to_complex_map C := by
             simp [zj, map_sub]
          rw [h_vi_map, h_vj_map, hk_eq]
          dsimp [t]
          apply Complex.ext
          · simp only [Complex.add_re, Complex.ofReal_re, Complex.ofReal_im, Complex.mul_re]
            dsimp [t]
            field_simp [hk_nz, (by linarith : 1 - k ≠ 0)]
            ring
          · simp only [Complex.add_im, Complex.ofReal_re, Complex.ofReal_im, Complex.mul_im]
            dsimp [t]
            field_simp [hk_nz, (by linarith : 1 - k ≠ 0)]
            ring
...
lemma is_convex_polygon_angular_order {n : ℕ} {s : Finset V}
  ...
  have h_total : IsTotal V R_le := ⟨fun x y => le_total (angle x) (angle y)⟩
  ...
  have h_sorted_le : l.Sorted R_le := by
    rw [h_sort_eq]
    exact @List.sorted_insertionSort V R_le (by infer_instance) h_total h_trans s.toList
...
-- 'Erdos93.erdos_93' depends on axioms: [propext, Classical.choice, Quot.sound]
