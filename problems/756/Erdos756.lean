...
lemma regular_polygon_card (m : ℕ) (hm : m ≥ 1) : (regular_polygon m).card = m := by
  ...
  · -- The polynomial $X^m - 1$ is split over the complex numbers, so its roots are exactly the $m$-th roots of unity.
    have h_split : Polynomial.Splits (algebraMap ℂ ℂ) (Polynomial.X ^ m - 1 : Polynomial ℂ) := by
      exact IsAlgClosed.splits_domain (Polynomial.X ^ m - 1);
    simp_all +decide [ Polynomial.splits_iff_card_roots ];
    rw [ Polynomial.natDegree_sub_eq_left_of_natDegree_lt ] at h_split <;> aesop;
...
#print axioms erdos_756
-- 'Erdos756.erdos_756' depends on axioms: [propext, Classical.choice, Quot.sound]
