...
Erdős Problem 152. For any `M ≥ 1`, if `A ⊂ ℕ` is a sufficiently large finite Sidon set then there are at least `M` many `a ∈ A + A` with `a + 1, a - 1 ∉ A + A`. `erdos_152` proves this.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos152.lean`, a single self-contained file whose only import is
`Mathlib`. It is reproduced here with the file-level comment header, the upstream trust-base
print lines and the trailing `alias` removed, wrapped once in `namespace Erdos152`, and with the
final theorem named `erdos_152` (upstream `erdos_152_unbounded`, the `Tendsto` form that
matches the site's statement); upstream's quantitative `erdos_152` (`f ≫ n²`) is kept as
`erdos_152_quantitative`. No mathematical content is changed.
-/
...
lemma universal_parity_3 (X : Set ℤ) (hX : X.Finite) :
  ...
  · have hcard :
        (hX.toFinset.filter (fun x => x + 2 ∈ hX.toFinset)).card =
          {x ∈ X | x + 2 ∈ X}.ncard := by
      rw [Set.ncard_eq_toFinset_card _ (hX.sep _)]
      congr 1
      ext x
      simp
    omega
...
lemma num_isolated_Z_rel (A : Set ℕ) (hS : (A + A).Finite) :
  I_Z (Z_S (A + A)) ≤ num_isolated A + 1 := by
  change
    {z : ℤ | z ∈ (fun x : ℕ => (x : ℤ)) '' (A + A) ∧
      z - 1 ∉ (fun x : ℕ => (x : ℤ)) '' (A + A) ∧
      z + 1 ∉ (fun x : ℕ => (x : ℤ)) '' (A + A)}.ncard ≤
      {m : ℕ | m - 1 ∉ A + A ∧ m ∈ A + A ∧ m + 1 ∉ A + A}.ncard + 1
  let T : Set ℕ := {m | m - 1 ∉ A + A ∧ m ∈ A + A ∧ m + 1 ∉ A + A}
  have hT : T.Finite := hS.subset fun _ hm => hm.2.1
  have hsub :
      {z : ℤ | z ∈ (fun x : ℕ => (x : ℤ)) '' (A + A) ∧
          z - 1 ∉ (fun x : ℕ => (x : ℤ)) '' (A + A) ∧
          z + 1 ∉ (fun x : ℕ => (x : ℤ)) '' (A + A)} ⊆
        (fun m : ℕ => (m : ℤ)) '' T ∪ {0} := by
    rintro z ⟨⟨m, hm, rfl⟩, hleft, hright⟩
    ...
    · left
      ...
      · intro hpred
        apply hleft
        exact ⟨m - 1, hpred, by norm_num [Nat.cast_pred hmpos]⟩
      · intro hnext
        apply hright
        exact ⟨m + 1, hnext, by norm_num⟩
  calc
    _ ≤ ((fun m : ℕ => (m : ℤ)) '' T ∪ {0}).ncard :=
      Set.ncard_le_ncard hsub ((hT.image _).union (Set.finite_singleton 0))
    _ ≤ ((fun m : ℕ => (m : ℤ)) '' T).ncard + 1 := by
      simpa using Set.ncard_union_le ((fun m : ℕ => (m : ℤ)) '' T) {0}
    _ = T.ncard + 1 := by rw [Set.ncard_image_of_injective _ Nat.cast_injective]
...
lemma quad_lower_2_exact (A : Set ℕ) (k : ℕ) (hA : A.Finite) :
    N_k_Z (D_set A) k ≤ (quad_k_N A k).ncard := by
  let φ : (ℕ × ℕ × ℕ × ℕ) → ℤ := fun q => (q.1 : ℤ) - q.2.2.2
  have hQ : (quad_k_N A k).Finite := by
    apply (hA.prod (hA.prod (hA.prod hA))).subset
    intro q hq
    exact ⟨hq.1, hq.2.1, hq.2.2.1, hq.2.2.2.1⟩
  have hsurj : {z ∈ D_set A | z + (k : ℤ) ∈ D_set A} ⊆
      φ '' quad_k_N A k := by
    rintro z ⟨⟨a, d, ha, hd, haz⟩, ⟨c, b, hc, hb, hcz⟩⟩
    subst z
    refine ⟨(a, b, c, d), ?_, rfl⟩
    refine ⟨ha, hb, hc, hd, ?_⟩
    have hInt : (a : ℤ) + b + k = c + d := by omega
    exact_mod_cast hInt
  unfold N_k_Z
  exact (Set.ncard_le_ncard hsurj (hQ.image φ)).trans (Set.ncard_image_le hQ)

lemma quad_lower_2 (A : Set ℕ) (k : ℕ) (_hSidon : IsSidon A) (hA : A.Finite) :
    N_k_Z (D_set A) k ≤ (quad_k_N A k).ncard + 2 * A.ncard := by
  exact (quad_lower_2_exact A k hA).trans (Nat.le_add_right _ _)

lemma quad_upper_2_exact (A : Set ℕ) (k : ℕ) (hSidon : IsSidon A) (hA : A.Finite) :
    (quad_k_N A k).ncard ≤ 4 * N_k_N (A + A) k := by
  let σ : (ℕ × ℕ × ℕ × ℕ) → ℕ := fun q => q.1 + q.2.1
  have hQ : (quad_k_N A k).Finite := by
    apply (hA.prod (hA.prod (hA.prod hA))).subset
    intro q hq
    exact ⟨hq.1, hq.2.1, hq.2.2.1, hq.2.2.2.1⟩
  have hE : {s ∈ A + A | s + k ∈ A + A}.Finite := (hA.add hA).sep _
  have himage : hQ.toFinset.image σ ⊆ hE.toFinset := by
    intro s hs
    obtain ⟨q, hq, rfl⟩ := Finset.mem_image.mp hs
    have hq' := hQ.mem_toFinset.mp hq
    rw [hE.mem_toFinset]
    refine ⟨⟨q.1, hq'.1, q.2.1, hq'.2.1, rfl⟩, ?_⟩
    rw [hq'.2.2.2.2]
    exact ⟨q.2.2.1, hq'.2.2.1, q.2.2.2, hq'.2.2.2.1, rfl⟩
  have hfiber : ∀ s ∈ hQ.toFinset.image σ,
      (hQ.toFinset.filter fun q => σ q = s).card ≤ 4 := by
    intro s hs
    obtain ⟨q₀, hq₀, rfl⟩ := Finset.mem_image.mp hs
    ...
    calc
      (hQ.toFinset.filter fun q => σ q = σ (a, b, c, d)).card ≤ T.card := by
        apply Finset.card_mono
        intro q hq
        rw [Finset.mem_filter] at hq
        rcases q with ⟨a', b', c', d'⟩
        have hq' := hQ.mem_toFinset.mp hq.1
        have hsumAB : a' + b' = a + b := by simpa [σ] using hq.2
        have hab : (a' = a ∧ b' = b) ∨ (a' = b ∧ b' = a) := by
          apply hSidon a' hq'.1 a hq₀'.1 b' hq'.2.1 b hq₀'.2.1
          exact hsumAB
        have hcd : (c' = c ∧ d' = d) ∨ (c' = d ∧ d' = c) := by
          apply hSidon c' hq'.2.2.1 c hq₀'.2.2.1 d' hq'.2.2.2.1 d hq₀'.2.2.2.1
          have hqEq : a' + b' + k = c' + d' := by simpa using hq'.2.2.2.2
          have hq₀Eq : a + b + k = c + d := by simpa using hq₀'.2.2.2.2
          omega
        rcases hab with hab | hab <;> rcases hcd with hcd | hcd <;>
          simp_all [T, Prod.ext_iff]
      _ ≤ 4 := Finset.card_le_four
  rw [Set.ncard_eq_toFinset_card _ hQ, N_k_N,
    Set.ncard_eq_toFinset_card _ hE]
  exact (Finset.card_le_mul_card_image hQ.toFinset 4 hfiber).trans
    (Nat.mul_le_mul_left 4 (Finset.card_le_card himage))

lemma quad_upper_2 (A : Set ℕ) (k : ℕ) (hSidon : IsSidon A) (hA : A.Finite) :
    (quad_k_N A k).ncard ≤ 4 * N_k_N (A + A) k + 2 * A.ncard := by
  exact (quad_upper_2_exact A k hSidon hA).trans (Nat.le_add_right _ _)
...
lemma num_isolated_lower_bound (n : ℕ) (hn : n > 0) (A : Set ℕ) (h_card : A.ncard = n) (h_sidon : IsSidon A) :
  ...
  have hI1 := num_isolated_Z_rel A hSF
...
theorem erdos_152_quantitative :
...
#print axioms erdos_152
-- 'Erdos152.erdos_152' depends on axioms: [propext, Classical.choice, Quot.sound]
