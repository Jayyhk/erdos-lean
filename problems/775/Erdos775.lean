...
theorem layered_tree_bounded_one (C : ℕ) :
  ...
  specialize h_degree ⟨ 0, Nat.zero_lt_succ t ⟩ ; simp_all +decide [ OrderedRootedTree.degree ];
  unfold OrderedRootedTree.numChildren at h_degree; aesop;
...
lemma phi_zero_eq_root (hm : T.numChildren ⟨0, Nat.zero_lt_succ t⟩ ≤ t) :
  ...
  convert ( keptVertices T ).min_of_mem _;
  rotate_left;
  exact ⟨ 0, Nat.zero_lt_succ t ⟩;
  · exact root_mem_keptVertices T
  · constructor;
    · have := Finset.min_of_mem ( root_mem_keptVertices T );
      lia;
    · rintro ⟨ b, hb ⟩;
      simp +decide;
...
lemma contractionParentFn_root_self
  ...
  unfold contractionParentFn;
  simp +zetaDelta at *;
  intro h;
  convert OrderIso.symm_apply_eq _ |>.2 _;
  ext;
  convert T.root_self;
  have := phi_zero_eq_root T hm;
  aesop
...
lemma contractionParentFn_parent_lt
  ...
  field_simp;
  intros i hi;
  by_cases h : T.parent ( ( keptVertices T ).orderIsoOfFin ( by
    have := keptVertices_card T; omega; ) i ) ∈ keptVertices T
  all_goals generalize_proofs at *;
  · have h_parent_lt : T.parent ( ( keptVertices T ).orderIsoOfFin ( by
      assumption ) i ) < ( keptVertices T ).orderIsoOfFin ( by
      assumption ) i := by
      apply T.parent_lt;
      contrapose! hi;
      simp +zetaDelta at *;
      exact ( ( keptVertices T ).orderEmbOfFin _ ).injective ( hi.trans ( phi_zero_eq_root T hm |> Eq.symm ) )
    generalize_proofs at *;
    simp +zetaDelta at *;
    split_ifs ; simp_all +decide;
    · exact (OrderIso.symm_apply_lt ((keptVertices T).orderIsoOfFin _)).mpr h_parent_lt
  · aesop
  ...
  convert keptVertices_card T using 1 ; omega) j).val := by
    ...
    · -- Let $p$ be the parent of $j$ in the contraction tree.
      obtain ⟨p, hp⟩ : ∃ p : Fin (t - T.numChildren ⟨0, Nat.zero_lt_succ t⟩ + 1), (contractionTree T hm).parent ⟨j, ih⟩ = p ∧ (keptVertices T).orderIsoOfFin ‹_› p = T.parent ((keptVertices T).orderIsoOfFin ‹_› ⟨j, ih⟩).val := by
        unfold contractionTree;
        unfold contractionParentFn; aesop;
      generalize_proofs at *;
      ...
      · have h_contra : T.parent ((keptVertices T).orderIsoOfFin ‹_› ⟨j, ih⟩).val = ⟨0, Nat.zero_lt_succ t⟩ := by
        ...
        generalize_proofs at *;
        grind +locals;
    · have h_contra : (contractionTree T hm).depth ⟨j, ih⟩ = 1 := by
        ...
        · exact hj_pos;
      have h_contra : T.depth (T.parent ((keptVertices T).orderIsoOfFin ‹_› ⟨j, ih⟩).val) ≥ 1 := by
        have h_contra : (T.parent ((keptVertices T).orderIsoOfFin ‹_› ⟨j, ih⟩).val).val ≠ 0 := by
      ...
      linarith
  generalize_proofs at *;
  by_cases hi : 0 < i.val <;> simp_all +decide [ IsKCLayeredTree ];
  ...
  assumption) i).val.val ≤ i.val + T.numChildren ⟨0, Nat.zero_lt_succ t⟩ := by
    ...
    have h_card_le : Finset.card (Finset.filter (fun j => j.val ≤ ((keptVertices T).orderIsoOfFin ‹_› i).val.val ∧ (j.val = 0 ∨ (T.parent j).val ≠ 0)) (Finset.univ : Finset (Fin (t + 1)))) ≤ i.val + 1 := by
      ...
      · ext; simp;
        constructor;
        · intro h;
          obtain ⟨ j, hj ⟩ := Finset.mem_image.mp ( show ( ‹Fin ( t + 1 ) › : Fin ( t + 1 ) ) ∈ Finset.image ( fun j : Fin ( t - T.numChildren ⟨ 0, Nat.zero_lt_succ t ⟩ + 1 ) => ( keptVertices T ).orderEmbOfFin ‹_› j ) ( Finset.univ : Finset ( Fin ( t - T.numChildren ⟨ 0, Nat.zero_lt_succ t ⟩ + 1 ) ) ) from by
                                                      have := Finset.eq_of_subset_of_card_le ( show Finset.image ( fun j : Fin ( t - T.numChildren ⟨ 0, Nat.zero_lt_succ t ⟩ + 1 ) => ( keptVertices T ).orderEmbOfFin ‹_› j ) Finset.univ ⊆ keptVertices T from Finset.image_subset_iff.mpr fun j _ => Finset.orderEmbOfFin_mem _ _ _ ) ; aesop; )
          generalize_proofs at *;
          exact ⟨ j, by simpa [ ← hj.2 ] using h.2, hj.2.symm ⟩;
        · rintro ⟨ j, hj, rfl ⟩ ; exact ⟨ Finset.orderEmbOfFin_mem _ _ _, by simpa using hj ⟩ ;
    generalize_proofs at *;
...
def prefixSubtree (T : OrderedRootedTree t) (s : ℕ) (hs : s ≤ t) :
  ...
  root_self := by
    ext
    show (T.parent ⟨(0 : Fin (s + 1)).val, by omega⟩).val = (0 : Fin (s + 1)).val
    have := T.root_self
    exact congrArg Fin.val this
  parent_lt := fun i hi => by
    show (T.parent ⟨i.val, by omega⟩).val < i.val
...
lemma prefixSubtree_depth (T : OrderedRootedTree t) (s : ℕ) (hs : s ≤ t)
  ...
  · unfold OrderedRootedTree.depth;
    ...
    · exact T.parent_lt ⟨ i, by linarith ⟩ hi;
  ...
  have h_inj : ∀ j : Fin (t - T.numChildren ⟨0, Nat.zero_lt_succ t⟩ + 1), (contractionTree T hm).parent j = ⟨0, Nat.zero_lt_succ (t - T.numChildren ⟨0, Nat.zero_lt_succ t⟩)⟩ → j ≠ ⟨0, Nat.zero_lt_succ (t - T.numChildren ⟨0, Nat.zero_lt_succ t⟩)⟩ → (φ j).val ∈ Finset.univ.filter (fun v : Fin (t + 1) => T.parent v ∈ rootChildrenFinset T ∧ v ≠ T.parent v) := by
    ...
    have h_parent : T.parent (φ j).val ∉ keptVertices T := by
      ...
      have h_contra : φ.symm ⟨T.parent (φ j).val, hj⟩ ≠ ⟨0, Nat.zero_lt_succ (t - T.numChildren ⟨0, Nat.zero_lt_succ t⟩)⟩ := by
        ...
        have h_contra : (φ j).val ∈ rootChildrenFinset T := by
          simp_all +decide [ rootChildrenFinset ];
          grind +suggestions;
        have h_contra : (φ j).val ∉ keptVertices T := by
          unfold rootChildrenFinset at h_contra; unfold keptVertices; aesop;
        exact h_contra <| φ j |>.2;
...
theorem sum_rootChildren_le_iterBound {k C : ℕ}
  ...
  revert T;
  induction' t using Nat.strong_induction_on with t ih generalizing k C;
  intros T hT
...
lemma walkDownAux_ge {n bound : ℕ} (X : ℕ → Finset (Fin n))
  ...
  induction' fuel with fuel ih generalizing u hu
  · exact le_rfl
  · by_cases h : ∃ v : ℕ, ∃ hv : v < bound, (rec v hv).1 = u ∧ v ≠ u ∧
        X v ∩ (rec u hu).2.2 = X bound ∩ (rec u hu).2.2
    · grind +locals
    · simp [walkDownAux]; grind
...
lemma walkDownAux_stopping {n bound : ℕ} (X : ℕ → Finset (Fin n))
  ...
  induction' fuel with fuel fuel_ih generalizing u hu <;>
...
lemma clParent_stopping {n : ℕ} (X : ℕ → Finset (Fin n))
  ...
  · intro v hv
    by_cases hv_zero : v = 0 <;> simp +decide [hv_zero]
    · unfold clTreeState; aesop
...
lemma walkdown_numChildren_le {n : ℕ} (X : ℕ → Finset (Fin n))
  ...
  · simp_all +decide [Finset.card_sdiff]; convert this using 1
...
lemma ancestor_chain_squeeze {n : ℕ} (X : ℕ → Finset (Fin n))
  ...
  · contrapose! h_lt;
    ...
    induction' ( d₀ - ( d + 1 ) ) with k hk <;> simp_all +decide [ Function.iterate_succ_apply' ];
...
lemma walkDownAux_ancestors_matching {n : ℕ} (X : ℕ → Finset (Fin n))
  ...
  · intro d hd_pos hd_anc hd_gt
    ...
    · have := ‹∃ v, ∃ hv : v < b, ( clTreeState X v ).1 = u ∧ v ≠ u ∧ X v ∩ ( clTreeState X u ).2.2 = X b ∩ ( clTreeState X u ).2.2›.choose_spec.choose_spec;
      ...
      · -- Apply the IH at u := h.choose. The third precondition `h.choose < walkDownAux`
        -- follows from h_case (h.choose ≤ (clParent X)^[d+1] walkDownAux) combined with
        -- (clParent X)^[d+1] walkDownAux < walkDownAux (via clParent_iterate_strict_anti).
        refine ih _ _ d hd_pos h_case ?_
        have hwa_pos : 0 < (walkDownAux X (fun j hj => clTreeState X j)
            ‹∃ v, ∃ hv : v < b, ( clTreeState X v ).1 = u ∧ v ≠ u ∧ X v ∩ ( clTreeState X u ).2.2 = X b ∩ ( clTreeState X u ).2.2›.choose
            ‹∃ v, ∃ hv : v < b, ( clTreeState X v ).1 = u ∧ v ≠ u ∧ X v ∩ ( clTreeState X u ).2.2 = X b ∩ ( clTreeState X u ).2.2›.choose_spec.choose fuel).val :=
          lt_of_lt_of_le hd_pos (clParent_iterate_le X _ d)
        exact lt_of_le_of_lt h_case (clParent_iterate_strict_anti X _ (Nat.zero_lt_succ d) hwa_pos)
...
lemma walkdown_empty_encoding_subset {n : ℕ} (X : ℕ → Finset (Fin n))
  ...
  · -- By Lemma~\ref{lem:walkdown_ancestor_matching}, since $x ∈ X j ∩ clSetB X i = ∅$, we have $x ∈ X i$.
    ...
    · exfalso; push_neg at hi_pos; have hi0 : i = 0 := by omega
...
lemma path_iterate_pos_of_depth {n t : ℕ} (X : ℕ → Finset (Fin n))
  ...
  contrapose! hm;
  induction' m with m ih generalizing i <;> simp_all +decide [ Function.iterate_succ_apply' ];
  · unfold OrderedRootedTree.depth; aesop;
...
-- 'Erdos775.erdos_775' depends on axioms: [propext, Classical.choice, Quot.sound]
