...
-/
/-- **Erdős Problem 1051** (Erdős–Graham [ErGr80, p.64; Er88c, p.106], proved by
Aletheia [Fe26]): if `1 ≤ a₁ < a₂ < ⋯` is a sequence of integers with
`liminf aₙ^(1/2ⁿ) > 1`, then `∑ 1/(aₙ aₙ₊₁)` is irrational. -/
theorem erdos_1051
  (a : ℕ → ℕ)
  (h_mono : StrictMono a)
  (h_pos : ∀ n, 0 < a n)
