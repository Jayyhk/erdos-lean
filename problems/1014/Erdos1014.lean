...
theorem IndepSetFree.comap {α β : Type*} {G : SimpleGraph α} {H : SimpleGraph β} {n : ℕ}
  ...
  exact _root_.SimpleGraph.CliqueFree.comap
    ((_root_.SimpleGraph.Embedding.complEquiv (G := H) (H := G)).toFun f) h
...
lemma ramseyProperty_of_card {k l n : ℕ} {α : Type*} [Fintype α]
  ...
  have hcf : H.CliqueFree k := hbad.1.comap e.symm.toEmbedding
  have hcfCompl : Hᶜ.CliqueFree l := by
    have hGCompl : Gᶜ.CliqueFree l := by
      simpa [SimpleGraph.cliqueFree_compl] using hbad.2
    exact hGCompl.comap
      ((SimpleGraph.Embedding.complEquiv (G := H) (H := G)).toFun e.symm.toEmbedding)
...
lemma ramseyProperty_mono {k l n m : ℕ} (hnm : n ≤ m) :
  ...
  have hcf : (G.comap f).CliqueFree k :=
    hbad.1.comap (SimpleGraph.Embedding.comap f G)
      ...
      | succ l ihl =>
          ...
          · let H : SimpleGraph (G.neighborSet v) := G.induce (G.neighborSet v)
            ...
            have hcf : H.CliqueFree k := by
              ...
              have htInd :
                ...
                simpa [H, SimpleGraph.induce_eq_coe_induce_top] using ht
          ...
          · let H : SimpleGraph (Gᶜ.neighborSet v) := G.induce (Gᶜ.neighborSet v)
            ...
            have hcf : H.CliqueFree (k + 1) := by
              exact hbad.1.comap (SimpleGraph.Embedding.induce (G := G) (s := Gᶜ.neighborSet v))
            have hif : H.IndepSetFree l := by
              intro t ht
              let t' : Finset (Fin (m + n + 1)) :=
                Finset.map ⟨Subtype.val, Subtype.val_injective⟩ t
...
theorem ramseyNumber_three_three : ramseyNumber 3 3 = 6 := by
  ...
  have hnot5 : ¬ RamseyProperty 3 3 5 := by
    ...
    have hcycleIndepFree : (SimpleGraph.cycleGraph 5).IndepSetFree 3 := by
      ...
      exact SimpleGraph.CliqueFree.comap e.toEmbedding hcycleCliqueFree
    intro h
...
theorem Iso.cliqueFree_iff {α β : Type*} {G : SimpleGraph α} {H : SimpleGraph β} {n : ℕ}
  ...
  constructor
  · intro h
    exact _root_.SimpleGraph.CliqueFree.comap e.symm.toEmbedding h
  · intro h
    exact _root_.SimpleGraph.CliqueFree.comap e.toEmbedding h
...
lemma ramseyGap_step1_upper_bound :
  ...
  | succ m =>
      ...
      have hchoose :
        ...
        simpa [hu1, Nat.add_assoc, Nat.add_left_comm, Nat.add_comm] using
          ramseyNumber_le_choose (u - 1) (m + 1)
      have hpow :
  ...
  have hHcf : H.CliqueFree u := by
    ...
    have hC : G.IsNClique u C := by
      have hCsub' :
        ...
        simpa [H, SimpleGraph.induce_eq_coe_induce_top] using hCsub
      simpa [C] using
        (SimpleGraph.IsNClique.of_induce
  ...
  calc
    ...
    _ =
          Finset.sum A (fun s => ∫⁻ ω,
            (if (SimpleGraph.fromEdgeSet ω).IsNClique u s then (1 : ℝ≥0∞) else 0) ∂
              setBer((Sym2.diagSetᶜ : Set (Sym2 (Fin N))), p)) := by
            apply MeasureTheory.lintegral_finset_sum
            intro s hs
...
lemma ramseyGap_step1_eq4 {Ω : Type*} [MeasurableSpace Ω] (μ : Measure Ω)
  ...
  · subst hN0
    ...
    have hnull : μ {ω | 0 < X ω} = 0 := by
      simpa [ae_iff, pos_iff_ne_zero] using hX0
...
lemma ramseyGap_step2_minDegree_bound (k l : ℕ) (hk : 3 ≤ k) :
  ...
  have hHcf : H.CliqueFree k := by
    exact hcf.comap (SimpleGraph.Embedding.induce (G := G) (s := Gᶜ.neighborSet v))
...
lemma ramseyGap_step4_key_inequality (s t q : ℕ)
  ...
  · have hlpos : 0 < l := Nat.pos_of_ne_zero hl
    ...
    have hnot : ¬ RamseyProperty k (l + 1) n := by
      ...
      simpa [k, n, ramseyCriticalSize] using
...
-- 'Erdos1014.erdos_1014' depends on axioms: [propext, Classical.choice, Quot.sound]
