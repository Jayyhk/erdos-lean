...
Erdős Problem 899. Let `A ⊆ ℕ` be infinite with `|A ∩ {1,…,N}| = o(N)`. Is `limsup_N |(A−A) ∩ {1,…,N}| / |A ∩ {1,…,N}| = ∞`? `erdos_899` proves this.

The formalisation is by plby (github.com/plby/lean-proofs),
`src/latest/ErdosProblems/Erdos899.lean`, a single self-contained file whose only import is
`Mathlib`. It is reproduced here with the file-level comment header, the upstream trust-base
print lines and the trailing `alias` removed, wrapped once in `namespace Erdos899`, and with the
final theorem named `erdos_899`. No mathematical content is changed.
-/
...
#print axioms erdos_899
-- 'Erdos899.erdos_899' depends on axioms: [propext, Classical.choice, Quot.sound]
