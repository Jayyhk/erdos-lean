...
Erdős Problem 37. Say `A ⊆ ℕ` is an *essential component* if `d_s(A + B) > d_s(B)` for
every `B ⊆ ℕ` with `0 < d_s(B) < 1`, where `d_s` is the Schnirelmann density. Can a lacunary
set `A ⊆ ℕ` be an essential component? `erdos_37` proves that it cannot: no lacunary set is
an essential component.

The answer is no by Ruzsa, who proved that an essential component `A` must satisfy
`|A ∩ {1, …, N}| ≥ (log N)^(1+c)` for some `c > 0` and all large `N`.

`IsLacunary A` asks that the positive part of `A` be infinite and that its increasing
enumeration have ratios bounded below by a single `q > 1`. Enumerating the *positive* part
is what makes a possible `0 ∈ A` — which matters for the pointwise sumset but not for the
counting function or for lacunarity — handled correctly.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos37.lean` -/

section
/- leanprover/lean4:v4.33.0  mathlib v4.33.0 -/
/- Original license: Apache 2.0. Note: This file has been modified. -/
/-
This is a Lean formalization of a solution to Erdős Problem 37.
https://www.erdosproblems.com/forum/thread/37
...
URLs:
- https://github.com/plby/lean-proofs/blob/main/ErdosProblems/Erdos37.md
-/
...
namespace DirectSpecialBridge.StageSelection
...
end DirectSpecialBridge.StageSelection
...
-- 'Erdos37.erdos_37' depends on axioms: [propext, Classical.choice, Quot.sound]
