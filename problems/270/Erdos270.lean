...
Erdős Problem 270. Let `f(n) → ∞` as `n → ∞`. Is it true that

  `∑_{n ≥ 1} 1 / ((n + 1) ⋯ (n + f n))`

is always irrational? `erdos_270` proves that it is not: the answer is no.

Crmarić and Kovač gave the strongest possible negative answer — every `α ∈ (0, ∞)` is the
value of such a series — and `erdos_270_resolution` is that statement. `erdos_270` is the
consequence asked for on the problem page, taking `α = 1`.

Indices below are zero-based, so the term of `erdos_270`'s series at `n` is the original
one-based term at `n + 1`: `productTerm n k = ((n + 2).ascFactorial k)⁻¹`, which is
`1 / ((n + 2) ⋯ (n + 1 + k))`. The counterexample function is required to be positive
everywhere, which makes the negation stated here stronger than the page demands.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos270.lean` -/
...
-- 'Erdos270.erdos_270' depends on axioms: [propext, Classical.choice, Quot.sound]
