...
This project request had uuid: 74b02a65-d854-4576-a005-876dff1abf76
...
- lemma kleitman_case_T_empty (z : Fin n → ℂ) (hz : ∀ i, 1 ≤ ‖z i‖) (c : ℂ)
    ...
    (validA : (∑ i, (εA i : ℂ) * z i) ∈ Metric.ball c 1)
    (validB : (∑ i, (εB i : ℂ) * z i) ∈ Metric.ball c 1)
    (h_T_empty : ((B : Set (Fin n)) \ (A : Set (Fin n)) ∩ {i : Fin n | 0 ≤ (Classical.choose (exists_unique_alignedZ (z i))).re}) = ∅) :
    False
...
- lemma kleitman_case_Tc_empty (z : Fin n → ℂ) (hz : ∀ i, 1 ≤ ‖z i‖) (c : ℂ)
    ...
    (validA : (∑ i, (εA i : ℂ) * z i) ∈ Metric.ball c 1)
    (validB : (∑ i, (εB i : ℂ) * z i) ∈ Metric.ball c 1)
    (h_Tc_empty : ((B : Set (Fin n)) \ (A : Set (Fin n)) ∩ {i : Fin n | 0 ≤ (Classical.choose (exists_unique_alignedZ (z i))).re}ᶜ) = ∅) :
    False
...
- lemma chain_extension_disjoint {α : Type*} [DecidableEq α] {C : Finset (Finset α)}
    (C1 C2 : Finset (Finset (Option α))) (h1 : C1 ∈ chain_extension C) (h2 : C2 ∈ chain_extension C) (hne : C1 ≠ C2) :
    Disjoint (C1 : Set (Finset (Option α))) (C2 : Set (Finset (Option α)))

- lemma chain_extension_covers_base {α : Type*} [DecidableEq α] {C : Finset (Finset α)} {S : Finset (Option α)} :
...
lemma eq_neg_of_ne_baseSign {z : ℂ} {s ε : ℤ} (hs : IsBaseSign z s) (hε : ε = 1 ∨ ε = -1) (hne : ε ≠ s) : ε = -s := by
  ...
  · rw [←Finset.sum_sub_distrib, Finset.mul_sum]
    have h_zero : ∀ i, i ∉ B \ A → (εB i : ℂ) * z i - (εA i : ℂ) * z i = 0 := by
      ...
      · have hAi : i ∈ A := by rcases hi_out with h | h; contradiction; exact not_not.mp h
        have hBin : IsBaseSign (z i) (εB i) := by rw [←mem_signToSet_iff, hB, Finset.mem_coe]; exact hBi
        have hAin : IsBaseSign (z i) (εA i) := by rw [←mem_signToSet_iff, hA, Finset.mem_coe]; exact hAi
        rw [hunique (εB i) hBin, hunique (εA i) hAin]; ring
      · have hAi : i ∉ A := fun h => hBi (hAB h)
        have hBout : ¬IsBaseSign (z i) (εB i) := by rw [←mem_signToSet_iff, hB, Finset.mem_coe]; exact hBi
        have hAout : ¬IsBaseSign (z i) (εA i) := by rw [←mem_signToSet_iff, hA, Finset.mem_coe]; exact hAi
        have eb : εB i = -s := eq_neg_of_ne_baseSign hs (hεB i) (fun h => hBout (by rw [h]; exact hs))
        have ea : εA i = -s := eq_neg_of_ne_baseSign hs (hεA i) (fun h => hAout (by rw [h]; exact hs))
        rw [eb, ea]; ring
    have h_eq : ∀ i ∈ B \ A, (εB i : ℂ) * z i - (εA i : ℂ) * z i = 2 * w i := by
      intro i hi; rw [Finset.mem_sdiff] at hi
      obtain ⟨s, hs, hunique⟩ := exists_unique_baseSign (z i)
      have hBin : IsBaseSign (z i) (εB i) := by rw [←mem_signToSet_iff, hB, Finset.mem_coe]; exact hi.1
      have hAout : ¬IsBaseSign (z i) (εA i) := by rw [←mem_signToSet_iff, hA, Finset.mem_coe]; exact hi.2
    ...
    calc
      ∑ i, (↑(εB i) * z i - ↑(εA i) * z i) = ∑ i ∈ B \ A, (↑(εB i) * z i - ↑(εA i) * z i) := by
...
def IsSymmetricChainDecomposition {α : Type*} [Fintype α] (D : Finset (Finset (Finset α))) : Prop :=
...
noncomputable def chain_extension {α : Type*} [DecidableEq α] (C : Finset (Finset α)) : Finset (Finset (Finset (Option α))) :=
...
/-
A sufficient condition for a chain to be symmetric: it has a minimum and maximum element, the sum of their cardinalities is the total cardinality, and the chain's length corresponds to the difference in their cardinalities plus one (implying no gaps).
-/
lemma check_symmetric {α : Type*} [Fintype α] (C : Finset (Finset α)) (start last : Finset α)
    ...
    Erdos498.IsSymmetricChain C := by
      constructor;
      refine' ⟨ last.card - start.card, h_chain, _, _, _, _, _ ⟩;
      all_goals try exact?;
      · linarith [ Nat.sub_add_cancel ( show start.card ≤ last.card from Finset.card_le_card ( h_min _ h_last ) ) ];
      · exact ⟨ last, h_last, h_max, by rw [ add_tsub_cancel_of_le ( show start.card ≤ last.card from Finset.card_le_card ( h_min last h_last ) ) ] ⟩

/-
If C is a symmetric chain with maximum element `top`, then the extension C1 (which is C lifted plus `top ∪ {none}`) is a symmetric chain in `Option α`.
Proof idea:
...
lemma symmetric_chain_extension_C1 {α : Type*} [Fintype α] [DecidableEq α]
    ...
    (htop : top ∈ C)
    (hmax : ∀ X ∈ C, X ⊆ top) :
    Erdos498.IsSymmetricChain (insert (insert none (top.map Function.Embedding.some)) (C.map (Finset.mapEmbedding Function.Embedding.some).toEmbedding)) := by
      ...
      · intro x hx y hy hxy; simp_all +decide [ Finset.subset_iff ] ;
        rcases hx with ( rfl | ⟨ x, hx, rfl ⟩ ) <;> rcases hy with ( rfl | ⟨ y, hy, rfl ⟩ ) <;> simp_all +decide;
        · simp_all +decide [ Finset.mapEmbedding ];
          simp_all +decide [ Finset.ext_iff, OrderEmbedding.ofMapLEIff ];
          exact fun x hx => hmax _ hy hx;
        · exact Or.inl fun y hy => by rcases Finset.mem_map.mp hy with ⟨ z, hz, rfl ⟩ ; exact Or.inr ⟨ z, hmax _ hx hz, rfl ⟩ ;
      ...
      · refine' ⟨ by linarith, Or.inl ⟨ _, _ ⟩ ⟩;
        · simp_all +decide [ Finset.subset_iff ];
          rintro X hX x hx; rcases Finset.mem_map.mp hx with ⟨ y, hy, rfl ⟩ ; exact Or.inr ⟨ y, hmax X hX hy, rfl ⟩ ;
        · obtain ⟨ last, hlast₁, hlast₂, hlast₃ ⟩ := hk₆; have := hmax _ hlast₁; simp_all +decide [ Finset.subset_iff ] ;
          linarith [ show top.card = last.card from by rw [ show top = last from Finset.Subset.antisymm ( by aesop ) ( by aesop ) ] ]
...
lemma chain_map_card_injective {α : Type*} (C : Finset (Finset α))
    ...
    Set.InjOn Finset.card (C : Set (Finset α)) := by
      -- Since C is a chain, if two elements have the same cardinality, they must be subset of each other, hence equal.
      intros X hX Y hY h_eq_card
      have h_sub : X ⊆ Y ∨ Y ⊆ X := by
        exact h_chain.total hX hY;
      cases' h_sub with h_sub h_sub <;> [ exact Finset.eq_of_subset_of_card_le h_sub ( by linarith ) ; exact Eq.symm ( Finset.eq_of_subset_of_card_le h_sub ( by linarith ) ) ]

/-
If a chain C has length equal to the difference of the cardinalities of its max and min elements plus one, then the set of cardinalities of elements in C is exactly the interval [min_card, max_card].
-/
lemma card_image_eq_Icc_of_chain_card_diff {α : Type*} (C : Finset (Finset α))
    ...
    C.image Finset.card = Finset.Icc start.card last.card := by
      ...
      · simp +zetaDelta at *;
        exact ⟨ by obtain ⟨ a, ha, rfl ⟩ := hx; exact Finset.card_le_card ( h_min a ha ), by obtain ⟨ a, ha, rfl ⟩ := hx; exact Finset.card_le_card ( h_max a ha ) ⟩;
...
lemma exists_second_top {α : Type*} [Fintype α] (C : Finset (Finset α)) (top : Finset α)
    ...
    ∃ second_top ∈ C.erase top, (∀ X ∈ C.erase top, X ⊆ second_top) ∧ second_top.card = top.card - 1 := by
      rcases hS with ⟨ start, k, h_chain, h_card, h_start, h_min, h_max ⟩;
      -- By definition of $C.erase top$, there exists an element $second_top$ in $C.erase top$ such that $second_top.card = top.card - 1$.
      obtain ⟨second_top, h_second_top⟩ : ∃ second_top ∈ C.erase top, second_top.card = top.card - 1 := by
        have h_card_image : C.image Finset.card = Finset.Icc start.card top.card := by
          ...
          aesop;
        replace h_card_image := Finset.ext_iff.mp h_card_image ( top.card - 1 ) ; rcases k with ( _ | k ) <;> simp_all +decide ;
        · rw [ Finset.card_eq_one ] at h_card ; aesop;
        · contrapose! h_card_image;
          have := Finset.eq_of_subset_of_card_le ( show start ⊆ top from fun x hx => hmax _ h_start hx ) ; simp_all +decide ;
          grind;
      use second_top;
      have h_second_top_max : ∀ X ∈ C.erase top, X ⊆ second_top := by
        ...
        have h_card : X.card ≤ second_top.card := by
          have h_card : X.card ≤ top.card - 1 := by
            exact Nat.le_sub_one_of_lt ( lt_of_le_of_ne ( Finset.card_le_card ( hmax X ( Finset.mem_of_mem_erase hX ) ) ) fun h => by have := Finset.eq_of_subset_of_card_le ( hmax X ( Finset.mem_of_mem_erase hX ) ) ( by linarith ) ; aesop );
          grind
        have h_card_eq : X ⊆ second_top ∨ second_top ⊆ X := by
          have := h_chain.total ( Finset.mem_coe.mpr ( Finset.mem_of_mem_erase hX ) ) ( Finset.mem_coe.mpr ( Finset.mem_of_mem_erase h_second_top.1 ) ) ; aesop;
...
lemma symmetric_chain_extension_C2 {α : Type*} [Fintype α] [DecidableEq α]
    ...
    (hmax : ∀ X ∈ C, X ⊆ top)
    (hrest : (C.erase top).Nonempty) :
    Erdos498.IsSymmetricChain ((C.erase top).image (fun A => insert none (A.map Function.Embedding.some))) := by
      obtain ⟨ start, k, h_chain, hk, h_start, h_min, h_top, h_max, h_card_sum, h_card_diff ⟩ := hS;
      have h_second_top : ∃ second_top ∈ C.erase top, (∀ X ∈ C.erase top, X ⊆ second_top) ∧ second_top.card = h_max.card - 1 := by
        convert Erdos498.exists_second_top C top _ _ _ _ using 1;
        · congr! 1;
          congr! 2;
          · convert rfl;
          · simp +decide [ Finset.subset_iff ];
          · rw [ show h_max = top from Finset.Subset.antisymm ( hmax _ h_card_sum ) ( h_card_diff.1 _ htop ) ];
        · exact ⟨ start, k, h_chain, hk, h_start, h_min, by linarith, ⟨ h_max, h_card_sum, h_card_diff ⟩ ⟩;
        · exact htop;
        · exact hmax;
        · grind;
      obtain ⟨ second_top, h_second_top_mem, h_second_top_max, h_second_top_card ⟩ := h_second_top;
      ...
      · grind;
      · grind;
      · rw [ Finset.card_insert_of_notMem, Finset.card_insert_of_notMem ] <;> simp +decide [ h_card_diff, h_second_top_card ];
        grind;
      · rw [ Finset.card_image_of_injective ] <;> norm_num [ Function.Injective ];
        · rw [ Finset.card_insert_of_notMem, Finset.card_insert_of_notMem ] <;> simp +decide [ Finset.mem_map, Function.Embedding.some ];
          grind;
        · simp +contextual [ Finset.ext_iff ];
          intro a₁ a₂ h a; specialize h ( Option.some a ) ; aesop;

/-
If C is a symmetric chain with maximum element `top` and `C \ {top}` is nonempty, then the extension C2 (which is C \ {top} lifted plus {none}) is a symmetric chain in `Option α`.
-/
...
lemma chain_extension_is_symmetric {α : Type*} [Fintype α] {C : Finset (Finset α)}
  ...
  · have := Classical.choose_spec ( Finset.exists_max_image C Finset.card ‹_› );
    ...
    have h_top_max : ∀ X ∈ C, X ⊆ Classical.choose (Finset.exists_max_image C Finset.card ‹_›) := by
      cases' hS with start k hS_subset;
      -- Since C is a chain, any two elements are comparable. So if X is in C, then either X is a subset of the chosen top or the chosen top is a subset of X.
    ...
    exact ⟨ Erdos498.symmetric_chain_extension_C1 _ _ hS this.1 h_top_max, Erdos498.symmetric_chain_extension_C2_aux _ _ hS this.1 h_top_max ‹_› ⟩;
  · convert Erdos498.symmetric_chain_extension_C1 _ _ _ _ _;
    all_goals try assumption;
    · exact Classical.choose_spec ( Finset.exists_max_image _ _ ‹_› ) |>.1;
    · exact fun X hX => Classical.not_not.1 fun h => Finset.notMem_empty _ <| ‹C.erase _ = ∅› ▸ Finset.mem_erase_of_ne_of_mem ( by aesop ) hX
...
lemma chain_extension_disjoint {α : Type*} [DecidableEq α] {C : Finset (Finset α)}
  ...
  have h_distinct : ∀ x ∈ C1, x ∉ C2 := by
    ...
    · -- Since the top element is unique in the chain C, if x is in C and not equal to the top element, then their images under the embedding can't be equal.
      have h_top_unique : ∀ x ∈ C, x ≠ Classical.choose (Finset.exists_max_image C Finset.card ‹_›) → ¬Insert.insert Option.none ((Finset.mapEmbedding Function.Embedding.some) x) = Insert.insert Option.none ((Finset.mapEmbedding Function.Embedding.some) (Classical.choose (Finset.exists_max_image C Finset.card ‹_›))) := by
        intro x hx hx'; intro h; simp_all +decide [ Finset.ext_iff ] ;
        obtain ⟨ y, hy ⟩ := hx'; specialize h ( Option.some y ) ; simp_all +decide ;
      -- For the second part, if x is in C and not the top element, then their images can't be equal because the top element's image has an extra element (none).
      have h_second_part : ∀ x ∈ C, x ≠ Classical.choose (Finset.exists_max_image C Finset.card ‹_›) → ∀ a ∈ C, ¬Insert.insert Option.none ((Finset.mapEmbedding Function.Embedding.some : Finset α → Finset (Option α)) x) = (Finset.mapEmbedding Function.Embedding.some : Finset α → Finset (Option α)) a := by
        intro x hx hx' a ha; intro h; replace h := Finset.ext_iff.mp h Option.none; simp_all +decide ;
      exact ⟨ fun x hx hx' => h_top_unique x hx' hx, fun a ha x hx hx' => h_second_part x hx' hx a ha ⟩;
...
lemma chain_extension_covers_base {α : Type*} [DecidableEq α] {C : Finset (Finset α)} {S : Finset (Option α)} :
  ...
  · split_ifs <;> simp +decide [ Finset.Nonempty ] at *;
    · -- Since S is in the second chain, it must be of the form insert none (map some A) for some A in C.
      apply Iff.intro;
      · rintro ( ( rfl | ⟨ A, hA, rfl ⟩ ) | ⟨ A, ⟨ hA₁, hA₂ ⟩, rfl ⟩ ) <;> [ exact Or.inr ⟨ _, Classical.choose_spec ( Finset.exists_max_image C Finset.card h ) |>.1, rfl ⟩ ; exact Or.inl ⟨ _, hA, rfl ⟩ ; exact Or.inr ⟨ _, hA₂, rfl ⟩ ];
      · grind;
    · grind;
  · aesop
...
lemma option_finset_cases {α : Type*} [DecidableEq α] (S : Finset (Option α)) :
    ∃ A : Finset α, S = A.map Function.Embedding.some ∨ S = insert none (A.map Function.Embedding.some) := by
      -- Let `A` be the set of `x` such that `some x ∈ S`.
      set A := S.filterMap id;
      by_cases h : Option.none ∈ S <;> [ refine' ⟨ A _, Or.inr _ ⟩ ; refine' ⟨ A _, Or.inl _ ⟩ ];
...
lemma chain_extension_partition {α : Type*} [Fintype α] (D : Finset (Finset (Finset α)))
  ...
  · intro A;
    -- By `option_finset_cases`, there exists `A' : Finset α` such that `A = emb A'` or `A = emb A' ∪ {none}`.
    obtain ⟨A', hA'⟩ : ∃ A' : Finset α, A = Finset.map Function.Embedding.some A' ∨ A = Finset.map Function.Embedding.some A' ∪ {none} := by
      exact Erdos498.option_finset_cases A |> fun ⟨ A', hA' ⟩ => ⟨ A', by simpa using hA' ⟩;
    obtain ⟨C, hC⟩ : ∃! C ∈ D, A' ∈ C := by
      exact hD.2 A';
    -- By `chain_extension_covers_base`, there exists a unique chain `C' ∈ chain_extension C` such that `A ∈ C'`.
    obtain ⟨C', hC'⟩ : ∃! C' ∈ chain_extension C, A ∈ C' := by
      ...
      have h_disjoint : Disjoint (C' : Set (Finset (Option α))) (C'' : Set (Finset (Option α))) ∨ C' = C'' := by
        exact Classical.or_iff_not_imp_left.2 fun h => by_contradiction fun hne => h <| chain_extension_disjoint _ _ hC'.1 hC''.1 hne;
      exact h_disjoint.resolve_left ( Set.not_disjoint_iff_nonempty_inter.mpr ⟨ A, hC'.2, hC''.2 ⟩ ) ▸ rfl;
    ...
    · intro y x hx hy hyA
      have hx_eq_C : x = C := by
        ...
        simp +zetaDelta at *;
        rcases this with ( ⟨ B, hB, rfl ⟩ | ⟨ B, hB, rfl ⟩ ) <;> simp_all +decide [ Finset.mapEmbedding ];
        · simp_all +decide [ Finset.ext_iff, OrderEmbedding.ofMapLEIff ];
...
lemma scd_equiv {α β : Type*} [Fintype α] [Fintype β] (e : α ≃ β)
  ...
  · intro C hC;
    ...
    obtain ⟨ start, k, h_chain, h_card, h_start_in, h_start_min, h_symm, ⟨ last, h_last_in, h_last_max, h_last_card ⟩ ⟩ := hD.1 C' hC';
    refine' ⟨ start.image e, k, _, _, _, _, _, ⟨ last.image e, _, _, _ ⟩ ⟩;
    · intro x hx y hy aesop;
      obtain ⟨ A, hA, rfl ⟩ := Finset.mem_image.mp hx; obtain ⟨ B, hB, rfl ⟩ := Finset.mem_image.mp hy; specialize h_chain ( show A ∈ C' from hA ) ( show B ∈ C' from hB ) ; simp_all +decide [ Finset.subset_iff ] ;
      exact h_chain fun a => aesop (congrArg (Finset.image ⇑e) a);
    · rw [ Finset.card_image_of_injective _ fun x y hxy => by simpa using Finset.image_injective e.injective hxy, h_card ];
  ...
  · intro A;
    ...
    · rintro y ⟨ hy₁, hy₂ ⟩;
      ...
      specialize hC₂ C' ⟨ hC'₁, by
        rw [ Finset.mem_image ] at hy₂; obtain ⟨ B, hB₁, rfl ⟩ := hy₂; simp +decide [ Finset.image_image ] ;
        exact hB₁ ⟩ ; aesop

lemma exists_SCD_fin (n : ℕ) : ∃ D : Finset (Finset (Finset (Fin n))), IsSymmetricChainDecomposition D := by
  induction n with
  | zero =>
...
lemma exists_SCD (α : Type*) [Fintype α] : ∃ D : Finset (Finset (Finset α)), IsSymmetricChainDecomposition D := by
...
lemma kleitman_grid_bound {α : Type*} [Fintype α] (T : Set α) [DecidablePred (· ∈ T)]
  ...
  have h_card_le_C : F_part.ncard ≤ C.card := by
    have h_card_le_C : F_part.ncard ≤ (Finset.image (fun X => X ∩ Finset.filter (fun x => x ∈ T) Finset.univ) (Finset.filter (fun X => X ∈ F_part) Finset.univ)).card := by
      ...
      · intro X hX Y hY h_eq;
        apply kleitman_bound_unique_proj_C T C D hD h_suppC h_suppD F_part h_inter h_subset X Y hX hY;
        simp_all +decide [ Finset.ext_iff, Set.ext_iff ];
    refine' h_card_le_C.trans ( Finset.card_le_card _ );
    intro aesop;
    simp +zetaDelta at *;
    intro x hx hx'; obtain ⟨ U, hU, V, hV, rfl ⟩ := h_subset hx; simp_all +decide [ Finset.ext_iff, Set.subset_def ] ;
    convert hU using 1;
    grind;
  refine' le_min h_card_le_C ( _ : F_part.ncard ≤ D.card );
  -- By definition of $F_part$, for any $X \in F_part$, there exist $U \in C$ and $V \in D$ such that $X = U \cup V$.
  have h_exists_UV : ∀ X ∈ F_part, ∃ U ∈ C, ∃ V ∈ D, X = U ∪ V := by
    exact fun X hX => by obtain ⟨ U, hU, V, hV, rfl ⟩ := h_subset hX; exact ⟨ U, hU, V, hV, rfl ⟩ ;
  -- By definition of $F_part$, for any $X \in F_part$, there exist $U \in C$ and $V \in D$ such that $X = U \cup V$. Therefore, we can define a function $f : F_part \to D$ by $f(X) = V$.
  have h_f : ∃ f : Finset α → Finset α, (∀ X ∈ F_part, f X ∈ D) ∧ (∀ X Y, X ∈ F_part → Y ∈ F_part → X ≠ Y → f X ≠ f Y) := by
    choose! U hU V hV hUV using h_exists_UV;
    refine' ⟨ V, hV, fun X Y hX hY hne h => _ ⟩;
    -- Since $V X = V Y$, we have $X = U X ∪ V X$ and $Y = U Y ∪ V Y$. Thus, $X \subset Y$ or $Y \subset X$.
  ...
  have h_f_image : (Set.image f F_part).ncard ≤ D.card := by
    rw [ ← Set.ncard_coe_finset ] ; exact Set.ncard_le_ncard ( Set.image_subset_iff.mpr fun x hx => Finset.mem_coe.mpr ( hf₁ x hx ) ) ;
  rwa [ Set.ncard_image_of_injOn fun X hX Y hY hXY => by contrapose! hXY; exact hf₂ X Y hX hY hXY ] at h_f_image

/-- Mapping a chain from a subtype preserves the chain property. -/
lemma map_chain_is_chain {α β : Type*} [Fintype α] [Fintype β] (emb : β ↪ α) (C : Finset (Finset β))
...
lemma scd_partition_cover {α : Type*} [Fintype α] (T : Set α) [DecidablePred (· ∈ T)]
  ...
  obtain ⟨ U, hU ⟩ := ‹∀ A : Finset T, ∃! C : Finset ( Finset T ), C ∈ CT ∧ A ∈ C› ( Finset.filter ( fun y => y.val ∈ x ) Finset.univ );
  obtain ⟨ V, hV ⟩ := ‹∀ A : Finset ( Tᶜ : Set α ), ∃! C : Finset ( Finset ( Tᶜ : Set α ) ), C ∈ DTC ∧ A ∈ C› ( Finset.filter ( fun y => y.val ∈ x ) Finset.univ );
...
lemma IsSymmetricChain_ranks {α : Type*} [Fintype α] {C : Finset (Finset α)}
  ...
  obtain ⟨start, k, h_chain, h_card, h_start_in, h_start_min, h_symm, ⟨last, h_last_in, h_last_max, h_last_card⟩⟩ := hS
  use start.card
  intro r
  let ranks := C.image Finset.card
  have h_inj : Set.InjOn Finset.card C.toSet := by
    intro X hX Y hY h_eq
  ...
  have h_ranks_eq : ranks = Finset.Icc start.card (start.card + k) := by
    ...
    · intro r' hr'
      ...
      rw [Finset.mem_Icc]
      exact ⟨Finset.card_le_card (h_start_min X hXC), (Finset.card_le_card (h_last_max X hXC)).trans h_last_card.le⟩
...
lemma symmetric_interval_inter_card {nA nB kC kD S : ℕ}
  ...
  simp +zetaDelta at *;
  rw [ show Finset.Icc kC ( nA - kC ) ∩ Finset.Icc ( S - ( nB - kD ) ) ( S - kD ) = Finset.Icc ( Max.max kC ( S - ( nB - kD ) ) ) ( Min.min ( nA - kC ) ( S - kD ) ) from ?_, Nat.card_Icc ];
  · omega;
  · ext; aesop

/-- Filtering a product of two intervals by their sum is equivalent to an intersection of intervals. -/
lemma filter_Icc_sum_eq_S_card {nA nB kC kD S : ℕ} (hS : S = (nA + nB) / 2)
    (h_nA_2kC : 2 * kC ≤ nA) (h_nB_2kD : 2 * kD ≤ nB) :
    let IC := Finset.Icc kC (nA - kC)
    let ID := Finset.Icc kD (nB - kD)
    let IC_shifted := Finset.Icc (S - (nB - kD)) (S - kD)
    (Finset.filter (fun p : ℕ × ℕ => p.1 + p.2 = S) (IC ×ˢ ID)).card = (IC ∩ IC_shifted).card := by
  intro IC ID IC_shifted
  let f : ℕ × ℕ → ℕ := fun p => p.1
  have hf : ∀ p ∈ (Finset.filter (fun p : ℕ × ℕ => p.1 + p.2 = S) (IC ×ˢ ID)), f p ∈ IC ∩ IC_shifted := by
    intro p hp
    simp only [Finset.mem_filter, Finset.mem_product, Finset.mem_inter, IC, ID, IC_shifted, f, Finset.mem_Icc] at hp ⊢
    repeat constructor <;> (zify at *; omega)
  have hf_inj : Set.InjOn f (Finset.filter (fun p : ℕ × ℕ => p.1 + p.2 = S) (IC ×ˢ ID)).toSet := by
...
lemma rectangle_middle_inter_size {α β : Type*} [Fintype α] [Fintype β]
    ...
    let P : Set (Finset (α ⊕ β)) := {U.map ⟨Sum.inl, Sum.inl_injective⟩ ∪ V.map ⟨Sum.inr, Sum.inr_injective⟩ | (U ∈ C) (V ∈ D)}
    (P ∩ rankSet (α ⊕ β) (n / 2)).ncard = min C.card D.card := by
  intro total_n P_val
  let p_emb : Finset α × Finset β → Finset (α ⊕ β) := fun p => p.1.map ⟨Sum.inl, Sum.inl_injective⟩ ∪ p.2.map ⟨Sum.inr, Sum.inr_injective⟩
  have h_f_inj : Function.Injective p_emb := by
    intro p1 p2 h; apply Prod.ext
    · ext x; have h_mem := congrArg (fun s => Sum.inl x ∈ s) h; simp [p_emb] at h_mem; exact h_mem
    · ext x; have h_mem := congrArg (fun s => Sum.inr x ∈ s) h; simp [p_emb] at h_mem; exact h_mem
  ...
  let Pairs := { p : Finset α × Finset β | p.1 ∈ C ∧ p.2 ∈ D ∧ p.1.card + p.2.card = total_n / 2 }
  ...
  let IC := Finset.Icc kC (nA - kC)
  let ID := Finset.Icc kD (nB - kD)
  have h_Pairs_fin : Pairs.Finite := Set.Finite.subset (Finset.finite_toSet (C ×ˢ D)) (fun p hp => by simp [hp.1, hp.2.1])
  let fr : Finset α × Finset β → ℕ × ℕ := fun p => (p.1.card, p.2.card)
  have hr_inj : Set.InjOn fr (C ×ˢ D : Finset _).toSet := by
  ...
  have h_Pairs_card : Pairs.ncard = (Finset.filter (fun (p : ℕ × ℕ) => p.1 + p.2 = S) (IC ×ˢ ID)).card := by
    have h_img : fr '' Pairs = (Finset.filter (fun (p : ℕ × ℕ) => p.1 + p.2 = S) (IC ×ˢ ID)).toSet := by
    ...
    rw [← Set.ncard_image_of_injOn (hr_inj.mono (fun p hp => by simp [Pairs] at hp; simp [hp.1, hp.2.1])), h_img]
    exact Set.ncard_coe_finset _
  rw [h_Pairs_card]
  have h_nA_2kC : 2 * kC ≤ nA := by
    obtain ⟨startC, k, h_chain, h_card, h_start_in, h_start_min, h_symm, ⟨last, h_last_in, h_last_max, h_last_card⟩⟩ := hC
    have h_start_mem : (rankSet α startC.card ∩ C).Nonempty := ⟨startC, by simp [rankSet, h_start_in]⟩
    obtain ⟨h_low, h_high⟩ := (hRC startC.card).mp h_start_mem
    omega
  have h_nB_2kD : 2 * kD ≤ nB := by
    obtain ⟨startD, k, h_chain, h_card, h_start_in, h_start_min, h_symm, ⟨last, h_last_in, h_last_max, h_last_card⟩⟩ := hD
    have h_start_mem : (rankSet β startD.card ∩ D).Nonempty := ⟨startD, by simp [rankSet, h_start_in]⟩
  ...
  have h_C_card : C.card = nA - 2 * kC + 1 := by
    ...
    have h_kC_val : kC = startC.card := by
      ...
      have h2 : startC.card ≤ kC := by rw [← hX_rank]; exact Finset.card_le_card (h_start_min X hXC)
      omega
  ...
  have h_D_card : D.card = nB - 2 * kD + 1 := by
    ...
    have h_kD_val : kD = startD.card := by
      ...
      have h2 : startD.card ≤ kD := by rw [← hX_rank]; exact Finset.card_le_card (h_start_min X hXD)
      omega
...
lemma sum_ncard_product_eq_ncard_biUnion {I J X : Type*} [DecidableEq X]
  ...
  have h_sum_prod_val : ∑ i ∈ s, ∑ j ∈ t, (f i j).ncard = ∑ p ∈ ht_fin.toFinset, (f p.1 p.2).ncard := by
...
lemma scd_middle_rank_sum {G : Type*} [Fintype G] (T : Set G) [DecidablePred (· ∈ T)]
  ...
  have h_union : ⋃ C ∈ CT, ⋃ D ∈ DTC, {U.map (Function.Embedding.subtype fun x => x ∈ T) ∪ V.map (Function.Embedding.subtype fun x => x ∈ Tᶜ) | (U ∈ C) (V ∈ D)} = Set.univ := by
    have := Erdos498.scd_partition_cover T CT DTC hCT hDTC;
    exact this.symm;
  convert Set.ncard_eq_toFinset_card' ( Set.univ ∩ Erdos498.rankSet G ( Fintype.card G / 2 ) ) using 1;
  · rw [ ← h_union, Set.ncard_eq_toFinset_card' ];
    ...
    · rw [ Set.ncard_eq_toFinset_card' ];
      ...
      exact fun x => ⟨ fun ⟨ a, b, h, h', hx ⟩ => ⟨ ⟨ a, h'.1, b, h'.2, h.choose, h.choose_spec.1, h.choose_spec.2.choose, h.choose_spec.2.choose_spec.1, h.choose_spec.2.choose_spec.2 ⟩, hx ⟩, fun ⟨ ⟨ a, ha, b, hb, U, hU, V, hV, hx ⟩, hx' ⟩ => ⟨ a, b, ⟨ U, hU, V, hV, hx ⟩, ⟨ ha, hb ⟩, hx' ⟩ ⟩;
    · infer_instance;
    · intro p hp q hq hpq;
      have h_disjoint : Disjoint ( {U.map (Function.Embedding.subtype fun x => x ∈ T) ∪ V.map (Function.Embedding.subtype fun x => x ∈ Tᶜ) | (U ∈ p.1) (V ∈ p.2)} : Set (Finset G) ) ( {U.map (Function.Embedding.subtype fun x => x ∈ T) ∪ V.map (Function.Embedding.subtype fun x => x ∈ Tᶜ) | (U ∈ q.1) (V ∈ q.2)} : Set (Finset G) ) := by
        have := scd_partition_disjoint T CT DTC hCT hDTC;
        exact this _ ( Finset.mem_product.mp hp |>.1 ) _ ( Finset.mem_product.mp hp |>.2 ) _ ( Finset.mem_product.mp hq |>.1 ) _ ( Finset.mem_product.mp hq |>.2 ) ( by simpa using hpq );
...
/-- The global summation identity: sum of min chain lengths equals the central binomial coefficient. -/
lemma scd_product_identity {α β : Type*} [Fintype α] [Fintype β]
  ...
  let P (C : Finset (Finset α)) (D : Finset (Finset β)) : Set (Finset G) :=
    { (U.map ⟨Sum.inl, Sum.inl_injective⟩ ∪ V.map ⟨Sum.inr, Sum.inr_injective⟩ : Finset G) | (U ∈ C) (V ∈ D) }
  ...
  have h_disj : (Cα.product Cβ : Set (Finset (Finset α) × Finset (Finset β))).PairwiseDisjoint (fun p => P p.1 p.2 ∩ rankSet G (n / 2)) := by
  ...
  calc
    ...
    _ = (Fintype.card α + Fintype.card β).choose ((Fintype.card α + Fintype.card β) / 2) := by
      have h_eq : (⋃ p ∈ Cα.product Cβ, P p.1 p.2 ∩ rankSet G (n / 2)) = (⋃ p ∈ Cα.product Cβ, P p.1 p.2) ∩ rankSet G (n / 2) := by
        ext x; simp only [Set.mem_iUnion, Set.mem_inter_iff, exists_and_right]
      rw [h_eq, h_univ, Set.univ_inter, Set.ncard_eq_toFinset_card (rankSet G (n / 2)) (Set.toFinite _)]
...
theorem kleitman_bound_theorem {α : Type*} [Fintype α] (F : Set (Set α))
  ...
  have total_card : F_fin.ncard = ∑ C ∈ CT, ∑ D ∈ DTC, (F_part C D).ncard := by
    ...
    have hf_part : ∀ C D, (F_part C D).Finite := fun C D => hf_fin.subset (@Set.inter_subset_left _ F_fin (P C D))
    rw [h_cover]
    have hf_union : (⋃ x ∈ CT.product DTC, F_part x.1 x.2).Finite := by rw [←h_cover]; exact hf_fin
    rw [Set.ncard_eq_toFinset_card _ hf_union]
    have h_toF : hf_union.toFinset = (CT.product DTC).biUnion (fun p => (hf_part p.1 p.2).toFinset) := by
      ext S
    ...
    · intro p hp q hq hne
      ...
      have disj := scd_partition_disjoint T CT DTC hCT hDTC p.1 (Finset.mem_product.mp hp).1 p.2 (Finset.mem_product.mp hp).2 q.1 (Finset.mem_product.mp hq).1 q.2 (Finset.mem_product.mp hq).2 hne
      exact Set.disjoint_left.mp (Set.disjoint_of_subset (@Set.inter_subset_right _ F_fin (P p.1 p.2)) (@Set.inter_subset_right _ F_fin (P q.1 q.2)) disj) hx
...
/-
If a nonempty set of complex numbers all lie in the second quadrant (negative real part, non-negative imaginary part) and have norm at least 1, then the norm of their sum is at least 1.
-/
lemma norm_sum_ge_one_of_quadrant {α : Type*} {S : Finset α} {w : α → ℂ}
    (hS : S.Nonempty)
    (h_quad : ∀ i ∈ S, (w i).re < 0 ∧ 0 ≤ (w i).im)
    (h_norm : ∀ i ∈ S, 1 ≤ ‖w i‖) :
    1 ≤ ‖∑ i ∈ S, w i‖ := by
      by_contra! h;
      -- Since $|w_i| \geq 1$ for all $i \in S$, we have $\sum_{i \in S} |w_i|^2 \geq |S|$.
      have h_sum_sq_ge_card : ∑ i ∈ S, ‖w i‖^2 ≥ S.card := by
        exact le_trans ( by norm_num ) ( Finset.sum_le_sum fun i hi => one_le_pow₀ ( h_norm i hi ) );
      -- Since $|\sum_{i \in S} w_i|^2 = \sum_{i \in S} |w_i|^2 + \sum_{i \ne j} \Re(w_i \overline{w_j})$, and $\Re(w_i \overline{w_j}) \geq 0$ for all $i \ne j$, we have $|\sum_{i \in S} w_i|^2 \geq \sum_{i \in S} |w_i|^2$.
      have h_sum_sq_ge_sum_sq : ‖∑ i ∈ S, w i‖^2 ≥ ∑ i ∈ S, ‖w i‖^2 := by
        have h_sum_sq_ge_sum_sq : ‖∑ i ∈ S, w i‖^2 = ∑ i ∈ S, ‖w i‖^2 + ∑ i ∈ S, ∑ j ∈ S, if i ≠ j then Complex.re (w i * starRingEnd ℂ (w j)) else 0 := by
          simp +decide [ Complex.normSq, Complex.sq_norm, Finset.sum_ite, Finset.filter_ne ];
          simp +decide [ Finset.mul_sum _ _ _, Finset.sum_add_distrib, mul_comm ];
          simp +decide [ ← Finset.sum_add_distrib ];
          exact Finset.sum_congr rfl fun i hi => by rw [ ← Finset.sum_erase_add _ _ hi ] ; ring;
        simp_all +decide [ Complex.mul_re ];
        exact Finset.sum_nonneg fun i hi => Finset.sum_nonneg fun j hj => by split_ifs <;> nlinarith only [ h_quad i hi, h_quad j hj ] ;
      exact absurd h_sum_sq_ge_sum_sq ( by nlinarith [ show ( S.card : ℝ ) ≥ 1 by exact_mod_cast Finset.card_pos.mpr hS, norm_nonneg ( ∑ i ∈ S, w i ) ] )
...
lemma kleitman_case_T_empty (z : Fin n → ℂ) (hz : ∀ i, 1 ≤ ‖z i‖) (c : ℂ)
    ...
    (h_T_empty : ((B : Set (Fin n)) \ (A : Set (Fin n)) ∩ {i : Fin n | 0 ≤ (Classical.choose (exists_unique_alignedZ (z i))).re}) = ∅) :
    False := by
    -- By `IsAlignedZ_isUHP`, `w i` is in the Upper Half Plane (UHP), meaning `(w i).im > 0` or `((w i).im = 0 ∧ (w i).re ≥ 0)`.
    -- Since `(w i).re < 0`, the condition `(w i).im = 0 ∧ (w i).re ≥ 0` cannot hold. Thus, `(w i).im > 0`.
    have h_w_im_pos : ∀ i ∈ B \ A, 0 ≤ (w i).im := by
      intro i hi
      have h_w_i_UHP : Erdos498.inUHP (w i) := by
        exact Erdos498.IsAlignedZ_isUHP ( hw_align i );
      cases h_w_i_UHP <;> linarith;
    -- By `IsAlignedZ_isUHP`, `w i` is in the Upper Half Plane (UHP), meaning `(w i).re < 0`.
    have h_w_re_neg : ∀ i ∈ B \ A, (w i).re < 0 := by
      simp_all +decide [ Set.ext_iff ];
      intro i hi hi'; specialize h_T_empty i hi hi'; rw [ show w i = ( Classical.choose ( Erdos498.exists_unique_alignedZ ( z i ) ) ) from ?_ ] ; exact h_T_empty;
      have := Classical.choose_spec ( Erdos498.exists_unique_alignedZ ( z i ) );
      exact this.2 _ ( hw_align i ) ▸ rfl;
    -- By `Erdos498.norm_sum_ge_one_of_quadrant`, we have `‖∑ i ∈ B \ A, w i‖ ≥ 1`.
    have h_sum_ge_one : 1 ≤ ‖∑ i ∈ B \ A, w i‖ := by
      convert Erdos498.norm_sum_ge_one_of_quadrant ( show ( B \ A ).Nonempty from Finset.nonempty_of_ne_empty <| by intro t; simp_all +decide [ Finset.ssubset_def ] ) ( fun i hi => ⟨ h_w_re_neg i hi, h_w_im_pos i hi ⟩ ) ( fun i hi => by
                                                      obtain ⟨ s, hs₁, hs₂ ⟩ := hw_align i;
                                                      rw [ hs₂, norm_mul ] ; specialize hz i ; rcases hs₁ with ⟨ hs₁, hs₂ ⟩ ; rcases eq_or_ne s 1 with rfl | hs₃ <;> simp_all +decide ) using 1;
    have := norm_sub_le ( ∑ i, ( εB i : ℂ ) * z i - c ) ( ∑ i, ( εA i : ℂ ) * z i - c ) ; simp_all +decide [ dist_eq_norm ] ; linarith;

noncomputable section AristotleLemmas

open Complex BigOperators

lemma pure_sum_norm_ge_sqrt {ι : Type*} (S : Finset ι) (w : ι → ℂ)
    (h_pure : (∀ i ∈ S, 0 ≤ (w i).re ∧ 0 ≤ (w i).im) ∨ (∀ i ∈ S, (w i).re ≤ 0 ∧ 0 ≤ (w i).im))
    (hNorm : ∀ i ∈ S, 1 ≤ ‖w i‖) :
    (S.card : ℝ) ≤ ‖∑ i ∈ S, w i‖^2 := by
      cases' h_pure with h_pure h_pure;
      · simp_all +decide [ Complex.normSq, Complex.sq_norm ];
        -- Since each $w_i$ has a norm of at least 1, we have $(w_i).re^2 + (w_i).im^2 \geq 1$ for all $i \in S$.
        have h_norm_sq : ∀ i ∈ S, (w i).re^2 + (w i).im^2 ≥ 1 := by
          simp_all +decide [ Complex.normSq, Complex.norm_def, sq ];
        have := Finset.sum_le_sum h_norm_sq;
        simp_all +decide [ Finset.sum_add_distrib, sq ];
        exact le_trans this ( add_le_add ( by simpa only [ Finset.sum_mul _ _ _ ] using Finset.sum_le_sum fun i hi => mul_le_mul_of_nonneg_left ( Finset.single_le_sum ( fun i _ => h_pure i ‹_› |>.1 ) hi ) ( h_pure i ‹_› |>.1 ) ) ( by simpa only [ Finset.sum_mul _ _ _ ] using Finset.sum_le_sum fun i hi => mul_le_mul_of_nonneg_left ( Finset.single_le_sum ( fun i _ => h_pure i ‹_› |>.2 ) hi ) ( h_pure i ‹_› |>.2 ) ) );
      · norm_num [ Complex.normSq, Complex.sq_norm ] at *;
        -- Since each $w_i$ has a norm of at least 1, we have $(w_i).re^2 + (w_i).im^2 \geq 1$ for all $i \in S$.
        have h_norm_sq : ∀ i ∈ S, (w i).re^2 + (w i).im^2 ≥ 1 := by
          simp_all +decide [ Complex.normSq, Complex.norm_def ];
          simpa only [ sq ] using hNorm;
        have := Finset.sum_le_sum fun i ( hi : i ∈ S ) => h_norm_sq i hi;
        simp_all +decide [ Finset.sum_add_distrib, Finset.mul_sum _ _ _, Finset.sum_mul _ _ _, sq ];
        refine' le_trans this _;
        gcongr <;> simp +decide [ ← Finset.sum_mul ];
        · rw [ Finset.sum_eq_add_sum_diff_singleton ‹_› ];
          nlinarith only [ h_pure _ ‹_›, show ∑ x ∈ S \ { ‹_› }, ( w x |> Complex.re ) ≤ 0 from Finset.sum_nonpos fun x hx => h_pure x ( Finset.mem_sdiff.mp hx |>.1 ) |>.1 ];
        · exact mul_le_mul_of_nonneg_right ( Finset.single_le_sum ( fun i _ => h_pure i ‹_› |>.2 ) ‹_› ) ( h_pure _ ‹_› |>.2 )

open Complex Metric BigOperators

open Complex Erdos498

lemma aligned_implies_im_ge_zero {z w : ℂ} (h : IsAlignedZ z w) : 0 ≤ w.im := by
  -- By definition of `IsAlignedZ`, there exists a base sign `s` such that `w = s * z` and `s = 1` if `z` is in the upper half-plane or on the non-negative real axis, and `s = -1` if `z` is in the lower half-plane or on the negative real axis.
  obtain ⟨s, hs⟩ := h;
  -- By definition of `IsBaseSign`, we know that if `z` is in the upper half-plane or on the non-negative real axis, then `s = 1`, and if `z` is in the lower half-plane or on the negative real axis, then `s = -1`.
  cases' hs.left with hz hz_neg;
  -- By definition of `inUHP`, we know that if `z` is in the upper half-plane or on the non-negative real axis, then `s = 1`, and if `z` is in the lower half-plane or on the negative real axis, then `s = -1`.
  by_cases hz : Erdos498.inUHP z <;> simp_all +decide [ Erdos498.inUHP ];
  -- By definition of `hz`, we know that either `z.im > 0` or `z.im = 0` and `z.re ≥ 0`.
  cases hz <;> simp_all +decide;
  -- Since $0 < z.im$, we have $0 \leq z.im$.
  exact le_of_lt ‹0 < z.im›
...
lemma kleitman_case_Tc_empty (z : Fin n → ℂ) (hz : ∀ i, 1 ≤ ‖z i‖) (c : ℂ)
    ...
    (h_Tc_empty : ((B : Set (Fin n)) \ (A : Set (Fin n)) ∩ {i : Fin n | 0 ≤ (Classical.choose (exists_unique_alignedZ (z i))).re}ᶜ) = ∅) :
    False := by
    -- By `pure_sum_norm_ge_sqrt` (first case), $|S| \le \|\sum_{i \in S} w_i\|^2$.
    have h_card_le_norm_sq : (Finset.card (B \ A)) ≤ ‖∑ i ∈ B \ A, w i‖^2 := by
      apply Erdos498.pure_sum_norm_ge_sqrt
      generalize_proofs at *; (
      -- Since $w_i$ is aligned with $z_i$, we have $w_i.re = \pm |w_i| \cos \theta$ and $w_i.im = \pm |w_i| \sin \theta$ for some angle $\theta$.
      have h_w_re_im : ∀ i ∈ B \ A, (w i).re = (Classical.choose (exists_unique_alignedZ (z i))).re ∧ (w i).im = (Classical.choose (exists_unique_alignedZ (z i))).im := by
        intro i hi
        have h_w_eq : w i = Classical.choose (exists_unique_alignedZ (z i)) := by
          exact Classical.choose_spec ( exists_unique_alignedZ ( z i ) ) |>.2 _ ( hw_align i ) ▸ rfl
        generalize_proofs at *; (
        exact ⟨ congr_arg Complex.re h_w_eq, congr_arg Complex.im h_w_eq ⟩)
      generalize_proofs at *; (
      simp_all +decide [ Set.ext_iff ];
      exact Or.inl fun i hi hi' => by have := h_Tc_empty i hi hi'; have := Erdos498.aligned_implies_im_ge_zero ( Classical.choose_spec ( ‹∀ i, ∃ x, Erdos498.IsAlignedZ ( z i ) x ∧ ∀ y, Erdos498.IsAlignedZ ( z i ) y → y = x› i ) |>.1 ) ; linarith;));
      intro i hi; specialize hw_align i; rcases hw_align with ⟨ s, hs, h ⟩ ; rw [ h ] ; norm_num [ hs, hz ] ;
      exact one_le_mul_of_one_le_of_one_le ( mod_cast abs_pos.mpr ( show s ≠ 0 from by rintro rfl; exact absurd hs ( by unfold Erdos498.IsBaseSign; aesop ) ) ) ( hz i );
    -- Since $\|\sum_{i \in S} w_i\| < 1$, we have $|S| < 1$.
    have h_card_lt_one : ‖∑ i ∈ B \ A, w i‖ < 1 := by
      have := dist_triangle ( ∑ i, ( εB i : ℂ ) * z i ) ( c ) ( ∑ i, ( εA i : ℂ ) * z i ) ; simp_all +decide [ dist_eq_norm ] ;
      linarith [ norm_sub_rev c ( ∑ i, ( εA i : ℂ ) * z i ) ];
    exact h_card_le_norm_sq.not_lt <| by exact lt_of_lt_of_le ( pow_lt_one₀ ( norm_nonneg _ ) h_card_lt_one <| by norm_num ) <| mod_cast Finset.card_pos.mpr <| Finset.card_pos.mp <| Finset.card_pos.mpr <| Finset.nonempty_of_ne_empty <| by intro t; simp_all +decide [ Finset.ssubset_def ] ;
...
lemma kleitman_compatibility (z : Fin n → ℂ) (hz : ∀ i, 1 ≤ ‖z i‖) (c : ℂ)
    ...
    (validA : (∑ i, (εA i : ℂ) * z i) ∈ Metric.ball c 1)
    (validB : (∑ i, (εB i : ℂ) * z i) ∈ Metric.ball c 1) :
    ((B : Set (Fin n)) \ (A : Set (Fin n)) ∩ {i : Fin n | 0 ≤ (Classical.choose (exists_unique_alignedZ (z i))).re}).Nonempty ∧
    ((B : Set (Fin n)) \ (A : Set (Fin n)) ∩ {i : Fin n | 0 ≤ (Classical.choose (exists_unique_alignedZ (z i))).re}ᶜ).Nonempty := by
...
lemma compatibility_helper (z : Fin n → ℂ) (hz : ∀ i, 1 ≤ ‖z i‖) (c : ℂ)
    ...
    (h_valid : ∀ ε ∈ valid_sums, (∀ i, ε i = 1 ∨ ε i = -1) ∧ (∑ i, (ε i : ℂ) * z i) ∈ Metric.ball c 1)
  ...
  have ballA : (∑ i, (εA i : ℂ) * z i) ∈ Metric.ball c 1 := (h_valid εA hAF_val).2
  have ballB : (∑ i, (εB i : ℂ) * z i) ∈ Metric.ball c 1 := (h_valid εB hBF_val).2
...
theorem erdos_498 (n : ℕ) (z : Fin n → ℂ) (hz : ∀ i, 1 ≤ ‖z i‖) (c : ℂ) :
    ...
    let valid_sums : Set (Fin n → ℤ) := {ε | ε ∈ all_coeffs ∧ (∑ i, (ε i : ℂ) * z i) ∈ Metric.ball c 1}
  ...
  have hinj : Set.InjOn (signToSet z) valid_sums := by
    ...
    have h1 : ∀ i, ε₁ i = 1 ∨ ε₁ i = -1 := by
      intro i; have := hε₁.1 i; simp [signs] at this; rcases this with h|h <;> rw [h] <;> norm_num
    have h2 : ∀ i, ε₂ i = 1 ∨ ε₂ i = -1 := by
      intro i; have := hε₂.1 i; simp [signs] at this; rcases this with h|h <;> rw [h] <;> norm_num
    exact signToSet_injective_on_signs z ε₁ ε₂ h1 h2 heq

  have card_eq : valid_sums.ncard = F.ncard := (Set.ncard_image_of_injOn hinj).symm
  ...
  have bound : F.ncard ≤ (Fintype.card (Fin n)).choose (Fintype.card (Fin n) / 2) := by
    ...
    let helper_valid : ∀ ε ∈ valid_sums, (∀ i, ε i = 1 ∨ ε i = -1) ∧ (∑ i, (ε i : ℂ) * z i) ∈ Metric.ball c 1 := by
      intro ε hε
      constructor
      · intro i; have := hε.1 i; simp [signs] at this; rcases this with h|h <;> rw [h] <;> norm_num
      · exact hε.2
    let res := compatibility_helper z hz c valid_sums helper_valid A B εA εB hA_val hB_val T hA_eq hB_eq hsub (by rfl)
    exact res
