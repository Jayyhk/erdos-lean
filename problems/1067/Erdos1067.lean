...
/-- **Erdős Problem 1067** (Erdős–Hajnal [ErHa66, p.77; ErHa85], refuted): not every
graph with chromatic number `ℵ₁` contains an (induced) subgraph which is infinitely
connected (i.e. has no finite vertex cut between any pair of distinct vertices) and
still has chromatic number `ℵ₁`. -/
theorem erdos_1067 :
    ¬ (∀ {V : Type 1} (G : SimpleGraph V),
        (Nonempty (G.Coloring (Set.Iio (Ordinal.omega.{0} 1))) ∧
          uncountably_chromatic G) →
          ∃ (S : Set V),
            (Nonempty ((G.induce S).Coloring (Set.Iio (Ordinal.omega.{0} 1))) ∧
              uncountably_chromatic (G.induce S)) ∧
            ∀ u v : S, u ≠ v →
              ¬ finite_independent_paths (G.induce S) u v) := by
  intro h
  classical
  have hG :
...
#print axioms erdos_1067
-- 'Erdos1067.erdos_1067' depends on axioms: [propext, Classical.choice, Quot.sound]
