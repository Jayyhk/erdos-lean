    ...
    {col u, col v} = (Finset.image col B_Finset).erase k := by
      ...
      obtain ⟨v', w', hv', hw', h_adj⟩ : ∃ v' w' : Fin 5, v' ≠ w' ∧ ¬(SimpleGraph.cycleGraph 5).Adj v' w' ∧ {col (H_V.b v'), col (H_V.b w')} = (Finset.image (fun i => col (H_V.b i)) (Finset.univ : Finset (Fin 5))).erase (col (H_V.b u')) := by
        have h_obs : ∀ (c : Fin 5 → Fin 6), (∀ i j, (SimpleGraph.cycleGraph 5).Adj i j → c i ≠ c j) → (Finset.image c (Finset.univ : Finset (Fin 5))).card = 3 → ∀ (k : Fin 5), ∃ v' w' : Fin 5, v' ≠ w' ∧ ¬(SimpleGraph.cycleGraph 5).Adj v' w' ∧ {c v', c w'} = (Finset.image c (Finset.univ : Finset (Fin 5))).erase (c k) := by
          -- Restate over the 5 underlying values `c0..c4 : Fin 6` so `decide` can enumerate.
          -- Chunk on `c0, c1, c2` (6 × 6 × 6 = 216 chunks); each chunk's `decide` handles
          -- 6 × 6 × 5 = 180 cases (over `c3, c4, k`), fitting within kernel stack limits.
          suffices h_obs' :
              ∀ (c0 c1 c2 c3 c4 : Fin 6) (k : Fin 5),
                let c : Fin 5 → Fin 6 := ![c0, c1, c2, c3, c4]
                (∀ i j, (SimpleGraph.cycleGraph 5).Adj i j → c i ≠ c j) →
                (Finset.image c (Finset.univ : Finset (Fin 5))).card = 3 →
                ∃ v' w' : Fin 5, v' ≠ w' ∧ ¬(SimpleGraph.cycleGraph 5).Adj v' w' ∧
                  {c v', c w'} = (Finset.image c (Finset.univ : Finset (Fin 5))).erase (c k) by
            intro c h_adj h_card k
            have hc_eq : c = ![c 0, c 1, c 2, c 3, c 4] := by
              ext i; fin_cases i <;> rfl
            simpa [← hc_eq] using h_obs' (c 0) (c 1) (c 2) (c 3) (c 4) k
              (by simpa [← hc_eq] using h_adj) (by simpa [← hc_eq] using h_card)
          intro c0 c1 c2 c3 c4 k
          fin_cases c0 <;> fin_cases c1 <;> fin_cases c2 <;> (revert c3 c4 k; decide)
        apply h_obs;
