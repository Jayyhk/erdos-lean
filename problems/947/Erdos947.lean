...
theorem coeff_geometric_progression_mul (a n m : ℕ) (h_a : a < n) :
  coeff m ((1 - (X : PowerSeries ℂ)^n) * geometric_progression a n) = if m = a then 1 else 0 := by
    ...
    · simp +decide [ PowerSeries.coeff_X_pow, Finset.Nat.sum_antidiagonal_eq_sum_range_succ_mk ];
      ...
      · rw [ show m = n + ( m - n ) by rw [ Nat.add_sub_cancel' ( by linarith ) ] ] ; aesop;
...
theorem power_series_identity_geometric (l : List (ℤ × ℕ)) (h : IsExactCoveringSystem l) :
  (∑ i : Fin l.length, geometric_progression (l.get i).1.toNat (l.get i).2) = geometric_progression 0 1 := by
    ...
    have h_coeff : ∀ m : ℕ, (∑ i : Fin l.length, (geometric_progression ((l.get i).1.toNat) ((l.get i).2) : PowerSeries ℂ).coeff m) = (geometric_progression 0 1 : PowerSeries ℂ).coeff m := by
      intro m; obtain ⟨ i, hi, hiu ⟩ := h_unique m; simp_all +decide [ PowerSeries.coeff ] ;
      unfold geometric_progression; simp_all +decide [ MvPowerSeries.coeff ] ;
      simp_all +decide [ Nat.mod_one, LinearMap.proj ];
      simp_all +decide [PowerSeries.mk];
      exact Finset.card_eq_one.mpr ⟨ i, Finset.eq_singleton_iff_unique_mem.mpr ⟨ Finset.mem_filter.mpr ⟨ Finset.mem_univ _, hi.symm ▸ Nat.mod_eq_of_lt ( show l[(↑i : ℕ)].1.toNat < l[(↑i : ℕ)].2 from by have := h.1 ( l.get i ) ( by simp +decide ) ; aesop ) ⟩, fun j hj => hiu j ( by simpa [ Nat.ModEq, Nat.mod_eq_of_lt ( show l[(↑j : ℕ)].1.toNat < l[(↑j : ℕ)].2 from by have := h.1 ( l.get j ) ( by simp +decide ) ; aesop ) ] using Finset.mem_filter.mp hj |>.2 ) ⟩ ⟩;
...
/-- **Erdős Problem 947** (Mirsky–Newman / Davenport–Rado).
There is no exact covering system $\{a_i \pmod{n_i}\}_{i=1}^k$ with $k \geq 2$
and distinct moduli $n_i$. -/
theorem erdos_947 : ¬ ∃ l : List (ℤ × ℕ),
    IsExactCoveringSystem l ∧
    l.Pairwise (fun p q => p.2 ≠ q.2) ∧
    l.length ≥ 2 := by
  rintro ⟨l, h_exact, h_distinct, h_len⟩
  exact exact_covering_system_distinct_moduli_impossible l h_exact h_distinct h_len

end

#print axioms erdos_947
-- 'Erdos947.erdos_947' depends on axioms: [propext, Classical.choice, Quot.sound]
