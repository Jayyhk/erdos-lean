...
/-- `phi_A_site n k` is `φ_A(k)` exactly as stated on erdosproblems.com/1000:
the number of integers `1 ≤ m ≤ n_k` for which the reduced denominator `n_k / gcd(m, n_k)`
is not equal to `n_j` for any `1 ≤ j < k`. The sequence `n` is read 1-indexed
(`n 1 = n₁`, `n 2 = n₂`, …); `n 0` is unused. -/
noncomputable def phi_A_site (n : ℕ → ℕ) (k : ℕ) : ℕ :=
  ((Finset.Icc 1 (n k)).filter (fun m =>
    ∀ j ∈ Finset.Icc 1 (k - 1), n k / Nat.gcd m (n k) ≠ n j)).card

/-- Bridge: for the 1-shifted sequence `n' k = if k = 0 then 0 else n (k-1)`,
the site-form `phi_A_site n' (k+1)` equals `phi_A n k`. -/
private lemma phi_A_site_shifted (n : ℕ → ℕ) (k : ℕ) :
    phi_A_site (fun j => if j = 0 then 0 else n (j - 1)) (k + 1) = phi_A n k := by
  unfold phi_A_site phi_A
  have hn' : (fun j => if j = 0 then 0 else n (j - 1)) (k + 1) = n k := by simp
  rw [hn']
  -- Reduce both filtered Finsets to the same set.
  have hLHS :
      (Finset.Icc 1 (n k)).filter (fun m =>
        ∀ j ∈ Finset.Icc 1 (k + 1 - 1),
          n k / Nat.gcd m (n k) ≠ if j = 0 then 0 else n (j - 1)) =
      (Finset.range (n k + 1)).filter
        (fun m => 1 ≤ m ∧ ∀ j, j < k → n k / Nat.gcd m (n k) ≠ n j) := by
    ext m
    simp only [Finset.mem_filter, Finset.mem_Icc, Finset.mem_range]
    refine ⟨?_, ?_⟩
    · rintro ⟨⟨hm1, hm2⟩, hcond⟩
      refine ⟨by omega, hm1, fun j hj => ?_⟩
      have h := hcond (j + 1) ⟨Nat.succ_le_succ (Nat.zero_le _), by omega⟩
      have hne : (j + 1 : ℕ) ≠ 0 := Nat.succ_ne_zero _
      simp [hne] at h
      exact h
    · rintro ⟨hmr, hm1, hcond⟩
      refine ⟨⟨hm1, by omega⟩, fun j ⟨hj1, hj2⟩ => ?_⟩
      have hj0 : j ≠ 0 := Nat.one_le_iff_ne_zero.mp hj1
      have hj_lt : j - 1 < k := by omega
      simp [hj0]
      exact hcond (j - 1) hj_lt
  rw [hLHS]

/-- **Erdős Problem 1000** (proved by Haight): there exists an infinite, strictly
increasing sequence of positive integers `A = {n₁ < n₂ < ⋯}` such that
`lim_{N → ∞} (1/N) ∑_{k=1}^{N} φ_A(k)/n_k = 0`, where `φ_A(k)` counts the integers
`1 ≤ m ≤ n_k` for which `n_k / gcd(m, n_k) ≠ n_j` for all `1 ≤ j < k`. -/
theorem erdos_1000 :
    ∃ n : ℕ → ℕ,
      (∀ k, 1 ≤ k → 0 < n k) ∧
      (∀ i j, 1 ≤ i → i < j → n i < n j) ∧
      Tendsto (fun N : ℕ => (1 / (N : ℝ)) *
        ∑ k ∈ Finset.Icc 1 N, (phi_A_site n k : ℝ) / (n k : ℝ))
        atTop (𝓝 (0 : ℝ)) := by
  obtain ⟨n0, hmono0, hpos0, hlim0⟩ := erdos_1000_true
  refine ⟨fun k => if k = 0 then 0 else n0 (k - 1), ?_, ?_, ?_⟩
  · -- positivity from index 1
    intro k hk
    have hk0 : k ≠ 0 := Nat.one_le_iff_ne_zero.mp hk
    simp [hk0]
    exact hpos0 _
  · -- strict mono from index 1
    intro i j hi hij
    have hi0 : i ≠ 0 := Nat.one_le_iff_ne_zero.mp hi
    have hj0 : j ≠ 0 := Nat.one_le_iff_ne_zero.mp (le_of_lt (lt_of_le_of_lt hi hij))
    simp only [hi0, hj0, if_false]
    exact hmono0 (Nat.sub_lt_sub_right hi hij)
  · -- The Cesàro mean → 0: relate to `cesaroPhi n0 → 0`.
    have hphi_eq : ∀ k, phiA n0 k = phi_A n0 k := by
      intro k
      unfold phiA phi_A
      congr 1
      ext m
      ...
      refine ⟨?_, ?_⟩
      · rintro ⟨⟨hm1, hm2⟩, h⟩
        exact ⟨by omega, hm1, fun j hj => h j hj⟩
      · rintro ⟨_, hm1, h⟩
        exact ⟨⟨hm1, by omega⟩, fun j hj => h j hj⟩
    have key : ∀ N : ℕ,
        (1 / (N : ℝ)) * ∑ k ∈ Finset.Icc 1 N,
          (phi_A_site (fun j => if j = 0 then 0 else n0 (j - 1)) k : ℝ) /
            ((if k = 0 then (0 : ℕ) else n0 (k - 1)) : ℝ) =
        cesaroPhi n0 N := by
      intro N
      unfold cesaroPhi
      rw [one_div]
      congr 1
      rw [show Finset.Icc 1 N = Finset.Ico 1 (N + 1) from by ext; simp [Nat.lt_succ_iff]]
      rw [Finset.sum_Ico_eq_sum_range]
      refine Finset.sum_congr (by simp) (fun k _ => ?_)
      have hk1 : (1 + k : ℕ) ≠ 0 := by omega
      have hidx : (1 + k : ℕ) - 1 = k := by omega
      have h_n_eq : ((if (1 + k : ℕ) = 0 then (0 : ℕ) else n0 ((1 + k) - 1)) : ℝ) = (n0 k : ℝ) := by
        simp [hk1, hidx]
      rw [h_n_eq]
      have h_phi : phi_A_site (fun j => if j = 0 then 0 else n0 (j - 1)) (1 + k) = phi_A n0 k := by
        have := phi_A_site_shifted n0 k
        rw [show (1 + k : ℕ) = k + 1 from by ring]
        exact this
      rw [h_phi, ← hphi_eq]
    refine hlim0.congr' (Filter.Eventually.of_forall (fun N => ?_))
    rw [← key]
    -- align cast-over-ite forms
    push_cast
    rfl

#print axioms erdos_1000
-- 'Erdos1000.erdos_1000' depends on axioms: [propext, Classical.choice, Quot.sound]
