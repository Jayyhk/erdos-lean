...
The answer is **no**: Cambie and Kalviainen constructed an infinite walk in $\mathbb{Z}^3$ with
bounded steps containing no three collinear points.

The construction and the no-three-in-line argument below are vendored from the Lean formalisation
accompanying that paper, `ekalvi/erdos-193`, flattened into a single file.
-/

/-! ### Upstream module `Hilbert193/GaussianValuation.lean` -/
...
/-! ### Upstream module `Hilbert193/Gaussian.lean` -/
...
/-! ### Upstream module `Hilbert193/Construction.lean` -/
...
/-! ### Upstream module `Hilbert193/Continuity.lean` -/
...
/-! ### Bridge to the statement of the problem -/

/-- Collinearity over `ℝ` of three integer points implies the two ordered-minor identities that
`OrderedCollinear` records: writing the second and third points as `s • d + u` and `t • d + u`,
both sides of each identity become the same product of `d`-coordinates. -/
private lemma minors_of_collinear {u v w : Fin 3 → ℤ}
    (h : Collinear ℝ ({(fun m => (u m : ℝ)), (fun m => (v m : ℝ)),
      (fun m => (w m : ℝ))} : Set (Fin 3 → ℝ))) :
    (w 2 - v 2) * (v 0 - u 0) = (v 2 - u 2) * (w 0 - v 0) ∧
    (w 2 - v 2) * (v 1 - u 1) = (v 2 - u 2) * (w 1 - v 1) := by
  rw [collinear_iff_of_mem (Set.mem_insert _ _)] at h
  obtain ⟨d, hd⟩ := h
  obtain ⟨s, hs⟩ := hd (fun m => (v m : ℝ)) (by simp)
  obtain ⟨t, ht⟩ := hd (fun m => (w m : ℝ)) (by simp)
  have hv : ∀ m, (v m : ℝ) = s * d m + (u m : ℝ) := fun m => by
    have := congrFun hs m
    simpa [Pi.add_apply, Pi.smul_apply, smul_eq_mul] using this
  have hw : ∀ m, (w m : ℝ) = t * d m + (u m : ℝ) := fun m => by
    have := congrFun ht m
    simpa [Pi.add_apply, Pi.smul_apply, smul_eq_mul] using this
  constructor
  · have key : ((w 2 - v 2 : ℤ) : ℝ) * ((v 0 - u 0 : ℤ) : ℝ)
        = ((v 2 - u 2 : ℤ) : ℝ) * ((w 0 - v 0 : ℤ) : ℝ) := by
      push_cast
      rw [hv 2, hv 0, hw 2, hw 0]
      ring
    exact_mod_cast key
  · have key : ((w 2 - v 2 : ℤ) : ℝ) * ((v 1 - u 1 : ℤ) : ℝ)
        = ((v 2 - u 2 : ℤ) : ℝ) * ((w 1 - v 1 : ℤ) : ℝ) := by
      push_cast
      rw [hv 2, hv 1, hw 2, hw 1]
      ring
    exact_mod_cast key

/-- The walk visits infinitely many distinct points: its height coordinate is strictly
increasing. -/
private lemma taggedLift_injective : Function.Injective taggedLift := by
  have hmono : StrictMono fun n => (taggedLift n).z := by
    intro m n h
    have := taggedHeight_strictMono h
    simpa [taggedLift] using this
  exact fun m n hmn => hmono.injective (congrArg Point3.z hmn)

/-- The walk as a sequence of vectors in `Fin 3 → ℤ`. -/
private def walk (n : ℕ) : Fin 3 → ℤ :=
  ![(taggedLift n).x, (taggedLift n).y, (taggedLift n).z]

/-- The sixteen-element step menu as a set of vectors in `Fin 3 → ℤ`. -/
private def stepMenu : Set (Fin 3 → ℤ) :=
  (fun p : Point3 => ![p.x, p.y, p.z]) '' finiteStepMenu

theorem erdos_193 :
    ¬ ∀ (S : Set (Fin 3 → ℤ)) (a : ℕ → (Fin 3 → ℤ)),
        S.Finite → Function.Injective a → (∀ i, a (i + 1) - a i ∈ S) →
        ∃ i j k : ℕ, i < j ∧ j < k ∧
          Collinear ℝ ({(fun m => (a i m : ℝ)), (fun m => (a j m : ℝ)),
            (fun m => (a k m : ℝ))} : Set (Fin 3 → ℝ)) := by
  intro hconj
  have hfin : stepMenu.Finite := finiteStepMenu_finite.image _
  have hinj : Function.Injective walk := by
    intro m n hmn
    refine taggedLift_injective (Point3.ext ?_ ?_ ?_)
    · simpa [walk] using congrFun hmn 0
    · simpa [walk] using congrFun hmn 1
    · simpa [walk] using congrFun hmn 2
  have hstep : ∀ n, walk (n + 1) - walk n ∈ stepMenu := by
    intro n
    refine ⟨displacement (taggedLift n) (taggedLift (n + 1)), taggedLift_step_mem n, ?_⟩
    funext m
    fin_cases m <;> simp [walk, displacement]
  obtain ⟨i, j, k, hij, hjk, hcol⟩ := hconj stepMenu walk hfin hinj hstep
  obtain ⟨h0, h1⟩ := minors_of_collinear hcol
  refine taggedLift_no_three hij hjk ⟨?_, ?_⟩
  · simpa [walk] using h0
  · simpa [walk] using h1

#print axioms erdos_193
-- 'Erdos193.erdos_193' depends on axioms: [propext, Classical.choice, Quot.sound]
