...
theorem erdos_844_upper (N : ℕ) (A : Finset ℕ)
...
/-- The proposed optimal set `{1,…,N} ∩ (even ∪ ¬squarefree)` itself satisfies the constraint:
for any two of its elements, the product is not squarefree. -/
lemma erdosSarkozySet_valid (N : ℕ) :
    ∀ a ∈ erdosSarkozySet N, ∀ b ∈ erdosSarkozySet N, ¬ Squarefree (a * b) := by
  intro a ha b hb hsf
  rw [erdosSarkozySet, Finset.mem_filter] at ha hb
  obtain ⟨_, ha'⟩ := ha
  obtain ⟨_, hb'⟩ := hb
  rcases ha' with h2a | hnsa
  · rcases hb' with h2b | hnsb
    · exact absurd (hsf 2 (mul_dvd_mul h2a h2b)) (by simp)
    · exact hnsb (hsf.squarefree_of_dvd (dvd_mul_left b a))
  · exact hnsa (hsf.squarefree_of_dvd (dvd_mul_right a b))

/-- Site-form statement of Erdős #844 (Erdős–Sárközy): the maximum size of an
`A ⊆ {1,…,N}` with `a · b` non-squarefree for all `a, b ∈ A` is `|erdosSarkozySet N|`
(achieved by `erdosSarkozySet N` itself = `{evens} ∪ {odd non-squarefree}`). -/
theorem erdos_844 (N : ℕ) :
    IsGreatest
      { c : ℕ | ∃ A : Finset ℕ, A ⊆ Finset.Icc 1 N ∧
        (∀ a ∈ A, ∀ b ∈ A, ¬ Squarefree (a * b)) ∧ A.card = c }
      (erdosSarkozySet N).card := by
  refine ⟨⟨erdosSarkozySet N, ?_, erdosSarkozySet_valid N, rfl⟩, ?_⟩
  · intro x hx
    exact (Finset.mem_filter.mp hx).1
  · rintro c ⟨A, hA_sub, hA_prod, rfl⟩
    exact erdos_844_upper N A hA_sub hA_prod

#print axioms erdos_844
-- 'Erdos844.erdos_844' depends on axioms: [propext, Classical.choice, Quot.sound]
