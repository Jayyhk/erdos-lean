...
/-! ## §4 Maximal-Sidon iff form (Route B) -/

theorem erdos_42_maximal_form :
...
/-! ## §4b Extension lemma: every Sidon set in `[1, N]` extends to a maximal one -/

lemma exists_maximal_sidon_extension {A : Set ℕ} {N : ℕ}
    (hA_sub : A ⊆ Set.Icc 1 N) (hA_sidon : IsSidon A) :
    ∃ A', A ⊆ A' ∧ IsMaximalSidonSetIn A' N := by
  classical
  have hA_fin : A.Finite := ((Set.Icc 1 N).toFinite).subset hA_sub
  let Af : Finset ℕ := hA_fin.toFinset
  have hAf_coe : (Af : Set ℕ) = A := by simp [Af]
  let S : Finset (Finset ℕ) :=
    (Finset.Icc 1 N).powerset.filter (fun B => Af ⊆ B ∧ IsSidon (B : Set ℕ))
  have hAf_in_S : Af ∈ S := by
    refine Finset.mem_filter.mpr ⟨?_, Finset.Subset.refl _, ?_⟩
    · refine Finset.mem_powerset.mpr (fun x hx => ?_)
      exact Finset.mem_Icc.mpr (hA_sub ((hA_fin.mem_toFinset).mp hx))
    · rw [hAf_coe]; exact hA_sidon
  obtain ⟨B, hB_in_S, hB_max⟩ := S.exists_max_image Finset.card ⟨Af, hAf_in_S⟩
  refine ⟨(B : Set ℕ), ?_, ?_, ?_, ?_⟩
  · intro x hx
    have hxAf : x ∈ Af := (hA_fin.mem_toFinset).mpr hx
    exact Finset.mem_coe.mpr ((Finset.mem_filter.mp hB_in_S).2.1 hxAf)
  · intro x hx
    have hsub : B ⊆ Finset.Icc 1 N :=
      Finset.mem_powerset.mp (Finset.mem_filter.mp hB_in_S).1
    exact Finset.mem_Icc.mp (hsub (Finset.mem_coe.mp hx))
  · exact (Finset.mem_filter.mp hB_in_S).2.2
  · intro x hxIcc hxB hSidon_insx
    have hxB_finset : x ∉ B := fun h => hxB (Finset.mem_coe.mpr h)
    let B' : Finset ℕ := insert x B
    have hB'_in_S : B' ∈ S := by
      refine Finset.mem_filter.mpr ⟨?_, ?_, ?_⟩
      · refine Finset.mem_powerset.mpr (fun y hy => ?_)
        rcases Finset.mem_insert.mp hy with rfl | hyB
        · exact Finset.mem_Icc.mpr hxIcc
        · exact Finset.mem_powerset.mp (Finset.mem_filter.mp hB_in_S).1 hyB
      · exact (Finset.mem_filter.mp hB_in_S).2.1.trans (Finset.subset_insert _ _)
      · have hcoe : ((B' : Finset ℕ) : Set ℕ) = insert x (B : Set ℕ) := by
          simp [B', Finset.coe_insert]
        rw [hcoe]; exact hSidon_insx
    have h_lt : B.card < B'.card := by
      simp [B', Finset.card_insert_of_notMem hxB_finset]
    have h_le : B'.card ≤ B.card := hB_max B' hB'_in_S
    omega
...
theorem erdos_42 :
  ...
  have hMax := erdos_42_maximal_form.mp trivial M hM
  filter_upwards [hMax] with N hN A hA_sub hA_sidon hA_ne
  obtain ⟨A', hA_le_A', hA'_max⟩ := exists_maximal_sidon_extension hA_sub hA_sidon
  obtain ⟨B, hB_sub, hB_sidon, hB_card, hAB⟩ := hN A' hA'_max
  refine ⟨B, hB_sub, hB_sidon, hB_card, ?_⟩
  have h_AA_sub : (A - A : Set ℕ) ⊆ A' - A' := by
    rintro y ⟨a, ha, b, hb, rfl⟩
    exact ⟨a, hA_le_A' ha, b, hA_le_A' hb, rfl⟩
  have h_sub : ((A - A) ∩ (B - B) : Set ℕ) ⊆ {0} := by
    calc ((A - A) ∩ (B - B) : Set ℕ)
        ⊆ (A' - A') ∩ (B - B) := Set.inter_subset_inter h_AA_sub (Set.Subset.refl _)
      _ = {0} := hAB
  have h_zero_AA : (0 : ℕ) ∈ A - A := by
    obtain ⟨a, ha⟩ := hA_ne; exact ⟨a, ha, a, ha, by simp⟩
  have hB_ne : B.Nonempty := by
    by_contra h
    rw [Set.not_nonempty_iff_eq_empty] at h
    rw [h, Set.ncard_empty] at hB_card; omega
  have h_zero_BB : (0 : ℕ) ∈ B - B := by
    obtain ⟨b, hb⟩ := hB_ne; exact ⟨b, hb, b, hb, by simp⟩
...
theorem erdos_42 :
    True ↔ erdos42RHS :=
  Iff.trans Erdos42.erdos_42_maximal_form erdos42RHS_iff_localRHS.symm
