...
/-- The constructive witness for the disproof of Erdős Problem 753: for every
`c > 0` and every `N`, there exists `n ≥ N` and a graph `G` on `n` vertices such
that `χ_L(G) + χ_L(Gᶜ) ≤ n^{1/2 + c}`.
...
/-- **Erdős Problem 753** (DISPROVED). The page asks: *"Does there exist some
constant `c > 0` such that `χ_L(G) + χ_L(Gᶜ) > n^{1/2+c}` for every graph `G` on
`n` vertices?"* The answer is **No** — there is no such `c`. -/
theorem erdos_753 :
    ¬∃ c : ℝ, c > 0 ∧ ∃ N : ℕ, ∀ n : ℕ, N ≤ n → ∀ (G : SimpleGraph (Fin n)),
        (n : ℝ) ^ ((1 : ℝ) / 2 + c) <
          ((listChromaticNumber G : ℝ) + (listChromaticNumber Gᶜ : ℝ)) := by
  intro ⟨c, hc, N, hN⟩
  obtain ⟨n, hn_ge, G, hG⟩ := erdos_753_witness c hc N
