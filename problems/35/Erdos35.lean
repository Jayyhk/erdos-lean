...
Erdős Problem 35. Let `B ⊆ ℕ` be an additive basis of order `k` with `0 ∈ B`. Is it true
that for every `A ⊆ ℕ`

  `d_s(A + B) ≥ α + α(1 - α)/k`,

where `α = d_s(A)` is the Schnirelmann density? `erdos_35` proves that it is.

`IsAdditiveBasisOfOrder B k` is `k • B = Set.univ`, i.e. the `k`-fold sumset of `B` is all
of `ℕ`, and `schnirelmannDensity` is Mathlib's `d_s`. The route is Ruzsa's observation that
the bound follows from Plünnecke's stronger `d_s(A + B) ≥ α ^ (1 - 1/k)`, together with the
elementary comparison `α + α(1 - α)/k ≤ α ^ (1 - 1/k)` recorded as `erdos35_le_rpow`.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos35.lean` -/
...
#print axioms erdos_35
-- 'Erdos35.erdos_35' depends on axioms: [propext, Classical.choice, Quot.sound]
