...
Erdős Problem 395. If `z₁, …, zₙ ∈ ℂ` with `|zᵢ| = 1`, is it true that the probability that

  `|ε₁ z₁ + ⋯ + εₙ zₙ| ≤ √2`,

with `εᵢ ∈ {-1, 1}` uniformly at random, is `≫ 1/n`? `erdos_395` proves that it is.

This is a reverse Littlewood--Offord problem, solved by He, Juškevičius, Narayanan and
Spiro. The constant is absolute: it is chosen before `n` and before the `zᵢ`.
`uniformProbability P` is literal normalized counting measure, `|{ε | P ε}| / 2 ^ n`, on the
Boolean sign cube.

Erdős originally asked this with `√2` replaced by `1`, but Carnielli and Carolino observed
that this is false — take `z₁ = 1` and `z_k = i` for `2 ≤ k ≤ n` with `n` even, so the sum
always has modulus at least `√2`. The `1/n` rate is best possible.
-/

/-! ### Upstream module `/tmp/plby-fresh/src/latest/ErdosProblems/Erdos395.lean` -/
...
-- 'Erdos395.erdos_395' depends on axioms: [propext, Classical.choice, Quot.sound]
