...
/-- Site-form statement of Erdős #818: for finite sets of integers `A` with `|A+A| ≪ |A|`,
`|A·A| ≫ |A|² / (log |A|)^C` for some `C > 0`. -/
theorem erdos_818 :
    ∃ C : ℕ, 0 < C ∧ ∀ c : ℕ, ∃ K : ℕ, 0 < K ∧
      ∀ A : Finset ℝ, 5 ≤ A.card → (A + A).card ≤ c * A.card →
        A.card ^ 2 ≤ K * (Nat.clog 2 A.card) ^ C * (A * A).card := by
  refine ⟨1, Nat.one_pos, fun c => ⟨324 * c ^ 2 + 1, Nat.succ_pos _, fun A hA hAA => ?_⟩⟩
  have h := erdos_818_explicit A hA c hAA
  calc A.card ^ 2
      ≤ 324 * c ^ 2 * Nat.clog 2 A.card * (A * A).card := h
    _ ≤ (324 * c ^ 2 + 1) * Nat.clog 2 A.card * (A * A).card := by gcongr; omega
    _ = (324 * c ^ 2 + 1) * (Nat.clog 2 A.card) ^ 1 * (A * A).card := by ring

end

#print axioms erdos_818
-- 'Erdos818.erdos_818' depends on axioms: [propext, Classical.choice, Quot.sound]
