...
Files may be lightly modified from their original source — typically renaming the headline theorem to fit our `Erdos<N>.erdos_<N>` convention and wrapping in a `namespace Erdos<N>` block. The original source is recorded in the catalog's `source` field.
...
<!-- TABLE:START -->
4 proofs in the catalog (out of 1217+ Erdős problems).
